...
This [parameter](parameters.md#include-exportmps) does not influence the way calculations are carried
out,
...
- When the optimization parameter [simplex-range](parameters.md#simplex-range) is set on `week`:  
  Problem-MC year-week number-date-time.mps  
  Criterion-MC year-week number-date-time.txt
- When the optimization parameter [simplex-range](parameters.md#simplex-range) is set on `day`:  
...
This [parameter](parameters.md#include-unfeasible-problem-behavior) can take one of the four values:
...
in [Output Files](outputs.md). This
...
positive for generation, negative for pumping) defined in [Output Files](outputs.md). This option
should be used if
