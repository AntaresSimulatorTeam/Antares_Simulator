...
The folder "reserves" is used to define the reserves in each area. For each area, the folder "areaName" contains a _reserves.yml_ file.

- _reserves.yml_: defines the reserves in the area. Each reserve is caracterized by a name, a failure cost, a spillage cost and a type (up, down).
...
Then, each plant has to define their participation to the defined reserves in the _reserve-participations.yml_ file in the plant folder. Thermal, short term storage and long term storage plants can participate to reserves. The files are located in:
...
**EXAMPLE OF RESERVE-PARTICIPATIONS.YML FILE FOR THERMAL PLANTS**
```yaml
participations:
  - cluster: nuclear_1
    certifications:
...
- The "Parameters" tab displays six 8760-hour times-series, which are:
  ...
  - Loop flow: amount of power flowing circularly through the grid when all "nodes" are perfectly balanced (no import and no export). Such loop flows may be expected on any "simplified" grid in which large regions (or even countries) are modeled by a small number of "macro" nodes, and should accordingly be accounted for.

  - PST min (denoted $Y^-$): lower bound of phase-shifting that can be reached by a PST installed on the link, if any (note : the effect of the active loop flow generated by the PST may be superimposed to that of the passive loop flow)

  - PST max (denoted $Y^+$): upper bound of phase-shifting that can be reached by a PST installed on the link, if any (note : the effect of the active loop flow generated by the PST may be superimposed to that of the passive loop flow)
...
_Note that such specific binding constraints used to be automatically generated by an auxiliary "Kirchhoff's Constraints Generator" module; this standalone tool was removed (see [#3469](https://github.com/AntaresSimulatorTeam/Antares_Simulator/pull/3469)) and these constraints must now be defined manually._
