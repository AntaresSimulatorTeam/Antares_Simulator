...
- **variables** _(optional)_: a collection of optimization variables that are defined for this model
    ...
    - **lower-bound**: an [expression](#expressions) representing the lower bound of the variable. Must use scalars
      and/or parameters only. If missing default to -inf for continuous and integer type or 0 for binary.
    - **upper-bound**: an [expression](#expressions) representing the upper bound of the variable. Must use scalars
      and/or parameters only. If missing default to +inf for continuous and integer type or 1 for binary.
