...
| --simulation-table-stages=VALUE | Override [which resolution stages get a simulation table](04-parameters.md#simulation-table-stages): `all`, `last`, or a comma-separated list among `optim-nb-1`, `optim-nb-2`, `peak-shaving`, `adq-patch`. `last` is the final stage the run reaches. Must not be empty. Default: `all`. Requires --output [all\|simulation-tables] |
