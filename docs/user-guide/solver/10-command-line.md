...
| --simulation-table-stages=VALUE | Override [which resolution stages get a simulation table](04-parameters.md#simulation-table-stages): `all`, or a comma-separated list among `optim-nb-1`, `optim-nb-2`, `shave-peaks`, `adq-patch`. Default: `all`. Requires --output [all\|simulation-tables] |
