...
| `LegacyExtraOutputsContext` | `src/solver/optimisation/include/antares/solver/optimisation/LegacyExtraOutputsContext.h` | Carries study-data lookups (reservoir capacities, link transmission capacities) that derived outputs need but cannot read from the problem's objective. Populated from `PROBLEME_HEBDO` once per weekly fill. |
...
| `level_percentage` (area) | each `HydroLevel` variable | `X[i] / reservoirCapacity[area]` | quantity by index; reservoir capacity from `LegacyExtraOutputsContext` (skipped if the area has no reservoir) |
| `is_directly_congested` (link) | each `DirectFlow` variable | `X[i] >= directCapacity[link, pdt] - eps ? 1 : 0` | flow by index; per-pdt direct capacity from `LegacyExtraOutputsContext` (skipped if the link's capacity is unknown) |
| `is_indirectly_congested` (link) | each `DirectFlow` variable | `-X[i] >= indirectCapacity[link, pdt] - eps ? 1 : 0` | flow by index; per-pdt indirect capacity from `LegacyExtraOutputsContext` |
| `co2_emissions` … `op5_emissions` (thermal cluster) | each `DispatchableProduction` variable | `X[i] × emission_factor[pollutant]` | generation by index; per-cluster emission factors from `LegacyExtraOutputsContext`, keyed `area$$cluster` (skipped if the cluster is unknown) |
| `cluster_availability` (thermal cluster) | each `DispatchableProduction` variable | `max(availability, minStablePower × ceil(availability / unitSize))` | per-cluster availability/unit data from `LegacyExtraOutputsContext`, keyed `area$$cluster` |
| `up_margin` (thermal cluster) | each `DispatchableProduction` variable | `cluster_availability - X[i]` | as above |
| `min_gen_power` (thermal cluster) | each `DispatchableProduction` variable | `min(X[i], minGenPower)` | generation by index; per-cluster min-gen series from `LegacyExtraOutputsContext` |
| `down_margin` (thermal cluster) | each `DispatchableProduction` variable | `X[i] - min(cluster_availability, minGenPower)` | as above |
...
4. If the formula uses a parameter that isn't an objective coefficient (a capacity, a quota, ...), add the field to `LegacyExtraOutputsContext` and populate it in `BuildLegacyExtraOutputsContext` from `PROBLEME_HEBDO`. Key by `info.component` (and `pdt` for per-hour data); helpers must skip the row when the lookup is empty so studies that don't carry the data still emit a coherent table.
5. Emit through `AddExtraOutputEntry` so block/time/scenario conventions stay uniform.
6. If the companion key may not be unique (cluster-level variables), do not use the view — extend the recorded info first.
7. Add a case to `src/tests/src/solver/optimisation/test_legacy_extra_outputs.cpp` and, when the value is deterministically derivable from a test study, a row to the cucumber scenario in `src/tests/cucumber/features/solver-features/legacy_simulation_table.feature`.
...
- **Study data not present in the problem.** Some formulas use parameters that are not objective coefficients. The infrastructure for this is `LegacyExtraOutputsContext`: the caller (in `opt_appel_solveur_lineaire.cpp::BuildLegacyExtraOutputsContext`) reads the needed fields from `PROBLEME_HEBDO` once per weekly solve and passes them to `AddLegacyExtraOutputs`; helpers look up the value by the row's `component` (and `pdt` for per-hour data) and emit nothing when the lookup is empty. Reservoir capacity (for `level_percentage`) and per-pdt link transmission capacities (for `is_directly/indirectly_congested`) are plumbed this way, as are the thermal margin inputs (unit size, min stable power, and the per-hour availability / min-generation series, all already in `PaliersThermiquesDuPays`). Thermal emission rates were **not** carried in `PaliersThermiquesDuPays`, so they were added there (`emissionFactors`, populated in `sim_calcul_economique.cpp` from `cluster->emissions.factors`) — the one case so far that needed a field added to the weekly problem rather than just snapshotting an existing one.
  - **Spinning and the margin formulas.** The thermal availability series and `nominalCapacityWithSpinning` are already multiplied by `(1 - spinning/100)` at load time (`cluster.cpp::calculationOfSpinning`). The spec writes `cluster_availability` with the *raw* `cluster_max_generation` and `max_power_per_unit` and re-applies `(1 - spinning/100)`; algebraically the factor cancels, so `cluster_availability = max(availability, minStablePower × ceil(availability / unitSize))` using the spinning-adjusted quantities the problem already holds — no spinning value or raw nominal capacity needs plumbing. Likewise the spec's `num_units` in the min-generation term is the cluster's static `unitCount` (it appears via `PthetaInf = modulation × unitCount × nominalCapacity`), not `NODU`, so the margin outputs do not depend on unit-commitment mode.
...
- **Component qualification.** `LegacyVariableInfo` now carries an `area` field (captured from the namer's current area at record time), so cluster-level study data can be keyed uniquely as `area$$cluster` — used by the thermal emissions and margin outputs. The public `component` is left unchanged (still the cluster name), and `LegacySolutionView` still keys on `component` alone, so cross-area lookups *through the view* (e.g. a future thermal `profit` reading its area price) remain blocked until the view is taught to use the area too.
...
The cucumber feature now references three small fixtures cloned into `hybrid/` to exercise the variant-specific outputs end-to-end: `008 Thermal fleet - Accurate unit commitment` for `actual_num_units_on`, `Hurdle-cost link` (a 21-day clone of `short-tests/025 Hurdle costs - 2`) for `abs_flow`, link `prop_cost`, `capacity_shadow_price`, and the two congestion indicators, and `Accurate hydro pricing` (a 7-day clone of a `valid-hydroPricing/Accurate/...` study) for `hydro_shadow_price` and `level_percentage`. The saturated-link case for `is_directly/indirectly_congested` stays unit-test-only — the 21-day window of the link fixture never pushes the link to its 3000 MW capacity.

The `008` scenario also covers the thermal emissions and margin outputs: `co2_emissions` uses the cluster CO2 factors from `list.ini` (1.2 / 0.6 / 0.7) times the at-maximum generation (3600 / 1500 / 800); and because the asserted hour is a shortfall, every cluster runs at its full availability, so `cluster_availability` equals the generation, `up_margin` is 0, the (zero) min-gen modulation makes `min_gen_power` 0, and `down_margin` equals the generation. These are quantities / study data, so they carry no anti-degeneracy noise and use the default tolerance (unlike `prop_cost`/`non_prop_cost`, which read noisy objective coefficients and use `1e-4`).
