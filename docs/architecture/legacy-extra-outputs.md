...
| `shave-peaks` | `simulation-table-{year}-shave-peaks` | `RemixHydroPostProcessCmd` (shave-peaks) | economy (both post-process lists) and adequacy |
...
That condition is deliberately **not** "this optimisation pass writes a table". A selection such as `--simulation-table-stages=shave-peaks` leaves both passes without a table of their own, and tying the retention to that would silently reduce every stage the run *did* ask for to its legacy half.

**Caveat: a stage table mixes two vintages.** Post-processing mutates the legacy result structures and cannot move a modeler variable, so in a hybrid study the legacy rows are *post*-treatment while the modeler component rows are the values the solver left. A `gen1` row in `shave-peaks` must not be read as "after remix" — it is identical to its `optim-nb-2` row by construction.
