...
"Inactive" status for anything driven by time-series content is decided **once per study**, over the whole loaded chronicle (every loaded column, not just the ones a given run happens to select via the scenario builder) — never re-evaluated per week or per MC year. Re-deciding per week/year would make the simulation table's column set drift between weeks or years for the same object, which breaks downstream tools that expect a stable schema.
...
- `BuildInactiveComponentsAnalyzer(const Data::Study&)` (`InactiveComponentsAnalyzerBuilder.h/.cpp`) computes the flags: `Matrix<T,ReadWriteT>::containsOnlyZero()` (`matrix.hxx`) per area's `load.series`/`hydro.series->ror`/`solar.series`/`wind.series`/`hydro.series->storage` (the inflow, aka "apports", series — distinct from `ror`), one `containsOnlyZero()` call per misc-gen column (`area.miscGen[column]`, ordered like `Data::MiscGenIndex`), and a link flag when **both** `directCapacities` and `indirectCapacities` are entirely zero (there is no explicit "disabled" flag on `AreaLink`, so this is the closest structural equivalent).
