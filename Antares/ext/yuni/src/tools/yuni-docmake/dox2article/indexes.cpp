...
void CreateIndex()
{
    ...
    {
        file << "<title>Doxygen</title>\n"
             << "<pragma:weight value=\"0.55\" />\n"
             << "<pragma:directoryindex content=\"nofollow\" />"
             << "\n\n"
             << "<h2>API Reference</h2>\n"
             << "<p><b>Beware: THIS DOCUMENTATION IS INCOMPLETE AND WE'RE CURRENTLY WORKING ON "
                "IT</b></p>\n"
             << "<ul>\n"
...
void CreateNamespaceIndex()
{
    ...
    if (file.openRW(filename))
    {
        ...
        for (Compound::Map::iterator i = allSymbolsByName[kdNamespace].begin(); i != end; ++i)
        {
            ...
            file << "<tr><td>\n";
            file << "<a href=\"../" << compound.htdocs << "/@{INDEX}\">" << compound.name
                 << "</a>\n";
            file << "</td><td></td></tr>\n";
            ++total;
        }
        ...
        {
        case 0:
            file << "<p><i>(no namespace)</i></p>";
            break;
        case 1:
            file << "<p><i>(total: 1 namespace)</i></p>";
            break;
        default:
            file << "<p><i>(total: " << total << " namespaces)</i></p>";
            break;
        }
        file << "\n";
...
void CreateClassIndex()
{
    ...
    if (file.openRW(filename))
    {
        ...
        {
        case 0:
            file << "<p><i>(no class)</i></p>";
            break;
        case 1:
            file << "<p><i>(total: 1 class)</i></p>";
            break;
        default:
            file << "<p><i>(total: " << total << " classes)</i></p>";
            break;
        }
        file << "\n";
