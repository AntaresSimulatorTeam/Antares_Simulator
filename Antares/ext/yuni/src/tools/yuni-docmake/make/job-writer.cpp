...
public:
    ...
    public:
        Curve() : showPoints(true), fill(true)
        {
        }
...
public:
    Chart(TiXmlElement* parentNode) : xmlNode(parentNode), legendPosition("ne")
    {
    }
    ...
    void generateJS(Clob& script, unsigned int id) const
    {
        ...
        for (unsigned int i = 0; i != curves.size(); ++i)
        {
            ...
            script << "],\n";
            script << "\t\t\t" << curve.type
                   << ": { show: true, fill: " << ((curve.fill) ? "true" : "false") << " }";
...
XMLVisitor::XMLVisitor(ArticleData& article, TiXmlDocument& document) :
 pDocument(document), pArticle(article), pCurrentTOCItemIndex(0), pH2Index(0), pH3Index(0)
{
}
    ...
    if (!(!pCurrentChart))
    {
        if (tag == "curve")
        {
            ...
            if (!type || (type != "lines" && type != "bars" && type != "points"))
            {
                if (not type.empty())
                    logs.warning()
                      << pArticle.relativeFilename << ": invalid curve type, got '" << type << "'";
        ...
        if (tag == "legend")
        {
            ...
            if (not pos.empty())
            {
                ...
                {
                    logs.warning() << pArticle.relativeFilename
                                   << ": invalid legend position (valid values: ne, nw, se, sw)";
                    return false;
                }
...
void JobWriter::Add(const String& input, const String& htdocs, const ArticleData& article)
{
    ...
    if (!newArticle.wordCount.empty())
    {
        ...
        DocIndex::RegisterWordIDsForASingleArticle(
          articleID, wordIDs, countInArticle, weights, wIx);
...
JobWriter::JobWriter(const String& input, const String& htdocs, const ArticleData& article) :
 pInput(input), pHtdocs(htdocs), pArticle(article), pArticleID(-1)
{
}
...
void JobWriter::prepareVariables(const String& filenameInHtdocs)
{
    ...
    //! @{MODIFIED_ISO8601}
    {
        DateTime::TimestampToString(
          pVars["MODIFIED_ISO8601"], "%Y-%m-%d", pArticle.modificationTime);
    }
    ...
    if (not pArticle.originalFilename.empty())
    {
        ...
        {
            logs.error() << pArticle.relativeFilename << ", l" << doc.ErrorRow() << ": "
                         << doc.ErrorDesc();
            return;
        }
        {
    ...
    // @{URL_PARTS}
    {
        ...
        else
        {
            ...
            {
                address << "<li><a href=\"@{ROOT}/en/documentation/" << pVars["INDEX"]
                        << "\">Documentation</a></li>\n";
                address << "<li><a href=\"@{ROOT}/en/downloads/" << pVars["INDEX"]
                        << "\">Downloads</a></li>\n";
                address << "<li><a href=\"@{ROOT}/en/developers/" << pVars["INDEX"]
                        << "\">Developers</a></li>\n";
                address << "<li><a href=\"@{ROOT}/en/links/" << pVars["INDEX"]
                        << "\">Links</a></li>\n";
                address << "<li><a href=\"@{ROOT}/en/contacts/" << pVars["INDEX"]
                        << "\">Contacts</a></li>\n";
            }
        }
    ...
    if (pArticle.tocItems.size() != 0 && pArticle.showTOC)
    {
        ...
        for (unsigned int i = 0; i != pArticle.tocItems.size(); ++i)
        {
            ...
            if (item.level == 2)
            {
                ...
                tmp << "\t\t<li><a href=\"#" << item.hrefID << "\">" << item.caption
                    << "</a></li>\n";
            ...
            else
            {
                ...
                tmp << "\t<li><a href=\"#" << item.hrefID << "\">" << item.caption << "</a>\n";
                hasOL = false;
                if (i + 1 == pArticle.tocItems.size())
                    tmp << "\t</li>\n";
            }
...
void JobWriter::onExecute()
{
            ...
            if (end < content.size())
            {
                if (end - offset < 64)
                {
                    ...
                    else
                    {
                        offset += 2;
                        logs.warning()
                          << pArticle.relativeFilename << ": unknown variable '" << key << "'";
                    }
                    continue;
                }
                else
    ...
    {
        logs.error() << "impossible to generate '" << pArticle.htdocsFilename << "' into '"
                     << filenameInHtdocs << "'";
    }
}
