...
bool XMLVisitor::VisitEnter(const TiXmlElement& element, const TiXmlAttribute* /*attr*/)
{
    ...
    case ArticleData::stNone:
    {
            ...
            else if (tag == "pragma:directoryindex")
            {
                ...
                if (not content.empty())
                {
                    if (content == "nofollow")
                        ...
                        logs.error() << "invalid directory index content flag (expected: 'all', "
                                        "'nofollow' or 'noindex')";
                }
                ...
                if (!string && !content)
                    logs.warning()
                      << pFilename
                      << ": pragma:directoryindex: missing attribute 'src' or 'content'";
            }
        ...
        if (pWithinParagraph
            && pArticle.allowedTagsInParagraph.find(tag) == pArticle.allowedTagsInParagraph.end())
        {
...
void CompileJob::onExecute()
{
    ...
    for (unsigned int i = 0; i != sourceCount; ++i)
    {
        ...
        pArticle.relativeFilename.assign(entry.c_str() + pInput.size() + 1,
                                         entry.size() - pInput.size() - 1);
