...
Program::Program() : printVersion(false)
{
}
...
bool Program::parseCommandLine(int argc, char** argv)
{
    ...
    opts.add(
      ...
      'j',
      "jobs",
      String() << "Number of concurrent jobs (default: " << nbJobs << ", max: " << cpuCount << ')');
    ...
    {
        logs.error()
          << "Please specify an input folder (--input, see --help for more informations)";
        return false;
    }
    if (!htdocs)
    {
        logs.error()
          << "Please specify a htdocs folder (--htdocs, see --help for more informations)";
        return false;
    }
...
bool Program::readProfile(const String& filename)
{
    ...
    while (pProfileFile.readline(buffer))
    {
        ...
        if (key == "[")
        {
            ...
            {
                logs.error()
                  << "invalid profile : the first section must be 'global' for global options";
                return false;
            }
