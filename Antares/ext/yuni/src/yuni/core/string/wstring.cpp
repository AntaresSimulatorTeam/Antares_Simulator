...
namespace Yuni
{
WString::WString(const WString& rhs) : pSize(rhs.pSize)
{
...
void WString::prepareWString(const AnyString& string, bool uncprefix)
{
...
#ifdef YUNI_OS_WINDOWS
    {
        ...
        const int sizeRequired
          = MultiByteToWideChar(CP_UTF8, 0, string.c_str(), string.size(), nullptr, 0);
        if (sizeRequired <= 0)
        {
            clear();
            return;
...
#else
    {
        ...
        memset(&state, '\0', sizeof(state));
        size_t written
          = mbsnrtowcs(pWString + offset, &wcstr, string.size(), pSize - offset, &state);
