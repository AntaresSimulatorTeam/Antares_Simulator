...
inline void Mutex::copy(const Mutex& rhs)
{
    ...
    if (0 == ::pthread_mutexattr_gettype(&rhs.pAttr, &type))
    {
        ...
        {
#if defined(YUNI_OS_DARWIN) or defined(YUNI_OS_FREEBSD) or defined(YUNI_OS_SOLARIS) \
  or defined(YUNI_OS_SUNOS) or defined(YUNI_OS_HAIKU) or defined(YUNI_OS_CYGWIN)
...
Mutex::Mutex(const Mutex& rhs) : NonMovable(rhs)
{
    copy(rhs);
}
