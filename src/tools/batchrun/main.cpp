...
int main(int argc, char* argv[])
{
	...
	// Command Line options
	{
		...
		options.addParagraph(String()
			<< "Antares Batch Simulation Launcher v" << VersionToCString() << "\n");
		// Input
		...
		options.addFlag(optNoOutput, ' ', "no-output", "Do not write the results in the output folder");
		options.addFlag(optYearByYear, ' ', "year-by-year", "Force the writing of the result output for each year");

		options.addParagraph("\nOptimization");
		options.addFlag(optNoTSImport, ' ', "no-ts-import", "Do not import timeseries into the input folder.");
	...
	if (optSolver.empty())
	{
		Solver::Feature feature = (not optSwap)
			? Solver::standard
			...
			if (-1 == system(cmd.c_str()))
			{
				...
				switch (errno)
				{
					...
					case ENOENT:
						...
						break;
					case ENOEXEC:
						logs.error() << "Command-interpreter file has invalid format and is not executable";
						break;
					case ENOMEM:
