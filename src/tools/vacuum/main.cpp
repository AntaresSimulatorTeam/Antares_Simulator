...
static void NotifyStatistics(const String& logprefix, const FSWalker::Statistics& stats, uint64 duration)
{
	String message;
	...
	{
		case 0:  message << "0 file";break;
		case 1:  message << "1 file";break;
		default: message << stats.fileCount << " files";
	}
	message << " and ";
	switch (stats.folderCount)
	{
		case 0:  message << "0 folder";break;
		case 1:  message << "1 folder";break;
		default: message << stats.folderCount << " folders";
	}
	message << " analyzed in ";
	switch (duration)
	{
...
int main(int argc, char** argv)
{
	...
	// Command Line options
	{
		...
		options.addParagraph(Yuni::String()
			<< "Antares Vacuum v" << VersionToCString() << "\n");
		// Input
		options.remainingArguments(optInput);
		// Output
		options.add(optInput, 'i', "input", "Add an folder to clean.");
		options.add(optEachFolderIn, ' ', "each-folder-in", "Add all available folders to clean from a given directory");
		options.add(optMaxDays, 'm', "max-days",
			String("The maximum number of days without modification (default: ")
			<< optMaxDays << " days)");

		bool optDelete = false;
		options.addFlag(optDelete, 'd', "delete", "Destroy all files and folders which are considered as too old (disabled by default)");
	...
	foreach (auto& input, inputFolders)
		...
		logs.warning() << "no directory to analyze";
