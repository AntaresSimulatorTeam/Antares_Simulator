...
static void PrepareTheWork(const String::Vector& outputs, const DataFile::Vector& dataFiles,
	const StudyData::Vector& studydata, const String::Vector& columns)
{
	...
	for (uint indx = 0; indx != (uint) outputs.size(); ++indx)
	{
		...
		logs.debug() << "  " << info.directory() << " : from " << minYear << " to "
...
static void ReadCommandLineOptions(int argc, char** argv)
{
	...
	// Command Line options
	{
		...
		options.add(optTimes,   't', "time",   "add a time interval ('hourly', 'daily', 'weekly', 'monthly', 'annual')");
		...
		options.add(optJobs, 'j', "jobs",
					String() << "The number of jobs to run simultaneously (default: " << optJobs << ", 4 should be a maxmimum due to i/o performance considerations)");
		// --swap
		options.add(optSwap, ' ', "swap-folder",
	...
	for (Output::Vector::iterator i = AllOutputs.begin(); i != end; ++i)
	{
		...
		Output::Ptr output = *i;
		{
			...
			if (!columnCount)
				continue;
			if (columnCount > 1)
				logs.info() << "  checking " << output->path << "  (" << columnCount << " variables)";
			else
		...
		for (; r != rend; ++r)
		{
			...
			for (; t != tend; ++t)
			{
				...
				for (; av != avend; ++av)
				{
					...
					for (uint v = 0; v != allvars.size(); ++v)
					{
						progressBar.message(String() << "Writing results for '" << studyItemName
											<< "' ("
						...
						for (uint y = 0; y != matrix.width; ++y)
						{
							...
							if (column.height)
							{
								...
								else
								{
									...
									{
										logs.error() << "All columns must have the same number of rows ("
											<< column.height << " found, " << requiredHeight << " expected): "
												<< output->path;
