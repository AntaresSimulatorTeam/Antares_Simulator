...
bool LibConfigProgram::parseCommandLine(int argc, char** argv)
{
    ...
    opts.addFlag(pOptPrintCompilerByDefault,
                 ' ',
                 "compiler-default",
                 "Print the compiler used by default and exit");

    // All required modules
    opts.addParagraph("\nModules:");
    opts.add(pOptModules, 'm', "modules", "Add one or several requested modules (*)");
    opts.addFlag(pOptModuleList,
                 ' ',
                 "module-list",
                 "Print all available modules of the selected version and exit (*)");
    opts.addFlag(pOptPrintModulesDeps,
                 ' ',
                 "module-deps",
                 "Print all modules and the dependencies required and exit (*)");

    // Prefix
    opts.addParagraph("\nSearching paths:");
    opts.add(
      pOptPrefix, 'p', "prefix", "Add a prefix folder for searching available versions of libyuni");
    opts.addFlag(pOptNoDefaultPath,
                 ' ',
                 "no-default-path",
                 "Do not use the default paths to find available versions of libyuni");
    opts.addFlag(pOptDefaultPathList,
                 ' ',
                 "default-path-list",
                 "Print the default paths that would be used and exit (empty if 'no-default-path' "
                 "is enabled)");

    opts.addParagraph("\nVersions:");
    opts.addFlag(pOptList,
                 'l',
                 "list",
                 "Print the list of all versions of libyuni which have been found (with complete "
                 "informations) and exit");
    opts.addFlag(pOptListOnlyVersions,
                 ' ',
                 "list-only-versions",
                 "Print the list of all versions of libyuni which have been found and exit");
...
void LibConfigProgram::findRootPath(const char* a0)
{
    ...
    {
        IO::parent_path(pRootPath, argv0);
    }
    else
    {
...
bool LibConfigProgram::displayInformations()
{
    ...
    if (pOptDefaultPathList)
    {
        ...
        for (String::List::const_iterator i = pDefaultPathList.begin(); i != pDefaultPathList.end();
             ++i)
            std::cout << *i << std::endl;
        return true;
    }
...
bool LibConfigProgram::checkForDependencies(LibConfig::VersionInfo::Settings& version)
{
    ...
    for (String::List::const_iterator i = pOptModules.begin(); i != end; ++i)
    {
        if (not isCoreModule(*i)
            && version.modules.end()
                 == std::find(version.modules.begin(), version.modules.end(), *i))
        {
            pExitStatus = 3;
            // if (pOptPrintErrors)
...
bool LibConfigProgram::displayInformationAboutYuniVersion(LibConfig::VersionInfo::Settings& version)
{
    ...
    String::List options;
    if (not version.configFile(options, pOptPrintErrors)
        or not version.parserModulesOptions(options, pOptPrintErrors))
    {
        pExitStatus = 1;
        return false;
    }
...
{
    typedef LibConfig::VersionInfo::Settings::SettingsPerModule::OptionMap::const_iterator
      const_iterator;
...
void LibConfigProgram::createArguments(LibConfig::VersionInfo::Settings& version) const
{
    ...
    if (pOptCxxFlags)
    {
        ...
        for (String::List::const_iterator i = pOptModules.begin(); i != end; ++i)
        {
            LibConfig::VersionInfo::Settings::SettingsPerModule& modSettings
              = version.moduleSettings[*i];
