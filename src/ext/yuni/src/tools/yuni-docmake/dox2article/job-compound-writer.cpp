...
void CompoundWriter::Dispatch()
{
    // Write articles
    {
        ...
        {
        case 0:
            logs.info() << "No article";
            break;
        case 1:
            logs.info() << "writing 1 article";
            break;
        default:
            logs.info() << "writing " << count << " articles";
            break;
        }
    }
    {
...
CompoundWriter::CompoundWriter(Compound::Ptr& compound) : pCompound(compound)
{
}
    ...
    if (file.openRW(filename))
    {
        ...
        if (not pCompound->description.empty())
        {
            fileOut << "<div>" << pCompound->description
                    << "</div><div style=\"margin-top:3em;border:1px solid "
                       "#aaa;border-bottom-style:none;border-left-style:none;border-right-style:"
                       "none\"></div>\n";
        }
...
void CompoundWriter::prepareClassSubtitle(const Section& section)
{
    ...
    if (not section.caption.empty())
    {
        ...
        subtitle << "<h3 class=\"doxygen_section\">" << sectionName
                 << " <code class=\"doxygen_visibility\">" << visibility << "</code></h3>\n";
    }
    else
    {
        sectionName.clear();
        subtitle << "<h3 class=\"doxygen_section\">" << visibility
                 << " <code class=\"doxygen_visibility\">" << visibility << "</code></h3>\n";
    }
    subtitle << "</td></tr>\n";
}
...
void CompoundWriter::appendClassSection(const Section& section, bool isAbstract)
{
    ...
    for (uint j = 0; j != memcount; ++j)
    {
        ...
        {
        case kdFunction:
            out << "<td class=\"doxygen_fun\">";
            break;
        case kdTypedef:
            out << "<td class=\"doxygen_typedef\">";
            break;
        case kdVariable:
            out << "<td class=\"doxygen_var\">";
            break;
        case kdEnum:
            out << "<td class=\"doxygen_enum\">";
            break;
        default:
            out << "<td>";
            break;
...
void CompoundWriter::appendClassFunction(const Member& member, bool isPublic)
{
    ...
    if (name.first() == '~')
    {
        String t = name;
        t.replace("~", "<b> ~ </b>");
        out << " <span class=\"method\"><a name=\"" << member.htmlID << "\" href=\"#\">"
            << umlSymbol << ' ' << t << "</a></span>";
        outIx << " <span class=\"method\"><a href=\"#" << member.htmlID << "\">" << t
              << "</a></span>";
    }
    else
    {
        out << " <span class=\"method\"><a name=\"" << member.htmlID << "\" href=\"#\">"
            << umlSymbol << ' ' << name << "</a></span>";
        outIx << " <span class=\"method\"><a href=\"#" << member.htmlID << "\">" << name
              << "</a></span>";
    }
...
void CompoundWriter::appendClassTypedef(const Member& member, bool isPublic)
{
    ...
    if (outIx.empty())
        outIx << "<table class=\"nostyle\">";
    out << "<span class=\"method\"><a name=\"" << member.htmlID << "\" href=\"#\">" << umlSymbol
        << ' ' << name << "</a></span>"
        << ": <span class=\"keyword\">typedef</span> " << type << ";\n";
    outIx
      << "<tr><td class=\"doxygen_index\"><code><span class=\"keyword\">typedef</span></code></td>"
      << "<td class=\"doxygen_index_def\"><code><span class=\"method\"><a name=\"#" << member.htmlID
      << "\" href=\"#\">" << name << "</a></span> : " << type << "</code></td></tr>\n";
}
...
void CompoundWriter::appendClassIndex(Clob& output,
                                      bool isPublic,
