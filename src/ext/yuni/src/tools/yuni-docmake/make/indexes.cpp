...
static sint64 RetrieveArticleID(const StringT& href)
{
    ...
    sqlite3_stmt* stmt;
    if (SQLITE_OK
        != sqlite3_prepare_v2(gDB, "SELECT id FROM articles WHERE html_href = $1", -1, &stmt, NULL))
        return -1;
...
Yuni::sint64 ArticleLastModificationTimeFromCache(const String& filename)
{
    ...
    if (SQLITE_OK
        != sqlite3_prepare_v2(
          gDB, "SELECT modified FROM articles WHERE rel_path = $1", -1, &stmt, 0))
        return -1;
...
void Write(ArticleData& article)
{
    ...
    {
        sqlite3_bind_text(
          stmt, 1, article.relativeFilename.c_str(), article.relativeFilename.size(), NULL);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
    }
    ...
    {
        query.clear()
          << "INSERT INTO articles (parent_order, weight, show_quick_links"
          << ", show_history"
        ...
        else
        {
            ...
            sqlite3_bind_text(
              stmt, 7, article.relativeFilename.c_str(), article.relativeFilename.size(), NULL);
            sqlite3_bind_text(
              stmt, 8, article.htdocsFilename.c_str(), article.htdocsFilename.size(), NULL);
    ...
    if (not article.accessPath.empty())
    {
        ...
        else
        {
            ...
            sqlite3_bind_text(
              stmt, 2, article.relativeFilename.c_str(), article.relativeFilename.size(), NULL);
            sqlite3_step(stmt);
            sqlite3_finalize(stmt);
        }
    }
...
// We want UNIX-style paths
    ...
    if (!article.tocItems.empty())
    {
        for (unsigned int i = 0; i != article.tocItems.size(); ++i)
        {
            ...
            sqlite3_bind_text(
              stmt, 1, article.htdocsFilename.c_str(), article.htdocsFilename.size(), NULL);
    ...
    if (!article.tags.empty())
    {
        ...
        for (Dictionary::TagSet::const_iterator i = article.tags.begin(); i != end; ++i)
        {
            ...
            query.clear() << "INSERT INTO tags_per_article (article_id,tagname) VALUES ("
                          << article.id << ",$1);";
            int error;
            if (SQLITE_OK
                == (error = sqlite3_prepare_v2(gDB, query.c_str(), query.size(), &stmt, NULL)))
            {
                ...
                logs.error() << "impossible to register tag for " << article.htdocsFilename
                             << ", err." << error;
        }
    }
}
...
void RemoveNonExistingEntries()
{
    ...
    if (SQLITE_OK
        != sqlite3_get_table(
          gDB, "SELECT rel_path FROM articles", &result, &rowCount, &colCount, NULL))
        return;
...
bool AppendArticleTitleFromPath(String& out, const String& path)
{
    ...
    if (SQLITE_OK
        != sqlite3_prepare_v2(
          gDB, "SELECT title FROM articles WHERE html_href = $1", -1, &stmt, NULL))
        return false;
...
void BuildSitemap()
{
    ...
    const char* const query
      = "SELECT html_href,weight,modified FROM articles WHERE weight > 0.2 ORDER BY html_href";
    if (SQLITE_OK == sqlite3_get_table(gDB, query, &result, &rowCount, &colCount, NULL))
    {
        String tmp;
...
static int TryToFindWordID(const Dictionary::Word& term)
{
    sqlite3_stmt* stmt;
    if (SQLITE_OK
        != sqlite3_prepare_v2(gDB, "SELECT id FROM terms WHERE term = $1", -1, &stmt, NULL))
        return -1;
...
void RegisterWordIDsForASingleArticle(ArticleID articleid,
                                      const int* termid,
                                      ...
                                      unsigned int count)
{
    ...
    for (unsigned int i = 0; i != count; ++i)
    {
        query << "INSERT INTO terms_per_article (term_id,article_id,count_in_page,weight) VALUES ("
              << termid[i] << ',' << articleid << ',' << countInArticle[i] << ',' << weights[i]
              << ");\n";
    }
    query << "COMMIT;\n";
...
{
    const char* const query = "SELECT SUM(count_in_page) AS s from terms_per_article GROUP BY "
                              "term_id ORDER BY s DESC LIMIT 1;";
    sqlite3_stmt* stmt;
...
static void UpdateAllRelativeTermWeight(uint64 maxO)
{
    ...
    int rowCount, colCount;
    const char* const query
      = "SELECT term_id, SUM(count_in_page) FROM terms_per_article GROUP BY term_id;";
    if (SQLITE_OK != sqlite3_get_table(gDB, query, &result, &rowCount, &colCount, NULL))
        return;
...
void BuildSEOArticlesReference()
{
    ...
    if (SQLITE_OK
        != sqlite3_prepare_v2(
          gDB, "SELECT id,html_href,title,weight FROM articles;", -1, &stmt, NULL))
        return;
...
void AppendChildrenList(Yuni::String& text, const String& path, const String& current)
{
    ...
    if (SQLITE_OK
        != sqlite3_prepare_v2(gDB,
                              "SELECT title,html_href FROM articles WHERE parent = $1 ORDER BY "
                              "parent_order,LOWER(title)",
                              -1,
                              &stmt,
                              NULL))
        return;
...
void RetrieveTagList(ArticleData& article)
{
    ...
    if (SQLITE_OK
        != sqlite3_prepare_v2(
          gDB, "SELECT tagname FROM tags_per_article WHERE article_id = $1", -1, &stmt, NULL))
        return;
