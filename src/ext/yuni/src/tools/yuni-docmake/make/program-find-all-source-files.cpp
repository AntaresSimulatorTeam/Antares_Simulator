...
void Program::findAllSourceFiles()
{
    ...
    for (IO::Directory::Info::recursive_iterator i = info.recursive_begin();
         i != info.recursive_end();
         ++i)
    {
        // assert
        assert(!(!(*i)));
        ...
        else
        {
            ...
            if (!Program::clean)
            {
                ...
                relative.assign(filename.c_str() + input.size() + 1,
                                filename.size() - input.size() - 1);
                sint64 lastWriteFromCache
                  = DocIndex::ArticleLastModificationTimeFromCache(relative);
