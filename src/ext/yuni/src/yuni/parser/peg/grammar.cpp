...
public:
    SubRulePart() : type(rtRule), line((uint)-1)
    {
    }

    SubRulePart(const NullPtr&) : type(rtRule), line((uint)-1)
    {
    }
...
private:
    ...
    bool prepareRuleIdentifierName(AnyString& line,
                                   uint lineIndex,
                                   VectorPairYAndLine& inlinePragmas);
    bool prepareNodeFromPragmas(const AnyString& source,
                                const AnyString& rulename,
                                Node& node,
                                const VectorPairYAndLine& pragmas);
...
static inline void RuleTypeToString(StreamT& out, RuleType type)
{
    switch (type)
    {
    case rtRule:
...
inline RuleParser::RuleParser(Grammar& grammar, Node::Map& rules) :
 grammar(grammar), grammarRules(rules)
{
}
...
inline bool RuleParser::prepareRuleIdentifierName(AnyString& line,
                                                  uint lineIndex,
                                                  VectorPairYAndLine& inlinePragmas)
{
...
inline bool RuleParser::checkForRulesExistence() const
{
    ...
    for (std::map<AnyString, bool>::const_iterator i = unusedList.begin(); i != unusedList.end();
         ++i)
    {
        if (not i->second)
...
inline bool RuleParser::prepareNodeFromPragmas(const AnyString& source,
                                               const AnyString& rulename,
                                               Node& node,
                                               const VectorPairYAndLine& pragmas)
{
    ...
    for (uint i = 0; i != pragmas.size(); ++i)
    {
        ...
        if (list[0] == "hidden")
        {
            ...
            if (rulename == "start")
                error(errmsg.clear()
                      << source << ':' << line << ": the rule 'start' can not be hidden");
            continue;
        }
        if (list[0] == "capture")
        {
...
inline bool RuleParser::prepareNodeFromSubrules(const String& rulename,
                                                Node& node,
                                                SubRulePart::Vector& rules)
{
    if (rules.empty())
        return true;
    ...
    for (uint i = 0; i != rules.size(); ++i)
    {
        ...
        case rtModifier:
        {
            ...
            {
                errmsg.clear() << source << ':' << firstLine
                               << ": repeat pattern without content: " << text;
                return error(errmsg);
            }
    ...
    do
    {
        ...
        if (not AnyString::IsSpace(line[0]))
        {
            if (not line.startsWith("//"))
            {
                ...
                {
                    errmsg.clear() << source << ':' << lineIndex << ": rule '" << currentRuleName
                                   << "' already exists";
                    return error(errmsg);
                }
...
bool RuleParser::analyzeEachRule()
{
    ...
    for (; i != end; ++i)
    {
        ...
        for (; si != send; ++si)
        {
                        ...
                        if (c == stringQuote)
                        {
                            ...
                            else
                            {
                                if (not newsubrules.back().empty())
                                {
                                    ...
                                    warns(errmsg.clear()
                                          << source << ':' << lineIndex << ": empty string at "
                                          << instringStartUtf8);
                    ...
                    default:
                    {
                        if (hasBackslash)
                        {
                            ...
                            case 'n':
                                ...
                                break;
                            case 'r':
                                newsubrules.back().text += '\r';
                                break;
                            case 'v':
                                newsubrules.back().text += '\v';
                                break;
                            case 'f':
                                newsubrules.back().text += '\f';
                                break;
                            default:
                            {
                                error(errmsg.clear()
                                      << source << ':' << lineIndex
                                      << ": invalid escape sequence at " << offsetutf8);
                                return false;
                            }
                            }
                            hasBackslash = false;
                        ...
                        else
                        {
                            if (--bracketDepth == 0)
                            {
                                if (not newsubrules.back().empty())
                                {
                                    ...
                                    warns(errmsg.clear()
                                          << source << ':' << lineIndex
                                          << ": empty list of chars at " << instringStartUtf8);
            ...
            if (state == stInString)
            {
                error(errmsg.clear() << source << ':' << lineIndex
                                     << ": unfinished string at offset " << instringStartUtf8);
                localerror = true;
                break;
            }
            ...
            {
                error(errmsg.clear()
                      << source << ':' << lineIndex << ": unfinished list of char at offset "
