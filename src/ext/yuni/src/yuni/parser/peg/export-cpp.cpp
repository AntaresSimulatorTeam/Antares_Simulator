...
inline void CPPConverter::generateH()
{
    ...
    h << '\n';
    h << '\n';
    h << "	//! Get if the rule should be ignored when duplucating an AST (starting from 'tk-' "
    ...
    h << '\n';
    h << '\n';
    h << "	class YUNI_DECL Node final : public Yuni::IIntrusiveSmartPtr<Node, false, "
         "Yuni::Policy::SingleThreaded>\n";
    h << "	{\n";
    h << "	public:\n";
    h << "		//! Ancestor\n";
    h << "		typedef Yuni::IIntrusiveSmartPtr<Node, false, "
         "Yuni::Policy::SingleThreaded> inherited;\n";
    h << "		//! Threading policy\n";
    ...
    h << "		static void Export(Yuni::Clob& out, const Node& node, bool color, "
         "ExportCallback callback = nullptr);\n";
    ...
    h << "		//! Export the tree node into a JSON object (with callback for additional "
         "data)\n";
    h << "		static void ExportToJSON(Yuni::Clob& out, const Node& node, void "
         "(*callback)(Yuni::Dictionary<AnyString, YString>::Unordered&, const Nany::Node&));\n";
    h << '\n';
    ...
    h << '\n';
    h << "		template<class StringT> bool extractFirstChildText(StringT& out, enum "
         "ASTRule rule) const;\n";
    h << '\n';
    h << "		template<class StringT> bool extractChildText(StringT& out, enum ASTRule "
         "rule, const AnyString& separator = nullptr) const;\n";
    h << '\n';
    ...
    h << "	{\n";
    h << "	public:\n";
    h << "		typedef Yuni::Bind<bool (Yuni::Clob& out, const AnyString& uri)>   "
         "OnURILoading;\n";
    h << "		typedef Yuni::Bind<bool (const AnyString& filename, uint line, uint "
         "offset, Error, const YString::Vector&)>  OnError;\n";
    h << '\n';
    h << "	public:\n";
    h << "		Parser();\n";
    h << "		~Parser();\n";
...
inline void CPPConverter::generateHXX()
{
    ...
    hxx << "		if (metadata)\n";
    hxx << "		{\n";
    hxx << "			assert(metadataRelease != nullptr and \"invalid callback for "
           "releasing metadata\");\n";
    ...
    hxx << '\n';
    hxx << '\n';
    hxx << "	template<class StringT> inline bool Node::extractFirstChildText(StringT& out, enum "
           "ASTRule rule) const\n";
    hxx << "	{\n";
    hxx << "		for (unsigned int i = 0; i != (unsigned) children.size(); ++i)\n";
    hxx << "		{\n";
    ...
    hxx << '\n';
    hxx << '\n';
    hxx << "	template<class StringT> inline bool Node::extractChildText(StringT& out, enum "
           "ASTRule rule, const AnyString& separator) const\n";
    hxx << "	{\n";
    ...
    hxx << "						for (unsigned int j = 0; j != (unsigned) "
           "subnode.children.size(); ++j)\n";
    hxx << "						{\n";
    hxx << "							if (subnode.children[j]->rule == "
           "*it)\n";
    hxx << "							{\n";
    hxx << "								result = "
           "subnode.children[j];\n";
...
static inline void GenerateFunctionForEachRule(Clob& cpp,
                                               uint& sp,
                                               const Node::Map& rules,
...
inline void CPPConverter::generateCPP()
{
    ...
    for (Node::Map::const_iterator i = rules.begin(); i != end; ++i)
        cpp << "		" << (i->second.attributes.capture ? "true" : "false") << ", // "
            << i->second.enumID << '\n';
    cpp << "		false, // rgEOF\n";
    cpp << "	};\n";
    cpp << '\n';
    ...
    cpp << "		false, // rgUnknown\n";
    for (Node::Map::const_iterator i = rules.begin(); i != end; ++i)
        cpp << "		" << (i->second.attributes.important ? "true" : "false") << ", // "
            << i->second.enumID << '\n';
    cpp << "		false, // rgEOF\n";
    cpp << "	};\n";
    cpp << '\n';
    ...
    cpp << '\n';
    cpp << '\n';
    cpp << "	void Node::Export(Clob& out, const Node& node, bool color, ExportCallback "
           "callback)\n";
    cpp << "	{\n";
    cpp << "		assert(&node and \"invalid reference to node\");\n";
    cpp << '\n';
    ...
    cpp << '\n';
    cpp << '\n';
    cpp << "	void Node::ExportToJSON(Yuni::Clob& out, const Node& node, void "
           "(*callback)(Yuni::Dictionary<AnyString, YString>::Unordered&, const Nany::Node&))\n";
    cpp << "	{\n";
    cpp << "		String tmp;\n";
    cpp << "		String indent;\n";
    ...
    cpp << "		if (rhs.metadata)\n";
    cpp << "		{\n";
    cpp << "			assert(metadataClone != nullptr and \"invalid callback for cloning "
           "metadata\");\n";
    ...
    cpp << "			for (unsigned int i = 0; i != (unsigned) rhs.children.size(); "
           "++i)\n";
    ...
    cpp << "		if (metadata)\n";
    cpp << "		{\n";
    cpp << "			assert(metadataRelease != nullptr and \"invalid callback for "
           "releasing metadata\");\n";
