...
{
    AutoReset(bool enabled, Clob& out, uint& depth) : enabled(enabled), out(out), depth(depth)
    {
        if (enabled)
        {
            out << "\n";
...
static inline void PrintSubNodesLinks(Clob& out,
                                      ...
                                      bool inverseColor)
{
    ...
    if (not node.children.empty())
    {
        ...
        default:
        {
            ...
            for (uint i = 1; i < (uint)node.children.size(); ++i)
                PrintSubNodesLinks(
                  out, rules, node.children[i], node.children[i - 1].id, not inverseColor);
        }
        }
...
bool Node::checkRules(AnyString& error,
                      const Node::Map& rules,
                      std::map<AnyString, bool>& unusedList) const
{
    if (rule.type == asRule)
    {
                     ...
                     uint& sp) const
{
    ...
    case Node::asAND:
    {
        ...
        else
        {
            ...
            for (uint i = 0; i != (uint)children.size(); ++i)
            {
                ...
                helpers[hi] << "\tstatic inline bool __helper" << (from + i)
                            << "(Datasource& ctx)\n";
                helpers[hi] << "\t{\n";
    ...
    case Node::asOR:
    {
        ...
        {
            PrintTabs(out, d) << "{\n";
            PrintTabs(out, d) << "\tassert(sp" << osp
                              << " < ctx.offset and \"invalid offset if\");\n";
    ...
    assert(out.size()
           < 1024 * 1024 * 100); // arbitrary - consistency check - 100MiB should be enough
    ...
    if (match.min == 0 and match.max == 1)
    {
        if (not safeFromComplexity and attributes.canEat)
        {
            ...
            PrintTabs(out, d) << "{\n";
            PrintTabs(out, d) << "\tassert(sp" << osp
                              << " < ctx.offset and \"invalid offset match ?\");\n";
            PrintTabs(out, d) << "\tctx.offset = sp" << osp
                              << "; // ignore the results if it does not match\n";
            PrintTabs(out, d) << "}\n";
        }
        else
        {
            // others are guaranted to not modify the stack
            PrintTabs(out, d) << "(" << stmt
                              << "); // ? - ignore the result if it does not match\n";
        }
        return;
    }
        ...
        if (not safeFromComplexity)
        {
            ...
            else
            {
                ...
                if (attributes.canEat)
                {
                    PrintTabs(out, d) << "{\n";
                    PrintTabs(out, d) << "\tassert(sp" << osp
                                      << " < ctx.offset and \"invalid offset match min == 1\");\n";
                    PrintTabs(out, d) << "\tctx.offset = sp" << osp << ";\n";
                    PrintTabs(out, d) << "\t" << returnfalse << '\n';
                    PrintTabs(out, d) << "}\n";
                }
