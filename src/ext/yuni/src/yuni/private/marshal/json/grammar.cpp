...
{
#ifndef NDEBUG
    std::cout << "   [!!] node enter: uncaught symbol " << node.symbolName
              << "  (enum: " << (uint)node.symbol << ")\n";
#endif
}
...
{
#ifndef NDEBUG
    std::cout << "   [!!] node leave: uncaught symbol " << node.symbolName
              << "  (enum: " << (uint)node.symbol << ")\n";
#endif
}
...
public:
    ...
    {
    public:
        Stack() : lalr((uint)-1), next(nullptr)
        {
        }
...
Internal::Internal(EventError& error, const AnyString& content) :
 grammar(Private::JSON::grammar), content(content), notifyError(error)
{
    cursor.line = 1;
    cursor.column = 1;
...
void Internal::prepareDiagnostic(String& diagnostic, const Cursor& cursor, uint tokenlength) const
{
    ...
    while (previousLinefeed < content.size()
           and (content[previousLinefeed] == '\t' or content[previousLinefeed] == ' '))
        ++previousLinefeed;
...
bool Internal::error(Error error, const Stack* current) const
{
    ...
    if (current)
    {
        ...
        return notifyError(error,
                           current->token.text,
                           diagnostic,
                           current->token.cursor.line,
                           current->token.cursor.column);
    }
    else
    {
    ...
    while (data.offset + length < content.size())
    {
        ...
        if (symbolLength == 1)
        {
            for (uint i = 0; i != stateCount; ++i)
            {
                ...
                AnyString adapter(grammar.charsets.table[charsetIndex].ascii,
                                  grammar.charsets.table[charsetIndex].asciiSize);
...
Private::JSON::LALR::ActionType Internal::parseToken(Stack& current)
{
    ...
    if (trim.reductions and grammar.productions.table[rule].symbolCount == 1
        and grammar.symbols.table[grammar.productions.table[rule].symbols[0]].kind
              == Private::JSON::Symbols::kdNonTerminal)
    {
    ...
    do
    {
        ...
        if (kind == Private::JSON::Symbols::kdTerminal or kind == Private::JSON::Symbols::kdEnd)
        {
            ...
            if ((uint)action >= Private::JSON::LALR::accept)
            {
                ...
                {
                    if (grammar.symbols.table[(uint)current->token.symbol].kind
                        == Private::JSON::Symbols::kdEnd)
                    {
