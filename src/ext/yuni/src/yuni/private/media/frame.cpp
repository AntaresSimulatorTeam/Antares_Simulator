...
Frame::Frame(uint index, double pts) : pIndex(index), pTimestamp(pts), pImpl(new FrameImpl())
{
}
...
bool Frame::valid() const
{
    ...
    return pImpl->frame and pImpl->frame->linesize[0] > 0 and pImpl->frame->linesize[1] > 0
           and pImpl->frame->linesize[2] > 0;
}
