...
{
#ifndef YUNI_OS_MSVC
    return (*argv == '-') ? ((argv[1] == '-') ? ttLongOption : ttShortOption) : ttParameter;
#else
    return (*argv == '-' or *argv == '/') ? ((argv[1] == '-') ? ttLongOption : ttShortOption)
                                          : ttParameter;
#endif
}
...
bool Context::findNextParameter(IOption* option, int argc, char* argv[])
{
    ...
    while ((++pParameterIndex) < argc)
    {
        ...
        if (ttParameter == GetTokenType(argv[pParameterIndex]))
        {
            ...
            option->addValue(argv[pParameterIndex],
                             static_cast<String::size_type>(::strlen(argv[pParameterIndex])));
