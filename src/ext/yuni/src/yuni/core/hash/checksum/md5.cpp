...
{
    MD5TypeUInt32 a = pms->abcd[0], b = pms->abcd[1], c = pms->abcd[2], d = pms->abcd[3];
    MD5TypeUInt32 t;
#if BYTE_ORDER > 0
...
#define xbuf X // (static only)
            ...
            for (uint i = 0; i != 16; ++i, xp += 4)
                xbuf[i] = static_cast<MD5TypeUInt32>(xp[0] + (xp[1] << 8) + (xp[2] << 16)
                                                     + (xp[3] << 24));
        }
#endif
    }
...
const String& MD5::fromFile(const String& filename)
{
    ...
    if (stream.open(filename))
    {
        ...
        while (0 != (len = stream.read((char*)buffer, 1024u)))
            md5ImplAppend(
              &state, reinterpret_cast<const MD5TypeByte*>(buffer), static_cast<uint>(len));
