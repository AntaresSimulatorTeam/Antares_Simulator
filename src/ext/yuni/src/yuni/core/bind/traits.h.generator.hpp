...
<% require File.dirname(__FILE__) + '/../../../tools/generators/commons.rb' generator
 = Generator.new() %><%= generator.thisHeaderHasBeenGenerated("traits.h.generator.hpp") %>
...
{
    // Forward declaration
    template<typename P, class Dummy>
    class Bind;
...
*/
template<class U, class C, class P>
class BoundWithSmartPtrMemberAndUserData;
...
<% (0..i-1).each do |j| %> // Argument <%=j%>
	...
	struct Argument<R(<%=generator.list(i)%>), <%=j%>>
{
    typedef A<%= j %> Type;
};
<% end end %>
  ...
  struct Parameter
{
    typedef const typename Static::Remove::Const<typename Static::Remove::RefOnly<T>::Type>::Type&
      Type;
};
...
public:
    ...
    BoundWithFunction(R (*pointer)(<%= generator.list(i) %>)) : pPointer(pointer)
    {
...
public:
    ...
    {
        return (*pPointer)(<%= generator.list(i - 1, 'a', "", ", ")
    ...
    virtual bool compareWithPointerToFunction(
      R (*pointer)(<%= generator.list(i - 1) %>)) const override
    {
        return (reinterpret_cast<void*>(pPointer) == reinterpret_cast<void*>(pointer));
    }
...
public:
    ...
    //@{
    //! Constructor
    BoundWithMember(C* c, R (C::*member)(<%= generator.list(i) %>)) : pThis(c), pMember(member)
    {
...
public:
    ...
    virtual R invoke(<%= generator.variableList(i - 1) %>) const override
    {
        return (pThis->*pMember)(
          <%= generator.list(i - 1, 'a', "", ", ") %> const_cast<UserDataTypeByCopy&>(pUserdata));
    }
...
public:
    ...
    virtual const IEventObserverBase* observerBaseObject() const override
    {
        return Static::DynamicCastWhenInherits<C, IEventObserverBase>::PerformConst(
          pThis.pointer());
    }
