...
{
template<typename T>
inline Vector3D<T> Vector3D<T>::Mean(const Vector3D<T>& p1, const Vector3D<T>& p2)
{
    return Vector3D<T>().mean(p1, p2);
}

template<typename T>
inline Vector3D<T>::Vector3D() : x(), y(), z()
{
}

template<typename T>
inline Vector3D<T>::Vector3D(const Vector3D<T>& rhs) : x(rhs.x), y(rhs.y), z(rhs.z)
{
}
...
inline Vector3D<T>::Vector3D(const U& x1, const V& y1, const W& z1) :
 x(static_cast<T>(x1)), y(static_cast<T>(y1)), z(static_cast<T>(z1))
{
}
...
inline Vector3D<T> Vector3D<T>::CrossProduct(const Point3D<U>& p1,
                                             const Point3D<V>& p2,
                                             const Point3D<W>& p3)
{
