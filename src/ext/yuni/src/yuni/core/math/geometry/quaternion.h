...
public:
    ...
    */
    ...
    //! Point rotation
    template<class U>
    static Point3D<T> Rotate(const Point3D<T>& p,
...
public:
    ...
    //! From glm quaternion
    Quaternion(const glm::quat& quat) : w(quat.w), v(quat.x, quat.y, quat.z)
    {
    }
#endif
    //@}
...
template<class T>
inline const Yuni::Quaternion<T> operator+(const Yuni::Quaternion<T>& lhs,
                                           const Yuni::Quaternion<T>& rhs)
{
