...
public:
    ...
    */
    template<class U, class V, class W>
    void move(const U x1, const V y1, const W z1)
    {
        x = (T)x1;
        y = (T)y1;
        z = (T)z1;
    ...
    */
    template<class U>
    void translate(const Point3D<U>& p)
    {
        x += (T)p.x;
        y += (T)p.y;
        z += (T)p.z;
    ...
    */
    ...
    {
        return Math::Abs(x - rhs.x) < delta and Math::Abs(y - rhs.y) < delta
               and Math::Abs(z - rhs.z) < delta;
    }
    ...
    */
    template<class U>
    void operator()(const Point3D<U>& p)
    {
        x = (T)p.x;
        y = (T)p.y;
        z = (T)p.z;
    ...
    */
    template<class U>
    bool operator==(const Point3D<U>& rhs) const
    {
        return Math::Equals<T>(rhs.x, x) and Math::Equals<T>(rhs.y, y)
               and Math::Equals<T>(rhs.z, z);
    }
