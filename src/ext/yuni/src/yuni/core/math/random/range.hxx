...
template<class D, int LoValue, int HiValue, typename T>
inline const T Range<D, LoValue, HiValue, T>::min()
{
    return T(LoValue);
}

template<class D, int LoValue, int HiValue, typename T>
inline const T Range<D, LoValue, HiValue, T>::max()
{
    return T(HiValue);
}

template<class D, int LoValue, int HiValue, typename T>
inline void Range<D, LoValue, HiValue, T>::reset()
{
    pDistribution.reset();
}
...
struct AlgorithmFromType<D, LoValue, HiValue, float>
{
    ...
    {
        return float(float(d.next()) / ((float(d.max()) + 1.) / float(HiValue - LoValue))
                     + float(LoValue));
    }
};
...
struct AlgorithmFromType<D, LoValue, HiValue, long double>
{
    ...
    {
        return (long double)(d.next()) / (((long double)(d.max()) + 1.) / (long double)(HiValue))
               + (long double)(LoValue);
    }
};
...
template<class D, int LoValue, int HiValue, typename T>
inline const T Range<D, LoValue, HiValue, T>::next()
{
