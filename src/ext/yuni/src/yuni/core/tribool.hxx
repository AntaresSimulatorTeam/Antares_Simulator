...
{
    return (indeterminate()) ? (0 != pValue.flags[1]) // using the default value
                             : (0 != pValue.flags[0]);
}
...
inline void Tribool::print(StreamT& out) const
{
...
public:
    ...
    static bool Perform(const StringT& s, Yuni::Tribool& out)
    {
        AnyString s1 = s;
        s1.trim();
    ...
    template<class StringT>
    static Yuni::Tribool Perform(const StringT& s)
    {
        Tribool out;
        Perform(s, out);
        return out;
