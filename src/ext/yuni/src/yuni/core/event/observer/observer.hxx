...
inline Observer<D, TP>::~Observer()
{
    ...
    assert((pCanObserve == false)
           and "All ancestor of the class `Yuni::Observer` must call destroyingObserver() in their "
               "destructor");
}

template<class D, template<class> class TP>
void Observer<D, TP>::destroyingObserver()
{
    // Lock
...
template<class D, template<class> class TP>
void Observer<D, TP>::disconnectAllEventEmitters()
{
    // Disconnecting from all events
    typename ThreadingPolicy::MutexLocker locker(*this);
    if (not pEvents.empty())
...
template<class D, template<class> class TP>
void Observer<D, TP>::disconnectEvent(const IEvent* event)
{
    if (event)
    {
        // Lock
...
template<class D, template<class> class TP>
void Observer<D, TP>::internalAttachEvent(IEvent* evt)
{
    typename ThreadingPolicy::MutexLocker locker(*this);
...
template<class D, template<class> class TP>
inline void Observer<D, TP>::internalDetachEvent(const IEvent* evt)
{
    typename ThreadingPolicy::MutexLocker locker(*this);
