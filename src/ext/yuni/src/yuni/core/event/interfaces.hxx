...
inline IEventObserver<Derived, TP>::IEventObserver() : pBoundEventTable(new IEvent::Map())
{
}
...
inline IEventObserver<Derived, TP>::~IEventObserver()
{
    ...
    assert((NULL == pBoundEventTable)
           and "IEventObserver: The derived class must call `destroyBoundEvents()` by itself to "
               "prevent a corrupt vtable when destroying the object");
}
