...
void Event<P>::connect(C* o, typename Event<P>::template PointerToMember<C>::Type method)
{
    if (o)
    {
        Bind<P> b;
        b.bind(o, method);
        assert(b.isDescendantOfIEventObserverBase()
               and "Invalid class C. The calling class must inherit from Event::Observer<CRTP, "
                   "ThreadingPolicy>");
...
// list + increment ref counter
        ...
        (dynamic_cast<const IEventObserverBase*>(o))
          ->boundEventIncrementReference(dynamic_cast<IEvent*>(this));
        AncestorType::pEmpty = false;
        // Unlocking
    }
}
...
void Event<P>::remove(const U* object)
{
    if (object)
    {
        ...
        if (base)
        {
            ...
            if (not AncestorType::pBindList.empty())
            {
                typedef Yuni::Private::EventImpl::template PredicateRemoveObserverBase<
                  typename AncestorType::BindType>
                  RemoveType;
        ...
        else
        {
            ...
            if (not AncestorType::pBindList.empty())
            {
                typedef Yuni::Private::EventImpl::template PredicateRemoveObject<
                  typename AncestorType::BindType>
                  RemoveType;
...
void Event<P>::unregisterObserver(const IEventObserverBase* pointer)
{
    ...
    if (not AncestorType::pBindList.empty())
    {
        typedef Yuni::Private::EventImpl::template PredicateRemoveWithoutChecks<
          typename AncestorType::BindType>
          RemoveType;
