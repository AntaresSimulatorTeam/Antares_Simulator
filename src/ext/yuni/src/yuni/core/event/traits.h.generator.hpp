  ...
  namespace Yuni
{
    ...
    {
        typedef FoldTypeVoid value_type;
    };
...
<% (0..generator.argumentCount).each do |i| %>
    ...
    public:
        ...
        <% (0..i - 1).each do | j | %>
          ...
          typedef typename BindType::template Argument <<
          %= j % >> ::Type A<%= j %>;
        <% end %>
        ...
        */
        ...
        R fold(typename FoldType<R>::value_type initval,
               const CallbackT& accumulator<%= generator.variableList(i, "A", "a", ", ") %>) const
        {
            if (not pEmpty)
            {
        ...
        template<template<class> class PredicateT>
        typename PredicateT<R>::ResultType invoke(
          PredicateT<R> & predicate<%= generator.variableList(i, "A", "a", ", ") %>) const
        {
            if (not pEmpty)
            {
                typename ThreadingPolicy::MutexLocker locker(*this);
