...
struct Model
{
    ...
    protected:
        NullIterator(const StringType& s) : pString(s)
        {
        }

        NullIterator(const NullIterator& copy) : pString(copy.pString)
        {
        }

        template<class ModelT>
        NullIterator(const ModelT& copy) : pString(copy.string())
        {
        }
    ...
    public:
        ...
        */
        Size offset() const
        {
            return (pOffset >= pString.size()) ? (pString.size() - pChar.size())
                                               : (pOffset - pChar.size());
        }
    ...
    protected:
        ...
        explicit UTF8Iterator(const StringType& s) : pOffset(0), pString(const_cast<StringType&>(s))
        {
        }
    ...
    protected:
        ...
        explicit ByteIterator(const StringType& s) : pOffset(0), pString(const_cast<StringType&>(s))
        {
        }
        ByteIterator(const ByteIterator& copy) :
         pOffset(copy.rawOffset()), pString(const_cast<StringType&>(copy.pString))
        {
        }
        ByteIterator(const UTF8Iterator& copy) :
         pOffset(copy.offset()), pString(const_cast<StringType&>(copy.string()))
        {
        }
