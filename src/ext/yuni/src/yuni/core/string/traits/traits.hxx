 ...
 size(rhs.size), capacity(rhs.size), data(NULL)
{
    if (size)
    {
        if (chunkSize != 0)
        {
            ...
            YUNI_MEMCPY(const_cast<void*>(static_cast<const void*>(data)),
...
template<uint ChunkSizeT, bool ExpandableT>
inline void Data<ChunkSizeT, ExpandableT>::adapt(const char* const cstring)
{
...
template<uint ChunkSizeT, bool ExpandableT>
inline void Data<ChunkSizeT, ExpandableT>::adapt(const char* const cstring, Size length)
{
    data = const_cast<char*>(cstring);
...
template<uint ChunkSizeT, bool ExpandableT>
inline void Data<ChunkSizeT, ExpandableT>::clear()
{
    if (static_cast<uint>(zeroTerminated))
    {
        if (size)
...
template<uint ChunkSizeT, bool ExpandableT>
inline void Data<ChunkSizeT, ExpandableT>::shrink()
{
    if (data)
    {
        if (size)
        {
            ...
            data = reinterpret_cast<char*>(
              ::realloc(const_cast<char*>(data), static_cast<uint>(capacity)));
        }
        else
        {
            capacity = 0;
...
template<uint ChunkSizeT, bool ExpandableT>
inline void Data<ChunkSizeT, ExpandableT>::insert(Size offset,
                                                  const C* const buffer,
                                                  const Size len)
{
    ...
    (void)::memmove(const_cast<char*>(data) + sizeof(C) * (offset + len),
                    const_cast<char*>(data) + sizeof(C) * (offset),
                    sizeof(C) * (size - offset));
    // Copying the given buffer
    YUNI_MEMCPY(const_cast<char*>(data) + sizeof(C) * (offset),
                static_cast<uint>(capacity),
                buffer,
                sizeof(C) * len);
    // Updating the size
...
template<uint ChunkSizeT, bool ExpandableT>
inline void Data<ChunkSizeT, ExpandableT>::put(const C rhs)
{
...
void Data<ChunkSizeT, ExpandableT>::reserve(Size mincapacity)
{
    ...
    if (static_cast<uint>(capacity) < mincapacity)
    {
        ...
        // static_cast<uint>(capacity) = minstatic_cast<uint>(capacity) +
        // minstatic_cast<uint>(capacity) % chunkSize; Especially when chunkSize is not a power of 2
...
typename Data<ChunkSizeT, false>::Size Data<ChunkSizeT, false>::assignWithoutChecking(
  const C* const block,
  Size blockSize)
{
    ...
    if (blockSize > static_cast<uint>(capacity))
    {
        ...
        YUNI_MEMCPY(const_cast<char*>(data),
                    static_cast<uint>(capacity),
                    block,
                    sizeof(C) * static_cast<uint>(capacity));
        // New size
...
template<uint ChunkSizeT>
typename Data<ChunkSizeT, false>::Size Data<ChunkSizeT, false>::appendWithoutChecking(
  const C* const block,
  Size blockSize)
{
    // We have to trunk the size if we are outer limits
...
template<uint ChunkSizeT>
inline typename Data<ChunkSizeT, false>::Size Data<ChunkSizeT, false>::assignWithoutChecking(
  const C c)
{
    data[0] = c;
    size = 1;
...
template<uint ChunkSizeT>
void Data<ChunkSizeT, false>::put(const C rhs)
{
    // Making sure that we have enough space
