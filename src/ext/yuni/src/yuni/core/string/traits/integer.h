...
template<class BaseT, class T, int KindT = TypeInfo<T>::kind>
class From
{
};

template<class BaseT, class T>
class From<BaseT, T, 0>
{ /* Unknown */
};
...
public:
    ...
    enum
    {
        ...
        */
        bufferCapacity = (BaseT::n >= 10) ? ((1 + sizeof(Type) * 8 * 309 / 1024)
                                             + ((sizeof(Type) * 8 * 309 / 1024) % 8))
                                          : (1 + sizeof(Type) * 8),
...
public:
    ...
    static void AppendTo(S& str, Type value, int precision = 3)
    {
        ...
        {
        case FP_NAN:
            str.appendWithoutChecking("nan", 3);
            return;
        case FP_INFINITE:
...
private:
    ...
    static int32_t Power10(int precision)
    {
        ...
        {
        case 3:
            return 1000; // The most widely used value first
        case 6:
            return 1000000;
        case 1:
            return 10;
