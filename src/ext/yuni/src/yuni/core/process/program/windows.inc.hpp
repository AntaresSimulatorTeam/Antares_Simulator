...
#if _WIN32_WINNT >= 0x0600
static inline bool fillProcAttributeList(HANDLE (&handles)[3],
...
void Program::ThreadMonitor::waitForSubProcess()
{
    ...
    do
    {
        ...
        if (/*waitStatus >= WAIT_OBJECT_0 and*/ waitStatus < (WAIT_OBJECT_0 + (uint)nbHandles))
        {
            ...
            DWORD exitCode = 0;
            std::cout << " :: event on handle " << (waitStatus - WAIT_OBJECT_0) << " = "
                      << signalled << std::endl;
            ...
            if (signalled == processHandle)
            {
                ...
                if (0 != ::GetExitCodeProcess(processHandle, &exitCode))
                {
                    ...
                    assert(exitCode != STILL_ACTIVE
                           and "Child process uses STILL_ACTIVE as exit status !");
            ...
            else // If the child wrote to stdout or stderr
            {
                ...
                // bool success = (FALSE != ::PeekNamedPipe(readFrom, nullptr, 0, nullptr,
                // &totalAvailBytes, nullptr)); if (success and totalAvailBytes > 0)
                do
                {
                    // std::cout << " ... peeking : " << totalAvailBytes << " bytes to read" <<
                    // std::endl;

                    std::cout << " ... reading\n" << std::flush;
                    bool success
                      = (FALSE
                         != ::ReadFile(readFrom, buffer, sizeof(char) * 1, &readBytes, nullptr));
        ...
        else // Abnormal completion
        {
            ...
            case WAIT_TIMEOUT: // Normally not possible with INFINITE
                assert(false
                       and "Program::ThreadMonitor::waitForSubProcess : Timeout on infinite !");
                // Keep looping
                break;
            default: // WAIT_ABANDONED_X is only for mutexes, this should never occur
                assert(false
                       and "Program::ThreadMonitor::waitForSubProcess : Unmanaged "
