...
bool Program::ThreadMonitor::spawnProcess()
{
    ...
    if (/*input*/ pipe(channels.outfd) or /*output*/ pipe(channels.infd) or pipe(channels.errd))
    {
        ...
        case EMFILE:
            ...
            break;
        case ENFILE:
            std::cerr << "pipe failed: The system limit on the total number of open files has been "
                         "reached\n";
            break;
        }
        return false;
    ...
    do
    {
        ...
        // the process is likely already dead
        {
            ...
            else
            {
                ...
                {
                    // getting the execution time ASAP to have the most precise value even in debug
                    // mode (especially when the precision is set to 'milliseconds')
                    pEndTime = currentTime();
                    break;
                }
            }
