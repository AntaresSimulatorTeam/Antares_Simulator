...
void Program::increaseInstructionCapacity()
{
    ...
    instructions = reinterpret_cast<InstructionType*>(
      ::realloc(instructions, static_cast<size_t>(instructionCapacity)));
}
...
void Program::reserveOperands(uint count)
{
    if (count > operandCapacity)
    {
        ...
        operands
          = reinterpret_cast<char*>(::realloc(operands, static_cast<size_t>(operandCapacity)));
    }
}
