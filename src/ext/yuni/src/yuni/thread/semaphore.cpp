...
void Semaphore::initialize(uint readers)
{
    ...
    // In every cases, for all UNIX operating systems, readers must not be greater than
    // SEM_VALUE_MAX
    ...
    if (0 != sem_init(&pSemaphore, 0, readers))
    {
        ...
        case EINVAL:
        {
            ...
            assert(false
                   and "impossible to initialize a semaphore: `readers` exceeds SEM_VALUE_MAX");
            break;
        }
        case ENOSYS:
        {
            ...
            assert(false
                   and "impossible to initialize a semaphore: the system does not support "
...
Semaphore::Semaphore(uint readers) : pMaxReaders(readers)
{
    initialize(readers);
}

Semaphore::Semaphore(const Semaphore& rhs) : pMaxReaders(rhs.pMaxReaders)
{
