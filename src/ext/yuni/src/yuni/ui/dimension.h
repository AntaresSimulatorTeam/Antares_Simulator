...
public:
    Dimension(float value, Unit unit) : pUnit(unit), pValue(value)
    {
    }

    Dimension(const Dimension& other) : pUnit(other.pUnit), pValue(other.pValue)
    {
    }
    ...
    float operator()(const ConversionData& conversion) const
    {
