...
public:
    ...
    explicit Glyph(::FT_GlyphSlot slot, uint glyphIndex, bool antiAliased = true) :
     pIndex(glyphIndex), pGlyph(nullptr)
    {
...
public:
    //! Constructor
    FTFontImpl(const AnyString& faceName, float size) : pFace(nullptr), pValid(false)
    {
...
FTFont::FTFont(const AnyString& name,
               float size,
               bool bold,
               bool italic,
               bool underline,
               bool strikeout) :
 Font(name, size, bold, italic, underline, strikeout), pImpl(nullptr)
{
    initialize();
}
...
void FTFont::draw(const AnyString& text,
                  Gfx3D::Texture::Ptr& texture,
                  bool antiAliased,
                  bool useKerning,
                  uint tabWidth) const
{
...
void FTFont::measure(const AnyString& text,
                     uint& width,
                     uint& height,
                     int& ascent,
                     int& descent,
