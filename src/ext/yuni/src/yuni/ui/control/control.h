...
public:
    ...
    //! Mouse down callback
    Yuni::Event<EventPropagation(IControl* sender, Input::IMouse::Button btn, float x, float y)>
      onMouseDown;
    //! Mouse up callback
    Yuni::Event<EventPropagation(IControl* sender, Input::IMouse::Button btn, float x, float y)>
      onMouseUp;
...
public:
    ...
    //! Draw the control
    virtual void draw(DrawingSurface::Ptr& surface,
                      float xOffset = 0.0f,
                      float yOffset = 0.0f) const = 0;

    //! Recursively search for the deepest child control containing the given point
    IControl* getControlAt(float x, float y);
    IControl* getControlAt(const Point2D<float>& point)
    {
        return getControlAt(point.x, point.y);
    }

    //! Add child controls
    void addChild(const IControl::Ptr& child)
    {
...
protected:
    ...
    {
        return x >= pPosition.x && x <= pPosition.x + pSize.x && y >= pPosition.y
    ...
    virtual EventPropagation mouseMove(float, float)
    {
        return epContinue;
    }
    //! Control reaction to mouse down
    virtual EventPropagation mouseDown(Input::IMouse::Button, float, float)
    {
        return epContinue;
    }
    //! Control reaction to mouse up
    virtual EventPropagation mouseUp(Input::IMouse::Button, float, float)
    {
        return epContinue;
    }
    //! Control reaction to mouse double-click
    virtual EventPropagation mouseDblClick(Input::IMouse::Button, float, float)
    {
        return epContinue;
    }
    //! Control reaction to mouse scroll
    virtual EventPropagation mouseScroll(float, float, float)
    {
        return epContinue;
    }
    //! Control reaction to mouse enter
    virtual EventPropagation mouseEnter()
    {
        return epContinue;
    }
    //! Control reaction to mouse leave
    virtual EventPropagation mouseLeave()
    {
        return epContinue;
    }
    //! Control reaction to mouse hover
    virtual EventPropagation mouseHover(float, float)
    {
        return epContinue;
    }
    //! Control reaction to key up
    virtual EventPropagation keyDown(Input::Key)
    {
        return epContinue;
    }
    //! Control reaction to key down
    virtual EventPropagation keyUp(Input::Key)
    {
        return epContinue;
    }
    //! Control reaction to char input
    virtual EventPropagation charInput(const AnyString&)
    {
        return epContinue;
    }
