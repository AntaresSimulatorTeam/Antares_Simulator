...
void Button::draw(DrawingSurface::Ptr& surface, float xOffset, float yOffset) const
{
    ...
    surface->drawFilledRectangle(
      theme.borderColor, pBackColor, pos.x, pos.y, pSize.x, pSize.y, theme.borderWidth);
    // Try drawing images on top
    if (pBeingClicked && !(!pImageClicking))
        surface->drawImage(pImageClicking,
                           pos.x,
                           pos.y,
                           pSize.x,
                           pSize.y,
    ...
    if (!pText.empty())
        surface->drawTextOnColorInRect(
          pText, theme.font, pTextColor, pBackColor, pos.x, pos.y, pSize.x, pSize.y);
    surface->endClipping();
    pModified = false;
}
