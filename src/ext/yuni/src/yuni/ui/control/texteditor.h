...
public:
    ...
    void backColor(float r, float g, float b)
    {
        pBackColor.assign(r, g, b);
        invalidate();
    }
    void backColor(float r, float g, float b, float a)
    {
        pBackColor.assign(r, g, b, a);
        invalidate();
    }
    void backColor(const Color::RGB<float>& color)
    {
        pBackColor = color;
        invalidate();
    }
    ...
    const Point2D<uint>& cursorPos() const
    {
        return pCursorPos;
    }
    ...
    //! Get line height
    const Dimension& lineHeight() const
    {
        return pLineHeight;
    }
    ...
    {
        return pLineHeight(pConversion);
    }
    //! Set line height
    void lineHeight(const Dimension& newValue)
    {
        pLineHeight = newValue;
    }
    //! Set line height in pixels
    void lineHeightPixels(float newValue)
    {
        pLineHeight.reset(newValue, Dimension::uPixel);
    }
...
private:
    ...
    uint columnCount(const BeginT& begin, const EndT& end) const
    {
        return makeView(begin, end)
          .map([&](const AnyString::Char& c) -> uint { return c == '\t' ? pTabWidth : 1u; })
          .sum();
    }
