...
namespace Control
{
Panel::Panel(float x, float y, float width, float height) : IControl(x, y, width, height)
{
    pColor = Theme::Current()->panelColor;
}

Panel::Panel(const Point2D<float>& position, const Point2D<float>& size) : IControl(position, size)
{
    pColor = Theme::Current()->panelColor;
}
...
void Panel::draw(DrawingSurface::Ptr& surface, float xOffset, float yOffset) const
{
    ...
    if (pColor.alpha > 0)
    {
        surface->drawFilledRectangle(
          pColor, pColor, pos.x, pos.y, pSize.x, pSize.y, theme.borderWidth);
    }
    // Draw the children
