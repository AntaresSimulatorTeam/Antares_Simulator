...
void TextEditor::draw(DrawingSurface::Ptr& surface, float xOffset, float yOffset) const
{
      ...
      [&](AnyString& line) -> bool {
          ...
          if (!line.empty())
          {
              ...
              if (!line.empty())
              {
                  ...
                  surface->drawText(
                    line, pFont, pColor, x, y - pixelLineHeight / 2, pTabWidth, true);
              }
          }
          y += pixelLineHeight;
          return true;
    ...
    surface->drawText(
      String(pCursorPos.y + 1) << ':' << pCursorPos.x, pFont, pColor, pSize.x - 50, pSize.y - 15);
...
void TextEditor::cursorMoveDown(uint offset)
{
    ...
    byteToCursor(pCursorPos,
                 Math::Min(absLineStart + pCursorPos.x, currentLine(absLineStart).size()));
    invalidate();
}
...
uint TextEditor::XToColumn(float x) const
{
    ...
    return (uint)Math::Round((x - pHorizMargin)
                             / (pFont->size() / 1.25f)); // For now, use a not-quite-random ratio
}
...
{
    // TODO : I need the real advance of the clicked text to find the proper column value
    return (float)col * ((float)pFont->size() / 1.25f)
           + (float)pHorizMargin; // For now, use a not-quite-random ratio
}
