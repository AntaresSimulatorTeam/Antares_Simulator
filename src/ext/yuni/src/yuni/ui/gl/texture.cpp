...
Texture::Ptr Texture::LoadFromFile(const AnyString& filePath, bool mipmaps)
{
    ...
    if (!data)
        return nullptr;
    Texture::Ptr texture
      = Texture::New((uint)width, (uint)height, (uint)depth, UInt8, data, mipmaps);
...
Texture::Ptr Texture::LoadFromMemory(uint length, const uint8* data)
{
    ...
    if (!data)
        return nullptr;
    uint8* texData
      = ::stbi_load_from_memory(data, length, &width, &height, &depth, 0); // 0 means auto-detect
    if (!texData)
        return nullptr;

    Texture::Ptr texture
      = Texture::New((uint)width, (uint)height, (uint)depth, UInt8, texData, true);
...
Texture::Ptr Texture::New(uint width,
                          uint height,
                          uint colorDepth,
                          DataType type,
                          const uint8* data,
                          bool mipmaps)
{
    ...
    ::glTexParameterf(
      GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, mipmaps ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
    // When texture area is large, bilinear filter the first mipmap
...
Texture::Ptr Texture::New3D(uint width,
                            uint height,
                            uint depth,
                            uint colorDepth,
                            DataType type,
...
{
    Texture::Ptr newTex = New3D(other->width(),
                                other->height(),
                                other->depth(),
                                other->colorDepth(),
                                other->type(),
    ...
    ::glCopyTexImage2D(GL_TEXTURE_2D,
                       0,
                       DepthToGLEnumInternal(other->colorDepth()),
                       0,
                       0,
                       other->width(),
                       other->height(),
...
Texture::Ptr Texture::NewMS(uint width,
                            uint height,
                            uint colorDepth,
                            DataType type,
                            uint samples,
...
Texture::Texture(ID id, uint width, uint height, uint depth, uint colorDepth, DataType type) :
 pID(id), pWidth(width), pHeight(height), pDepth(depth), pColorDepth(colorDepth), pType(type)
{
}
...
void Texture::update(uint offsetX,
                     uint offsetY,
                     uint width,
                     uint height,
                     uint colorDepth,
                     const unsigned char* data)
{
    ...
    pColorDepth = colorDepth;
    ::glTexSubImage2D(
      GL_TEXTURE_2D, 0, offsetX, offsetY, width, height, format, GL_UNSIGNED_BYTE, data);
    if (!GLTestError("glTexSubImage2D Texture update"))
        std::cerr << "On texture " << pID << std::endl;
}
