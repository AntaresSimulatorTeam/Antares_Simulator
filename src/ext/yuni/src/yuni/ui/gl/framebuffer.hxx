...
inline FrameBuffer::FrameBuffer() :
 pID(-1), pTexture(nullptr), pDepth(-1), pSize(1, 1), pUsage(fbDraw)
{
}

inline FrameBuffer::FrameBuffer(uint width, uint height) :
 pID(-1), pTexture(nullptr), pDepth(-1), pSize(width, height), pUsage(fbDraw)
{
}
...
{
    assert(pUsage == fbPingPong
           && "FrameBuffer::swap() has no meaning when usage is not Ping-Pong !");
