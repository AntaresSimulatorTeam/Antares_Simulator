...
{
    assert(pImpl->locked
           && "DrawingSurface error : Cannot commit on an unlocked surface ! (try calling the "
              "begin() function)");
    assert(pImpl->clippings.empty()
           && "DrawingSurface commit : Too few endClipping() calls, stack is not empty !");
    pImpl->locked = false;
...
{
    assert(pImpl->locked
           && "DrawingSurface error : Cannot rollback on an unlocked surface ! (try calling the "
              "begin() function)");
...
void DrawingSurface::resize(float width, float height)
{
    ...
    if (width <= 0 || height <= 0 || (Math::Equals(width, size.x) && Math::Equals(height, size.y)))
        return;
    size(width, height);
    pImpl->fb.resize((uint)Math::Ceil(width), (uint)Math::Ceil(height));
}

void DrawingSurface::drawText(const String& text,
                              const FTFont::Ptr& font,
                              const Color::RGBA<float>& color,
                              float x,
                              float y,
                              uint tabWidth,
                              bool drawOnBaseline)
...
void DrawingSurface::drawTextOnColor(const String& text,
                                     const FTFont::Ptr& font,
                                     const Color::RGBA<float>& color,
                                     const Color::RGBA<float>& backColor,
                                     float x,
                                     float y,
                                     uint tabWidth)
{
...
void DrawingSurface::drawTextInRect(const String& text,
                                    const FTFont::Ptr& font,
                                    const Color::RGBA<float>& color,
                                    float x,
                                    float y,
...
void DrawingSurface::drawTextOnColorInRect(const String& text,
                                           const FTFont::Ptr& font,
                                           const Color::RGBA<float>& color,
                                           const Color::RGBA<float>& backColor,
                                           float x,
...
void DrawingSurface::drawLine(const Color::RGBA<float>& color,
                              float startX,
                              float startY,
                              float endX,
                              float endY,
...
void DrawingSurface::drawLine(const Color::RGBA<float>& color,
                              const Color::RGBA<float>& bgColor,
                              float startX,
                              float startY,
                              float endX,
                              float endY,
...
void DrawingSurface::drawImage(const Gfx3D::Texture::Ptr& texture,
                               float x,
                               float y,
                               float width,
                               float height,
                               const Color::RGBA<float>& fillColor,
                               ...
                               float imageOpacity)
{
    ...
    ::glEnableVertexAttribArray(Gfx3D::Vertex<>::vaPosition);
    const float vertices[]
      = {xStart, yEnd, xEnd, yStart, xStart, yStart, xStart, yEnd, xEnd, yEnd, xEnd, yStart};
...
{
    assert(pImpl->locked
           && "DrawingSurface error : Cannot manage clipping on an unlocked surface !");
...
void DrawingSurface::endClipping()
{
    ...
    if (!pImpl->clippings.empty())
    {
        ...
        ::glScissor(
          (int)coord.first.x, (int)coord.first.y, (uint)coord.second.x, (uint)coord.second.y);
    }
}
