...
/// A reference to NSApp.  Always a good idea, seeing as it's probably the most helpful thing in
/// CocoaLand
extern id NSApp;
...
bool OnAppDidFinishLaunching(AppDelegate* self, SEL _cmd, id notification)
{
    ...
    self->window
      = cmacs_window_init_msgSend(self->window,
    ...
    id view
      = cmacs_rect_msgSend1(cmacs_simple_msgSend((id)objc_getClass("YView"), sel_getUid("alloc")),
...
bool NSGLWindow::initialize()
{
    ...
    if (not appDelObj)
    {
        appDelObj = (AppDelegate*)cmacs_simple_msgSend((id)objc_getClass("AppDelegate"),
                                                       sel_getUid("alloc"));
    ...
    appDelObj->window
      = cmacs_simple_msgSend((id)objc_getClass("NSOpenGLWindow"), sel_getUid("alloc"));

    /// Create an instance of the window.
    appDelObj->window
      = cmacs_window_init_msgSend(appDelObj->window,
                                  sel_getUid("initWithContentRect:styleMask:backing:defer:"),
                                  (CMRect){{0, 0}, {1024, 460}},
                                  (NSTitledWindowMask | NSClosableWindowMask | NSResizableWindowMask
                                   | NSMiniaturizableWindowMask),
