...
const Input::Key KeyConversions[] = {
  ...
  Input::Key::Invalid,     Input::Key::Invalid,
  ...
  Input::Key::Invalid,     Input::Key::Invalid, Input::Key::Invalid, Input::Key::Invalid,
  Input::Key::Invalid, // 1A 	Undefined
  Input::Key::Escape,  // 1B 	ESC key
  Input::Key::Invalid, // 1C-1F 	Reserved for Kanji systems
  Input::Key::Invalid,     Input::Key::Invalid, Input::Key::Invalid,
  ...
  Input::Key::B,           Input::Key::C,       Input::Key::D,       Input::Key::E,
  Input::Key::F,           Input::Key::G,       Input::Key::H,       Input::Key::I,
  Input::Key::J,           Input::Key::K,       Input::Key::L,       Input::Key::M,
  Input::Key::N,           Input::Key::O,       Input::Key::P,       Input::Key::Q,
  Input::Key::R,           Input::Key::S,       Input::Key::T,       Input::Key::U,
  Input::Key::V,           Input::Key::W,       Input::Key::X,       Input::Key::Y,
...
LRESULT CALLBACK WGLWindow::MessageCallback(HWND handle, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        int nbChar
          = ::WideCharToMultiByte(CP_UTF8, 0, (wchar_t*)&wParam, 1, nullptr, 0, nullptr, nullptr);
        if (nbChar < 1)
            return 0;
        char uniChar[nbChar + 1];
        nbChar = ::WideCharToMultiByte(
          CP_UTF8, 0, (wchar_t*)&wParam, 1, uniChar, nbChar, nullptr, nullptr);
        if (nbChar < 1)
            return 0;
        uniChar[nbChar] = '\0';
        String str;
...
bool WGLWindow::enableFullScreen()
{
    ...
    ::SetWindowLong(pHWnd, GWL_STYLE, pPrevStyle & ~(WS_CAPTION | WS_THICKFRAME));
    ::SetWindowLong(
      pHWnd,
      GWL_EXSTYLE,
      pPrevExStyle
...
bool WGLWindow::initWindow()
{
    ...
    if (!(pHWnd = ::CreateWindowEx(dwExStyle,
                                   L"OpenGL",
                                   WString{pTitle}.c_str(),
                                   dwStyle | WS_CLIPSIBLINGS | WS_CLIPCHILDREN,
                                   pLeft,
                                   pTop, // Position
                                   windowRect.right - windowRect.left,
                                   windowRect.bottom - windowRect.top, // Dimensions
                                   nullptr,
                                   nullptr,
                                   pHInstance,
                                   nullptr)))
...
bool WGLWindow::initMultisamplePixelFormat(const PIXELFORMATDESCRIPTOR& pfd, bool set)
{
    ...
    if (not ::glewIsSupported("GL_ARB_multisample")
        or not ::wglewIsSupported("WGL_ARB_multisample"))
    {
        std::cerr << "Window init error : Multisampling is not available, falling back to standard "
                     "pixel format !"
                  << std::endl;
        return false;
    }
    ...
    int iAttributes[] = {WGL_DRAW_TO_WINDOW_ARB,
                         ...
                         WGL_FULL_ACCELERATION_ARB, // Hardware acceleration
                         WGL_COLOR_BITS_ARB,
                         (int)pBitDepth,
                         WGL_ALPHA_BITS_ARB,
                         0,
                         ...
                         true, // Activate multisampling
                         WGL_SAMPLES_ARB,
                         (int)msMultiplier, // Specify number of samples
                         WGL_PIXEL_TYPE_ARB,
                         WGL_TYPE_RGBA_ARB,
                         0};
    ...
    bool valid
      = ::wglChoosePixelFormatARB(pHDC, iAttributes, nullptr, 1, &pixelFormat, &numFormats);
...
void WGLWindow::icon(const AnyString& path)
{
    ...
    WString wstr(path);
    const HICON hIcon
      = (HICON)::LoadImage(nullptr, wstr.c_str(), IMAGE_ICON, 32, 32, LR_LOADFROMFILE);
    if (hIcon)
    ...
    const HICON hIconSmall
      = (HICON)::LoadImage(nullptr, wstr.c_str(), IMAGE_ICON, 16, 16, LR_LOADFROMFILE);
    if (hIconSmall)
...
RenderWindow* WindowFactory::CreateGLWindow(const AnyString& title,
                                            uint width,
                                            uint height,
                                            uint bits,
                                            bool fullScreen)
