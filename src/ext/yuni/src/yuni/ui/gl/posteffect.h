...
public:
    PostEffect(const AnyString& name,
               const AnyString& vertexShader,
               const AnyString& fragmentShader,
               float width,
               float height) :
     pName(name), pSize(width > 0.0f ? width : 0.0f, height > 0.0f ? height : 0.0f)
    {
    ...
    bool valid() const
    {
        return !(!pShaders) && pShaders->valid();
    }
