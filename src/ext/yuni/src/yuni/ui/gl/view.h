...
**
** \warning Overlapping is not recommended for the moment, because the implementation is
*sub-efficient.
*/
class View
{
...
public:
    ...
    //! Overlay management
    void addOverlay(const TextOverlay::Ptr& text)
    {
        pTexts.push_back(text);
    }
    void addOverlay(const PictureOverlay::Ptr& picture)
    {
        pPictures.push_back(picture);
    }
    void removeOverlay(const TextOverlay::Ptr& text)
    {
        pTexts.erase(std::remove_if(pTexts.begin(),
                                    pTexts.end(),
    ...
    //! Get the UI controls
    const IControl::Ptr& rootControl() const
    {
        return pControl;
    }
    IControl::Ptr& rootControl()
    {
        return pControl;
    }
...
protected:
    ...
    void drawPicture(const Gfx3D::Texture::Ptr& texture,
                     float x,
                     float y,
                     float width,
                     float height,
...
protected:
    ...
    **
    ** \note This is a hiding feature and has nothing to do with whether the view is hidden by other
    *views on top of it
    */
    bool pVisible;
