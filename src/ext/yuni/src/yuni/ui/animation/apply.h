...
public:
    static Point2D<T> Transition(const EasingFunc& easing,
                                 const Point2D<T>& start,
                                 const Point2D<T>& end,
                                 float ratio)
    {
        float newRatio = easing(ratio);
        return Point2D<T>(start.x + (end.x - start.x) * newRatio,
...
public:
    static Point3D<T> Transition(const EasingFunc& easing,
                                 const Point3D<T>& start,
                                 const Point3D<T>& end,
                                 float ratio)
    {
        float newRatio = easing(ratio);
        return Point3D<T>(start.x + (end.x - start.x) * newRatio,
                          start.y + (end.y - start.y) * newRatio,
...
public:
    static Yuni::Color::RGB<T> Transition(const EasingFunc& easing,
                                          const Yuni::Color::RGB<T>& start,
                                          const Yuni::Color::RGB<T>& end,
                                          float ratio)
    {
        float newRatio = easing(ratio);
        return Yuni::Color::RGB<T>(start.red + (end.red - start.red) * newRatio,
...
public:
    static Yuni::Color::RGBA<T> Transition(const EasingFunc& easing,
                                           const Yuni::Color::RGBA<T>& start,
                                           const Yuni::Color::RGBA<T>& end,
                                           float ratio)
    {
        float newRatio = easing(ratio);
        return Yuni::Color::RGBA<T>(start.red + (end.red - start.red) * newRatio,
                                    start.green + (end.green - start.green) * newRatio,
                                    start.blue + (end.blue - start.blue) * newRatio,
