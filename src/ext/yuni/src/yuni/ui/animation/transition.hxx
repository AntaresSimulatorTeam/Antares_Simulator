...
T Transition<T>::currentValue(uint timeMS) const
{
    ...
    for (; end != it; ++it, ++prev)
    {
        if (timeMS < it->first)
        {
            ...
            assert(easingIt != pEasings.end()
                   and "Transition : There must always be an easing defined out of any key frame.");
            return apply(easingIt->second,
                         prev->second,
                         it->second,
...
template<class T>
inline T Transition<T>::apply(const EasingFunc& easing,
                              const T& start,
                              const T& end,
                              float ratio) const
{
