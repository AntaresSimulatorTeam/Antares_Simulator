...
** \code
**    String outputString;
**	  IO::parent_path(outputString, "/tmp/foo.txt");
...
*/
void parent_path(String& out, const AnyString& path, bool systemDependant = false);
...
/*!
** \brief Simplifies a path by removing navigation elements such as "." and ".." to produce a
*direct, well-formed path
**
...
**
** \param[out] out            A string (any class compliant to std::string) where to write the
*result
** \param      in             A path/filename to normalize
** \param      replaceSlashes True to replace slashes according the local OS conventions. False to
*keep
**                            as it.
*/
