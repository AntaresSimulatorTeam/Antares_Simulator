...
inline bool Stream::readline(StringT& buffer, bool trim)
{
    return readline<StringT::chunkSize, StringT>(buffer, trim);
}

template<uint CustomChunkT, class StringT>
bool Stream::readline(StringT& buffer, bool trim)
{
...
inline uint64 Stream::read(CString<CSizeT, ExpT>& buffer, uint64 size)
{
    ...
    assert(buffer.capacity() != 0
           and "When reading a file, the buffer must have reserved some space");
