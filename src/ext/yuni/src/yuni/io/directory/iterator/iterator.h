...
public:
    //! The class ancestor
    typedef typename Yuni::
      IIntrusiveSmartPtr<IIterator<DetachedT>, true, Yuni::Policy::ObjectLevelLockable>
        Ancestor;
...
protected:
    ...
    */
    virtual Flow onBeginFolder(const String& filename,
                               const String& parent,
    ...
    ** \param name The name of the folder found only (ex: file.txt)
    */
    virtual void onEndFolder(const String& filename,
                             const String& parent,
                             const String& name) override;
    ...
    ** \return itAbort to abort the whole process, itSkip to skip the current folder and its
    *sub-folders
    */
...
**
**				if (match)
**					std::cout << " . found in file " << filename << " (" << size
<< " bytes)\n";
**			});
**		}
**		return IO::flowContinue;
