...
bool Source::prepareDispatched(uint source)
{
    ...
    if (nullptr != pAStream)
    {
        ...
        for (uint i = 0; i < maxBufferCount; ++i)
        {
            ...
            if (!Private::Media::OpenAL::SetBufferData(pIDs[i],
                                                       pAStream->alFormat(),
                                                       pData.data(),
                                                       size,
                                                       pAStream->rate() / pAStream->channels()))
                return false;
...
bool Source::updateDispatched(uint source)
{
    ...
    if (hasAudio())
    {
        ...
        if (!Private::Media::OpenAL::SetBufferData(
              buffer, pAStream->alFormat(), pData.data(), size, frequency / channels))
            return false;
...
{
    assert(hasVideo()
           && "Source::nextFrame : Cannot get a video frame from an audio-only source !");
