...
public:
    ...
    void stream(const AStreamPtr& audioStream)
    {
        pAStream = audioStream;
    }

    void stream(const VStreamPtr& videoStream)
    {
    ...
    bool valid() const
    {
        return hasAudio() or hasVideo();
    }

    //! Has a valid audio stream
    bool hasAudio() const
    {
        return nullptr != pAStream and pAStream->valid();
    }

    //! Has a valid video stream
    bool hasVideo() const
    {
        return nullptr != pVStream and pVStream->valid();
    }
    ...
    {
        assert(hasAudio());
        return pAStream->channels();
    }

    //! Bits per sample for audio. Only valid if there is audio !
    uint bitsPerSample() const
    {
        assert(hasAudio());
        return pAStream->bits();
    }
