...
namespace DBI
{
inline Cursor::Cursor(::yn_dbi_adapter& adapter, void* handle) : pAdapter(adapter), pHandle(handle)
{
}

inline Cursor::Cursor(Cursor&& other) : pAdapter(other.pAdapter), pHandle(other.pHandle)
{
...
inline DBI::Error Cursor::next()
{
    ...
    return (pHandle) ? (DBI::Error)pAdapter.cursor_go_to_next(pHandle) : DBI::errNoRow;
}

inline DBI::Error Cursor::previous()
{
    assert(pAdapter.cursor_go_to_previous != NULL);
    return (pHandle) ? (DBI::Error)pAdapter.cursor_go_to_previous(pHandle) : DBI::errNoRow;
}
...
inline DBI::Error Cursor::moveTo(uint64 rowindex)
{
    ...
    return (pHandle) ? (DBI::Error)pAdapter.cursor_go_to(pHandle, rowindex) : DBI::errNoRow;
}
