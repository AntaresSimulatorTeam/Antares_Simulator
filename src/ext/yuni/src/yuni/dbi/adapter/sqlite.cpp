...
// the current transaction (if any) and try again later. Otherwise, the
// system may become deadlocked.
int sqlite3_blocking_prepare_v2(sqlite3* db,
                                const char* zSql,
                                int nSql,
                                sqlite3_stmt** ppStmt,
                                const char** pz)
...
static yn_dbierr ynsqliteOpen(void** dbh,
                              const char* host,
                              uint /*port*/,
                              const char* /*user*/,
                              const char* /*pass*/,
...
static yn_dbierr ynsqliteTruncate(void* dbh, const char* tablename, uint length)
{
    ...
    {
        sqlite3_stmt* cursor = nullptr;
        int error = sqlite3_blocking_prepare_v2(
          (::sqlite3*)dbh, select.c_str(), (int)select.size(), &cursor, nullptr);
...
static yn_dbierr ynsqliteQueryPerformAndRelease(void* qh)
{
    ...
    yn_dbierr result
      = (error == SQLITE_ROW or error == SQLITE_DONE) ? yerr_dbi_none : ynsqliteError(error, stmt);
...
static yn_dbierr ynsqliteBindStr(void* qh, uint index, const char* str, uint length)
{
    assert(qh != NULL);
    int error
      = ::sqlite3_bind_text(((SQLiteQuery*)qh)->statement, (int)(index + 1), str, (int)length, 0);
