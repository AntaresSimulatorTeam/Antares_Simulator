...
inline Column::Column(::yn_dbi_adapter& adapter, void* handle, uint colindx) :
 pAdapter(adapter), pHandle(handle), pColumnIndex(colindx)
{
}

inline Column::Column(const Column& rhs) :
 pAdapter(rhs.pAdapter), pHandle(rhs.pHandle), pColumnIndex(rhs.pColumnIndex)
{
}
...
template<>
inline String Column::as<String>() const
{
    return asString();
}

template<>
inline bool Column::as<bool>() const
{
    return asBool();
}

template<>
inline double Column::as<double>() const
{
    return asDouble();
}

template<>
inline float Column::as<float>() const
{
    return (float)asDouble();
}

template<>
inline sint32 Column::as<sint32>() const
{
    return asInt();
}

template<>
inline sint64 Column::as<sint64>() const
{
    return asInt64();
}
