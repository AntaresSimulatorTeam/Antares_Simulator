...
inline DBI::Error Transaction::perform(const AnyString& script,
                                       const A1& a1,
                                       const A2& a2,
                                       const A3& a3)
{
...
inline DBI::Error Transaction::each(const AnyString& query, const A1& a1, const CallbackT& callback)
{
    Cursor stmt = prepare(query);
    stmt.map(a1);
    stmt.execute();
...
inline DBI::Error Transaction::each(const AnyString& query,
                                    const A1& a1,
                                    const A2& a2,
