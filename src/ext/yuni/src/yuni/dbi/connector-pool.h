...
public:
    ...
    ** \param host    host name (or filename)
    */
    Error open(Adapter::IAdapter* adapter,
               AnyString host,
               AnyString dbname,
               AnyString user,
               AnyString password);
    ...
    ** dedicated to maintenance
    **
    ** \param idleTime The maximum idle time in seconds (override the value provided by the
    *connection settings)
    ** \param[out] remainingCount the number of connections which remain active
    ** \param[out] closedCount The number of connection which have been closed
    */
    void closeIdleConnections(uint idleTime,
                              uint* remainingCount = nullptr,
                              uint* closedCount = nullptr);
    //@}
