...
Context::Context(Service& service) :
 userdata(nullptr), httpStatus(), remotePort(), service(service), httpStatusCode()
{
    ...
    {
        codes.header4xx[i - 400] << "HTTP/1.1 " << i << ' ' << Net::HttpStatusCodeToCString(i)
                                 << "\r\n";
    ...
    {
        codes.header5xx[i - 500] << "HTTP/1.1 " << i << ' ' << Net::HttpStatusCodeToCString(i)
                                 << "\r\n";
