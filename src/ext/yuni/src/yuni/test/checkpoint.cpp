...
Checkpoint::Checkpoint(const char* id) : pResult(true)
{
// ID
#ifdef YUNI_OS_MSVC
...
TimedCheckpoint::TimedCheckpoint(const char* id) : pResult(true)
{
// ID
#ifdef YUNI_OS_MSVC
    strncpy_s(pID, sizeof(pID), id, YUNI_TEST_ID_MAXLENGTH - 1);
...
TimedCheckpoint::~TimedCheckpoint()
{
    ...
    // \internal At the end of each checkpoint we force the
    // flush for stdout and stderr
    std::cout << "[test:cout] <end," << RESULT_TO_STR(pResult) << "," << duration << "ms> " << pID
              << std::endl;
    std::cerr << "[test:cerr] <end," << RESULT_TO_STR(pResult) << "," << duration << "ms> " << pID
              << std::endl;
}
