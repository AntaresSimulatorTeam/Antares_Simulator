...
namespace Display
{
List::List() : pNullMonitor(new Monitor(YUNI_DEVICE_DISPLAY_LIST_FAIL_SAFE_NAME))
{
    ...
    for (MonitorsFound::iterator it = lst.begin(); it != itEnd; ++it)
    {
        ...
        // beginning
        {
            ...
            for (OrderedResolutions::reverse_iterator j = resolutions.rbegin(); j != jEnd; ++j)
            {
                ...
                for (std::map<uint32, std::map<uint8, bool>>::reverse_iterator k
                     = j->second.rbegin();
                     k != j->second.rend();
                     ++k)
                {
                    ...
                    for (std::map<uint8, bool>::reverse_iterator l = k->second.rbegin();
                         l != k->second.rend();
                         ++l)
                    {
                        if (l->first >= minDepth)
