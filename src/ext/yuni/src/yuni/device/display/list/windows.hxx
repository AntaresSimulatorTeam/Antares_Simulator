...
static void refreshForWindows(MonitorsFound& lst)
{
    ...
    for (uint countDevices = 0;
         EnumDisplayDevicesW(NULL, countDevices, (DISPLAY_DEVICEW*)&displayDevice, 0);
         ++countDevices)
    {
        // Ignore mirrored displays
        if (not(displayDevice.StateFlags & DISPLAY_DEVICE_MIRRORING_DRIVER)
            and (displayDevice.StateFlags & DISPLAY_DEVICE_ATTACHED_TO_DESKTOP))
        {
            ...
            EnumDisplayDevicesW(
              displayDevice.DeviceName, 0, (DISPLAY_DEVICEW*)&monitorDisplayDevice, 0);
            ...
            if (newMonitor)
            {
                ...
                const int sizeRequired = WideCharToMultiByte(
                  CP_UTF8, 0, monitorDisplayDevice.DeviceString, -1, NULL, 0, NULL, NULL);
                if (sizeRequired <= 0)
                    continue;
                ...
                WideCharToMultiByte(CP_UTF8,
                                    0,
                                    monitorDisplayDevice.DeviceString,
                                    -1,
                                    (char*)newID.data(),
                                    sizeRequired,
                                    NULL,
                ...
                monitor = new Monitor(
                  newID, (Monitor::Handle)monitorDisplayDevice.DeviceID, mainDisplay, true, true);
