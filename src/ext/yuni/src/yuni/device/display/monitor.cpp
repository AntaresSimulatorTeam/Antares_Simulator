...
Monitor::Monitor() :
 pHandle(Monitor::InvalidHandle), pPrimary(false), pHardwareAcceleration(false), pBuiltin(false)
{
    addSafeResolutions();
}

Monitor::Monitor(const String& nm, const Monitor::Handle hwn, bool p, bool a, bool b) :
 pHandle(hwn), pProductName(nm), pResolutions(), pPrimary(p), pHardwareAcceleration(a), pBuiltin(b)
{
...
Resolution::Ptr Monitor::recommendedResolution() const
{
    ...
    return (!pResolutions.empty()) ? *(pResolutions.begin()) : new Resolution(640, 480);
}
