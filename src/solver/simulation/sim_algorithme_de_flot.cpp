...
{
    double AugmentationDuFlot;
    long TestDeConvergence;
    long NoeudDestination;
    long NoeudOrigine;
    long i;
    long j;
    long k;
    ...
    for (i = pt->PointeurSurIndices[0]; i < pt->PointeurSurIndices[1]; i++)
    {
        ...
        if (pt->ResultatsParPays[pt->IndicesDesVoisins[i] - 1]->Offre < 0.)
        {
            pt->ValeursEffectivesNTC[i]
              = -pt->ResultatsParPays[pt->IndicesDesVoisins[i] - 1]->Offre;
        }
        pt->ValeursEffectivesNTC[pt->PointeurSurIndices[pt->IndicesDesVoisins[i]]
                                 + pt->RangDansVoisinage[i]]
          = 0.0;
    }
...
LOOP2:;
    ...
    for (i = 0; i < (long)areaCount + 1; i++)
    {
        if (pt->X[i])
        {
            for (j = pt->PointeurSurIndices[i]; j < pt->PointeurSurIndices[i + 1]; j++)
            {
                ...
                {
                    TestDeConvergence = 1;
                    pt->S[k] = ((pt->S[i] < pt->ValeursEffectivesNTC[j] - pt->FluxDeSecours[j])
                                  ? pt->S[i]
                                  : pt->ValeursEffectivesNTC[j] - pt->FluxDeSecours[j]);
                    pt->A[k] = i;
                    pt->B[k] = j;
                    pt->X[k] = 1;
                    ...
                    LOOP3:;
                        pt->FluxDeSecours[pt->B[NoeudOrigine]]
                          = pt->FluxDeSecours[pt->B[NoeudOrigine]] + AugmentationDuFlot;
                        pt->FluxDeSecours
                          [pt->PointeurSurIndices[pt->IndicesDesVoisins[pt->B[NoeudOrigine]]]
                           + pt->RangDansVoisinage[pt->B[NoeudOrigine]]]
                          = -pt->FluxDeSecours[pt->B[NoeudOrigine]];
                        if (NoeudDestination)
                        {
    ...
    for (i = 0; i < (long)areaCount; i++)
    {
        if (pt->ResultatsParPays[i]->Offre == 0)
            pt->ResultatsParPays[i]->MargeAvecReseau = pt->ResultatsParPays[i]->MargeHorsReseau;
        if (pt->ResultatsParPays[i]->Offre > 0)
        {
            for (j = pt->PointeurSurIndices[i + 1]; j < pt->PointeurSurIndices[i + 2]; j++)
            {
                if (pt->IndicesDesVoisins[j] == (long)areaCount + 1)
                    pt->ResultatsParPays[i]->MargeAvecReseau
                      = pt->ResultatsParPays[i]->MargeHorsReseau - pt->FluxDeSecours[j];
            }
        }
        if (pt->ResultatsParPays[i]->Offre < 0)
        {
