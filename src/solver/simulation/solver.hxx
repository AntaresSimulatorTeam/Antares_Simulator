...
namespace Simulation
{
	...
	void ISimulation<Impl>::run()
	{
		...
		else
		{
			...
			logs.info() << "MC-Years : ["
				<< (study.runtime->rangeLimits.year[Data::rangeBegin] + 1)
				<< " .. "
				<< (1 + study.runtime->rangeLimits.year[Data::rangeEnd]) << "], total: "
				<< study.runtime->rangeLimits.year[Data::rangeCount];
			...
			logs.info() << " TSGenerator destroy";
			// Destroy the TS Generators if any
			// It will export the time-series into the output in the same time
			Solver::TSGenerator::DestroyAll(study);

			// Post operations
			logs.info() << " simulation end";
			ImplementationType:: simulationEnd();

			logs.info() << " variable siumulation end";
	...
	void ISimulation<Impl>::writeResults(bool synthesis, uint year, uint numSpace)
	{
		...
		else
		{
			...
			// The target folder
			String newPath;
			newPath << study.folderOutput << IO::Separator
				<< ImplementationType::Name() << IO::Separator;
			if (synthesis)
				newPath << "mc-all";
			else
			{
	...
	{	
		...
		// Variables "NumeroChroniquesTireesParPays" and "ValeursGenereesParPays" (see : sim_allocation_tableaux.cpp)
	...
	void ISimulation<Impl>::estimateMemoryForWeeklyPb(Antares::Data::StudyMemoryUsage& u)
	{
		/*
			...
			Then	: multiplying this estimation by the number of years in parallel (== nb of cores)
		*/
	...
	template<class Impl>
	void ISimulation<Impl>::estimateMemoryForOptimizationPb(Antares::Data::StudyMemoryUsage& u, int & nbVars, int & nbConstraints)
	{
		...
		/*
			...
			These parameters are also required for the next estimation part of the solver estimation (simplex problem).
		*/
		...
		if(u.mode == Data::stdmAdequacyDraft) return;
		...
		// ------------------------------------
		int NombreDePasDeTempsPourUneOptimisation = (parameters.simplexOptimizationRange == Data::sorWeek) ? 168 : 24;
		...
		*/
		...
		NombreDeContraintes += nbLinks;	/* Contraintes pour modeliser la partie positive/negative du flux si l'interco est geree avec des couts */
		...
		*/
		if (u.mode == Data::stdmEconomy)
		{
			for(uint i = 0; i != study.areas.size(); i++)
			{
				...
				if ((int)nbClusters > mxPaliers) mxPaliers = (int)nbClusters;
			}
		}
	...
	template<class Impl>
	void ISimulation<Impl>::estimateMemoryForSplxPb(Antares::Data::StudyMemoryUsage& u, int & nbVars, int & nbConstraints)
	{
		/*
		...
		*/
		...
		linearCombination += nbConstraints * (60 * sizeof(double)  + 58 * sizeof(int) + 24 * sizeof(char));
	...
	void ISimulation<Impl>::regenerateTimeSeries(uint year)
	{
		...
		// Load
		if (pData.haveToRefreshTSLoad && (PreproOnly || !year || ((year % pData.refreshIntervalLoad) == 0)))
			GenerateTimeSeries<Data::timeSeriesLoad>(study, year);
		// Solar
		if (pData.haveToRefreshTSSolar && (PreproOnly || !year || ((year % pData.refreshIntervalSolar) == 0)))
			GenerateTimeSeries<Data::timeSeriesSolar>(study, year);
		// Wind
		if (pData.haveToRefreshTSWind && (PreproOnly || !year || ((year % pData.refreshIntervalWind) == 0)))
			GenerateTimeSeries<Data::timeSeriesWind>(study, year);
		// Hydro
		if (pData.haveToRefreshTSHydro && (PreproOnly || !year || ((year % pData.refreshIntervalHydro) == 0)))
			GenerateTimeSeries<Data::timeSeriesHydro>(study, year);
		// Thermal
		if (pData.haveToRefreshTSThermal && (PreproOnly || !year || ((year % pData.refreshIntervalThermal) == 0)))
														...
														)
	{
		...
		for (uint y = firstYear; y < endYear; ++y)
		{
			...
			refreshing = refreshing  ||  (	pData.haveToRefreshTSSolar  &&  (!y || ((y % pData.refreshIntervalSolar) == 0))		);
			refreshing = refreshing  ||  (	pData.haveToRefreshTSWind  &&  (!y || ((y % pData.refreshIntervalWind) == 0))		);
			refreshing = refreshing  ||  (	pData.haveToRefreshTSHydro  &&  (!y || ((y % pData.refreshIntervalHydro) == 0))		);
			refreshing = refreshing  ||  (	pData.haveToRefreshTSThermal  &&  (!y || ((y % pData.refreshIntervalThermal) == 0))	);						
			
			// We build a new set of parallel years if one of these conditions is fulfilled :
			//	- We have to refresh (or regenerate) some or all time series before running the current year
			//	- This is the first year (to be executed or not) after the previous set is full with years to be executed.
			//	  That is : in the previous set filled, the max number of years to be actually run is reached.
			...
			if (buildNewSet)
			{
				...
				if(refreshing)
				{
					...
					set->yearForTSgeneration = y;	// year number to be given to function "regenerateTimeSeries<false>(y /* year */)"
				}
			}
												...
												)
	{
		...
		for(ity = years.begin(); ity != years.end(); ++ity)
		{
			...
			for (unsigned int a = 0; a != nbAreas; ++a)
			{
				...
				{
					if (isPerformed)
						randomForYears.pYears[indexYear].pThermalNoisesByArea[a][c] = runtime.random[Data::seedThermalCosts].next();
					else
						runtime.random[Data::seedThermalCosts].next();
					// logs.info() << "      cluster : " << c << ", value : " << randomForYears.pYears[indexYear].pThermalNoisesByArea[a][c];
				}
			}
			...
			switch (powerFluctuations)
			{
				...
				case Data::lssMinimizeExcursions:
				{
					...
					{
						if (isPerformed)
							randomForYears.pYears[indexYear].pHydroCosts_rampingOrExcursion[areaIndex] = randomHydro();
						else
							randomHydro();
	...
	void ISimulation<Impl>::loopThroughYears(uint firstYear, uint endYear, std::vector<Variable::State> & state)
	{
		...
		// Gets information on each set of parallel years and returns the max number of years performed in a set
		...
		for(set_it = setsOfParallelYears.begin(); set_it != setsOfParallelYears.end(); ++set_it)
		{
			...
			for(year_it = set_it->yearsIndices.begin(); year_it != set_it->yearsIndices.end(); ++year_it)
			{
				...
				// gp - todo : ==================================================================================
				// gp - todo : Dans le cas d'une boucle "while" avant la boucle "for" concernant la gestion 
				...
				// Note that, when we enter for the first time in the "for" loop, all years
				// of the set have to be rerun (meaning : they must be run once).
				// if(!set_it->yearFailed[y]) continue;
			...
			ImplementationType::variables.computeSummary(set_it->spaceToPerformedYear, set_it->nbPerformedYears);
