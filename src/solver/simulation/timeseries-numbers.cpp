                                 ...
                                 AreaList& areas)
{
    areas.each([&](Area& area) {
        ...
        if (isTSintramodal[indexTS])
            area.hydro.series->timeseriesNumbersHydroMaxPower[year] = intramodal_draws[indexTS];
                                           ...
                                           Study& study)
{
    study.areas.each([&](Area& area) {
        ...
        if (!isTSintramodal[indexTS])
        {
            area.solar.series.timeseriesNumbers[year]
        ...
        if (!isTSintramodal[indexTS])
        {
            area.wind.series.timeseriesNumbers[year]
        ...
        if (!isTSintramodal[indexTS])
        {
            area.hydro.series->timeseriesNumbers[year]
        ...
        if (!isTSintramodal[indexTS])
        {
            ...
            if (nbTimeSeries != 1)
            {
                area.hydro.series->timeseriesNumbersHydroMaxPower[year] = static_cast<uint32_t>(
        ...
        for (auto& cluster : area.thermal.list.all())
        {
            ...
            else
            {
                if (!isTSintramodal[indexTS])
                {
                    cluster->series.timeseriesNumbers[year] = (uint32_t)(
        ...
        if (!isTSintramodal[indexTS])
        {
            for (auto it = area.links.begin(); it != area.links.end(); ++it)
            {
                ...
                if (nbTimeSeries > 1)
                {
                    link.timeseriesNumbers[year] = (uint32_t)(
                        floor(study.runtime->random[seedTimeseriesNumbers].next() * nbTimeSeries));
                }
            }
        }
...
static void applyMatrixDrawsToInterModalModesInArea(Data::TimeSeriesNumbers& tsNumbersMtx,
                                             Area& area,
                                             const array<bool, timeSeriesCount>& isTSintermodal,
                                             const uint years)
{
    for (uint year = 0; year < years; ++year)
    {
        ...
        if (isTSintermodal[ts_to_tsIndex.at(timeSeriesHydroMaxPower)])
            area.hydro.series->timeseriesNumbersHydroMaxPower[year] = draw;
