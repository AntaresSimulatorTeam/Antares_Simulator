		...
		for (uint w = 0; w != pNbWeeks; ++w)
		{
			...
			if ((state.simplexHasBeenRan = simplexIsRequired(hourInTheYear, numSpace)))
			{
				...
				for (uint ar = 0; ar != nbAreas; ++ar)
				{
					...
					{
						double& conso  = pProblemesHebdo[numSpace]->ConsommationsAbattues[hw]->ConsommationAbattueDuPays[ar];
						double& conso2 = pProblemesHebdo[numSpace]->ConsommationsAbattuesRef[hw]->ConsommationAbattueDuPays[ar];
						double stratReserve = area.reserves[Data::fhrStrategicReserve][hw + hourInTheYear];
			...
			else
			{
				...
				for (uint j = 0; j != nbHoursInAWeek; ++j, ++indx)
				{
					...
					for (uint k = 0; k != nbAreas; ++k)
					{
						...
						hourlyResults.TurbinageHoraire[j] = valgen.HydrauliqueModulableQuotidien[dayInTheYear] / 24.;
