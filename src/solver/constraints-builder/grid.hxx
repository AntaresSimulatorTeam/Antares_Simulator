...
namespace Antares
{
	...
	typename Grid<NodeT>::NodeP Grid<NodeT>::addNode(NodeT& n, std::string id)
	{
		...
		auto nodeIT = std::find_if(pNodesList.begin(), pNodesList.end(),
			[&id](const NodeP& nodeP)->bool {return nodeP->getName() == id; });
	...
	void Grid<NodeT>::kruskal()
	{
		...
		std::stable_sort(tempEdgesList.begin(), tempEdgesList.end(), typename Graph::Edge<NodeT>::compareWeight());
	...
	bool Grid<NodeT>::buildMesh()
	{
		...
		for (auto i = pEdgesList.begin(); i != pEdgesList.end(); i++)
		{
			...
			if (std::find(pMinSpanningTree.begin(), pMinSpanningTree.end(), (*i)) == pMinSpanningTree.end())
		...
		for (int i = 0; i < linksToBeAdded.size(); i++)
		{
			...
			//retrait des arêtes (u,v) qui sont dan Ei et remplacement dans le graphe duliqué par(u+,v-) et (u-,v+)
			for (int j = 0; j < v.size(); j++)
			{

				if (std::find(adjacentNodes.begin(), adjacentNodes.end(), v[j]->getOrigin()) == adjacentNodes.end())
					adjacentNodes.push_back(v[j]->getOrigin());
				if (std::find(adjacentNodes.begin(), adjacentNodes.end(), v[j]->getDestination()) == adjacentNodes.end())
					adjacentNodes.push_back(v[j]->getDestination());


				EdgeP ei = polarisedDuplicate.findEdgeFromNodeNames(v[j]->getOrigin()->getName()+"+", v[j]->getDestination()->getName() + "+");
				...
				ei = polarisedDuplicate.findEdgeFromNodeNames(v[j]->getOrigin()->getName() + "-", v[j]->getDestination()->getName() + "-");
	...
	*/
	...
	bool Grid<NodeT>::getDuplicatedGrid(Grid<NodeT>& grid)
	{
		...
		{
			{//+
				auto nodeOrigIT = std::find_if(grid.pNodesList.begin(), grid.pNodesList.end(),
					[&e](const NodeP& nodeP)->bool {return nodeP->getName() == (*e)->getOrigin()->getName()+"+"; });

				auto nodeDestIT = std::find_if(grid.pNodesList.begin(), grid.pNodesList.end(),
					[&e](const NodeP& nodeP)->bool {return nodeP->getName() == (*e)->getDestination()->getName() + "+"; });
	...
	*/
	...
	typename Grid<NodeT>::VectorEdgeP Grid<NodeT>::twoLevelPath(VectorNodeP vN)
	{
		...
		{
			SP = findShortestPath(findNodeFromName((*n)->getName()+"+"), findNodeFromName((*n)->getName() + "-"));
			length = std::accumulate(SP.begin(), SP.end(), (long)0,typename Graph::Edge< NodeT>::addpWeight());
			if (length < minLength)
			{
				minLength = length;
				minSP = SP;
