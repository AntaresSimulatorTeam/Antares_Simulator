...
namespace Antares
{
	...
	{
	public:
		areaInfo(Data::Area* area) { ptr = area; nodeName = area->name.to<std::string>(); }
	...
	class linkInfo
		...
		public:
			...
			{
				return getWeightWithImpedance() < other.getWeightWithImpedance() ? true : (getWeightWithImpedance() > other.getWeightWithImpedance() ? false : getName() < other.getName());
			}
		}; // class linkInfo
	...
	{
	public:
		State(std::vector<double> impedancesList, uint time, double infinite = 1000000) : secondMember(3, time), impedances(impedancesList)
		{
	...
	{
	public:
		Cycle(const std::vector<linkInfo*>& linkList, double infinite = 1000000) : time(0), loop (linkList), opType(Data::BindingConstraint::opEquality), pInfinite(infinite)
		{
			...
			{
				if((*line)->nImpedanceChanges > 0 || ((*line)->type == Antares::Data::atAC && (!(*line)->hasPShiftsEqual)))
					opType = Data::BindingConstraint::opBoth;
				impedances.push_back((*line)->dataLink->entry[columnImpedance][0]);

				time = (*line)->dataLink->height;/*BC loading always expects 8786 values heigth will have to be resized*/
				...
				{
					if (previousLine->with == (*line)->ptr->from || previousLine->from == (*line)->ptr->with) //[(A/B),(B/C),... ] or [(A,B),(C/A),... ]
					{
						
					}
					else if(previousLine->from == (*line)->ptr->from || previousLine->with == (*line)->ptr->with)
					{
						currentLineSign *= -1;
					}
					else
		...
		{
			std::vector<State>::iterator stIT = std::find_if(states.begin(), states.end(), [&impedances](State& s)->bool {return s.impedances == impedances; });
	...
	public:
		...
		*/
		...
		{
			auto linkIT = std::find_if(pLink.begin(), pLink.end(),
				[&u, &v](const linkInfo* edgeP)->bool {	if (edgeP->ptr->from->id == u && edgeP->ptr->with->id == v)
				return true;
		...
		void buildAreaToLinkInfosMap()
		{
			...
			{
				auto a = area.second;
				std::for_each(pLink.begin(), pLink.end(),
					[&a,this](linkInfo* edgeP){	if (edgeP->ptr->from == a || edgeP->ptr->with == a)
		...
		*/
		...
		double setInfinite(const long value) { return infiniteSecondMember = value; }

		bool setCheckNodalLoopFlow(const bool value) { return checkNodalLoopFlow = value; }

		bool setLoopFlowInclusion(const bool value) { return includeLoopFlow = value; }

		bool setPhaseShiftInclusion(const bool value) { return includePhaseShift = value; }

		double getInfinite() { return infiniteSecondMember; }

		bool getCheckNodalLoopFlow() { return checkNodalLoopFlow; }

		bool getLoopFlowInclusion() { return includeLoopFlow; }

		bool getPhaseShiftInclusion() { return includePhaseShift; }

		bool setUpToDate(const bool value) { return isUpToDate = value; }

		bool getUpToDate() { return isUpToDate; }
	...
	private:
		...
		*/
		Antares::Data::BindingConstraint* addConstraint(const Data::ConstraintName& name, const Yuni::String& op, 
			const Yuni::String& type, const WeightMap& weights, const double& secondMember);

		void addConstraint(const Data::ConstraintName& name, const Yuni::String& op, 
			const Yuni::String& type, const WeightMap& weights, const Matrix<double, double>& secondMember);
