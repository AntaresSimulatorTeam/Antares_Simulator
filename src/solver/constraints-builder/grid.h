...
namespace Antares
{
	namespace Graph
	{
		...
		{
		public:
			Node(NodeT* data) { nodeProperties = data; }
		...
		public:
			...
			Edge(Node<NodeT>* from, Node<NodeT>* to) { originNode = from; destinationNode = to; }
			
			long getWeight() const { return weight; }
			void setWeight(long w) { weight = w; }
			Node<NodeT>* getOrigin() { return originNode; }
			Node<NodeT>* getDestination() { return destinationNode; }
			void setOrigin(Node<NodeT>* from) { originNode = from; }
			void setDestination(Node<NodeT>* to ) { destinationNode = to; }
	...
	*/
	...
	public:
		...
		{
			auto edgeIT = std::find_if(pEdgesList.begin(), pEdgesList.end(),
				[&u, &v](const EdgeP& edgeP)->bool {	if (edgeP->getOrigin()->getName() == u && edgeP->getDestination()->getName() == v)
															return true;
		...
		{
			auto edgeIT = std::find_if(pMinSpanningTree.begin(), pMinSpanningTree.end(),
				[&u, &v](const EdgeP& edgeP)->bool {	if (edgeP->getOrigin()->getName() == u && edgeP->getDestination()->getName() == v)
				return true;
			if (edgeP->getDestination()->getName() == u && edgeP->getOrigin()->getName() == v)
				return true;
		...
		{
			auto nodeIT = std::find_if(pNodesList.begin(), pNodesList.end(), [&name](NodeP& n)->bool {return n->getName() == name; });
		...
		*/
		...
		const VectorEdgeP& getEdges() { return pEdgesList;}
	...
	public:
		...
		EdgeIncidence incidenceXOR(EdgeIncidence& e1, EdgeIncidence& e2)
		{
			...
			std::transform(e1.begin(), e1.end(), e2.begin(),
				Ei.begin(), std::bit_xor<bool>());
