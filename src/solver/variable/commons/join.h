...
namespace Variable
{
	...
	*/
	...
	public:
		...
		{
			LeftType ::template computeSpatialAggregatesSummary(allVars, numSpaceToYear, nbYearsForCurrentSummary);
			RightType::template computeSpatialAggregatesSummary(allVars, numSpaceToYear, nbYearsForCurrentSummary);
		}
		...
		const double* retrieveHourlyResultsForCurrentYear() const
		{
			...
			return (!result)
				? RightType:: template retrieveHourlyResultsForCurrentYear <VCardToFindT>()
				: result;
		}
