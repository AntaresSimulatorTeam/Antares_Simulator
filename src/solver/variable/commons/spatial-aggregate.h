...
namespace Common
{
	...
	{
		static const char* Caption(const uint indx) { return NULL; }
	};
	...
	struct VCardProxy
	{
		...
		//! Caption
		static const char* Caption() {return VCardOrigin::Caption();}
		//! Unit
		static const char* Unit() {return VCardOrigin::Unit();}
		//! The short description of the variable
		static const char* Description() {return VCardOrigin::Description();}
		...
		typedef typename VCardOrigin::IntermediateValuesTypeForSpatialAg IntermediateValuesTypeForSpatialAg;
	...
	private:
		...
		void internalSpatialAggregate(V& allVars, uint year, const SetT& set)
		{
			...
			if (!set.empty())
			{
				...
				if (VCardType::VCardOrigin::spatialAggregate & Category::spatialAggregateAverage)
					VariableAccessorType::MultiplyHourlyResultsBy(pValuesForTheCurrentYear[0], 1. / set.size());
				// The spatial cluster may be an average
				if (VCardType::VCardOrigin::spatialAggregate & Category::spatialAggregateSumThen1IfPositive)
		...
		void internalSpatialAggregateForCurrentYear(V& allVars, const SetT& set, uint numSpace)
		{
			...
			if (!set.empty())
			{
				...
				// The spatial cluster may be an average
				if (VCardType::VCardOrigin::spatialAggregate & Category::spatialAggregateAverage)
					VariableAccessorType::MultiplyHourlyResultsBy(pValuesForTheCurrentYear[numSpace], 1. / set.size());
				// The spatial cluster may be an average
				if (VCardType::VCardOrigin::spatialAggregate & Category::spatialAggregateSumThen1IfPositive)
