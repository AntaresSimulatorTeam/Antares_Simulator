...
namespace LINK_NAMESPACE
{
	...
	inline void Links::yearEndBuildPrepareDataForEachThermalCluster(State& state, uint year, unsigned int numSpace)
	{
		for (uint i = 0; i != pLinkCount; ++i)
		{
	...
	inline void Links::computeSummary(std::map<unsigned int, unsigned int> & numSpaceToYear, unsigned int nbYearsForCurrentSummary)
	{
		for (uint i = 0; i != pLinkCount; ++i)
		{
	...
	inline void Links::buildSurveyReport(SurveyResults& results, int dataLevel, int fileLevel, int precision) const
	{
	...
	inline void Links::buildAnnualSurveyReport(SurveyResults& results, int dataLevel, int fileLevel, int precision, uint numSpace) const
	{
		if (count && (dataLevel & Category::link))
		{
			assert(results.data.link != NULL
