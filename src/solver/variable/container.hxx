                                      ...
                                      unsigned int numSpace)
{
    ...
    enum
    {
        ...
        //variables were automatically printed. 		It is no longer used since we've offered the
        //possibility of selecting the output variables to be printed. 		Note that it is computed at
        //compile time using recurrences. 		We keep it here because it is the model of the newer
        //(runtime) way to compute any report's max number of columns 		in the context of a possible
        //user's choice of variables to be printed. 		As this choice is necessarily made at runtime,
        //this computation has to be made at runtime as well.
        maxColumnsNeededForExportation = BrowseAllVariables<NextType>::maxValue,
    };

    /* -- Old way to create an instance of the survey results class : deprecated since we can choose
     * variables to be printed (see comments above) */
    // The survey itself.
    ...
    // logs.debug() << "  (for " << static_cast<unsigned int>(maxColumnsNeededForExportation) << "
    // columns)"; auto* survey = new SurveyResults(maxColumnsNeededForExportation, *pStudy, output);
