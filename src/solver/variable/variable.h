	...
	*/
	...
	public:
		...
		*/
		...
		/*!
		** \brief Notify to all variables to prepare data for the close to year end calculations for each thermal cluster
		**
		...
		*/
		...
		template<class V> void yearEndSpatialAggregates(V& allVars, uint year);

		template<class V, class SetT> void yearEndSpatialAggregates(V& allVars, uint year, const SetT& set);

		template<class V> void simulationEndSpatialAggregates(V& allVars);

		template<class V, class SetT> void simulationEndSpatialAggregates(V& allVars, const SetT& set);
		//@}
		...
		*/
		...
		template<class VCardToFindT> const double* retrieveHourlyResultsForCurrentYear(uint numSpace) const;
