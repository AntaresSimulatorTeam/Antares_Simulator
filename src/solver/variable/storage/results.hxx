...
namespace Variable
{
	...
	inline void
	Results<FirstDecoratorT,DecoratorForSpatialAggregateT>::initializeFromStudy(Antares::Data::Study& study)
	{
	...
	Results<FirstDecoratorT,DecoratorForSpatialAggregateT>::buildSurveyReport(SurveyResults& report,
		const S& results, int dataLevel, int fileLevel, int precision) const
	{
		// Ask to fullfil the report to the first decorator
		DecoratorType::template buildSurveyReport<S,VCardT>(report, results, dataLevel, fileLevel, precision);
	}
