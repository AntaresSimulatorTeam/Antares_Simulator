...
namespace Private
{
	...
	void InternalExportDigestLinksMatrix(const Data::Study& study, const String& originalOutput,
		String& output, const char* title, Clob& pFileBuffer, const Matrix<>& matrix)
	{
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							# ifdef YUNI_OS_MSVC
							sizePrintf = ::sprintf_s(conversionBuffer + 1, sizeof(conversionBuffer)-2, "%.0f", v);
							# else
							sizePrintf = ::snprintf(conversionBuffer + 1, sizeof(conversionBuffer)-2, "%.0f", v);
							# endif
							if (sizePrintf >= 0)
...
namespace Variable
{
	...
	static inline uint
	GetRangeLimit(const Data::Study& study, int fileLevel, int precisionLevel, int index)
	{
		...
		if (not Math::Zero(v))
		{
			...
			else
			{
				...
				else
				{
					...
					# ifdef YUNI_OS_MSVC
					int sizePrintf = ::sprintf_s(conversionBuffer + 1, sizeof(conversionBuffer) - 2,
						precision.c_str(), v);
					# else
					int sizePrintf = ::snprintf(conversionBuffer + 1, sizeof(conversionBuffer) - 2,
						precision.c_str(), v);
					# endif
	...
	*/
	...
	static inline void
	WriteIndexHeaderToFileDescriptor(int precisionLevel, S& s, const StringT& areaname,
		const CaptionT& captions, uint columnIndex)
	{
		switch (precisionLevel)
		{
			case Category::hourly:
				{
					switch (Row)
					{
						case 0:  s << areaname << "\thourly\t\t\t";break;
						case 2:  s << "\tindex\tday\tmonth\thour";break;
						default: s << "\t\t\t\t";
					}
					break;
				}
			case Category::daily:
				{
					switch (Row)
					{
						case 0:  s << areaname << "\tdaily\t\t";break;
						case 2:  s << "\tindex\tday\tmonth";break;
						default: s << "\t\t\t";
					}
					break;
				}
			case Category::weekly:
				{
					switch (Row)
					{
						case 0:  s << areaname << "\tweekly";break;
						case 2:  s << "\tweek";break;
						default: s << '\t';
					}
					break;
			...
			case Category::monthly:
				{
					switch (Row)
					{
						case 0:  s << areaname << "\tmonthly\t";break;
						case 2:  s << "\tindex\tmonth";break;
						default: s << "\t\t";
					}
					break;
				}
			case Category::annual:
				{
					switch (Row)
					{
						case 0:  s << areaname << "\tannual";break;
						case 2:  s << '\t';break;
						default: s << '\t';
					}
					break;
	...
	void SurveyResults::exportDigestAllYears()
	{
		...
		if (not out.opened())
		{
			logs.error() << "impossible to open the file '" << data.output
				<< "' for appending data";
			return;
		}
	...
	void SurveyResults::saveToFile(int dataLevel, int fileLevel, int precisionLevel)
	{
		...
		if (data.area)
		{
			...
			WriteIndexHeaderToFileDescriptor<0>(precisionLevel, data.fileBuffer, areaname, captions, data.columnIndex);
			WriteIndexHeaderToFileDescriptor<1>(precisionLevel, data.fileBuffer, areaname, captions, data.columnIndex);
			WriteIndexHeaderToFileDescriptor<2>(precisionLevel, data.fileBuffer, areaname, captions, data.columnIndex);
		}
		else
		{
			WriteIndexHeaderToFileDescriptor<0>(precisionLevel, data.fileBuffer, "system", captions, data.columnIndex);
			WriteIndexHeaderToFileDescriptor<1>(precisionLevel, data.fileBuffer, "system", captions, data.columnIndex);
			WriteIndexHeaderToFileDescriptor<2>(precisionLevel, data.fileBuffer, "system", captions, data.columnIndex);
		}
	...
	void SurveyResults::EstimateMemoryUsage(uint maxVars, Data::StudyMemoryUsage& u)
	{
			...
			|| (u.study.parameters.yearByYear && u.mode != Data::stdmAdequacyDraft))
		{
			...
			// Il y a un thread par année MC et chaque thread construit dynamiquement un objet de type SurveyResults
			// pour ses outputs (voir container.hxx : void List<NextT>::exportSurveyResults(...))
