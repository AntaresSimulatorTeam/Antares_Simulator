...
namespace Variable
{
	...
	struct VariableAccessor
	{
		...
		static void ComputeSum(U& out, const VarT& var, uint numSpace)
		{
			for (uint i = 0; i != ColumnCountT; ++i)
			{
				Antares::Memory::Stored<double>::ConstReturnType src = var.retrieveRawHourlyValuesForCurrentYear(i, numSpace);
	...
	struct VariableAccessor<ResultsT, Category::dynamicColumns>
	{
		...
		template<class U> static void Reset(U& out)
		{
			const typename Type::const_iterator end = out.end();
			for (typename Type::const_iterator i = out.begin(); i != end; ++i)
				(*i).reset();
	...
	struct VariableAccessor<ResultsT, Category::singleColumn /* The default */ >
	{
		...
		{
			for (uint y = 0; y != maxHoursInAYear; ++y)
				intermediateValues.hour[y] = Yuni::Math::Abs(intermediateValues.hour[y]) > 0. ? 100. : 0.;
		}
		...
		static void ComputeSum(U& out, const VarT& var, uint numSpace)
		{
			Antares::Memory::Stored<double>::ConstReturnType src = var.retrieveRawHourlyValuesForCurrentYear(-1, numSpace);
			# ifdef ANTARES_SWAP_SUPPORT
			assert(src.valid());
			# else
			assert(src != NULL);
	...
	struct VariableAccessor<ResultsT, Category::noColumn>
	{
		...
		template<class U> static void Reset(U&)
		{
			// Do nothing
		}
	...
	struct Storage
	{
		...
		typedef typename VariableAccessor<
			typename VCardT::ResultsType, VCardT::columnCount>::Type ResultsType;
	};
	...
	{
		template<class U, class VarT> static void Perform(U&, const VarT&, uint) {}
	};
	...
	struct SpatialAggregateOperation<true, Category::spatialAggregateSum, VCardT>
	{
		...
		static void Perform(U& intermediateResults, const VarT& var, uint numSpace)
		{
			...
			VariableAccessor<ResultsType, VCardT::columnCount>::ComputeSum(intermediateResults, var, numSpace);
		}
	};
	...
	struct SpatialAggregateOperation<true, Category::spatialAggregateMax, VCardT>
	{
		...
		{
			typedef typename VCardT::ResultsType  ResultsType;
			VariableAccessor<ResultsType, VCardT::columnCount>::ComputeMax(intermediateResults, var, numSpace);
		}
	};
