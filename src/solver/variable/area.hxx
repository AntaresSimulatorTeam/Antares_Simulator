...
namespace Variable
{
	...
	template<class NextT>
	inline void Areas<NextT>::initializeFromThermalCluster(Data::Study*, Data::Area*, Data::ThermalCluster*)
	{
	...
	template<class NextT>
	void Areas<NextT>::buildSurveyReport(SurveyResults& results, int dataLevel, int fileLevel, int precision) const
	{
		if (count && (dataLevel & Category::link || dataLevel & Category::area || dataLevel & Category::thermalAggregate))
		{
	...
	template<class NextT>
	void Areas<NextT>::buildAnnualSurveyReport(SurveyResults& results, int dataLevel, int fileLevel, int precision, uint numSpace) const
	{
		if (count && (dataLevel & Category::link || dataLevel & Category::area || dataLevel & Category::thermalAggregate))
		{
			assert(results.data.area != NULL
			...
			// Build the survey results for the given area
			pAreas[area.index].buildAnnualSurveyReport(results, dataLevel, fileLevel, precision, numSpace);
		}
	}
	...
	template<class VCardToFindT>
	inline void
	Areas<NextT>::retrieveResultsForArea(typename Storage<VCardToFindT>::ResultsType** result, const Data::Area* area)
	{
	...
	template<class VCardToFindT>
	inline void
	Areas<NextT>::retrieveResultsForThermalCluster(typename Storage<VCardToFindT>::ResultsType** result, const Data::ThermalCluster* cluster)
	{
		pAreas[cluster->parentArea->index].template retrieveResultsForThermalCluster<VCardToFindT>(result, cluster);
	}
	...
	template<class VCardToFindT>
	inline void
	Areas<NextT>::retrieveResultsForLink(typename Storage<VCardToFindT>::ResultsType** result, const Data::AreaLink* link)
	{
