...
void AstGraphVisitor::emitNode(int id, const BoxStyle& box_style, std::ostream& os)
{
    os << "  " << id << " [label=\"" << box_style.label << "\", shape=\"" << box_style.shape
       << "\", style=\"" << box_style.style << "\", color=\"" << box_style.color << "\"];\n";
}
