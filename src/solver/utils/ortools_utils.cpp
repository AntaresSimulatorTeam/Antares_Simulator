...
static void setGenericParameters(MPSolverParameters& params)
{
    ...
    // ortools default is 1e-7 for primal tolerance, but this may be too high as we manipulate large values in the problem. Then 1e-7 may be too hard to achieve and has lead to declare some problems infeasible whereas they were not (contraint were active but not violated). Sirius uses 1e-6 (and this cannot be changed with ortools), this has effect for all solvers except sirius
