...
void HourlyCSRProblem::constructVariableFlows()
{
    ...
    for (uint32_t Interco = 0; Interco < problemeHebdo_->NombreDInterconnexions; Interco++)
    {
                 ...
                 == Antares::Data::AdequacyPatch::physicalAreaInsideAdqPatch)
        {
            ...
            int directVar;
            int indirectVar;
            algebraicFluxVar = variableManager_.FluxDirect(Interco, triggeredHour)
              = NumberOfVariables;
            problemeAResoudre_.TypeDeVariable[NumberOfVariables] = VARIABLE_BORNEE_DES_DEUX_COTES;
            logs.debug()
            ...
            directVar = variableManager_.FluxDirectPositif(Interco, triggeredHour)
              = NumberOfVariables;
            problemeAResoudre_.TypeDeVariable[NumberOfVariables] = VARIABLE_BORNEE_DES_DEUX_COTES;
            logs.debug() << NumberOfVariables << " direct flow[" << Interco << "]. ";
            NumberOfVariables++;

            indirectVar = variableManager_.FluxIndirectPositif(Interco, triggeredHour)
              = NumberOfVariables;
            problemeAResoudre_.TypeDeVariable[NumberOfVariables] = VARIABLE_BORNEE_DES_DEUX_COTES;
