...
ex: calling FluxDirect() implies adding Direct Flux Variable to the current Constraint
...
public:
    ...
    ConstraintBuilder& FluxDirect(unsigned int index, double coeff, int offset = 0, int delta = 0);

    ConstraintBuilder& FluxDirectPositif(unsigned int index, double coeff);

    ConstraintBuilder& FluxIndirectPositif(unsigned int index, double coeff);
