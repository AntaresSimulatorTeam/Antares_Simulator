...
void OPT_ConstruireLaMatriceDesContraintesDuProblemeLineaire(PROBLEME_HEBDO * ProblemeHebdo, uint numSpace)
{
	...
	for (Pays = 0; Pays < ProblemeHebdo->NombreDePays; Pays++) {
		...
		if (presenceHydro == OUI_ANTARES && TurbEntreBornes == OUI_ANTARES)
		{
			...
			ProblemeHebdo->NumeroDeContrainteMinEnergieHydraulique[Pays] = ProblemeAResoudre->NombreDeContraintes;
		...
		if (presenceHydro == OUI_ANTARES && TurbEntreBornes == OUI_ANTARES)
		{
			...
			ProblemeHebdo->NumeroDeContrainteMaxEnergieHydraulique[Pays] = ProblemeAResoudre->NombreDeContraintes;
	...
	for (Pdt = 0; Pdt < NombreDePasDeTempsPourUneOptimisation; Pdt++) {
		...
		for (Pays = 0; Pays < ProblemeHebdo->NombreDePays; Pays++) {
			if (ProblemeHebdo->CaracteristiquesHydrauliques[Pays]->SuiviNiveauHoraire == OUI_ANTARES)
			{
				...
				if (Pdt > 0)
				{
					Var1 = ProblemeHebdo->CorrespondanceVarNativesVarOptim[Pdt - 1]->NumeroDeVariablesDeNiveau[Pays];
					if (Var1 >= 0) {
						Pi[NombreDeTermes] = -1.0;
						Colonne[NombreDeTermes] = Var1;
						NombreDeTermes++;
				...
				CorrespondanceCntNativesCntOptim->NumeroDeContrainteDesNiveauxPays[Pays] = ProblemeAResoudre->NombreDeContraintes;
