...
void
OPT_InitialiserLesPminHebdo(PROBLEME_HEBDO* problemeHebdo)
{
    ...
    for (uint32_t pays = 0; pays < problemeHebdo->NombreDePays; pays++)
    {
        ...
        for (int palier = 0; palier < PaliersThermiquesDuPays.NombreDePaliersThermiques; palier++)
        {
            const std::vector<double>&
                    PuissanceMinDuPalierThermique = PuissanceDisponibleEtCout[palier]
                                                            .PuissanceMinDuPalierThermique;
            std::vector<double>&
                    PuissanceMinDuPalierThermiqueRef = PuissanceDisponibleEtCout[palier]
