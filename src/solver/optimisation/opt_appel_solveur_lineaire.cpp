                                          ...
                                          IResultWriter& writer)
{
    ...
    SingleOptimOptions modifiedOptions(options);
    modifiedOptions.solverUsesBasis &= problemeHebdo->weekInTheYear > 0;
    solver = ORTOOLS_Simplexe(ProblemeAResoudre.get(), solver, modifiedOptions);
    if (solver != nullptr)
    {
