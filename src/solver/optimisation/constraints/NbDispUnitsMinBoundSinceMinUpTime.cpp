...
void
NbDispUnitsMinBoundSinceMinUpTime::add(int pays, int index, int pdt)
{
    auto cluster = data.PaliersThermiquesDuPays[pays]
                           .NumeroDuPalierDansLEnsembleDesPaliersThermiques[index];
    const int DureeMinimaleDeMarcheDUnGroupeDuPalierThermique =
            data.PaliersThermiquesDuPays[pays]
                    .DureeMinimaleDeMarcheDUnGroupeDuPalierThermique[index];

    data.CorrespondanceCntNativesCntOptim[pdt]
            .NumeroDeContrainteDesContraintesDeDureeMinDeMarche[cluster] = -1;
    if (!data.Simulation)
    {
