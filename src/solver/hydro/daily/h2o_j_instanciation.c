...
DonneesMensuelles->ProblemeHydraulique = (PROBLEME_HYDRAULIQUE *) malloc( sizeof( PROBLEME_HYDRAULIQUE ) );
...
ProblemeHydraulique->NbJoursDUnProbleme = (int *) malloc( ProblemeHydraulique->NombreDeProblemes * sizeof( int ) );
...
ProblemeHydraulique->CorrespondanceDesVariables = (CORRESPONDANCE_DES_VARIABLES **) malloc( NombreDeProblemes * sizeof( CORRESPONDANCE_DES_VARIABLES ) );  
if ( ProblemeHydraulique->CorrespondanceDesVariables == NULL ) {
  return( 0 );
}
ProblemeHydraulique->CorrespondanceDesContraintes = (CORRESPONDANCE_DES_CONTRAINTES **) malloc( NombreDeProblemes * sizeof( CORRESPONDANCE_DES_CONTRAINTES ) );  
if ( ProblemeHydraulique->CorrespondanceDesContraintes == NULL ) {
  return( 0 );
}
ProblemeHydraulique->ProblemeLineairePartieFixe = (PROBLEME_LINEAIRE_PARTIE_FIXE **) malloc( NombreDeProblemes * sizeof( PROBLEME_LINEAIRE_PARTIE_FIXE ) );  
if ( ProblemeHydraulique->ProblemeLineairePartieFixe == NULL ) {
  return( 0 );
}
ProblemeHydraulique->ProblemeLineairePartieVariable = (PROBLEME_LINEAIRE_PARTIE_VARIABLE **) malloc( NombreDeProblemes * sizeof( PROBLEME_LINEAIRE_PARTIE_VARIABLE ) );
...
for ( i = 0 ; i < NombreDeProblemes ; i++ ) {
  CorrespondanceDesVariables[i] = (CORRESPONDANCE_DES_VARIABLES *) malloc( sizeof( CORRESPONDANCE_DES_VARIABLES ) );
  if ( CorrespondanceDesVariables[i] == NULL ) {
    return( 0 );
  }
  CorrespondanceDesContraintes[i] = (CORRESPONDANCE_DES_CONTRAINTES *) malloc( sizeof( CORRESPONDANCE_DES_CONTRAINTES ) );
  if ( CorrespondanceDesContraintes[i] == NULL ) {
    return( 0 );
  }
  ProblemeLineairePartieFixe[i] = (PROBLEME_LINEAIRE_PARTIE_FIXE *) malloc( sizeof( PROBLEME_LINEAIRE_PARTIE_FIXE ) );
  if ( ProblemeLineairePartieFixe[i] == NULL ) {
    return( 0 );
  }
  ProblemeLineairePartieVariable[i] = (PROBLEME_LINEAIRE_PARTIE_VARIABLE *) malloc( sizeof( PROBLEME_LINEAIRE_PARTIE_VARIABLE ) );
...
for ( i = 0 ; i < NombreDeProblemes ; i++ ) {
  ...
  H2O_j_ConstruireLesVariables( NbJoursDUnProbleme[i],
  ...
  H2O_J_ConstruireLesContraintes( NbJoursDUnProbleme[i],
                                  CorrespondanceDesVariables[i]->NumeroDeVariableTurbine,
  ...
  ProblemeLineairePartieFixe[i]->CoutLineaire[CorrespondanceDesVariables[i]->NumeroDeLaVariableMu] = 1.0;
  ProblemeLineairePartieFixe[i]->CoutLineaire[CorrespondanceDesVariables[i]->NumeroDeLaVariableXi] = 1.0;
