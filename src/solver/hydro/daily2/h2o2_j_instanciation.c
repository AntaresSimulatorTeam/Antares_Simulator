...
{
	int i; int * NbJoursDUnProbleme; int NombreDeProblemes; int NbPdt; int j; int NombreDeVariables;
	int NombreDeContraintes; int NombreDeTermesAlloues;
	...
	DonneesMensuellesEtendues = (DONNEES_MENSUELLES_ETENDUES *) malloc( sizeof(DONNEES_MENSUELLES_ETENDUES) );
	if ( DonneesMensuellesEtendues == NULL )
		return( NULL );


	DonneesMensuellesEtendues->ProblemeHydrauliqueEtendu = (PROBLEME_HYDRAULIQUE_ETENDU *) malloc( sizeof( PROBLEME_HYDRAULIQUE_ETENDU ) );
	...
	ProblemeHydrauliqueEtendu->NbJoursDUnProbleme = (int *) malloc( ProblemeHydrauliqueEtendu->NombreDeProblemes * sizeof( int ) );
	...
	DonneesMensuellesEtendues->TurbineCible = (double *) malloc( NbJoursDUnProbleme[3] * sizeof( double ) );
	...
	DonneesMensuellesEtendues->niveauxFinJours = (double *) malloc(NbJoursDUnProbleme[3] * sizeof(double));
	...
	ProblemeHydrauliqueEtendu->CorrespondanceDesVariables = (CORRESPONDANCE_DES_VARIABLES_PB_ETENDU **) malloc( NombreDeProblemes * sizeof( CORRESPONDANCE_DES_VARIABLES_PB_ETENDU ) );  
	if ( ProblemeHydrauliqueEtendu->CorrespondanceDesVariables == NULL )
		return( 0 );

	ProblemeHydrauliqueEtendu->ProblemeLineaireEtenduPartieFixe = (PROBLEME_LINEAIRE_ETENDU_PARTIE_FIXE **) malloc( NombreDeProblemes * sizeof( PROBLEME_LINEAIRE_ETENDU_PARTIE_FIXE ) );  
	if ( ProblemeHydrauliqueEtendu->ProblemeLineaireEtenduPartieFixe == NULL )
		return( 0 );

	ProblemeHydrauliqueEtendu->ProblemeLineaireEtenduPartieVariable = (PROBLEME_LINEAIRE_ETENDU_PARTIE_VARIABLE **) malloc( NombreDeProblemes * sizeof( PROBLEME_LINEAIRE_ETENDU_PARTIE_VARIABLE ) );
	...
	for ( i = 0 ; i < NombreDeProblemes ; i++ ) 
	{
		CorrespondanceDesVariables[i] = (CORRESPONDANCE_DES_VARIABLES_PB_ETENDU *) malloc( sizeof( CORRESPONDANCE_DES_VARIABLES_PB_ETENDU ) );
	...
	for ( i = 0 ; i < NombreDeProblemes ; i++ ) 
	{
		...
		PlFixe->CoefficientsDeLaMatriceDesContraintes = (double *) malloc( NombreDeTermesAlloues * sizeof( double ) );
		...
		PlVariable->AdresseOuPlacerLaValeurDesVariablesOptimisees = (double **) malloc( NombreDeVariables * sizeof( double * ) );
		...
		PlVariable->CoutsMarginauxDesContraintes = (double *) malloc( NombreDeContraintes * sizeof( double ) );
	...
	for ( i = 0 ; i < NombreDeProblemes ; i++ ) 
	{
		...
		H2O2_J_ConstruireLesContraintes(	NbJoursDUnProbleme[i],
		...
		for (int Pdt = 0; Pdt < NbPdt; Pdt++)
			ProblemeLineaireEtenduPartieFixe[i]->CoutLineaire[CorrespondanceDesVariables[i]->NumeroVar_niveauxFinJours[Pdt]] = -1./32.;
		
		
		for(int Pdt = 0; Pdt < NbPdt; Pdt++)
			ProblemeLineaireEtenduPartieFixe[i]->CoutLineaire[CorrespondanceDesVariables[i]->NumeroVar_overflow[Pdt]] = 32 * 68 + 1;

		
		for (int Pdt = 0; Pdt < NbPdt; Pdt++)
		 	ProblemeLineaireEtenduPartieFixe[i]->CoutLineaire[CorrespondanceDesVariables[i]->NumeroVar_deviations[Pdt]] = 1.;

		
		for (int Pdt = 0; Pdt < NbPdt; Pdt++)
			ProblemeLineaireEtenduPartieFixe[i]->CoutLineaire[CorrespondanceDesVariables[i]->NumeroVar_violations[Pdt]] = 68.;
		...
		ProblemeLineaireEtenduPartieFixe[i]->CoutLineaire[CorrespondanceDesVariables[i]->NumeroVar_violationMax] = 68.0;
