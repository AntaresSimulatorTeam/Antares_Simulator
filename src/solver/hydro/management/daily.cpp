...
namespace Antares
{
	...
	inline void HydroManagement::prepareDailyOptimalGenerations(Solver::Variable::State & state, Data::Area& area, uint y, uint numSpace)
	{
			...
			for (uint month = 0; month != 12; ++month)
			{
				...
				if (area.hydro.followLoadModulations)
				{
					...
					if (not Math::Zero(demandMax))
					{
						...
						{
							auto dYear = day + dayYear;
							dtg[dYear] = coeff * Math::Power(data.DLE[dYear] / demandMax, area.hydro.interDailyBreakdown);
						}
					}
					else
					{
		...
		if (not area.hydro.reservoirManagement)
		{
			for (uint month = 0; month != 12; ++month)	
			{
				...
				case OUI:
					...
					break;
				case NON:
					logs.fatal() << "Year : " << y + 1 << " - hydro: " << area.name << " [daily] no solution found";
		...
		else
		{
			...
			for (uint month = 0; month != 12; ++month)	
			{
				...
				problem.TurbineDuMois = (data.MOG[realmonth] + wasteFromPreviousMonth) / reservoirCapacity;
				...
				for (uint day = firstDay; day != endDay; ++day)
				{
					...
					problem.TurbineCible[dayMonth] = (dtg[day] + wasteFromPreviousMonth / daysPerMonth) / reservoirCapacity;
				...
				case OUI:
					...
					for (uint day = firstDay; day != endDay; ++day)
					{
						...
						valgen.HydrauliqueModulableQuotidien[day] = problem.Turbine[dayMonth] * reservoirCapacity;
					...
					for (uint day = firstDay + 1; day != endDay; ++day)
						valgen.NiveauxReservoirsDebutJours[day] = valgen.NiveauxReservoirsFinJours[day - 1];
			...
			// Level from scenario builder
			double levelFromSceanrioBuilder = study.scenarioHydroLevels[z][y];
			if(study.parameters.useCustomTSNumbers and levelFromSceanrioBuilder > 0)
				state.problemeHebdo->previousSimulationFinalLevel[z] = levelFromSceanrioBuilder * reservoirCapacity;
				...
				if (IO::Directory::Create(folder))
				{
					...
					if (file.openRW(filename))
					{
						...
						for (uint month = 0; month != 12; ++month)	
						{
							...
							file << monthName << "\n";
							file << "-------------\n";
							file << "\t\t\tNiveauMin\tApports\t\tTurbMax\t\tTurbCible\tTurbCible MAJ\tNiveaux D\tNiveaux F\tTurbines\t";
							file << "Overflows\tDeviations\tViolations\tDeviation Max\tViolation Max\tWaste\t\tCout total\tTurb mois no previous W\t\tTurb mois + previous W \n";
							...
							for (uint day = firstDay; day != endDay; ++day)
							{
								...
								double turbCibleUpdated = dtg[day] / reservoirCapacity + previousMonthWaste[realmonth] / daysPerMonth;
								file
									<< day << '\t'
									<< '\t'
									<< dayMonth << '\t'
								...
								if (dayMonth == 1)
								{
									file
										...
										<< '\t'
										<< '\t'
										<< (data.MOG[realmonth] / reservoirCapacity + previousMonthWaste[realmonth]) * 100;
								}
								file << '\n';
	...
	void HydroManagement::prepareDailyOptimalGenerations(Solver::Variable::State & state, uint y, uint numSpace)
	{
