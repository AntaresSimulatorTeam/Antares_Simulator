...
namespace Antares::Optimisation {
    VariablesBulkAddition::VariablesBulkAddition(
        ...
        Optimization::VariableDictionary &variableDictionary): linear_problem_(linear_problem),
    ...
    ComponentFiller::ComponentFiller(const ModelerStudy::SystemModel::Component &component,
                                     ...
                                     const ScenarioGroupRepository &scenarioGroupRepository): component_(component),
    ...
    void ComponentFiller::addVariables(Optimisation::LinearProblemApi::ILinearProblem &pb,
        ...
        for (const auto &variable: component_.getModel()->Variables() | std::views::values) {
            ...
            if (variable.isTimeDependent()) {
                Optimisation::LinearProblemApi::IScenario::TimeSeriesNumber chronicle = scenario.getData(
                    ctx.getYear());
                const Optimization::Dimensions dim(Optimization::IntegerInterval{
                                                       chronicle,
                                                       chronicle
                                                   }, /*TODO Handle range of chronicle ? */
                                                   Optimization::IntegerInterval(
                                                       ctx.getFirstTimeStep(), ctx.getLastTimeStep()));
                ...
                variableDictionary_.addVariable(
                    dim,
                    key,
                    [&pb, &lb, &ub, &variable](const Optimization::TimeAndScenario &, const std::string &name) {
