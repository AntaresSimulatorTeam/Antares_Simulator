...
namespace TSGenerator
{
	...
	namespace 
	{
		...
		private:
			...
			template<class T>
			void prepareIndispoFromLaw(Data::ThermalLaw law, double volatility, double A[], double B[],
				const T& duration);
		...
		void GeneratorTempData::writeResultsToDisk(const Data::Area& area, const Data::ThermalCluster& cluster)
		{
			if (not study.parameters.noOutput)
			{
				...
				pTempFilename.clear()
					<< study.folderOutput << SEP
					<< "ts-generator" << SEP << "thermal" << SEP << "mc-" << currentYear
					<< SEP << area.id
					<< SEP << cluster.id() << ".txt";
		...
		void GeneratorTempData::prepareOutputFoldersForAllAreas(uint year)
		{
			...
			study.areas.each([&] (Data::Area& area)
			{
				...
				{
					folder.clear();
					folder << study.folderOutput << SEP << "ts-generator"
						<< SEP << "thermal" << SEP << "mc-" << year << SEP << area.id;
		...
		template<class T>
		void GeneratorTempData::prepareIndispoFromLaw(Data::ThermalLaw law, double volatility, double A[], double B[],
			const T& duration)
		{
			switch (law)
			{
		...
		int GeneratorTempData::durationGenerator(Data::ThermalLaw law , int expec, double volat, double a, double b)
		{
				...
				for (uint dayInTheYear = 0; dayInTheYear < daysPerYear; ++dayInTheYear)
				{
					...
					if (lp[dayInTheYear] > 0. and lp[dayInTheYear] <= FAILURE_RATE_EQ_1)
					{
						...
						if (A > last)
						{
							...
							{
								last   = ((last * pp[dayInTheYear]) * ((double)(AUN_app + 1. - d))) / (double)d;
								cumul += last;
								POC = d;
								if (A <= cumul)
									break;
					...
					if (PFO != 0 or MXO != 0)
						FOD_reel = durationGenerator(f_law, FODOfTheDay, f_volatility, af[dayInTheYear], bf[dayInTheYear]);
					if (PPO != 0 or MXO != 0)
						POD_reel = durationGenerator(p_law, PODOfTheDay, p_volatility, ap[dayInTheYear], bp[dayInTheYear]);
