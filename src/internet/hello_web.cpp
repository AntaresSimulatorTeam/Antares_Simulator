...
bool CheckActivationKeyValidity(uint version,
                                Yuni::String& activationKey,
                                Yuni::String* error,
                                bool ignoreLicenseServer)
{
    if (error)
        error->clear();
    lastError = errFailed;
    ...
    foreach (auto& keyvalue, licenseproperties)
    {
        ...
        case 'h':
        {
            if (key == "host.allow")
            {
                ...
                {
                    LICENSE_DEBUG("checking hostid : " << hostid << ", from activation key: "
                                                       << value << ", status: failed");
                    continue; // can have several hostid
                }
                checkHostID = true;
                LICENSE_DEBUG("checking hostid : " << hostid << ", from activation key: " << value
                                                   << ", status: success");
                Limits::hostID
                  = hostid; // If value=* we send the hostid to the server and not the * value
            }
            break;
        }
        case 'l':
        ...
        case 'm':
        {
            ...
            {
                uint available
                  = (uint)Math::Round(((double)System::Memory::Total() / 1024 / 1024 / 1024));
    ...
    case stWaiting:
    {
        ...
        if (Antares::License::proxy.enabled)
        {
            ...
            if (!proxy.login.empty())
            {
                ...
                char* user
                  = curl_easy_escape(curl_handle, proxy.login.c_str(), proxy.login.length());
