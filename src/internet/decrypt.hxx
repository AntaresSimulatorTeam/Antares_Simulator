...
static bool DecodeAntaresKey(License::Properties& properties, StringT& content)
{
    ...
    // In a first time, we will extract the encrypted key (encrypted by our public RSA key, base64
    // encoded) used to encrypt informations about the host
    Yuni::String encryptedKey;
    {
        ...
        auto length = base64_decode_block(
          rawEncryptedKey.c_str(), rawEncryptedKey.size(), encryptedKey.data(), &state);
        if (length <= 0)
            return false;
    ...
    if (ForHostKey)
    {
        decryptedLen = RSA_private_decrypt(encryptedKey.size(),
                                           (const unsigned char*)encryptedKey.c_str(),
    ...
    {
        decryptedLen = RSA_public_decrypt(encryptedKey.size(),
                                          (const unsigned char*)encryptedKey.c_str(),
                                          (unsigned char*)decryptedKey.data(),
                                          privkey,
                                          RSA_PKCS1_PADDING);
    ...
    if (decryptedLen > 0)
    {
        ...
        base64_init_decodestate(&state);
        auto length = base64_decode_block(
          encodedBase64.c_str(), encodedBase64.size(), encoded.data(), &state);
        if (length <= 0)
            return false;
