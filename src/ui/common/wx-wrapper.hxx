...
inline wxString
wxStringFromUTF8(const Yuni::CString<ChunkT, FixedT>& s)
{
	# if wxUSE_UNICODE
	return wxString::FromUTF8(s.c_str(), s.size());
	# else
...
wxString DoubleToWxStringS(double f)
{
	...
	switch (PrecisionT)
	{
		case 0: tmp.format("%.0f", f);break;
		case 1: tmp.format("%.1f", f);break;
		case 2: tmp.format("%.2f", f);break;
		case 3: tmp.format("%.3f", f);break;
		case 4: tmp.format("%.4f", f);break;
		case 5: tmp.format("%.5f", f);break;
		case 6: tmp.format("%.6f", f);break;
		case 7: tmp.format("%.7f", f);break;
		case 8: tmp.format("%.8f", f);break;
		case 9: tmp.format("%.9f", f);break;
		default:
			tmp.format("%.f", f);break;
	}
	if (tmp.contains('.'))
	{
		tmp.trimRight('0');
