...
namespace Component
{
	...
	void Spotlight::createComponents(Spotlight* parent, bool input, bool results)
	{
		...
		if (input)
		{
			...
			// The search input
			pEdit = new wxTextCtrl(inputPanel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
				wxTE_PROCESS_ENTER | wxTE_LEFT);

			pLayerFilter = new wxComboBox(inputPanel, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, 
			...
			pLayerFilter->Connect(pLayerFilter->GetId(), wxEVT_COMBOBOX,
				wxCommandEventHandler(Spotlight::onComboUpdated), nullptr, this);
			...
			pEdit->Connect(pEdit->GetId(), wxEVT_COMMAND_TEXT_UPDATED,
				wxCommandEventHandler(Spotlight::onInputUpdated), nullptr, this);
		}
...
//				parentSizer->AddSpacer(5);
	...
	void Spotlight::convertRawTextIntoSearchTokenVector(SearchToken::Vector& out, const Yuni::String& text)
	{
		// assert
		assert(&out != NULL);
		...
		for (String::Vector::const_iterator i = ts.begin(); i != end; ++i)
		{
			...
			for (String::const_utf8iterator ti = tok.utf8begin(); ti != tend; ++ti, offset += (*ti).size())
			{
				const char c = *ti;
				if (c == '-')
				{
	...
	void Spotlight::FrameShow(wxWindow* parent, Antares::Component::Spotlight::IProvider::Ptr provider,
		uint flags, uint width)
	{
		...
		if (globalSpotlight->pResults)
			frame->SetSize((int)width, std::min( (int)(globalSpotlight->pResults->size() * globalSpotlight->itemHeight()) + offset, 330));
		// Display the toolbar
		if (parent)
		{
