...
static void DrawTextWithSelection(const AnyString& text,
                                  const wxString& wxtext,
                                  wxDC& dc,
...
static void DrawTag(wxDC& dc, const Spotlight::IItem::Tag& tag, wxRect& bounds, uint itemHeight)
{
    ...
    pCachedColorText.Set((uchar)Math::MinMax<int>((int)(yy + 1.403f * v), 0, 255),
                         (uchar)Math::MinMax<int>((int)(yy - 0.344f * u - 0.714f * v), 0, 255),
                         (uchar)Math::MinMax<int>((int)(yy + 1.770f * u), 0, 255));

    // Border color
    wxColour border;
    border.Set((uchar)Math::MinMax<int>(r - NODE_DRAW_COLOR_VARIATION_DARK * 2, 0, 255),
               (uchar)Math::MinMax<int>(g - NODE_DRAW_COLOR_VARIATION_DARK * 2, 0, 255),
               (uchar)Math::MinMax<int>(b - NODE_DRAW_COLOR_VARIATION_DARK * 2, 0, 255));
...
void Spotlight::IItem::draw(wxDC& dc,
                            uint itemHeight,
                            wxRect& bounds,
                            bool selected,
                            const SearchToken::VectorPtr& tokens) const
{
    ...
    if ((selected || pSelected) && (mayHasSelection))
    {
        ...
        if (pSelected)
        {
            a.Set((uchar)Math::MinMax<int>(selectionColor.Red() + 40, 0, 255),
                  (uchar)Math::MinMax<int>(selectionColor.Green() + 40, 0, 255),
                  (uchar)Math::MinMax<int>(selectionColor.Blue() + 40, 0, 255),
                  120);
        }
        else
        {
            a.Set((uchar)Math::MinMax<int>(gColorBackgroundDefault.Red() - 25, 0, 255),
                  (uchar)Math::MinMax<int>(gColorBackgroundDefault.Green() - 25, 0, 255),
                  (uchar)Math::MinMax<int>(gColorBackgroundDefault.Blue() - 25, 0, 255));
            border.Set((uchar)Math::MinMax<int>(gColorBackgroundDefault.Red() - 45, 0, 255),
