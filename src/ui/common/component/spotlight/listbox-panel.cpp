...
namespace Component
{
			...
			if ((0 != (pFlags & Spotlight::optGroups)))
			{
				if (!(!group) && group != pLastGroupName)
				{
					...
					int x = bounds.x + pLonguestGroupNameSize.GetWidth()
						- horizontalSpaceBetweenGroupsAndItems
						- groupBounds.GetWidth();
	...
	void ListboxPanel::updateItems(const Spotlight::IItem::VectorPtr& vptr, const Spotlight::SearchToken::VectorPtr& tokens)
	{
		uint maxSize = 0;
		...
		switch (resultCount)
		{
			case 0:  pCacheResultText << wxT("No item found");break;
			case 1:  pCacheResultText << wxT("1 item found");break;
			default: pCacheResultText << resultCount << wxT(" items found");
		}
