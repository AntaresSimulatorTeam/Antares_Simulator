...
namespace Datagrid
{
		...
		pColorMappingRowLabels(true)
	{
		...
		// Dynamic event
		GetGridCornerLabelWindow()->Connect(wxEVT_PAINT, wxPaintEventHandler(DBGrid::evtCornerPaint),
			nullptr, GetGridCornerLabelWindow());
		GetGridColLabelWindow()->Connect(wxEVT_PAINT,
				wxPaintEventHandler(DBGrid::onDrawColLabels),
				nullptr, GetGridColLabelWindow());
		GetGridRowLabelWindow()->Connect(wxEVT_PAINT,
				wxPaintEventHandler(DBGrid::onDrawRowLabels),
				nullptr, GetGridRowLabelWindow());

		// Erase background
		GetGridCornerLabelWindow()->Connect(wxEVT_ERASE_BACKGROUND, wxEraseEventHandler(DBGrid::onEraseBackground));
		GetGridWindow()->Connect(wxEVT_ERASE_BACKGROUND, wxEraseEventHandler(DBGrid::onEraseBackground));

		// These two events should not be used, otherwise some parts
		// of the panel will not be redrawn
		// GetGridColLabelWindow()->Connect(wxEVT_ERASE_BACKGROUND, wxEraseEventHandler(DBGrid::onEraseBackground));
		// GetGridRowLabelWindow()->Connect(wxEVT_ERASE_BACKGROUND, wxEraseEventHandler(DBGrid::onEraseBackground));
	}
	...
	void DBGrid::evtCornerPaint(wxPaintEvent&)
	{
		...
		dc.GradientFillLinear(wxRect(0, 0, client_width, client_height),
			gradientColStart, gradientColEnd, wxSOUTH);
