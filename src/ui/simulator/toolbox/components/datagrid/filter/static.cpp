...
namespace Filter
{
	...
	struct ResultCaption
	{
		...
		static const wxChar* Default() {return wxT("(null)");}
		template<typename U>
		static const wxChar* Value(U) {return T::Caption();}
	};
	...
	struct ResultNewInstance
	{
		...
		static AFilterBase* Default() {return NULL;}
		static T* Value(Input* parent) {return new T(parent);}
	};



	template<template<class> class Result, typename I>
	static typename Result<void>::Type FindFilterFromName(const wxString& name, I i,
