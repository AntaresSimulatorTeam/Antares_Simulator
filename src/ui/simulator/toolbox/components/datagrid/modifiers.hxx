...
namespace // anonymous
{
	...
	// Forward declaration
	template<enum ModifierSet ModifierT> struct ModifierOperatorsData;
	...
	struct ModifierOperatorsData<modifierValues>
	{
		enum Operator
		{
			opPlus, opAssign, opMinus, opMult, opDiv, opAbs,
			opMax
		};
		...
		static const wchar_t* OperatorToCString(uint op)
		{
			if (op < (uint) opMax)
			{
				switch ((Operator) op)
				{
					case opPlus:   return L"+";
					case opAssign: return L"=";
					case opMinus:  return L"-";
					case opMult:   return L"*";
					case opDiv:    return L"/";
					case opAbs:    return L"ABS";
					case opMax:    break;
				}
			}
			return L"?";
	...
	struct ModifierOperatorsData<modifierDataset>
	{
		...
		static const wchar_t* OperatorToCString(uint op)
		{
			if (op < (uint) opMax)
			{
				switch ((Operator )op)
				{
					case opShiftRows:    return L"Shift rows until";
					case opResizeColumns: return L"Resize columns to";
					case opMax:    break;
				}
			}
			return L"?";
		...
		static void ApplyChanges(uint op, const YString& value, Renderer::IRenderer* renderer, VGridHelper*)
		{
			if (op >= (uint) opMax)
				return;
							...
							if (maxwidth > 0)
							{
								switch (newwidth)
								{
									...
									case 1:
										...
										break;
									default:
										logs.error() << "impossible to resize the matrix to " << newwidth << " columns";
								}
							}
							else
								logs.error() << "impossible to resize the matrix";
	...
	struct ModifierOperators
	{
		static uint OperatorCount(ModifierSet modifier)
		{
			switch (modifier)
			{
				case modifierValues:  return ModifierOperatorsData<modifierValues>::OperatorCount();
				case modifierDataset: return ModifierOperatorsData<modifierDataset>::OperatorCount();
				case modifierMax: break;
			}
			return 0;
		}
		...
		static const wchar_t* Name(ModifierSet modifier)
		{
			switch (modifier)
			{
				case modifierValues:  return ModifierOperatorsData<modifierValues>::Name();
				case modifierDataset: return ModifierOperatorsData<modifierDataset>::Name();
				case modifierMax: break;
			}
			return L"?";
		}
		...
		static const wchar_t* ApplyName(ModifierSet modifier, uint op)
		{
			switch (modifier)
			{
				case modifierValues:  return ModifierOperatorsData<modifierValues>::ApplyName(op);
				case modifierDataset: return ModifierOperatorsData<modifierDataset>::ApplyName(op);
				case modifierMax: break;
			}
			return L"?";
		}
		...
		static const wchar_t* OperatorToCString(ModifierSet modifier, uint op)
		{
			switch (modifier)
			{
				case modifierValues:  return ModifierOperatorsData<modifierValues>::OperatorToCString(op);
				case modifierDataset: return ModifierOperatorsData<modifierDataset>::OperatorToCString(op);
				case modifierMax: break;
			}
			return L"?";
		}
		...
		static OpInputType OperatorInputType(ModifierSet modifier, uint op)
		{
			switch (modifier)
			{
				case modifierValues:  return ModifierOperatorsData<modifierValues>::OperatorInputType(op);
				case modifierDataset: return ModifierOperatorsData<modifierDataset>::OperatorInputType(op);
				case modifierMax: break;
			}
			return opInputText;
		}

		static void ApplyChanges(ModifierSet modifier, uint op, const YString& value, Renderer::IRenderer* renderer, VGridHelper* grid)
		{
			if (!renderer)
				return;
			switch (modifier)
