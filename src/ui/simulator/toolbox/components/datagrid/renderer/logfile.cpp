...
namespace Renderer
{
	...
	wxString LogFile::columnCaption(int x) const
	{
		switch (x)
		{
			case 0: return wxT("                    Date                    ");
			//case 1: return wxT("Application");
			case 1: return wxT("Level");
			case 2: return wxT("                           Message                           ");
		}
	...
	wxString LogFile::cellValue(int x, int y) const
	{
		...
		switch (x)
		{
			case 0: return wxStringFromUTF8(entry.date);
			//case 1: return wxStringFromUTF8(entry.application);
			case 1: return wxStringFromUTF8(entry.verbosity);
	...
	wxColour LogFile::cellBackgroundColor(int x, int y) const
	{
		...
		{
			case LogEntry::vtInfo:        return entry.highlight ? wxColour(245, 247, 254) : wxColour(255, 255, 255);
			case LogEntry::vtWarning:     return (x == 1) ? wxColour(255, 134, 12)  : wxColour(255, 239, 222);
			case LogEntry::vtError:       return (x == 1) ? wxColour(206, 0,   0)   : wxColour(255, 228, 228);
			case LogEntry::vtCheckpoint:  return (x == 1) ? wxColour(120, 136, 206) : wxColour(221, 228, 244);
			case LogEntry::vtNotice:      return (x == 1) ? wxColour(50,  167, 34)  : wxColour(236, 244, 233);
			case LogEntry::vtDebug:       return wxColour(255, 255, 255);
		}
	...
	wxColour LogFile::cellTextColor(int x, int y) const
	{
		...
		switch (entry.verbosityType)
		{
			case LogEntry::vtInfo:
				return (x == 1) ? wxColour(121, 135, 163)  : (entry.highlight
					? wxColour(95, 7, 7) : wxColour(42, 42, 47));
