...
namespace Renderer
{
	...
	thermalScBuilderRenderer::thermalScBuilderRenderer(Toolbox::InputSelector::Area* notifier) : ScBuilderRendererBase()
	{
		if (notifier)
		{
	...
	wxString thermalScBuilderRenderer::rowCaption(int rowIndx) const
	{
		if (!(!study) && !(!pRules))
		{
			if (pArea && (uint) rowIndx < pArea->thermal.list.size())
				return wxString() << wxT(" ") << wxStringFromUTF8(pArea->thermal.list.byIndex[rowIndx]->name()) << wxT("  ");
		}
