				...
				wxString LayersUI::cellValue(int x, int y) const
				{
					if (!(!study) && (uint)x < study->layers.size() && pArea)
					{
						...
						case 1:
							...
							return layerPosition != layerListEnd ? wxString::Format(wxT("%d"), pArea->ui->layerX[layerIterator->first]) : wxT("-");

						case 2:

							return layerPosition != layerListEnd ? wxString::Format(wxT("%d"), pArea->ui->layerY[layerIterator->first]) : wxT("-");
						}
					}
					return wxEmptyString;
				}
