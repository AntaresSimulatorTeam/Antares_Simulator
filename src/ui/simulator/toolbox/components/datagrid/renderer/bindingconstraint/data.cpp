...
namespace BindingConstraint
{
	...
	int Data::width() const
	{
		return !study
			? 0
			: (uint) study->uiinfo->byOperator[pOperator][pType].size();
	}


	int Data::height() const
	{
		return (pType == Antares::Data::BindingConstraint::typeHourly)
			? 8760 : 365;
	}
	...
	void Data::applyLayerFiltering(size_t layerID, VGridHelper * gridHelper)
	{
		...
		for (int x = 0; x < gridHelper->virtualSize.x; ++x)
		{
			...
			Antares::Data::BindingConstraint* constraint = study->uiinfo->byOperator[pOperator][pType][x];
	...
	{
		return ((y % 2)
			? IRenderer::cellStyleDefaultAlternate
