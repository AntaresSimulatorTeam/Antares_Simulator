...
namespace Renderer
{
	...
	ThermalClusterCommonModulation::ThermalClusterCommonModulation(wxWindow* control, Toolbox::InputSelector::ThermalCluster* notifier)
		:Renderer::Matrix<>(control),
		pCluster(nullptr)
	{
		if (notifier)
			notifier->onThermalClusterChanged.connect(this, &ThermalClusterCommonModulation::internalThermalClusterChanged);
	}
	...
	wxString ThermalClusterCommonModulation::columnCaption(int column) const
	{
		switch (column)
		{
			case Data::thermalModulationCost: return wxT(" Marginal Cost \n modulation ");
			case Data::thermalModulationMarketBid: return wxT(" Market bid \n modulation ");
			case Data::thermalModulationCapacity: return wxT(" Capacity \n modulation ");
			case Data::thermalMinGenModulation: return wxT(" Min Gen \n modulation ");
			case (Data::thermalModulationCost + Data::thermalModulationMax): return wxT("Net\n Marginal Cost ");
			case (Data::thermalModulationMarketBid + Data::thermalModulationMax): return wxT("Net\n Market bid ");
			case (Data::thermalModulationCapacity + Data::thermalModulationMax): return wxT("Net\n Capacity ");
			case (Data::thermalMinGenModulation + Data::thermalModulationMax): return wxT("Net\n Min Gen ");
		...
		if (pMatrix && (uint) y < pMatrix->height && pCluster)
		{
			switch (x)
			{
				...
				case Data::thermalModulationMarketBid:
					return DoubleToWxString(Math::Round((*pMatrix)[Data::thermalModulationMarketBid][y], 3));
				...
				case (Data::thermalMinGenModulation + Data::thermalModulationMax):
					return DoubleToWxString(Math::Round((*pMatrix)[Data::thermalMinGenModulation][y]
						* pCluster->unitCount * pCluster->nominalCapacity , 2));
			}
		}
		return wxT("0");
	}
	...
	double ThermalClusterCommonModulation::cellNumericValue(int x, int y) const
	{
		if (pMatrix && (uint) y < pMatrix->height && pCluster)
		{
			...
			switch (x)
			{
				...
				case (Data::thermalMinGenModulation + Data::thermalModulationMax):
					return (*pMatrix)[Data::thermalMinGenModulation][y] * pCluster->unitCount * pCluster->nominalCapacity;
			}
		}
		return 0.;
	}
	...
	void ThermalClusterCommonModulation::internalThermalClusterChanged(Antares::Data::ThermalCluster* cluster)
	{
		if (pCluster != cluster)
		{
			pCluster = cluster;
	...
	IRenderer::CellStyle ThermalClusterCommonModulation::cellStyle(int col, int row) const
	{
		...
		{
			if (pCluster->minDivModulation.isCalculated && v/ceil(v) < pCluster->minDivModulation.border
