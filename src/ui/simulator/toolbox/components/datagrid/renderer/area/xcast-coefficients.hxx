...
namespace Renderer
{
	...
	inline XCastCoefficients<T>::XCastCoefficients(wxWindow* control, Toolbox::InputSelector::Area* notifier) :
	...
	{
		static const wxString captions[Data::XCast::dataMax] =
		{
			wxT("        alpha        "),
			wxT("        beta         "),
			wxT("       gamma       "),
						...
						case XCast::dtUniform:
							{
								...
								//	return MatrixAncestorType::cellValue(x, y, String(xcast.data.entry[XCast::dataCoeffDelta][y]));
								break;
							}
						case XCast::dtBeta:
							{
						...
						case XCast::dtUniform:
							{
								...
								//	return MatrixAncestorType::cellValue(x, y, xcast.data.entry[XCast::dataCoeffGamma][y]);
								break;
							}
						case XCast::dtBeta:
							{
	...
	IRenderer::CellStyle XCastCoefficients<T>::cellStyle(int col, int row) const
	{
		...
		return Yuni::Math::Zero(d)
			? IRenderer::cellStyleDefaultDisabled : IRenderer::cellStyleDefault;
	}
	...
	{
		return ((uint) rowIndx < 12)
			? wxString() << (1 + rowIndx) << wxT(" - ") << wxStringFromUTF8(Date::MonthToString(rowIndx))
			: wxString();
	}
