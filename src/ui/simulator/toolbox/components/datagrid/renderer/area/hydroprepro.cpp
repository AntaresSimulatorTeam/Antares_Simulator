...
HydroPreproHydroclusterCluster::HydroPreproHydroclusterCluster(wxWindow* control, 
...
HydroPreproHydroclusterCluster::~HydroPreproHydroclusterCluster()
{
    destroyBoundEvents();
}

int HydroPreproHydroclusterCluster::width() const
{
    return Data::PreproHydro::hydroPreproMax /* + 3 reservoir levels*/;
}

int HydroPreproHydroclusterCluster::height() const
{
    return 12;
}

wxString HydroPreproHydroclusterCluster::columnCaption(int colIndx) const
{
    switch (colIndx)
    {
    case 0:
...
wxString HydroPreproHydroclusterCluster::cellValue(int x, int y) const
{
    /*	if (x >= 5 && x < 8)
            {
                    if (!pArea)
...
double HydroPreproHydroclusterCluster::cellNumericValue(int x, int y) const
{ /*if (x >= 5 && x < 8)
         {
                 if (!pArea)
                         return 0.;
...
bool HydroPreproHydroclusterCluster::cellValue(int x, int y, const String& value)
{
    double v;
    if (!value.to(v))
        return false;
...
void HydroPreproHydroclusterCluster::internalHydroclusterClusterChanged
(Antares::Data::HydroclusterCluster* hydroclusterCluster)
{
    // FIXME for some reasons, the variable study here is not properly initialized
    ...
    auto* pPreproHydro = (hydroclusterCluster) ? hydroclusterCluster->prepro : nullptr;
    Renderer::ARendererHydroclusterCluster::internalHydroclusterClusterChanged(hydroclusterCluster);
    if (pPreproHydro)
    {
        MatrixAncestorType::matrix(&pPreproHydro->data);//MatrixAncestorType::matrix(nullptr);
    }
    else
        MatrixAncestorType::matrix(nullptr);
}

IRenderer::CellStyle HydroPreproHydroclusterCluster::cellStyle(int col, int row) const
{
    if (!pMatrix || (uint)Data::PreproHydro::hydroPreproMax > pMatrix->width
        || (uint)row >= pMatrix->height)
        return IRenderer::cellStyleWithNumericCheck(col, row);
...
wxString HydroPreproHydroclusterCluster::rowCaption(int rowIndx) const
{
    return wxStringFromUTF8(Date::MonthToString(rowIndx));
}

bool HydroPreproHydroclusterCluster::valid() const
{
    return MatrixAncestorType::valid();
}

bool HydroPreproHydroclusterCluster::circularShiftRowsUntilDate(MonthName month, uint daymonth)
{
    if (pHydroclusterCluster)
        pHydroclusterCluster->reservoirLevel.circularShiftRows(month);
    return MatrixAncestorType::circularShiftRowsUntilDate(month, daymonth);
}

void HydroPreproHydroclusterCluster::onStudyClosed()
{
    MatrixAncestorType::onStudyClosed();
    Renderer::ARendererHydroclusterCluster::onStudyClosed();
}

void HydroPreproHydroclusterCluster::onStudyLoaded()
{
    MatrixAncestorType::onStudyLoaded();
