...
namespace Datagrid
{
	...
	void InternalState::updaterModifiersControls()
	{
		...
		auto* opname = ModifierOperators::OperatorToCString(modifier.selectedSet, modifier.selectedAction);
		auto input   = ModifierOperators::OperatorInputType(modifier.selectedSet, modifier.selectedAction);
	...
	void InternalState::createAllInternalControls(const CreateOptions& flags)
	{
		...
		{
			pLayerFilter = new wxComboBox(component, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(-1, 22),
			...
			pLayerFilter->Connect(pLayerFilter->GetId(), wxEVT_COMBOBOX,
				wxCommandEventHandler(Component::onComboUpdated), nullptr, component);

			auto* ssz = new wxBoxSizer(wxHORIZONTAL);
			ssz->AddSpacer(15);
			ssz->Add(CreateLabel(component, wxT("Layer"), false, true), 0, wxALIGN_CENTER_VERTICAL | wxALL);
			ssz->Add(3, 3);
			ssz->AddSpacer(26);
		...
		if (flags.hasFilter)
		{
			auto* ssz = new wxBoxSizer(wxHORIZONTAL);
			ssz->AddSpacer(15);
			ssz->Add(CreateLabel(component, wxT("Filter"), false, true), 0, wxALIGN_CENTER_VERTICAL|wxALL);
			ssz->Add(3, 3);
			filter.component = new Toolbox::Filter::Component(component, precision);
			filter.component->onUpdateSearchResults.connect(this, &InternalState::applyLayerFiltering);
			filter.component->precision(precision);
	...
	Component::Component(wxWindow* parent, Renderer::IRenderer* renderer,
		const wxString& caption, bool hasFilter, bool colorMappingRowLabels, bool copypasteOnly,
		bool readonly, bool hasLayerFilter) :
	...
	void Component::createModifyPanel(wxSizer* sizer, bool copypasteOnly, bool readonly)
	{
...

		{
			...
			if (not readonly)
			{
				btn = new Antares::Component::Button(grp->subpanel, wxEmptyString, "images/16x16/open.png",
					this, &Component::onLoadMatrix);
				hz->Add(btn, 0, wxALIGN_CENTER_VERTICAL|wxALL);
			}

			btn = new Antares::Component::Button(grp->subpanel, wxEmptyString, "images/16x16/savestudy.png",
				this, &Component::onSaveMatrix);
			hz->Add(btn, 0, wxALIGN_CENTER_VERTICAL|wxALL);
			...
			// COPY / PASTE
			btn = new Antares::Component::Button(grp->subpanel, wxT("Copy"), "images/16x16/copy.png",
				this, &Component::onCopyValues);
			btn->dropDown(true);
			...
			if (not readonly)
			{
				btn = new Antares::Component::Button(grp->subpanel, wxT("Paste"), "images/16x16/paste.png",
					this, &Component::onPasteValues);
				hz->Add(btn, 0, wxALIGN_CENTER_VERTICAL|wxALL);
			}
		}
		...
		if (false and not copypasteOnly)
		{
			...
			auto* sbmp = Resources::StaticBitmapLoadFromFile(grp, wxID_ANY, "images/16x16/update_values.png");
			grp->leftSizer->Add(sbmp, 0, wxALL|wxALIGN_CENTRE_VERTICAL);

			auto* hz = grp->subpanel->GetSizer();
			auto* operatorGroup = new Antares::Component::Button(grp->subpanel, wxT("Change values"), nullptr);
			...
			color.Set(
				(unsigned char) Math::MinMax<int>(color.Red()   + 20, 0, 255),
			...
			auto* textValue = new wxTextCtrl(grp->subpanel, wxID_ANY, wxT("0"),
				wxDefaultPosition, wxSize(42,18), wxBORDER_NONE); // , Toolbox::Validator::Numeric());
			hz->Add(textValue, 0, wxALL|wxALIGN_CENTER_VERTICAL);
			internal.modifier.ui.textValue = textValue;

			auto* btnSelectCalendar = new Antares::Component::Button(grp->subpanel, wxT("(date not set)"));
			...
			Antares::Component::AddVerticalSeparator(grp->subpanel, hz, 2);
			auto* apply = new Antares::Component::Button(grp->subpanel, wxT("Apply"), nullptr, this, &Component::onModifyAll);
	...
	void Component::onLoadMatrix(void*)
	{
		...
		auto* dialog = new wxFileDialog(this,
			wxString(wxT("Load")),
		...
		if (dialog->ShowModal() == wxID_OK)
		{
			...
			{
				Matrix<> m;
				if (m.loadFromCSVFile(stdFilename, 1, internal.gridHelper->GetNumberRows(), Matrix<>::optImmediate))
				{
					wxString value;
	...
	void Component::onSaveMatrix(void*)
	{
		...
		wxFileDialog* dialog = new wxFileDialog(this,
			wxString(wxT("Save a "))
				<< internal.gridHelper->GetNumberCols() << wxT("x")
	...
	*/
	...
	void Component::copyToClipboard()
	{
		...
		if (internal.grid and internal.gridHelper)
		{
			...
			else
			{
				...
				{
					clipboard->add(internal.gridHelper,
						grid->currentPosition().x, grid->currentPosition().y,
						grid->currentPosition().x + 1, grid->currentPosition().y + 1,
						false, false);
				}
			}
			clipboard->copy();
...

		{
			...
			for (auto i = rows.begin(); i != end; ++i)
			{
				...
				else
				{
					...
					{
						logs.error() << "Impossible to paste the matrix "
							<< rect.x2 << 'x' << rect.y2
							<< " from the clipboard at position ("
							<< rect.x << ',' << rect.y << ')'
							<< " (Out of bounds)";
						return;
					}
				...
				for (auto j = cols.begin(); j != cend; ++j)
				{
					...
					logs.debug() << "  done writing  : " << (rect.x + x) << "x" << (rect.y + y) << " = " << *j;
					++x;
				}
				++y;
			}
	...
	void Component::createPopupCopy(Button&, wxMenu& menu, void*)
	{
		...
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::onCopyValuesFromMenu), nullptr, this);

		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Copy the entire matrix with styles (HTML)"));
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::onCopyAllValuesFromMenu), nullptr, this);
	}
	...
	void Component::onModifyAll(void*)
	{
		...
		ModifierOperators::ApplyChanges(internal.modifier.selectedSet, internal.modifier.selectedAction,
			value, internal.renderer, internal.gridHelper);
	...
	void Component::onPopupMenuModifierSet(Button&, wxMenu& menu, void*)
	{
		for (uint i = 0; i != (uint) modifierMax; ++i)
		{
			...
			Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
				wxCommandEventHandler(Component::onChangeModifierSet), userdata, this);
		}
	}
	...
	void Component::onPopupMenuModifierOperators(Button&, wxMenu& menu, void*)
	{
		...
		for (uint i = 0; i != count; ++i)
		{
			...
			auto* it = Menu::CreateItem(&menu, wxID_ANY, caption,
				"images/16x16/empty.png", wxEmptyString);
			Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
				wxCommandEventHandler(Component::onChangeModifierOperator), userdata, this);
		}
	}
