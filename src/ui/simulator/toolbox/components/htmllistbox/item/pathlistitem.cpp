...
namespace Item
{
	...
	bool PathListItem::HtmlContent(wxString& out, wxString name, const wxString& searchString, bool isFolder, Yuni::uint64 size)
	{
		...
		out	<< wxT("<td width=5></td><td width=16 bgcolor=\"")
			...
			<< ColorDarker(c.Red(), c.Green(), c.Blue()).GetAsString(wxC2S_HTML_SYNTAX)
			<< wxT("\">");
		if (isFolder)
			out << wxT("D");
		else
		...
		if (size > 1)
		{
			...
			{
				if (size < 1024 * 1024)
					out << wxT("<font size=\"-2\" color=\"#A9B2DB\">") << (size/1024) << wxT(" Kb</font>");
				else
					out << wxT("<font size=\"-2\" color=\"#A9B2DB\">") << (size/1024/1024) << wxT(" Mb</font>");
			}
		}
		// Post
		out << wxT("</td>");
