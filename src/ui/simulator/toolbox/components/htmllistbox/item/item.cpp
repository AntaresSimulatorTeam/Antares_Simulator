...
namespace Item
{
	...
	IItem::IItem()
		:pHighlighted(false), pVisible(true)
	{
	}
		...
		if (!toHighlight.empty())
		{
			...
			if (regex.Matches(s))
			{
				...
				regex.ReplaceFirst(&s,
					(wxString(wxT("<font color=red><u><b>")) << rep << wxT("</b></u></font>") ));
				return true;
			}
		}
		return false;
	...
	{
		return wxColour(
			Yuni::Math::MinMax<int>(r - level, 0, 255),
	...
	{
		return wxColour(
			Yuni::Math::MinMax<int>((int)c.Red()   - level, 0, 255),
