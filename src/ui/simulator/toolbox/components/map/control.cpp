...
namespace Map
{
	...
	wxBitmap Control::getRenderedMapImage(const MapRenderOptions& mapOptions)
	{
		...
		if	(mapOptions.transparentBackground && mapOptions.fileFormat != Antares::Map::mapImageFormat::mfJPG)
			bmp.SetMask(new wxMask(bmp, pBackgroundColor)); // suppress the background color to let it transparent
			...
			case mouseActionSelectionBox:
				{
					...
					// New coordinates
					const wxPoint a (pSelectionBox.first.x + pCachedOrigin.x, pCachedOrigin.y - pSelectionBox.first.y);
					const wxPoint b (pSelectionBox.second.x + pCachedOrigin.x - a.x, pCachedOrigin.y - pSelectionBox.second.y - a.y);
					// Draw
	...
	void Control::drawTools(DrawingContext& drawingContext)
	{
		...
		if (not pTools.empty() && (mouseActionNone == pMouseAction || mouseActionTool == pMouseAction))
		{
			...
			for (auto i = pTools.begin(); i != end; ++i)
				(*i)->draw(drawingContext, mouseDown && (pLastSelectedTool == *i), pCurrentMousePositionGraph, pCurrentMousePosition);
		}
	}
	...
	void Control::paintGraph(wxDC& dc, bool inFile)
	{
		...
		else
		{
			...
			//changing the drawing origin so the exported picture will have the bounding box drawn in the upper-left corner
		...
		// A drawing context
		DrawingContext  drawingContext(dc, cxtOrigin, cxtOffsetForSelectedNodes,cxtCurrentScroll, cxtClientSize, cxtBottomRight, getUid(), inFile);
	...
	void Control::refreshCache()
	{
		...
		BoundingBox box = nodes.boundingBox(getUid());
		pCachedBoxSize.x = Math::Max<int>(Math::Max<int>(abs(box.first.x), Math::Abs(box.second.x)) * 2, DEFAULT_MAP_WIDTH);
		pCachedBoxSize.y = Math::Max<int>(Math::Max<int>(abs(box.first.y), Math::Abs(box.second.y)) * 2, DEFAULT_MAP_HEIGHT);
	...
	void Control::addToolsForCurrentSelection()
	{
		...
		if (haveNodes || haveConnection)
		{
			Private::AddingToolsHelper helper(
				nodes, pTools,
				nodes.selectedItemsCount(), nodes.selectedItemsAsConnectionCount(),
	...
	void Control::removeTools(const Tool::LifeSpan filter)
	{
		if (!pTools.empty())
		{
			pTools.erase(std::remove_if(pTools.begin(), pTools.end(), RemovalToolPredicate(filter)), pTools.end());
		}
	}
	...
	void Control::addNewNodeWhereTheMouseIs()
	{
		...
		if (not newName.empty())
		{
			...
			Node* newNode = nodes.addNode(wxStringFromUTF8(id), wxStringFromUTF8(newName),
				pCurrentMousePositionGraph.x, pCurrentMousePositionGraph.y);
	...
	void Control::addNewNode(uint count)
	{
		...
		for (uint i = 1; i <= count; ++i)
		{
			...
			Data::AreaName id;
			TransformNameIntoID(newName, id);
			auto* newNode = nodes.addNode(wxStringFromUTF8(id), wxStringFromUTF8(newName),
				x + offsetX, y + offsetY);
