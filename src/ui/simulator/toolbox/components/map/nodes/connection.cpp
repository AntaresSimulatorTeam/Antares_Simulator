...
namespace Map
{
	...
	static inline wxPoint ComputeBarycentre(const wxPoint& a, const wxPoint& b, const int coeff1, const int coeff2)
	{
		if (!coeff1 && !coeff2)
			return wxPoint();
		return wxPoint(
			(coeff1 * a.x + coeff2 * b.x) / (coeff1+coeff2),
	...
	*/
	static int DistanceBetweenAPointAndASegment(const double pointX, const double pointY, const double x1,
	...
	*/
	...
	{
		return (pA && pB) &&
			(distance = (DistanceBetweenAPointAndASegment(x, y, pA->x(), pA->y(), pB->x(), pB->y()))
	...
	void Connection::drawArrow(wxDC& dc, const Connection::Direction direction, const wxPoint& rA, const wxPoint& rB)
	{
		wxPoint b1;
		wxPoint b2;
		wxPoint b3;
		...
		const wxPoint pBase (
			(int) (pCachedArrow[0].x + -ta * vecLine[0]),
	...
	void Connection::draw(DrawingContext& dc)
	{
		...
		if (!pCaption.empty())
		{
			...
			// Position
			pCachedAbsolutePosition.x = rCenterA.x + (rCenterB.x - rCenterA.x) / 2 - pCachedSize.x / 2 + 4;
			pCachedAbsolutePosition.y = rCenterA.y + (rCenterB.y - rCenterA.y) / 2 - pCachedSize.y / 2 + 2;
