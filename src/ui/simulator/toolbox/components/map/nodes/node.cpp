	...
	*/
	...
	void Node::refreshCache(wxDC& dc)
	{
		...
		if (pAttachedArea && isVisibleOnLayer(pManager.getActiveLayerId()))
		{
			...
			if (ui.layerX.find(activeLayerId) != ui.layerX.end() && ui.layerY.find(activeLayerId) != ui.layerY.end()  && ui.layerColor.find(activeLayerId) != ui.layerColor.end())
			{
				if (pX != ui.layerX[activeLayerId] || pY != ui.layerY[activeLayerId] || (int)pColor.Red() != ui.layerColor[activeLayerId][0])
				{
	...
	void Node::draw(DrawingContext& dc)
	{
		...
		refreshCache(device);
		wxPoint rTopLeft(dc.origin().x + pCachedPosition.x, dc.origin().y - pCachedPosition.y - pCachedSize.y);
		...
		if (rTopLeft.x < dc.scroll().x - pCachedSize.x || rTopLeft.x > dc.bottomRight().x + pCachedSize.x)
			return;
		if (rTopLeft.y < dc.scroll().y - pCachedSize.y || rTopLeft.y > dc.bottomRight().y + pCachedSize.y)
			return;
		...
		if (pSelected)
			device.DrawRoundedRectangle(rTopLeft.x - 3, rTopLeft.y - 3, pCachedSize.x + 6, pCachedSize.y + 6, 2);
		else
		...
		device.GradientFillLinear(wxRect(rTopLeft.x, rTopLeft.y + pCachedSize.y / 2, pCachedSize.x,
				pCachedSize.y / 2 + 1
				),
