...
namespace Map
{
	...
	{
		assert(gInstance != nullptr && "No manager Instance was created. Accessing this object will result into error");
		return gInstance;
	}
	...
	bool Manager::loadFromStudy(Data::Study& study)
	{
		...
		study.areas.each([&] (Data::Area& area)
		{
			...
			Node* node = this->addNode(wxStringFromUTF8(area.id),
				wxStringFromUTF8(area.name), area.ui->x, area.ui->y);
			...
			for (NodeByZPosition::iterator i = pAllNodes.begin(); i != end; ++i)
			{
				...
				for (NodeMap::iterator j = i->second.begin(); j != jEnd; ++j)
				{
					...
					if (node)
					{
						...
						for (Data::Area::Vector::const_iterator areaI = areaList.begin(); areaI != areaEnd; ++areaI)
						{
		...
		if (item and !pAllNodes.empty())
		{
			...
			if (cnx && mouseSelectionLinks)
			{
				...
				if (mouseSelectionConstraints)
				{
					...
					for (Data::BindConstList::iterator i = pStudy->bindingConstraints.begin(); i != cEnd; ++i)
					{
						bool stop = false;
						...
						for (Data::BindingConstraint::const_iterator ly = constraint->begin(); ly != lend; ++ly)
						{
		...
		if (!pAllNodes.empty())
		{
...

			{
				...
				for (NodeByZPosition::const_iterator i = pAllNodes.begin(); i != end; ++i)
				{
					...
					for (NodeMap::const_iterator j = i->second.begin(); j != jEnd; ++j)
					{
						bool gotcha = j->first->isContained(nx1, ny1, nx2, ny2) && j->first->isVisibleOnLayer(layerID);
						if (!gotcha)
						{
							j->first->selected(false);
							continue;
						...
						if (node)
						{
							...
							if (mouseSelectionPlants)
							{
								...
								for (Data::ThermalClusterList::iterator t = area->thermal.list.begin(); t != tend; ++t)
			...
			if (mouseSelectionConstraints)
			{
				...
				for (Data::BindConstList::iterator i = pStudy->bindingConstraints.begin(); i != end; ++i)
				{
					bool stop = false;
					// alias to the current constraint
					Data::BindingConstraint* constraint = *i;
