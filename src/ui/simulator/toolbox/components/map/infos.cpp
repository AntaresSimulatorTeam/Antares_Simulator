...
namespace Map
{
	...
	wxBoxSizer* PanelInfos::createColorPicker(const wxColour& color)
	{
		...
		wxButton* btn = new wxButton(this, PanelInfos::idDefaultColor, wxT("Default"), wxDefaultPosition, wxDefaultSize, wxBU_EXACTFIT);
		...
		if (!pItemList.empty())
		{
			...
			for (ItemList::iterator i = pItemList.begin(); i != end; ++i)
			{
				if ((*i)->type() == Item::tyConnection)
					(dynamic_cast<Connection*>(*i))->direction((evt.IsChecked() ? Connection::dirDirect : Connection::dirIndirect));
			}
			GetParent()->Refresh();
		}
	}
	...
	void PanelInfos::countItemSpecies(const PanelInfos::ItemList& list, int& nodes, int& connections) const
	{
		nodes = 0;
		connections = 0;
		if (!list.empty())
		{
			...
			for (ItemList::const_iterator i = list.begin(); i != end; ++i)
			{
				switch ((*i)->type())
				{
					case Item::tyNode : ++nodes; break;
					case Item::tyConnection : ++connections; break;
					default: break;
				}
			}
		}
	...
	void PanelInfos::layoutNoSelection()
	{
		...
		GetSizer()->Add(new wxStaticText(this, wxID_ANY, wxT("No selection"),
			wxDefaultPosition, wxDefaultSize, wxALIGN_CENTRE), 0, wxALL|wxEXPAND);
		// Another spacer
		GetSizer()->AddStretchSpacer();
	}
	...
	void PanelInfos::layoutPropertiesForNodes(const int nodeCount)
	{
		...
		if (1 == nodeCount)
		{
			...
			wxTextCtrl* edit = new wxTextCtrl(this, idAreaName, pItemList[0]->caption(),
		...
		gridSizer->Add(new wxStaticText(this, wxID_ANY, wxT("Color ")),
			0, wxRIGHT | wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL);
		gridSizer->Add(createColorPicker((nodeCount == 1)
				? pItemList[0]->color() : findCommonColour(pItemList)),
				0, wxALL|wxALIGN_CENTER_VERTICAL|wxALIGN_LEFT);
	...
	{
		wxSizer* sizer = GetSizer();
		layoutAddTitle(((connectionCount == 1) ? wxT("Inspector - Connection") : wxT("Inspector - Connections")));
		wxFlexGridSizer* gridSizer = new wxFlexGridSizer(2, 1, 0);
		sizer->Add(gridSizer, 0, wxALL|wxEXPAND, 5);
	...
	void PanelInfos::layoutFiltersForConnections(const int connectionCount)
	{
		...
		// Filters
		sizer->AddSpacer(10);
		layoutAddTitle(	(connectionCount == 1) ? wxT("Output Filters - Connection") : wxT("Output Filters - Connections"));
