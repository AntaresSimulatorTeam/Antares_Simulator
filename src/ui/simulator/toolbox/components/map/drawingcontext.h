...
namespace Map
{
	...
	*/
	...
	public:
		...
		*/
		DrawingContext(wxDC& dc, const wxPoint& origin, const wxPoint& offset,
			const wxPoint& scroll, const wxPoint& clientSize, const wxPoint& btRight, const size_t& layer = -1, bool forFileExport = false);
		//! Destructor
		~DrawingContext() {}
		//@}
		...
		*/
		const wxPoint& origin() const {return pOrigin;}

		/*!
		** \brief Get the layer id of the device context
		*/
		const size_t& getLayerId() const { return layerId; }
		
		/*!
		** \brief Get the device context
		*/
		bool isForFileExport() {return pForFileExport;}
		
		/*!
		** \brief Get the device context
		*/
		wxDC& device() {return pDC;}
		...
		** mouse release.
		*/
		const wxPoint& offsetForSelectedNodes() const {return pOffsetForSelectedNodes;}

		/*!
		** \brief Get the scrolling values for the horizontal and the vertical scrollbars
		*/
		const wxPoint& scroll() const {return pScroll;}

		/*!
		** \brief Get the client size
		*/
		const wxPoint& clientSize() const {return pClientSize;}

		/*!
		** \brief Get the absolute position of the point at the bottom right
		*/
		const wxPoint& bottomRight() const {return pBottomRight;}
