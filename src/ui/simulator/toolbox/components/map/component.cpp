...
namespace Map
{
		...
		pSelectionPopUpMenu(nullptr)
	{
		...
		Menu::CreateItem(pSelectionPopUpMenu, Antares::Forms::MenusID::mnIDPopupSelectionHide,
			wxT("Hide "),
			"images/16x16/empty.png", wxEmptyString);
		Menu::CreateItem(pSelectionPopUpMenu, Antares::Forms::MenusID::mnIDPopupSelectionShow,
			wxT("Show "),
			"images/16x16/empty.png", wxEmptyString);
		pSelectionPopUpMenu->Connect(Antares::Forms::MenusID::mnIDPopupSelectionHide, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(Component::evtOnSelectionHide), NULL, this);
		pSelectionPopUpMenu->Connect(Antares::Forms::MenusID::mnIDPopupSelectionShow, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(Component::evtOnSelectionShow), NULL, this);
	...
	void Component::onPageChanged(Antares::Component::Notebook::Page& page)
	{
		for (size_t i = 0, size = pNoteBook->pPages.size(); i < size; i++)
		{
			if (pNoteBook->pPages[i] == &page && i < mapLayersPtrList.size())
			{
				...
				pMapActiveLayer->pBtnSelectionConstraint->pushed(pMapActiveLayer->nodes.mouseSelectionConstraints);
			}
		}
		refresh();
	}
	...
	Antares::Component::MapNotebook::Page* Component::addNewLayer(wxString pageName, size_t uID)
	{
		...
		wxWindow* lblMouse = Antares::Component::CreateLabel(panelHeader, wxT("mouse :"), false, true, -1);
		lblMouse->SetToolTip(wxT("Mouse selection"));
		hs->Add(lblMouse, 0, wxALL | wxALIGN_CENTER_VERTICAL);
		hs->AddSpacer(4);

		pMapLayer->pBtnSelectionArea =
			new Antares::Component::Button(panelHeader, wxEmptyString, "images/16x16/area.png",
				this, &Component::onToggleMouseSelectionArea);
		...
		pMapLayer->pBtnSelectionLink =
			new Antares::Component::Button(panelHeader, wxEmptyString, "images/16x16/link.png",
				this, &Component::onToggleMouseSelectionLink);
		...
		pMapLayer->pBtnSelectionConstraint =
			new Antares::Component::Button(panelHeader, wxEmptyString, "images/16x16/constraint.png",
				this, &Component::onToggleMouseSelectionConstraint);
		...
		// New area
		auto* btnNewArea =
			new Antares::Component::Button(panelHeader, wxEmptyString, "images/16x16/plus.png", this, &Component::onNew);
		...
		// Copy
		auto* btnCopy =
			new Antares::Component::Button(panelHeader, wxT("Copy"), "images/16x16/copy.png", this, &Component::onCopy);
		...
		// Paste
		auto* btnPaste =
			new Antares::Component::Button(panelHeader, wxT("Paste"), "images/16x16/paste.png", this, &Component::onPaste);
		...
		// Select all
		auto* btnSelectAll =
			new Antares::Component::Button(panelHeader, wxT("Select all"), nullptr, this, &Component::onSelectAll);
		...
		// Re-center
		auto* btnCenter =
			new Antares::Component::Button(panelHeader, wxEmptyString, "images/16x16/target.png", this, &Component::onCenterXY);
		...
		wxString testString;
		auto* pointX = new wxTextCtrl(panelHeader, wxID_ANY, wxT("0"), wxDefaultPosition, wxSize(50, -1), wxTE_PROCESS_ENTER, wxTextValidator(wxFILTER_NUMERIC, &testString));
		pointX->SetBackgroundColour(Settings::background);
		pointX->Connect(wxEVT_COMMAND_TEXT_ENTER, wxCommandEventHandler(Component::onEvtCenterXY), 0, this);
		...
		auto* pointY = new wxTextCtrl(panelHeader, wxID_ANY, wxT("0"), wxDefaultPosition, wxSize(50, -1), wxTE_PROCESS_ENTER, wxTextValidator(wxFILTER_NUMERIC, &testString2));
		pointY->SetBackgroundColour(Settings::background);
		pointY->Connect(wxEVT_COMMAND_TEXT_ENTER, wxCommandEventHandler(Component::onEvtCenterXY), 0, this);
		...
		// Re-size
		auto* btnResize =
			new Antares::Component::Button(panelHeader, wxEmptyString, "images/16x16/crop.png", this, &Component::onCenter);
		...
		if (pageName == wxString(""))
		{
			...
			while (pNoteBook->find(wxString::FromUTF8("Map ") << pMapLayer->getUid() + numberOffset, false) != nullptr)
				numberOffset++;

			resultPage = pNoteBook->add(p, wxString::FromUTF8("Map ") << pMapLayer->getUid() + numberOffset, wxString::FromUTF8("Map ") << pMapLayer->getUid() + numberOffset);
	...
	{
		if (pNoteBook->pPages[0] == &page)//close action on the first layer is dealed with in a specific way
		{
	...
	void Component::attachStudy(Data::Study::Ptr study)
	{
		...
		if (pMapActiveLayer)
		{
			...
			if (pMapActiveLayer->nodes.study()->layers.size() > 0)
			{
				...
				std::map<size_t, std::string>::iterator iterator = pMapActiveLayer->nodes.study()->layers.begin();
				mapLayersPtrList[0]->pPage->name(iterator->second);
				size_t i = 1;
				for (iterator++; iterator != pMapActiveLayer->nodes.study()->layers.end(); iterator++, i++)
				{
					Antares::Component::Notebook::Page * newPage= addNewLayer(iterator->second, iterator->first);
	...
	bool Component::saveToAttachedStudy(bool incremental)
	{
		...
		return (!(!study))
			? saveToStudy(*study, incremental) : false;
	}

	bool Component::saveToImageFile(const AnyString& filePath, const Antares::Map::MapRenderOptions& options)
	{
		...
		switch (options.fileFormat)
		{
			...
			case mfSVG:
			{
				...
				wxSVGFileDC svgDC(wxStringFromUTF8(filePath), boundsRect.GetWidth()+80, boundsRect.GetHeight()+80);
	...
	void Component::refreshHeaderInformations()
	{
		if (pMapActiveLayer)
		{
			...
			if (cArea < 2)
				pMapActiveLayer->getpInfosAreaCount()->SetLabel(wxString(wxT(' ')) << cArea << wxT(" area, "));
			else
				pMapActiveLayer->getpInfosAreaCount()->SetLabel(wxString(wxT(' ')) << cArea << wxT(" areas, "));
	...
	{
		return pMapActiveLayer
			? pMapActiveLayer->nodes.selectedItemsCount() : 0u;
	}
	...
	void Component::onCopyDropdown(Antares::Component::Button&, wxMenu& menu, void*)
	{
		...
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Copy the selection to the clipboard\tCtrl+C"),
			"images/16x16/copy.png");
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtPopupCopy), nullptr, this);

		menu.AppendSeparator();

		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Copy all items"), nullptr);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtPopupCopyAll), nullptr, this);

		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Copy all areas"), nullptr);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtPopupCopyAllAreas), nullptr, this);
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Copy all links"), nullptr);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtPopupCopyAllLinks), nullptr, this);
	}
	...
	void Component::onPasteDropdown(Antares::Component::Button&, wxMenu& menu, void*)
	{
		...
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Paste\tCtrl+V"),
			"images/16x16/copy.png");
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtPopupPaste), nullptr, this);

		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Paste special..."), "images/16x16/empty.png");
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtPopupPasteSpecial), nullptr, this);
	}
	...
	{
			pMapActiveLayer->nodes.mouseSelectionConstraints = !pMapActiveLayer->nodes.mouseSelectionConstraints;
	}
	...
	void Component::onNewDropdown(Antares::Component::Button&, wxMenu& menu, void*)
	{
		...
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Create a new area"),
			"images/16x16/area.png");
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtNewArea), nullptr, this);

		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Tip : Press N to directly create a new area"),
			nullptr);
		it->Enable(false);
		...
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtNewArea2), nullptr, this);
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Create 4 new areas"), nullptr);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtNewArea4), nullptr, this);
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Create 6 new areas"), nullptr);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtNewArea6), nullptr, this);
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Create 8 new areas"), nullptr);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Component::evtNewArea8), nullptr, this);
	}
