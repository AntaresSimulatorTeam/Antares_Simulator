	...
	namespace Component
	{
			...
			Tabs(parent, notebook), undrawnLeftTabs(0), remainingRightTabs(0), pTabNameCtrl(nullptr)
		{
			Connect(GetId(), wxEVT_MOTION,
				wxMouseEventHandler(MapTabs::onMouseMove), NULL, this);
			Connect(GetId(), wxEVT_LEAVE_WINDOW,
				wxMouseEventHandler(MapTabs::onMouseLeave), NULL, this);
			//add Page button
			addPageButton = new tabButton("images/16x16/white_add_18.png", this, tabButton::btnNone, "images/16x16/grey_add_18.png");
			//right navigation button
			rightTabScroll = new tabButton("images/16x16/white_arrow_right_18.png", this, tabButton::btnRight, "images/16x16/grey_arrow_right_18.png");
			//left navigation button
			leftTabScroll = new tabButton("images/16x16/white_arrow_left_18.png", this, tabButton::btnLeft, "images/16x16/grey_arrow_left_18.png");
		}
		...
		void MapNotebook::MapTabs::onDraw(wxPaintEvent&)
		{
			...
			if (dc.IsOk())
			{
				...
				if (dc.IsOk())
				{
					...
					addPageButton->drawButton(dc, pRect.GetWidth() - addPageButton->buttonImage->GetWidth(), (pRect.height >> 2) - (addPageButton->buttonImage->GetHeight() >> 2));
		...
		void MapNotebook::MapTabs::drawOrientationTop(wxDC& dc)
		{
			...
			{
			case themeDefault: ori.Set(80, 80, 80); break;
			case themeLight:   ori.Set(101, 109, 123); break;
			}
			...
			// Gradient inside
			dc.GradientFillLinear(wxRect(pRect.x, pRect.y, pRect.width, pRect.height / 2),
				a1, b1, wxSOUTH);
			dc.GradientFillLinear(wxRect(pRect.x, pRect.y + pRect.height / 2, pRect.width, pRect.height / 2),
				a2, b2, wxSOUTH);
			...
			{
				if (pNotebook.pAlwaysDisplayTab || pNotebook.pPages.size() > 2 ||(pNotebook.pPages.size() == 2 && Data::Study::Current::Get()->showAllLayer))
				{
					...
					case alignLeft:
					{
						...
						if (!pNotebook.pCaption.empty())
						{
							...
							dc.DrawText(pNotebook.pCaption, 31, 1 + (pRect.height >> 1) - (size.GetHeight() >> 1));
							dc.SetTextForeground(wxColour(255, 166, 25));
							dc.DrawText(pNotebook.pCaption, 30, (pRect.height >> 1) - (size.GetHeight() >> 1));
					...
					case alignRight:
					{
						...
						if (!pNotebook.pCacheTitle.IsEmpty())
						{
							...
							dc.SetTextForeground(wxColour(50, 50, 50));
							dc.DrawText(pNotebook.pCacheTitle, 31, 1 + (pRect.height >> 1) - (size.GetHeight() >> 1));
							dc.SetTextForeground(wxColour(255, 166, 25));
							dc.DrawText(pNotebook.pCacheTitle, 30, (pRect.height >> 1) - (size.GetHeight() >> 1));
						}
						break;
					}
					}
		...
		void MapNotebook::MapTabs::doSizing(wxDC& dc)
		{
			...
			int totalSize = 18 + 21 + 15 + sizingOffset;//selected page button, add page button, left margin, and left scroll button (if any)
			int realTotalSize = totalSize;
			for (int i = 0, size = pNotebook.pPages.size(); i < size; i++)
			{
				if (i >= undrawnLeftTabs)
				{
					totalSize += pNotebook.pPages[i]->visible() ? dc.GetTextExtent(pNotebook.pPages[i]->caption()).GetWidth() + 8 * 2 : 0;
				}
				realTotalSize += pNotebook.pPages[i]->visible() ? dc.GetTextExtent(pNotebook.pPages[i]->caption()).GetWidth() + 8 * 2 : 0;
			}

			rightTabScroll->isVisible = false;
			if (totalSize >= pRect.GetWidth()) //the tabs at the right won't fit in, we have to draw the right scrolling button
			{
				//draw right button
				rightTabScroll->drawButton(dc, pRect.GetWidth() - 21 - 18, (pRect.height >> 2) - (rightTabScroll->buttonImage->GetHeight() >> 2));
			}
			...
			if (undrawnLeftTabs > 0) // some tabs on the left weren't drawn, we need to draw left scrolling button
			{
				sizingOffset = 8;
				//draw left button
				leftTabScroll->drawButton(dc, 1, (pRect.height >> 2) - (leftTabScroll->buttonImage->GetHeight() >> 2));
			}
		}
		...
		void MapNotebook::MapTabs::drawItemTop(wxDC& dc, Page* page, int& pos, Notebook::Alignment align)
		{
			...
			else
			{
				...
				wxRect rect(
					pos - ((align == alignRight) ? (textExtent.GetWidth() + 8 * 2) : 0), pRect.y + 3,
					textExtent.GetWidth() + 8 * 2, pRect.height - 6);
				...
				{
					dc.SetTextForeground(wxColour(50, 50, 50));
					dc.DrawText(page->caption(), rect.x + 8, pRect.y + pRect.height / 2 - textExtent.GetHeight() / 2 + 1);
					dc.SetTextForeground(wxColour(230, 230, 230));
					dc.DrawText(page->caption(), rect.x + 7, pRect.y + pRect.height / 2 - textExtent.GetHeight() / 2);
				}
				else
				{
					...
					const wxColour gColor(
						(unsigned char)Math::MinMax<int>(pColor.Red() - 50, 0, 255),
						(unsigned char)Math::MinMax<int>(pColor.Green() - 50, 0, 255),
						(unsigned char)Math::MinMax<int>(pColor.Blue() - 50, 0, 255));

					dc.GradientFillLinear(wxRect(rect.x, rect.y, rect.width, 4),
						colGray, gColor, wxSOUTH);
					dc.GradientFillLinear(wxRect(rect.x, rect.y + 4, rect.width, rect.height - 5),
						gColor, Map::Settings::background, wxSOUTH);
					...
					dc.DrawText(page->caption(), rect.x + 8, pRect.y + pRect.height / 2 - textExtent.GetHeight() / 2);
					...
					static_cast<MapNotebook&>(pNotebook).closePageBoundingBox.SetX(rect.x + rect.width - 17);
					...
					else if (page->selected() && page->pBoundingBox.Contains(p))
					{
						...
						else
						{
							if (pTabNameCtrl == nullptr)
							{
								...
								pTabNameCtrl->Connect(wxEVT_COMMAND_TEXT_ENTER, wxCommandEventHandler(MapNotebook::TabTextCtrl::OnTextEnter));
							}
							else
							{
		...
		MapNotebook::MapTabs::tabButton::tabButton(std::string imagePath, MapTabs* parentFrame, BtnType t, char* hoverImagePath):
			hoverImage(nullptr),
		...
		{
			coords = wxPoint(x, y);
			pBoundingBox = new wxRect(x-1, 0, buttonImage->GetWidth()+2, parentTabFrame->pRect.GetHeight());
		...
		{
			for (int i = 0, size = pNotebook->pPages.size(); i < size; ++i)// Check that there is no layer already having this name
			{
				if (pNotebook->pPages[i]->name() == GetValue() && pNotebook->pPages[i] != pPage)
				{
					logs.warning() << "There already is a layer named: " << std::string(GetValue().mb_str());
					SetValue(pPage->name());
					evt.Skip();
					return;
				}
