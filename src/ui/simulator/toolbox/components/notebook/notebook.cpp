...
namespace Component
{
		...
		pAlignment(alignRight)
	{
		...
		pOriginalColorDark.Set(
			(unsigned char) Math::MinMax<int>(pOriginalColor.Red()   - 70, 0, 255),
			...
			case themeLight:
				{
					...
					wxColour bgDark;
					bg.Set(
						(unsigned char) Math::MinMax<int>(pNotebook.pOriginalColor.Red()   + 40, 0, 255),
	...
	void Notebook::Tabs::drawItemOnCanvasNotSelected(Page* page, wxDC& dc, const int pos, const int h,
	...
	void Notebook::Tabs::drawItemOnCanvasSelected(Page* page, wxDC& dc, const int pos, const int h,
		const wxSize& textExtent)
	{
		switch (pNotebook.pTheme)
		{
			case themeDefault:
				{
					...
					dc.GradientFillLinear(wxRect(11, pos + 1, pRect.width - 11, 4),
						wxColor(145, 145, 145), gColor, wxSOUTH);
					...
					else
					{
						...
						for (int i = 0; i < size; i++)
						{
							...
							{
								pPts[0].x = pRect.width - 6;
								pPts[0].y = pos + subTabSize * (i + 1) + (subpageTextExtent.GetHeight() >> 1) + 1;
								pPts[1].x = pPts[0].x - 3;
								pPts[1].y = pPts[0].y - 3;
								pPts[2].x = pPts[0].x - 3;
								pPts[2].y = pPts[0].y + 3;
			...
			case themeLight:
				{
					...
					dc.GradientFillLinear(wxRect(20 , pos , pRect.width - 5, 1), pNotebook.pOriginalColorDark, bgDark2, wxEAST);
					dc.GradientFillLinear(wxRect(20 , pos + h - 1, pRect.width - 5, 1), pNotebook.pOriginalColorDark, bgDark2, wxEAST);

					dc.GradientFillLinear(wxRect(10, pos, 5, h), bg, pNotebook.pOriginalColorDark, wxEAST);

					dc.SetTextForeground(wxColour(255, 255, 255));
					dc.DrawText(page->caption(), pRect.width - textExtent.GetWidth() - 15, pos - 1 + (h >> 1) - (textExtent.GetHeight() >> 1) + 1);
	...
	void Notebook::addCommonControlTop(wxWindow* ctrnl, const int border, const wxPoint& recommendedSize)
	{
		if (ctrnl)
		{
	...
	Notebook::Page::Page(Notebook& notebook) :
		pNotebook(notebook), pControl(nullptr), pSelected(false),
		pVisible(true), pDisplayExtraControls(true)
	{
	...
	Notebook::Page::Page(Notebook& notebook, wxWindow* ctnrl, const wxString& name, const wxString& caption)
		:pNotebook(notebook),
		pControl(ctnrl),
		pName(name),
		pCaption(caption),
