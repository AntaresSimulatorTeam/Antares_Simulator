...
namespace Component
{
	...
	void MainPanel::addProperty(wxDC& dc,const wxString& caption, const wxString& text, const wxSize& size,
		int& posY) const
	{
	...
	void MainPanel::onDraw(wxPaintEvent&)
	{
			...
			// Property: Name of the study
			{
				...
				if (study->header.version != (uint) Data::versionLatest)
				{
					...
					if (StudyHasBeenModified())
						s << wxT("(*) ");
					s << pStudyCaption << wxT(" (v") << Data::VersionToWStr((Data::Version(study->header.version)))
						<< wxT(")");
		...
		else
		{
			if (!(!study) and study->readonly())
			{
				...
				dc.DrawRectangle(newRect.x - 1, newRect.y - 1, newRect.GetWidth() + 3, newRect.GetHeight() + 3);

				dc.SetPen(wxPen(wxColour(0, 0, 0)));
				dc.DrawRectangle(newRect.x - 1, newRect.y - 1, newRect.GetWidth() + 2, newRect.GetHeight() + 2);

				dc.GradientFillLinear(newRect,
					wxColour(255, 100, 0), wxColour(183, 0, 0), wxSOUTH);
