...
namespace Component
{
	...
	Button::Button(wxWindow* parent, const wxString& caption, const char* bitmap, const Button::OnClick& onclick) :
		Panel(parent),
		pCaption(caption),
		pOnClick(onclick),
		pUserData(nullptr),
	...
	void Button::onDraw(wxPaintEvent&)
	{
		...
		if (pSelected || pPushed)
		{
			...
			{
				a.Set(
					(unsigned char) Math::MinMax<int>(pColorOverridePushed.Red()   + 10, 0, 255),
			...
			else
			{
				...
				a.Set(
					(unsigned char) Math::MinMax<int>(selectionColor.Red()   + 30, 0, 255),
		...
		else
		{
			...
			{
				wxColour a(
					(unsigned char) Math::MinMax<int>(originalColor.Red()   - 80, 0, 255),
		...
		if (pIcon)
		{
			...
			if (!pCaption.empty())
			{
				x  = rect.x + pMiddleWidth - pCaptionExtent.GetWidth() / 2 - pIcon->GetWidth() - iconMargin;
		...
		if (pClickBehavior != clkDefault)
		{
			...
			if (pSelected)
			{
				...
				{
					dc.SetPen(wxPen(wxSystemSettings::GetColour(wxSYS_COLOUR_HIGHLIGHT), 1, wxPENSTYLE_SOLID));
					dc.DrawLine(x, rect.y + 2, x, rect.y + rect.height - 2);
				}
				dc.SetPen(wxPen(wxSystemSettings::GetColour(wxSYS_COLOUR_HIGHLIGHTTEXT), 1, wxPENSTYLE_SOLID));
				dc.SetBrush(wxBrush(wxSystemSettings::GetColour(wxSYS_COLOUR_HIGHLIGHTTEXT), wxBRUSHSTYLE_SOLID));
			}
			else
			{
				dc.SetPen(wxPen(wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT), 1, wxPENSTYLE_SOLID));
				dc.SetBrush(wxBrush(wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT), wxBRUSHSTYLE_SOLID));
			}
