...
namespace Process
{
	...
	*/
	...
	public:
		JobExecute(const wxString& command, const wxString& workingdirectory,
			const wxString& title, const wxString& subTitle, const char* icon);
		virtual ~JobExecute();
	...
	*/
	...
	JobExecute::JobExecute(const wxString& command, const wxString& workingdirectory,
		const wxString& title, const wxString& subTitle, const char* icon)
		:Jobs::Job(title, subTitle, icon),
		pCommand(command), pWDirectory(workingdirectory),
		pRunning(0),
		pProcess(nullptr)
	{
