...
namespace Toolbox
{
	...
	void Clipboard::add(Antares::Component::Datagrid::VGridHelper* m,
			...
			uint offsetX2, uint offsetY2,
			bool withStyle, bool withHeaders)
	{
		if (!m)
		{
		...
		if (m && m->GetNumberRows() && m->GetNumberCols() && m->renderer())
		{
			...
			*d << "<table>";
			...
			for (uint y = offsetY; y < posY2; ++y)
			{
				...
				assert(cellY >= 0 && cellY < m->renderer()->height() && "Clipboard: Invalid indice pointer for rows (header)");
				...
				*d << "<tr>";
				...
				for (uint x = offsetX; x < posX2; ++x)
				{
					...
					assert(cellX >= 0 && cellX < m->renderer()->width() && "Clipboard: Invalid indice pointer for columns (header)");
			...
			*d << "</table></body></html>";
			...
			{
				int cellY = indicesRows[y];
				assert(cellY >= 0 && indicesRows[y] < m->renderer()->height() && "Clipboard: Invalid indice pointer for rows (value)");

				bool firstColCell = true;
				for (uint x = offsetX; x < posX2; ++x)
				{
					assert(indicesCols[x] >= 0 && indicesCols[x] < m->renderer()->width() && "Clipboard: Invalid indice pointer for columns (value)");
