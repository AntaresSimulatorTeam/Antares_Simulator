...
namespace Handler
{
	...
	namespace // anonymous
	{
		...
		*/
		...
		bool checkConstraintSupportingElementsIntegrity(const Antares::Action::Context* context, const BuildContext* ctx)
		{
			...
			for (auto i = ctx->constraint.begin(); i != ctx->constraint.end(); ++i)
			{
				...
				for (Data::BindingConstraint::iterator j = bc->begin(); j != endWeights; ++j)
				{
					...
					{
						if(Antares::Window::Inspector::IsLinkSelected(areaLink->from->name, areaLink->with->name))
							--count;
					}
				}
				...
				if (count != 0)
				{
					...
					logs.error() << "Impossible to paste binding constraint: "<<bc->name()<<", missing supporting element";
					return false;
				}
		...
		bool checkLinkSupportingElementsIntegrity(const Antares::Action::Context* context, const BuildContext* ctx)
		{
			...
			for (BuildContext::LinkSet::const_iterator i = ctx->link.begin(); i != ctx->link.end(); ++i)
			{
				...
				for (std::map<Data::AreaName, bool>::const_iterator j = i->second.begin(); j != i->second.end(); ++j)
				{
					...
					if (not Antares::Window::Inspector::isAreaSelected(fromWantedName) || not Antares::Window::Inspector::isAreaSelected(toWantedName))
					{
						...
						{
							logs.error() << "Impossible to paste link: " << fromWantedName << "/" << toWantedName << ", missing supporting element";
							return false;
		...
		static void PreparePasteOperations(Antares::Action::Context* context, const String& text, uint offset, bool forceDialog)
		{
			// assert
			...
			do
			{
				...
				if (point != String::npos)
				{
					...
					if (!AppendCommand(ctx, command, param))
						logs.warning() << "paste from clipboard: invalid command: " << command << ", value = " << param;
				}
			...
			if (ctx.modifiedWhenCopied && context->shouldDestroyExtStudy)
			{
				...
				logs.error() << "Impossible to paste data from another instance. Please save the changes before.";
				return;
			}
			...
			if (not ctx.cluster.empty())
			{
				...
				for (BuildContext::LinkSet::const_iterator i = ctx.cluster.begin(); i != end; ++i)
				{
					...
					std::map<Data::ThermalClusterName, bool>::const_iterator send = i->second.end();
					for (std::map<Data::ThermalClusterName,bool>::const_iterator j = i->second.begin(); j != send; ++j)
					{
						*create += Antares::Action::AntaresStudy::Area::Create::StandardActionsToCopyThermalCluster(i->first, j->first);
					}
					*clusterNode += create;
				}
				*root += clusterNode;
			...
			if (not ctx.link.empty())
			{
				...
				for (BuildContext::LinkSet::const_iterator i = ctx.link.begin(); i != end; ++i)
				{
					...
					for (std::map<Data::AreaName,bool>::const_iterator j = i->second.begin(); j != send; ++j)
					{
						empty = false;
						...
						auto* create = (not Antares::Window::Inspector::IsLinkSelected(fromWantedName, toWantedName))
							? new Antares::Action::AntaresStudy::Link::Create(i->first, j->first)
							: new Antares::Action::AntaresStudy::Link::Create(i->first, fromWantedName, j->first, toWantedName);
			...
			if (!ctx.constraint.empty())
			{
				...
				for (auto i = ctx.constraint.begin(); i != end; ++i)
				{
					...
					? new Antares::Action::AntaresStudy::Constraint::Create(*i, sourceConstraint->type(), sourceConstraint->operatorType())
					: new Antares::Action::AntaresStudy::Constraint::Create(*i, *i, sourceConstraint->type(), sourceConstraint->operatorType());
		...
		public:
			...
			void folder(const wxString& f) {pFolder = f;}

			Data::Study* study() const {return pStudy;}
	...
	void AntaresStudy(Data::Study& target, const String& content, uint offset, PropertyMap& map, bool forceDialog)
	{
