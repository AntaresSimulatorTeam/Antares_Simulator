 ...
 pTotalChildrenCount(1)
{
    ...
    pLineColor.Set((unsigned char)Math::MinMax<int>(pBackgroundColor.Red() - 70, 0, 255),
...
void ActionPanel::update()
{
    ...
    {
    case stUnknown:
        pStateText = wxT("unknown");
        break;
    case stDisabled:
        pStateText = wxT("skip");
        break;
    case stError:
        pStateText = wxT("error");
        break;
    case stNothingToDo:
        pStateText = wxT("ready");
        break;
    case stReady:
        pStateText = wxT("ready");
        break;
    case stConflict:
        pStateText = wxT("conflict");
        break;
    case stMax:
        break;
    }
...
void ActionPanel::DrawBackgroundWithoutItems(wxWindow& obj, wxDC& dc, const wxRect& rect)
{
    ...
    pBorderColor.Set((unsigned char)Math::MinMax<int>(pBackgroundColor.Red() - 40, 0, 255),
...
void ActionPanel::onDraw(wxPaintEvent&)
{
    ...
    static const wxFont fontComments(
      wxFontInfo(fontSize - 1).Family(wxFONTFAMILY_SWISS).FaceName("Tahoma"));
    // The DC
    ...
    if (pCanDoSomething)
    {
        ...
        dc.DrawText(pStateText,
                    gutterLength + stateLength / 2 - size.GetWidth() / 2,
    ...
    if (pCanDoSomething)
    {
        ...
        dc.DrawText(pBehaviorText,
                    gutterLength - 16 - size.GetWidth(),
                    (rect.height >> 1) - (size.GetHeight() >> 1));
    }
    ...
    // Comments
    {
        ...
        dc.DrawLine(gutterLength + stateLength + captionLength,
                    1,
                    gutterLength + stateLength + captionLength,
                    rect.height - 3);
...
void ActionPanel::onMouseDownBehaviorSelect(wxWindow* obj)
{
    ...
    if (!pPopupMenu)
    {
        ...
        if (pAction->allowUpdate())
        {
            ...
            pPopupMenu->Connect(item->GetId(),
                                wxEVT_COMMAND_MENU_SELECTED,
                                wxCommandEventHandler(ActionPanel::onBehaviorMerge),
                                nullptr,
                                this);
        }
        ...
        if (pAction->allowOverwrite())
        {
            ...
            pPopupMenu->Connect(item->GetId(),
                                wxEVT_COMMAND_MENU_SELECTED,
                                wxCommandEventHandler(ActionPanel::onBehaviorOverwrite),
                                nullptr,
                                this);
        }
        ...
        if (pAction->allowSkip())
        {
            ...
            pPopupMenu->Connect(item->GetId(),
                                wxEVT_COMMAND_MENU_SELECTED,
                                wxCommandEventHandler(ActionPanel::onBehaviorSkip),
                                nullptr,
                                this);
        }
    }
...
void ActionPanel::onMouseDown(wxMouseEvent& evt)
{
    enum
    {
        offsetArrow = gutterLength + stateLength,
    };
    ...
    // collapse / expand
    if (pHasChildren && y <= itemHeight && x >= offsetArrow + pDepthSpace + 2
        && x <= offsetArrow + pDepthSpace + 20)
    {
