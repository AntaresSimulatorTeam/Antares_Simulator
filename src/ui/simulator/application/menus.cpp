...
namespace Antares
{
	...
	static void InternalRebuildRecentFiles(wxMenu* it, const RecentFiles::ListPtr& lst, bool addOpenMenu = true)
	{
		if (!it)
			return;
		...
		if (addOpenMenu)
		{
			...
			Menu::CreateItem(subMenu, Forms::mnIDOpen,
				wxT("&Open from a folder...   \tCtrl+O"),
				"images/16x16/open.png",
				wxEmptyString, wxITEM_NORMAL, true);
		...
		if (not lst->empty())
		{
			...
			for (RecentFiles::List::const_iterator i = lst->begin(); i != end; ++i)
			{
				...
				// Adding the item in the menu
				Menu::CreateItem(subMenu, Forms::mnIDOpenRecents_0 + (!indx ? 10 : indx) - 1,
					# ifndef YUNI_OS_WINDOWS
					mnCaption, "images/16x16/minibullet.png",
					# else
					mnCaption, "images/misc/study.png",
					# endif
			...
			subMenu->AppendSeparator();
			// Path to the study
			wxMenuItem* i = Menu::CreateItem(subMenu, Forms::mnIDOpenRecents_ToggleShowPaths,
					wxT("Show the path of the studies"), nullptr, wxEmptyString, wxITEM_CHECK);
			i->Check(showPaths);

			Menu::CreateItem(subMenu, Forms::mnIDOpenRecents_ClearHistory,
				wxT("Clear the history"), nullptr, wxEmptyString);
		}
		else
		{
			auto* menuitem = Menu::CreateItem(subMenu, wxID_ANY,
				wxT("(No recent files available)"), nullptr, wxEmptyString);
	...
	wxMenuItem* Menu::CreateItem(wxMenu* parent, const int id, const wxString& caption,
		const char* bitmap, const wxString& help, wxItemKind kind, const bool bold)
	{
	...
	wxMenuItem* Menu::CreateItemWithSubMenu(wxMenu* parent, const int id, const wxString& caption,
		const char* bitmap, const wxString& help, const bool bold)
	{
		assert(parent && "invalid parent handle");
		...
		auto* m = (help.empty())
			? new wxMenuItem(parent, id, newcaption, wxEmptyString, wxITEM_NORMAL)
			: new wxMenuItem(parent, id, newcaption, wxString(wxT("  ")) << help, wxITEM_NORMAL);
	...
	wxMenuItem* Menu::CreateItem(wxMenu* parent, const int id, const wxString& caption,
		const wxBitmap& bitmap, const wxString& help, wxItemKind kind, const bool bold)
	{
		assert(parent && "invalid parent handle");
