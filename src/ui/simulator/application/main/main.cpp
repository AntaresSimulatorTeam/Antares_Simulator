...
namespace Forms
{
	...
	ApplWnd::ApplWnd() :
		Component::Frame::WxLocalFrame(nullptr, wxID_ANY, wxT("Antares"), wxDefaultPosition, wxSize(minimalWidth, minimalHeight)),
		pMainSizer(nullptr),
		pMenu(nullptr),
		pMenuFile(nullptr),
	...
	void ApplWnd::exportMap(const Yuni::String& path, bool transparentBackground, const wxColour& backgroundColor, const std::list<uint16_t>& layers, int nbSplitParts, Antares::Map::mapImageFormat format)
	{
		if (Data::Study::Current::Valid())
			Antares::ExportMap(path, transparentBackground, backgroundColor, layers, nbSplitParts, format);
	}
	...
	public:
		...
		//! Sets
		Data::ScenarioBuilder::Sets*  scenarioBuilder() const {return pSets;}
	...
	void ApplWnd::title(const wxString& s)
	{
		...
		if (StudyHasBeenModified())
			t << wxT("* ");
		t << (s.IsEmpty() ? wxT("<untitled>") : s)
			<< wxT(" - Antares Simulator")
			# ifndef NDEBUG
			<< wxT(" - DEVELOPER PREVIEW")
			# endif
			# if ANTARES_RC != 0
	...
	bool ApplWnd::wouldYouLikeToSaveTheStudy()
	{
		...
		if (!(!study))
		{
			if (StudyHasBeenModified())
			{
				...
				Window::Message message(this, wxT("Save"),
					wxT("The study has been modified"), t, "images/misc/save.png");
