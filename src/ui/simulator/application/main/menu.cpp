...
namespace Forms
{
	...
	wxMenu* ApplWnd::createMenuFiles()
	{
		...
		// Wizard
		Menu::CreateItem(pMenuFile, mnIDWizard,
				wxT("Startup wizard"), "images/16x16/wizard.png", wxT("Display the startup wizard"));
		pMenuFile->AppendSeparator();

		// New
		Menu::CreateItem(pMenuFile, mnIDNew,
				wxT("New\tCtrl+N"), "images/16x16/new.png", wxT("Create a new study with default settings"));


		// Open a study
		Menu::CreateItem(pMenuFile, mnIDOpen,
				wxT("&Open...\tCtrl+O"), "images/16x16/open.png", wxT("Load a study"));
		pMenuFileRecents = pMenuFile->AppendSubMenu(new wxMenu(), wxT("Quick open"));
		Menu::RebuildRecentFiles(pMenuFileRecents);

		// Save
		Menu::CreateItem(pMenuFile, mnIDSave,
				wxT("Save\tCtrl+S"), "images/16x16/savestudy.png",
				wxT("Save the last changes"));
		Menu::CreateItem(pMenuFile, mnIDSaveAs,
				wxT("Save As..."), nullptr,
				wxT("Save the study as..."));
		
		pMenuFile->AppendSeparator();

		Menu::CreateItem(pMenuFile, mnIDExportMap,
				wxT("Export Map..."), nullptr,
		...
		if (System::windows)
		{
			Menu::CreateItem(pMenuFile, mnIDOpenExplorer,
				wxT("Open in Windows Explorer..."), "images/16x16/empty.png",
				wxT("Open the folder in Windows Explorer"));
		}
		else
		{
			Menu::CreateItem(pMenuFile, mnIDOpenExplorer,
				wxT("Open in Gnome Nautilus..."), "images/16x16/empty.png",
				wxT("Open the folder in Gnome Nautilus"));
		}

		Menu::CreateItem(pMenuFile, mnIDCleanOpenedStudyFolder,
				wxT("Clean the study folder..."), nullptr,
				wxT("Clean the study folder..."));

		pMenuFile->AppendSeparator();

		// Close
		Menu::CreateItem(pMenuFile, mnIDClose,
				wxT("Close"), "images/16x16/empty.png", wxT("Close the current study"));
	...
	wxMenu* ApplWnd::createMenuEdit()
	{
		...
		data.pEditCurrentLocation = Menu::CreateItem(menu, wxID_ANY,
			wxT("System Maps"), "images/16x16/empty.png", wxEmptyString);
		data.pEditCurrentLocation->Enable(false);

		Menu::CreateItem(menu, mnIDStudyEditCopy,
			wxT("&Copy\tCtrl+C"), "images/16x16/copy.png",
			wxT("Copy to the clipboard"));
		Menu::CreateItem(menu, mnIDStudyEditPaste,
			wxT("&Paste\tCtrl+V"), "images/16x16/paste.png",
			wxT("Paste from the clipboard"));
		Menu::CreateItem(menu, mnIDStudyEditPasteSpecial,
			wxT("Paste special..."), nullptr,
			wxT("Paste special from the clipboard"));

		menu->AppendSeparator();

		Menu::CreateItem(menu, mnIDStudyEditMapReverseSelection,
			wxT("Reverse  "), nullptr,
			wxT("Reverse the selection"));

		Menu::CreateItem(menu, mnIDStudyEditMapUnselectAll,
			wxT("Unselect a&ll"), nullptr,
			wxT("Unselect all items on the map"));

		Menu::CreateItem(menu, mnIDStudyEditMapSelectAll,
			wxT("Select a&ll"), nullptr,
	...
	wxMenu* ApplWnd::createMenuInput()
	{
		...
		pMenuInputCreation = Menu::CreateItem(pMenuInput, wxID_ANY,
			wxT("Created: "),
			"images/16x16/empty.png", wxEmptyString);

		pMenuInputLastSaved = Menu::CreateItem(pMenuInput, wxID_ANY,
			wxT("Last saved: none"), nullptr,
			wxEmptyString);
		...
		Menu::CreateItem(pMenuInput, mnIDStudyEditTitle,
				wxT("Name of the study"), "images/16x16/empty.png",
				wxT("Set the name of the study"));
		Menu::CreateItem(pMenuInput, mnIDStudyEditAuthors,
				wxT("Author(s) of the study"), "images/16x16/empty.png",
				wxT("Set the author(s) of the study"));

		pMenuInput->AppendSeparator();

		Menu::CreateItem(pMenuInput, mnIDViewAllSystem,
			wxT("View Map All"), "images/16x16/empty.png", wxT("View 'All'"));
		Menu::CreateItem(pMenuInput, mnIDViewSystem,
				wxT("View  System\tAlt+1"), "images/16x16/empty.png", wxT("View 'System Maps'"));
		Menu::CreateItem(pMenuInput, mnIDViewSimulation,
				wxT("View  Simulation\tAlt+2"), "images/16x16/empty.png", wxT("View 'Simulation'"));
		Menu::CreateItem(pMenuInput, mnIDViewNotes,
				wxT("View  User's Notes\tAlt+3"), "images/16x16/notes.png", wxT("View 'User's Notes'"));

		pMenuInput->AppendSeparator();

		Menu::CreateItem(pMenuInput, mnIDViewLoad,
				wxT("View  Load"), nullptr, wxT("View 'Load'"));
		Menu::CreateItem(pMenuInput, mnIDViewSolar,
				wxT("View  Solar"), nullptr, wxT("View 'Solar'"));
		Menu::CreateItem(pMenuInput, mnIDViewWind,
				wxT("View  Wind"), nullptr, wxT("View 'Wind'"));
		Menu::CreateItem(pMenuInput, mnIDViewHydro,
				wxT("View  Hydro"), nullptr, wxT("View 'Hydro'"));
		Menu::CreateItem(pMenuInput, mnIDViewThermal,
				wxT("View  Thermal"), nullptr, wxT("View 'Thermal'"));

		Menu::CreateItem(pMenuInput, mnIDViewMiscGen,
				wxT("View  Misc Gen."), nullptr, wxT("View 'Misc Gen.'"));
		Menu::CreateItem(pMenuInput, mnIDViewReservesDSM,
				wxT("View  Reserves / DSM"), nullptr, wxT("View 'Reserves / DSM'"));
		Menu::CreateItem(pMenuInput, mnIDViewInterconnections,
				wxT("View  Links"), nullptr, wxT("View 'Interconnections'"));
		Menu::CreateItem(pMenuInput, mnIDViewBindingConstraints,
				wxT("View  Binding constraints"), nullptr, wxT("View 'Binding constraints'"));
		Menu::CreateItem(pMenuInput, mnIDViewNodalOptimization,
				wxT("View  Economic Optimization"), nullptr, wxT("View 'Economic Optimization'"));
	...
	wxMenu* ApplWnd::createMenuSimulation()
	{
		...
		// Run !
		Menu::CreateItem(pMenuSimulation, mnIDRunTheSimulation,
				wxT("Run a Monte Carlo simulation\tF9"), "images/16x16/run-simulation.png",
				wxT("Run a Monte Carlo simulation"));
		Menu::CreateItem(pMenuSimulation, mnIDRunTheTSGenerators,
				wxT("Run the time-series generators  "), "images/16x16/empty.png",
				wxT("Run the time-series generators"));

		pMenuSimulation->AppendSeparator();

		Menu::CreateItem(pMenuSimulation, mnIDRunTheTSAnalyzer,
	...
	wxMenu* ApplWnd::createMenuOptions()
	{
		...
		Menu::CreateItem(pMenuOptions, mnIDOptionOptimizationPrefs,
				wxT("Optimization preferences"), "images/16x16/optimization-prefs.png",
				wxT("Set optimization preferences"));
		it = Menu::CreateItem(pMenuOptions, mnIDOptionAdvanced,
				wxT("Advanced parameters"), "images/16x16/empty.png",
	...
	wxMenu* ApplWnd::createMenuTools()
	{
		...
		// Memory Statistics
		Menu::CreateItem(pMenuTools, mnIDMemoryUsedByTheStudy,
				wxT("Resources monitor"), "images/16x16/resources.png", wxT("View 'Resources monitor'"));

		pMenuTools->AppendSeparator();

		// SWAP Folder
		Menu::CreateItem(pMenuTools, mnIDOptionTempFolder,
				wxT("Configure the swap folder"), nullptr,
	...
	wxMenu* ApplWnd::createMenuWindow()
	{
		...
		// Fullscreen
		Menu::CreateItem(pMenuWindow, mnIDFullscreen,
				wxT("Toggle Fullscreen"), "images/16x16/fullscreen.png",
				wxT("Toggle the Fullscreen mode"));

		pMenuWindow->AppendSeparator();

		Menu::CreateItem(pMenuWindow, mnIDInspector,
				wxT("Inspector"), "images/16x16/inspector.png",
				wxT("Show the inspector"));

		// Logs
		Menu::CreateItem(pMenuWindow, mnIDStudyLogs,
				wxT("Log viewer"), "images/16x16/logs.png", wxT("View 'Log viewer'"));
		...
		if (framelist.size() > 1)
		{
			...
			foreach (auto* frame, framelist)
			{
				...
				{
					Menu::CreateItem(pMenuWindow, evId, mnCaption,
						"images/16x16/minibullet.png",
						wxString() << wxT("Raise window `") << mnCaption << wxT('`'));
				}
				else
				{
					Menu::CreateItem(pMenuWindow, evId, wxT("Others..."),
						"images/16x16/minibullet.png",
	...
	wxMenu* ApplWnd::createMenuHelp()
	{
		...
		Menu::CreateItem(pMenuHelp, mnIDHelpContinueOnline,
			wxT("Continue online"), nullptr,
			wxT("Allow gathering of  anonymous usage metrics"));
		Menu::CreateItem(pMenuHelp, mnIDHelpContinueOffline,
			wxT("Continue offline"), nullptr,
		...
		Menu::CreateItem(pMenuHelp, mnIDHelpAbout,
				wxT("About Antares"), nullptr,
				wxT("About Antares"));
	...
	{
		Window::Message message(this, wxT("Recent files"),
		...
		if (message.showModal() == Window::Message::btnYes)
		{
			...
			Window::Message ok(this, wxT("Recent files"),
	...
	void ApplWnd::evtOnOpenStudyFolderInExplorer(wxCommandEvent&)
	{
		...
		{
			if (System::windows)
				wxExecute(wxString(wxT("explorer.exe \"")) << wxStringFromUTF8(study->folder) << wxT("\""));
			else
				wxExecute(wxString(wxT("gnome-open \"")) << wxStringFromUTF8(study->folder) << wxT("\""));
		}
	}
	...
	void ApplWnd::evtOnOpenOutputInExplorer(wxCommandEvent& evt)
	{
		...
		for (auto i = ListOfOutputsForTheCurrentStudy.begin(); i != end; ++i)
		{
			...
			{
				if (System::windows)
					wxExecute(wxString(wxT("explorer.exe \"")) << wxStringFromUTF8((*i)->path) << wxT("\""));
				else
					wxExecute(wxString(wxT("gnome-open \"")) << wxStringFromUTF8((*i)->path) << wxT("\""));
				return;
			}
		}
	}
