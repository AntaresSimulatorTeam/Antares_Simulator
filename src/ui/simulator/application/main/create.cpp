...
namespace Forms
{
	...
	public:
		...
		void Notify() override
		{
			...
			if (Antares::Memory::swapSupport)
			{
				...
				{
					if (pDelayedCount++ == 0)
						logs.info() << "memory flush delayed because a data update is currently performed";
					else
					{
						if (pDelayedCount > 5)
							pDelayedCount = 0;
	...
	static void CreateWindowToolbar(ApplWnd& mainfrm, wxAuiManager& auimanager)
	{
		...
		// Toolbar
		{
			...
			btn = new Antares::Component::Button(toolbar, wxEmptyString, "images/16x16/wizard.png",
				mainfrm.data(), &MainFormData::onToolbarWizard);
			s->Add(btn, 0, wxALL|wxEXPAND);

			// separator
			Antares::Component::AddVerticalSeparator(toolbar, s);

			btn = new Antares::Component::Button(toolbar, wxEmptyString, "images/16x16/new.png",
				mainfrm.data(), &MainFormData::onToolbarNewStudy);
			s->Add(btn, 0, wxALL|wxEXPAND);
			btn = new Antares::Component::Button(toolbar, wxT("Open"), "images/16x16/open.png",
				mainfrm.data(), &MainFormData::onToolbarOpenLocalStudy);
			btn->dropDown(true);
			btn->onPopupMenu(mainfrm.data(), &MainFormData::onToolbarOpenRecentMenu);
			s->Add(btn, 0, wxALL|wxEXPAND);
			btn = new Antares::Component::Button(toolbar, wxT("Save"), "images/16x16/savestudy.png",
				mainfrm.data(), &MainFormData::onToolbarSave);
			s->Add(btn, 0, wxALL|wxEXPAND);

			// separator
			Antares::Component::AddVerticalSeparator(toolbar, s);

			btn = new Antares::Component::Button(toolbar, wxT("Run a simulation"), "images/16x16/run-simulation.png",
				mainfrm.data(), &MainFormData::onToolbarRunSimulation);
			s->Add(btn, 0, wxALL|wxEXPAND);

			// separator
			Antares::Component::AddVerticalSeparator(toolbar, s);

			btn = new Antares::Component::Button(toolbar, wxEmptyString, "images/16x16/optimization-prefs.png",
				mainfrm.data(), &MainFormData::onToolbarOptimizationPreferences);
			s->Add(btn, 0, wxALL|wxEXPAND);

			btn = new Antares::Component::Button(toolbar, wxEmptyString, "images/16x16/logs.png",
				mainfrm.data(), &MainFormData::onToolbarLogs);
			s->Add(btn, 0, wxALL|wxEXPAND);

			// separator
			Antares::Component::AddVerticalSeparator(toolbar, s);

			btn = new Antares::Component::Button(toolbar, wxT("Inspector"), "images/16x16/inspector.png",
				mainfrm.data(), &MainFormData::onToolbarInspector);
			s->Add(btn, 0, wxALL|wxEXPAND);

			// separator
			Antares::Component::AddVerticalSeparator(toolbar, s);

			btn = new Antares::Component::Button(toolbar, wxEmptyString, "images/16x16/fullscreen.png",
				mainfrm.data(), &MainFormData::onToolbarFullscreen);
			s->Add(btn, 0, wxALL|wxEXPAND);
		...
		auimanager.AddPane(toolbar, wxAuiPaneInfo().Name(wxT("form_toolbar")).
			Top().Row(1).
			DockFixed(true).
			LeftDockable(false).RightDockable(false).
			TopDockable(false).BottomDockable(false).
			PaneBorder(false).MaximizeButton(false).
			GripperTop(false).
	...
	void ApplWnd::internalInitialize()
	{
		...
		# if defined(wxUSE_STATUSBAR)
		{
			...
			statusbar->Connect(statusbar->GetId(), wxEVT_CONTEXT_MENU,
				wxContextMenuEventHandler(ApplWnd::evtOnContextMenuStatusBar), nullptr, this);
		...
		//The Layers Notebook
		Component::Notebook * layersNotebook = new Component::Notebook(pNotebook, Component::Notebook::orTop);
		...
		// The first page
		Component::Notebook::Page* MapPage = layersNotebook->add(pMainMap, wxT("map"), wxT("Set Content"));

		// Create a standard page with an input selector
		std::pair<Component::Notebook*, Toolbox::InputSelector::Area*> layersUIPage =
			createStdNotebookPage<Toolbox::InputSelector::Area>(layersNotebook,
				wxT("properties"), wxT("Set Visibility"));

		Component::Datagrid::Renderer::LayersUI* areaUIPropertiesRenderer = new Component::Datagrid::Renderer::LayersUI(layersUIPage.second);
		Component::Datagrid::Component* areaUIPropertiesGrid = new Component::Datagrid::Component(layersUIPage.first, areaUIPropertiesRenderer);
		...
		Component::Datagrid::Renderer::LayersVisibility* visibilityRenderer = new Component::Datagrid::Renderer::LayersVisibility();
		Component::Datagrid::Component* visibilityGrid = new Component::Datagrid::Component(layersUIPage.first, visibilityRenderer);
		auto pageVisilityGrid = layersUIPage.first->add(visibilityGrid, wxT("areaVisibility"), wxT("Area Wise"));
		...
		pAUIManager.AddPane(pBigDaddy, wxAuiPaneInfo().Name(wxT("form_content")).
			CenterPane().PaneBorder(false).MaximizeButton(true));
...
//			Window::StartupWizard::Show();
	...
	void ApplWnd::installUserLicense(bool online)
	{
		...
		if (not IO::Directory::Create(filename))
		{
			...
			logs.error() << "impossible to install the license. Please check your user account privileges";
			return;
		}
	...
	void ApplWnd::startAntares()
	{
		...
		if (not logs.logfile())
		{
			...
			Window::Message message(this, wxT("Logs"),
				wxT("No Log file"), msg, "images/misc/warning.png");
	...
	void ApplWnd::createNBBindingConstraints()
	{
		...
		if (pWndBindingConstraints)
			pNotebook->add(pWndBindingConstraints, wxT("bindingconstraints"), wxT("Binding constraint"));
	}
	...
	void ApplWnd::createNBMisc()
	{
		...
		std::pair<Component::Notebook*, Toolbox::InputSelector::Area*> page =
			createStdNotebookPage<Toolbox::InputSelector::Area>(pNotebook,
			wxT("misc"), wxT("Misc. Gen."));

		// Time-series
		Component::Notebook::Page* p = page.first->add(
			new Component::Datagrid::Component(page.first,
			new Component::Datagrid::Renderer::Misc(page.first, page.second)),
			wxT("Misc. Gen."));
		p->select();
	}
	...
	void ApplWnd::createNBInterconnections()
	{
		...
		std::pair<Component::Notebook*, Toolbox::InputSelector::Connections*> page =
			createStdNotebookPage<Toolbox::InputSelector::Connections>(pNotebook,
			wxT("interconnections"), wxT("Links"));

		// Interco
		pageLinksDetails = page.first->add(new Window::Interconnection(page.first, page.second), wxT("  Details  "));
		pageLinksSummary = page.first->add(
			new Component::Datagrid::Component(page.first,
			new Component::Datagrid::Renderer::Links::Summary(page.first)),
			wxT(" Summary "));
	}
	...
	bool ApplWnd::checkGDPRStatus()
	{
		...
		else
		{
			...
			wxString msg = wxT("By letting us know that our software is now running for a new user, you\n\
...
Click on Continue to express consent");
			...
			Window::Message message(this, wxT(""),
				wxT("Congratulations for the successful installation of Antares_Simulator"), msg, "images/128x128/antares.png");
			message.add(Window::Message::btnContinue, true);
	...
	void ApplWnd::setGDPRStatus(bool checkOnline)
	{
		...
		if (IO::File::Exists(GDPR_filename))
		{
			...
			if(checkOnline)
			{
				...
				{
					Window::Message message(this, wxT(""),
						"Usage metrics", "Antares_Simulator signature is no longer valid. Please restart to be able to join again", "images/128x128/antares.png");
					message.add(Window::Message::btnOk, true);
					message.showModal();
				}
				else
				{
					if (consent == 1) //display  "  Antares_Simulator is already online (anonymous usage metrics)" // nothing to do
					{
						Window::Message message(this, wxT(""),"Usage metrics", "Antares_Simulator is already online (anonymous usage metrics)", "images/128x128/antares.png");
						message.add(Window::Message::btnOk, true);
						message.showModal();
					}
					else
					{
						Window::Message message(this, wxT(""), "Usage metrics", "Antares_Simulator will start on-line next time (anonymous usage metrics)", "images/128x128/antares.png");
						message.add(Window::Message::btnOk, true);
						message.showModal();
						consent = 1;
					}
			...
			else
			{
				if (banned == 0)
				{
					if (consent == -1) 
					{
						Window::Message message(this, wxT(""), "Usage metrics", "Antares_Simulator is already off-line", "images/128x128/antares.png");
						message.add(Window::Message::btnOk, true);
						message.showModal();
					} // no need to BAN	
					else
					{ 
						Window::Message message(this, wxT(""), "Usage metrics", "Antares_Simulator will continue off-line", "images/128x128/antares.png");
						message.add(Window::Message::btnOk, true);
						message.showModal();
						banned = 1; 
				...
				else // new call of the BAN command by this ui instance, or by another one ; nothing to do until next restart
				{
					Window::Message message(this, wxT(""), "Usage metrics", "Antares_Simulator will stay off-line", "images/128x128/antares.png");
					message.add(Window::Message::btnOk, true);
					message.showModal();
					banned = 1;
				}
