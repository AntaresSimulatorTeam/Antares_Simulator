...
namespace Antares
{
	...
	static void TheSimulationIsComplete(const wxString& duration)
	{
		...
		if (mainFrm)
		{
			Window::Message message (mainFrm, wxT("Simulation"),
				...
				wxString() << wxT("Time to complete the simulation : ") << duration);
	...
	namespace // anonymous
	{
		...
		{
		public:
			JobSaveStudy(Data::Study& study, const String& folder, bool copyOutput = false,
				bool copyUserData = false, bool copyLogs = false)
				:Toolbox::Jobs::Job(wxT("Saving the study"), wxT("Saving the study"),
				"images/32x32/open.png"),
				study(study),
				pSaveAs(false),
			...
			template<class StringT> void folder(const StringT& f) {pFolder = f;}

			void saveAs(bool b) {pSaveAs = b;}

			void shouldInvalidateStudy() { pShouldInvalidateStudy = true; }
		...
		protected:
			...
			*/
			virtual bool executeTask()
			{
				...
				{
					logs.debug() << "[ui] releasing the scenario builder data, since the page is not opened";
			...
			bool onCopyFile(IO::Directory::CopyState state, const String&, const String&, uint64 count, uint64 total)
			{
				if (0 == (--pCount))
				{
					pCount = 128;
						...
						case IO::Directory::cpsCopying:
							{
								...
								logs.info() << pTitle << " : " << ((100 * count) / total) << "%   ("
									<< (count / 1024 / 1024) << "Mo / " << (1 + (total / 1024 / 1024)) << "Mo)";
								return true;
							}
		...
		{
		public:
			explicit JobExportMap(const String& path, bool transparentBackground, const wxColour& backgroundColor, const std::list<uint16_t> &layers, int nbSplitParts, Antares::Map::mapImageFormat format)
			: Toolbox::Jobs::Job(wxT("Exporting the map"), wxT("Exporting the map"),
	...
	bool CheckIfInsideAStudyFolder(const AnyString& path, bool quiet)
	{
		...
		if (not quiet)
		{
			...
			Window::Message message(&mainFrm, wxT("Save As"),
				...
				wxString()
					<< wxT("Impossible to save the study into another one. That may lead to data loss.\n\nTitle : ")
	...
	SaveResult SaveStudy()
	{
		...
		{
			Window::Message message(&mainFrm, wxT("Save changes"),
					wxT("Impossible to save the study"),
		...
		{
			Window::Message message(&mainFrm, wxT("Study Upgrade"),
	...
	SaveResult ExportMap(const Yuni::String& path, bool transparentBackground, const wxColour& backgroundColor, const std::list<uint16_t>& layers,int nbSplitParts, Antares::Map::mapImageFormat format)
	{
		if (not Data::Study::Current::Valid() or path.empty())
			return svsCancel;
...

		{
			...
			auto* job = new JobExportMap(path, transparentBackground, backgroundColor, layers, nbSplitParts, format);
			job->run();
			job->Destroy();
			...
			case Data::version1xx:
				{
					logs.error() << studyfolder
						<< ": The support for the studies in 1.x format is no longer available since Antares v3.4.";
					return;
				}
			default:
				if ((uint)version > (uint)Data::versionLatest)
				{
					logs.error() << "A more recent version of Antares is required to open `"
						<< studyfolder
        ...
        bool useOrtools, const std::string& ortoolsSolver)
	{
...

		{
			...
			else
			{
				...
				{
					exec->subTitle(wxString()
						<< wxT("Running for ") << study->parameters.nbYears
	...
	bool StudyRenameArea(Data::Area* area, const AnyString& newname, Data::Study::Ptr study)
	{
		...
		if (study == currentstudy)
		{
			...
			if (beautifyname.empty())
				logs.error() << "impossible to rename the area '" << area->name << "' with an empty name";
			else
			{
