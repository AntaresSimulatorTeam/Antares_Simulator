...
namespace Simulation
{
	...
	void Run::gridAppend(wxFlexGridSizer& sizer, const wxString& title, const wxString& key, wxWindow* value, bool bold)
	{
		if (title.empty())
		{
		...
		{
			auto* t = Antares::Component::CreateLabel(pBigDaddy, wxString(wxT("   ")) << title << wxT("       "), true);
			t->Enable(false);
	...
	void Run::gridAppend(wxFlexGridSizer& sizer, const wxString& title, const wxString& key, const wxString& value)
	{
		...
		pFeatureIndex(0)
	{
		...
		// Header
		mnSizer->Add(Toolbox::Components::WizardHeader::Create(this,
			wxT("Run a simulation..."),
			"images/32x32/run.png",
			wxString(wxT("For the study `")) << wxStringFromUTF8(study.header.caption)
			<< wxT("`, ") << wxStringFromUTF8(Data::StudyModeToCString(study.parameters.mode))
			)
			, 0, wxALL|wxEXPAND|wxFIXED_MINSIZE);
		...
		// Space
		s->AddSpacer(8); s->AddSpacer(8); s->AddSpacer(8);

		// Informations about the INPUT
		{
			auto* lblMode = Antares::Component::CreateLabel(pBigDaddy, wxStringFromUTF8(study.header.caption));
		...
		gridAppend(*s, wxT("Time-series generator(s) : "), TimeSeriesToWxString(study.parameters.timeSeriesToGenerate));

		// Space
		s->AddSpacer(15); s->AddSpacer(15); s->AddSpacer(15);

		// Informations about the OUTPUT
		pSimulationName = new wxTextCtrl(pBigDaddy, wxID_ANY, wxEmptyString,
			wxDefaultPosition, wxSize(360, -1), 0, Toolbox::Validator::Default());
		gridAppend(*s, wxT("OUTPUT"), wxT("Simulation name : "), pSimulationName, true);


		if (false)
		{
			pSimulationComments = new wxTextCtrl(pBigDaddy, wxID_ANY, wxEmptyString,
				wxDefaultPosition, wxSize(360, 70), wxTE_MULTILINE|wxTE_PROCESS_TAB);
		...
		pPreproOnly->Connect(pPreproOnly->GetId(), wxEVT_COMMAND_CHECKBOX_CLICKED,
			wxCommandEventHandler(Run::evtOnPreprocessorsOnlyClick), nullptr, this);
		...
		uint & minNbCores = Data::Study::Current::Get()->minNbYearsInParallel;	// For run window's simulation cores field.
...

		{
			...
			pLblEstimationAvailable = Antares::Component::CreateLabel(pBigDaddy, wxEmptyString, false, true);
			...
			pLblDiskEstimationAvailable = Antares::Component::CreateLabel(pBigDaddy, wxEmptyString, false, true);
		...
		Connect(GetId(), wxEVT_MOTION,
			wxMouseEventHandler(Run::onInternalMotion), NULL, this);
	}


	Run::~Run()
	{
		if(pNbCores)		delete pNbCores;
		if(pTitleSimCores)	delete pTitleSimCores;
	...
	void Run::estimateMemoryUsage()
	{
		...
		if (not updating)
		{
			...
			pWarnAboutDiskLimit   = (diskFree != (uint64) -1) and (not study.folder.empty() and (diskFree < m.requiredDiskSpace));
	...
	bool Run::createCommentsFile(String& filename) const
	{
		...
		if (not content.empty())
		{
			...
			{
				fn.clear()
					<< temporary << IO::Separator << "antares-" << processID
					<< "-simu-comment-" << (void*) this << '-' << (++index) << ".txt";
	...
	int Run::checkForLowResources()
	{
		...
		Window::Message message(&mainFrm, wxT("Simulation"),
			msg, wxT("Try anyway ?"), "images/misc/warning.png");
	...
	void Run::onRun(void*)
	{
		...
		if (License::Limits::areaCount) // checking for license restrictions
		{
			...
			if (isTrial)
			{
				...
				{
					text << wxT("Simulation limited to ") << License::Limits::areaCount << wxT(" areas");
				}

				Window::Message message(this, wxT("Simulation"),
				...
				else
				{
					...
					if (!pAlreadyWarnedNoMCYear)
					{
						...
						Window::Message message(this, wxT("Simulation"),
							...
							wxT("The number of MC years is invalid (can not be null)"), "images/misc/error.png");
	...
	void Run::updateNbCores()
	{
		...
		if (Data::Study::Current::Valid())
		{
			// Minimum number of years in a set of parallel years (reduction from raw number of cores chosen by user).
	...
	void Run::onSelectMode(wxCommandEvent& evt)
	{
		...
		// In case of either default or swap support mode, MC years parallel computation is disabled (nb of cores is set to 1)
