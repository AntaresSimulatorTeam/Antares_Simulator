...
namespace Window
{
	...
	ExportMap::ExportMap(wxFrame* parent) :
		wxDialog(parent, wxID_ANY, wxT("Export Map"), wxDefaultPosition, wxDefaultSize,
	...
	{
		auto* t = new wxStaticText(pParentProperties, wxID_ANY,
			wxString() << wxT("    ") << text << wxT("  "),
			wxDefaultPosition, wxDefaultSize, wxALIGN_RIGHT);
		if (bold)
		{
			t->SetFont(wxFont(t->GetFont().GetPointSize(), wxFONTFAMILY_DEFAULT,
	...
	{
		auto* edit = new wxTextCtrl(pParentProperties, id, defaultValue,
			wxDefaultPosition, wxDefaultSize, 0, Toolbox::Validator::Default());
	...
	wxTextCtrl* ExportMap::propEdit(const wxString& defaultValue, const wxTextValidator& validator, int id)
	{
		wxTextCtrl* edit = new wxTextCtrl(pParentProperties, id, defaultValue,
	...
	void ExportMap::internalCreateComponents()
	{
		...
		auto* header = Toolbox::Components::WizardHeader::Create(this,
			...
			"images/32x32/exportmap.png", wxT(""));
		...
		propCaption(wxT("Save in:"));
		{
			...
			//
			wxButton* btnBrowse = new wxButton(pnl, mnIDBrowse, wxT(" Browse "),
				wxDefaultPosition, wxSize(-1, 22), wxBU_EXACTFIT);
			pnlSizer->Add(btnBrowse);
			btnBrowse->Connect(btnBrowse->GetId(), wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(ExportMap::onBrowse), nullptr, this);
			


			pMapFolderPath = new wxTextCtrl(pnl, wxID_ANY, wxEmptyString,
		...
		pMapFormatChoice = new wxChoice(pParentProperties, mnIDMapFormat, wxDefaultPosition, wxSize(-1,22), formatsList);
		pMapFormatChoice->SetSelection(0);
		pMapFormatChoice->Connect(pMapFormatChoice->GetId(), wxEVT_COMMAND_CHOICE_SELECTED, wxCommandEventHandler(ExportMap::onMapFormatChanged), nullptr, this);
		...
		// More Options
		//
		auto* colPanl = new wxCollapsiblePane(pParentProperties, mnIDCollapsePanel, wxT("More options"));
		...
		pLayers = new wxComboBox(pParentProperties, wxID_ANY, layersList.Item(0), wxDefaultPosition, wxSize(-1,22), layersList);
		pGridSizer->Add(pLayers);*/
		...
		pSplitNumberChoice = new wxChoice(pParentProperties, wxID_ANY, wxDefaultPosition, wxSize(-1,22), splitList);
		pSplitNumberChoice->SetSelection(1);
		*/
			
		// Using a background color. Default should be set to true if JPEG is selected, false otherwise.
		pUseBackgroundColor = (wxCheckBox*)propEdit(new wxCheckBox(pParentProperties, wxID_ANY,
			wxT("Use background colour")));
		pUseBackgroundColor->SetValue(false);
		pUseBackgroundColor->Connect(pUseBackgroundColor->GetId(), wxEVT_COMMAND_CHECKBOX_CLICKED, wxCommandEventHandler(ExportMap::onUseBackgroundChanged), nullptr, this);
		//pGridSizer->Add(pUseBackgroundColor);
		// Colour picker to choose the background color, if using a one 
		pBackgroundColor = new wxColourPickerCtrl(pParentProperties, wxID_ANY, Map::Settings::background); // On windows we avoid WX_WHITE and WX_BLACK because of a bug that prevent to pick the colour
		...
		auto* btnExportMap = Component::CreateButton(this, wxT("Export"), this, &ExportMap::onExportMap);
	...
	{
		wxDirDialog dialog(this, _T("Save map in directory:"),
			pMapFolderPath->GetValue(), wxDD_DEFAULT_STYLE & ~wxDD_DIR_MUST_EXIST);
	...
	void ExportMap::ensureUniqueFileName(Yuni::String& folderPath)
	{
		...
		if (pMapName->GetValue() != wxEmptyString && pMapName->GetValue() != wxStringFromUTF8(mapFileName))
		{
	...
	void ExportMap::onMapFormatChanged(wxCommandEvent&)
	{
		...
		pMapName->ChangeValue( s+wxStringFromUTF8(Antares::Map::formatExt[pMapFormatChoice->GetSelection()]) );
	...
	void ExportMap::onExportMap(void*)
	{
		...
		int nbSplitParts		= srcFolderNotEmpty and pSplitNumberChoice and pSplitNumberChoice->GetSelection();
		...
		// Checking for the root folder
		{
			...
			{
				Window::Message message(&mainFrm, wxT("Save Map"),
		...
		if (IO::Exists(path))
		{
			Window::Message message(&mainFrm, wxT("Save As"),
					wxT("Impossible to save the map"),
					wxString()
					<< wxT("A file or a folder named '") << wxStringFromUTF8(path) << wxT("' already exists")
			);
			message.add(Window::Message::btnCancel);
			message.showModal();
			return;
		...
		mainFrm.exportMap(path, transparentBackground, backgroundColor, layers, nbSplitParts, pMapFormat);
	}
