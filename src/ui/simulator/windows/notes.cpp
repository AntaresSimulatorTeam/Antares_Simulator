...
void Notes::saveToStudy()
{
    ...
    // ... and reloading it
    IO::File::LoadFromFile(study->simulationComments.comments, pTempFile);
}
...
void Notes::loadFromStudy()
{
    ...
    {
        pLocked = true;
        if (study.simulationComments.comments.empty())
        {
            pRichEdit->Clear();
        }
        else
        {
            // <?xml ?
            if (study.simulationComments.comments.startsWith("<?xml "))
            {
                // XML Version
            ...
            {
                // Txt version
                pRichEdit->SetValue(wxStringFromUTF8(study.simulationComments.comments));
            }
        }
        pLocked = false;
    }
