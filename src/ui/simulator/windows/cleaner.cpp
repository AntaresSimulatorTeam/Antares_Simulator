...
namespace Window
{
	...
	{
	public:
		CleaningThread(StudyCleaner* parent) :
			pParent(parent)
		{
		}
	...
	{
	public:
		CleaningInfoRefreshTimer(StudyCleaner* parent) :
			pParent(parent)
		{
			// Do Nothing
		}
	...
	StudyCleaner::StudyCleaner(wxFrame* parent) :
		wxDialog(parent, wxID_ANY, wxT("Cleaner"), wxDefaultPosition, wxDefaultSize,
		...
		pThread(nullptr)
	{
		...
		// Header
		mainSizer->Add(Toolbox::Components::WizardHeader::Create(this, wxT("Study folder cleaner"),
		...
		pnlBtns->AddSpacer(5);
		pBtnGo = Component::CreateButton(this, wxT("Perform a disk cleanup"), this, &StudyCleaner::onProceed);
	...
	void StudyCleaner::updateGUI(bool hasItems)
	{
		...
		if (hasItems and pInfos and !pInfos->intruders.empty())
		{
			...
			{
				*(pListbox) +=
					new Component::HTMLListbox::Item::PathListItem(i->first, i->second);
		...
		if (hasAtLeastOneItem and pInfos)
		{
			...
			pLblInfos->SetLabel(
				wxString::Format(wxT("Total size : %.1f Mb  (%u files)"),
				totalSize, pInfos->intruders.size()));
