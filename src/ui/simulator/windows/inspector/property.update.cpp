...
namespace Inspector
{
	...
	bool InspectorGrid::onPropertyChanging_A(wxPGProperty*, const PropertyNameType& name, const wxVariant& value)
	{
	...
	bool InspectorGrid::onPropertyChanging_C(wxPGProperty*, const PropertyNameType& name, const wxVariant& value)
	{
	...
	bool InspectorGrid::onPropertyChanging_L(wxPGProperty*, const PropertyNameType& name, const wxVariant& value)
	{
		InspectorData::Ptr& data = pCurrentSelection;
		if (!data)
			return false;
	...
	bool InspectorGrid::onPropertyChanging_Constraint(wxPGProperty*, const PropertyNameType& name, const wxVariant& value)
	{
		InspectorData::Ptr& data = pCurrentSelection;
		if (!data)
			return false;
	...
	bool InspectorGrid::onPropertyChanging_Cluster(wxPGProperty*, const PropertyNameType& name, const wxVariant& value)
	{
		InspectorData::Ptr& data = pCurrentSelection;
		if (!data)
			return false;
		...
		if (name == "cluster.group")
		{
			...
			if (not newgroup.empty())
			{
				...
				{
					if (index < 0 || index > arrayClusterGroupCount)
					{
						logs.error() << "The group index is invalid";
						return false;
					}
					//
					const wxChar* const wName = arrayClusterGroup[index];
		...
		if (name == "cluster.nominal_capacity")
		{
			...
			if (data)
			{
				Accumulator<PClusterNomCapacity>    :: Apply(pFrame.pPGClusterNominalCapacity, data->clusters);
				Accumulator<PClusterInstalled, Add> :: Apply(pFrame.pPGClusterInstalled, data->clusters);
				
				// apply check and colour
				AccumulatorCheck<PClusterMinStablePowerColor> :: ApplyTextColor(pFrame.pPGClusterMinStablePower, data->clusters);
				AccumulatorCheck<PClusterNomCapacityColor>    :: ApplyTextColor(pFrame.pPGClusterNominalCapacity, data->clusters);
				AccumulatorCheck<PClusterSpinningColor>       :: ApplyTextColor(pFrame.pPGClusterSpinning, data->clusters);
			}
			// Notify
			OnStudyThermalClusterCommonSettingsChanged();
			return true;
		...
		if (name == "cluster.forcedlaw")
		{
			...
			switch (index)
			{
				case 0: law = Data::thermalLawUniform; break;
				case 1: law = Data::thermalLawGeometric; break;
	...
	bool InspectorGrid::onPropertyChanging_S(wxPGProperty*, const PropertyNameType& name, const wxVariant& value)
	{
		InspectorData::Ptr& data = pCurrentSelection;
		if (!data)
			return false;
				...
				case 1: // manual
					{
						...
						if (error)
						{
							Window::Message message(this, wxT("Selection Mode"), wxT("MC Playlist"),
