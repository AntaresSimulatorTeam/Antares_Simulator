...
namespace ScenarioBuilder
{
	...
	namespace // anonymous
	{
		...
		inline wxString  RulesetCaption(const Data::ScenarioBuilder::Rules::Ptr& rules, bool selected = false)
		{
		...
		Antares::Component::Panel(parent)
	{
		...
		hz->Add(Component::CreateLabel(sub, wxT("   Active :"), true), 0, wxRIGHT | wxALIGN_CENTER_VERTICAL, 4);
	...
	{
		// New
		wxMenuItem* itNew = Menu::CreateItem(&menu, wxID_ANY, wxT("New ruleset     "), "images/16x16/new.png", wxEmptyString,
			wxITEM_NORMAL);
		menu.Connect(itNew->GetId(), wxEVT_COMMAND_MENU_SELECTED,
				wxCommandEventHandler(Panel::onFileNew), nullptr, this);
		...
		if (rulesetCount and !(!pRules))
		{
			...
			wxMenuItem* itRename = Menu::CreateItem(&menu, wxNewId(), s, "images/16x16/rename.png", wxEmptyString,
				wxITEM_NORMAL);
			menu.Connect(itRename->GetId(), wxEVT_COMMAND_MENU_SELECTED,
					wxCommandEventHandler(Panel::onFileRename), nullptr, this);
		...
		if (rulesetCount > 1 and !(!pRules))
		{
			...
			wxMenuItem* itDelete = Menu::CreateItem(&menu, wxNewId(), s, "images/16x16/cancel.png", wxEmptyString,
				wxITEM_NORMAL);
			menu.Connect(itDelete->GetId(), wxEVT_COMMAND_MENU_SELECTED,
					wxCommandEventHandler(Panel::onFileDelete), nullptr, this);
		...
		if (rulesetCount > 1)
		{
			...
			it = Menu::CreateItem(&menu, wxID_ANY, wxT("Rename..."), "images/16x16/rename.png", wxEmptyString, wxITEM_NORMAL);
			it->Enable(false);
			...
			for (Data::ScenarioBuilder::Sets::iterator i = sets.begin(); i != end; ++i)
			{
				...
				wxMenuItem* itRename = Menu::CreateItem(&menu, wxNewId(), s, nullptr, wxEmptyString,
					wxITEM_NORMAL);
				menu.Connect(itRename->GetId(), wxEVT_COMMAND_MENU_SELECTED,
					wxCommandEventHandler(Panel::onFileRename), nullptr, this);
			...
			it = Menu::CreateItem(&menu, wxID_ANY, wxT("Delete..."), "images/16x16/cancel.png", wxEmptyString, wxITEM_NORMAL);
			it->Enable(false);
			...
			for (Data::ScenarioBuilder::Sets::iterator i = sets.begin(); i != end; ++i)
			{
				...
				wxMenuItem* itDelete = Menu::CreateItem(&menu, wxNewId(), s, nullptr, wxEmptyString,
					wxITEM_NORMAL);
				menu.Connect(itDelete->GetId(), wxEVT_COMMAND_MENU_SELECTED,
						wxCommandEventHandler(Panel::onFileDelete), nullptr, this);
				...
				//
				//wxMenuItem* itSubDelete = Menu::CreateItem(deleteSub, wxNewId(), str, img, wxEmptyString,
				//	wxITEM_NORMAL);
				//deleteSub->Connect(itSubDelete->GetId(), wxEVT_COMMAND_MENU_SELECTED,
				//	wxCommandEventHandler(Panel::onFileDelete), nullptr, this);

				// Rename
				//wxMenuItem* itSubRename = Menu::CreateItem(renameSub, wxNewId(), str, img, wxEmptyString,
				//	wxITEM_NORMAL);
				//renameSub->Connect(itSubRename->GetId(), wxEVT_COMMAND_MENU_SELECTED,
				//	wxCommandEventHandler(Panel::onFileRename), nullptr, this);
	...
	void Panel::onActiveMenu(Antares::Component::Button&, wxMenu& menu, void*)
	{
		...
		if (sets.empty())
		{
			wxMenuItem* it = Menu::CreateItem(&menu, wxID_ANY, wxT("(No ruleset available)  "), "images/16x16/empty.png", wxEmptyString,
				wxITEM_NORMAL);
			it->Enable(false);
			return;
		}
		...
		for (; i != end; ++i)
		{
			...
			// New
			wxMenuItem* it = Menu::CreateItem(&menu, wxID_ANY, str, img, wxEmptyString,
				wxITEM_NORMAL, selected);
			// Mapping
			pActiveList[it->GetId()] = rules;

			menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
					wxCommandEventHandler(Panel::onActiveRuleset), nullptr, this);
		}
	}
	...
	void Panel::onFileDelete(wxCommandEvent& evt)
	{
		...
		{
			Window::Message message(this, wxT("Delete a ruleset"),
				wxEmptyString,
	...
	void Panel::onFileRename(wxCommandEvent& evt)
	{
		...
		do
		{
			...
			if (!newname)
			{
				Window::Message message(this, wxT("Rename"),
					wxEmptyString,
					wxT("Impossible to rename the ruleset. The new name must not be empty."), "images/misc/book.png");
				message.add(Window::Message::btnContinue, true);
				message.showModal();
				continue;
			}
			if (newname.contains('[') or newname.contains(']') or newname.contains('$') or newname.contains('#') or newname.contains(','))
			{
				Window::Message message(this, wxT("Rename"),
					wxEmptyString,
					wxT("Impossible to rename the ruleset.\n\nThe name must not contain the following characters : , $ # [ ]"), "images/misc/book.png");
				message.add(Window::Message::btnContinue, true);
				message.showModal();
				continue;
			}
			...
			if (sets.exists(newid))
			{
				Window::Message message(this, wxT("Rename"),
					wxEmptyString,
					wxT("The name is already used."), "images/misc/book.png");
				message.add(Window::Message::btnContinue, true);
				message.showModal();
				continue;
			}
		...
		if (!newrules)
		{
			Window::Message message(this, wxT("Rename"),
				wxEmptyString,
				wxT("Impossible to rename the ruleset"), "images/misc/book.png");
			message.add(Window::Message::btnContinue, true);
			message.showModal();
			return;
		}
