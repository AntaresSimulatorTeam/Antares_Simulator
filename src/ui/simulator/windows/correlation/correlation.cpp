...
namespace Window
{
		...
		wxPanel(parent, wxID_ANY)
	{
		...
		// Panel
		{
			static const wchar_t* const months[] =
			{
			...
			btn = Resources::BitmapButtonLoadFromFile(panel,
				wxID_ANY, "images/16x16/sort_alphabet.png");
			btn->Connect(btn->GetId(), wxEVT_COMMAND_BUTTON_CLICKED,
				wxCommandEventHandler(CorrelationPanel::onSortAlpha), nullptr, this);
			panel->GetSizer()->Add(btn, 0, wxALL|wxEXPAND, margin);

			// Sort - reverse
			btn = Resources::BitmapButtonLoadFromFile(panel,
				wxID_ANY, "images/16x16/sort_alphabet_descending.png");
			btn->Connect(btn->GetId(), wxEVT_COMMAND_BUTTON_CLICKED,
				wxCommandEventHandler(CorrelationPanel::onSortAlphaReverse), nullptr, this);
			panel->GetSizer()->Add(btn, 0, wxALL|wxEXPAND, margin);

			// Sort - color
			btn = Resources::BitmapButtonLoadFromFile(panel,
				wxID_ANY, "images/16x16/color.png");
			btn->Connect(btn->GetId(), wxEVT_COMMAND_BUTTON_CLICKED,
				wxCommandEventHandler(CorrelationPanel::onSortColor), nullptr, this);
			panel->GetSizer()->Add(btn, 0, wxALL|wxEXPAND, margin);
			...
			if (timeseries != Data::timeSeriesHydro)
			{
				for (uint i = 0; i != 12; ++i)
				{
					pData->pGridMonthly[i] = new Component::Datagrid::Component(notebook, pData->renderer[i]);
