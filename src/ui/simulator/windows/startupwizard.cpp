...
{ 
		...
		public:
			...
			*/
			...
			void extractNumber(const String& title, String& newtitle, String& number)
			{
				...
				{
					if (String::IsDigit(title[0]) and String::IsDigit(title[1]) and String::IsDigit(title[2]))
					{
	...
	StartupWizard::StartupWizard(wxFrame* parent) :
		wxDialog(parent, wxID_ANY, wxT("Getting started"), wxDefaultPosition, wxDefaultSize,
		...
		pBtnExamples(nullptr)
	{
...

		{
			...
			# ifdef ANTARES_BETA
			hz->Add(Resources::StaticBitmapLoadFromFile(this, wxID_ANY, "images/misc/beta.gif"), 0, wxALIGN_LEFT | wxALIGN_TOP);
			# elif ANTARES_RC != 0
			hz->Add(Resources::StaticBitmapLoadFromFile(this, wxID_ANY, "images/misc/rc.gif"), 0, wxALIGN_LEFT | wxALIGN_TOP);
			# else
			hz->AddSpacer(20);
			# endif
			...
			hz2->AddSpacer(20);
			wxStaticText* welcome = Component::CreateLabel(this, wxT("Welcome to Antares Simulator"), true, false, +4);
		...
		// New study
		addControls(wxT("new"), "images/32x32/new.png", wxT("Start a new empty Study"),
			wxT("Create an new study, with default settings and an empty system"));
		addControls(wxT("open"), "images/32x32/open.png", wxT("Open a Study"),
		...
		// Cancel
		auto* btnCancel = Component::CreateButton(pnlBtns, wxT("  Close  "),
			this, &StartupWizard::onClose);
		...
		Connect(GetId(), wxEVT_MOTION,
			wxMouseEventHandler(StartupWizard::onWindowMotion), nullptr, this);
	}
	...
	void StartupWizard::loadRecentFiles()
	{
		...
		if (not lst->empty())
		{
			addControls(wxString(), "images/32x32/recent.png", wxT("Open a recent Study"),
				wxEmptyString, false);
			if (sizer)
				sizer->AddSpacer(4);
	...
	void StartupWizard::addRecentItem(const wxString& mapping, const wxString& title, const wxString& path)
	{
		auto* bigPanel = new Component::Panel(this);
		bigPanel->SetBackgroundColour(wxColour(255, 255, 255));
		{
			pMapping[bigPanel->GetId()] = mapping;
			bigPanel->Connect(bigPanel->GetId(), wxEVT_MOTION,
				wxMouseEventHandler(StartupWizard::onMotion), nullptr, this);
			bigPanel->Connect(bigPanel->GetId(), wxEVT_LEFT_DOWN,
				wxCommandEventHandler(StartupWizard::onClick), nullptr, this);
		}
		...
		//
		lblTitle->Connect(lblTitle->GetId(), wxEVT_LEFT_DOWN,
			wxCommandEventHandler(StartupWizard::onClick), nullptr, this);
		lblTitle->Connect(lblTitle->GetId(), wxEVT_MOTION,
			wxMouseEventHandler(StartupWizard::onMotion), nullptr, this);
		lblPath->Connect(lblPath->GetId(),   wxEVT_LEFT_DOWN,
			wxCommandEventHandler(StartupWizard::onClick), nullptr, this);
		lblPath->Connect(lblPath->GetId(),   wxEVT_MOTION,
			wxMouseEventHandler(StartupWizard::onMotion), nullptr, this);
	...
	void StartupWizard::addControls(const wxString& mapping, const char* image,
		const wxString& title, const wxString& subtitle, bool useEvents)
	{
		auto* bigPanel = new Component::Panel(this);
		bigPanel->SetBackgroundColour(wxColour(255, 255, 255));
		if (useEvents)
		...
		if (image)
		{
			...
			if (useEvents)
			{
				bmp->Connect(bmp->GetId(), wxEVT_MOTION, wxMouseEventHandler(StartupWizard::onMotion), nullptr, this);
				bmp->Connect(bmp->GetId(), wxEVT_LEFT_DOWN, wxCommandEventHandler(StartupWizard::onClick), nullptr, this);
		...
		// Text
		{
			...
			if (useEvents)
			{
				if (lblTitle)
				{
					lblTitle->Connect(lblTitle->GetId(), wxEVT_MOTION, wxMouseEventHandler(StartupWizard::onMotion), nullptr, this);
					lblTitle->Connect(lblTitle->GetId(), wxEVT_LEFT_DOWN, wxCommandEventHandler(StartupWizard::onClick), nullptr, this);
					pMapping[lblTitle->GetId()] = mapping;
				}
				if (lblSubTitle)
				{
					lblSubTitle->Connect(lblSubTitle->GetId(), wxEVT_MOTION, wxMouseEventHandler(StartupWizard::onMotion), nullptr, this);
					lblSubTitle->Connect(lblSubTitle->GetId(), wxEVT_LEFT_DOWN, wxCommandEventHandler(StartupWizard::onClick), nullptr, this);
			...
			else
			{
				if (wxT("open") == mapping)
				{
					...
					else
					{
						...
						logs.error() << "This folder does not seem to be a valid study: " << studyfolder;
						return;
					}
				}
				else
