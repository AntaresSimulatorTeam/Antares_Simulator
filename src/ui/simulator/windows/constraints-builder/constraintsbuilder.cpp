...
namespace Window
{
	...
	ConstraintsBuilderWizard::ConstraintsBuilderWizard(wxFrame* parent) : wxDialog(parent, wxID_ANY, wxT("Constraints Builder"), wxDefaultPosition, wxSize(800, 420),
			wxCLOSE_BOX|wxCAPTION|wxCLIP_CHILDREN), pProceedTimer(nullptr)
	{
		...
		// Header
		mainSizer -> Add(Toolbox::Components::WizardHeader::Create(this, wxT("Kirchhoff's constraints generator"), "images/32x32/network.png", wxT("")), 0, wxALL|wxEXPAND | wxFIXED_MINSIZE);
...

		{
			...
			// Space
			flexSz->AddSpacer(8); flexSz->AddSpacer(8); flexSz->AddSpacer(8);

			auto* tmpF = Component::CreateLabel(panelGenerationSettings, wxString(wxT("   ")) << wxT("CHECK LOOP FLOW") << wxT("       "), true);
			tmpF->Enable(false);
			flexSz->Add(tmpF, 0, wxRIGHT | wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL);

			flexSz->AddSpacer(5);
			flexSz->AddSpacer(5);
			flexSz->AddSpacer(3); flexSz->AddSpacer(3); flexSz->AddSpacer(3);

			pCheckNodalLoopFlow = new wxCheckBox(panelGenerationSettings, wxID_ANY, wxT(""));
			pCheckNodalLoopFlow->SetValue(true);
			pCheckNodalLoopFlow->Bind(wxEVT_CHECKBOX, &ConstraintsBuilderWizard::onCheckNodalLoopFlow, this);
			flexSz->Add(pCheckNodalLoopFlow, 0, wxALIGN_BOTTOM | wxALIGN_RIGHT);
			flexSz->Add(Antares::Component::CreateLabel(panelGenerationSettings, wxT(" Check loop flow sum at node "), true), 0, wxALIGN_BOTTOM | wxALIGN_LEFT);
			flexSz->AddSpacer(5);
			...
			auto label = Antares::Component::CreateLabel(panelGenerationSettings, wxString(wxT("   ")) << wxT("INFINITE VALUE") << wxT("       "), true);
			label->Enable(false);
			...
			//flexSz->Add(pCheckNodalLoopFlow, 0, wxALIGN_BOTTOM | wxALIGN_RIGHT);
			flexSz->Add(Antares::Component::CreateLabel(panelGenerationSettings, wxT(" Infinite Value: "), true), 0, wxALIGN_BOTTOM | wxALIGN_RIGHT,3);

			pInfiniteValue = new wxTextCtrl(panelGenerationSettings, wxID_ANY, wxT("1000000"), wxDefaultPosition, wxSize(80, 20));
			...
			/*pCheckNodalLoopFlow = new wxCheckBox(panelGenerationSettings, wxID_ANY, wxT(""));
			pCheckNodalLoopFlow->SetValue(true);
			sizerGenerationSettings->Add(Antares::Component::CreateLabel(panelGrid, wxT(" Check loop flow sum at nodes "), true), 0, wxALIGN_BOTTOM | wxALIGN_LEFT);*/
		}
		...
		{
			auto* label = Antares::Component::CreateLabel(panelGrid, wxString(wxT("   ")) << wxT("WORKING MAP") << wxT("       "), true);
			label->Enable(false);
			flexSz->Add(label, 0, wxRIGHT | wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL);
			...
			std::string layerName = Data::Study::Current::Get()->layers.at(Data::Study::Current::Get()->activeLayerID);
			mapName = Antares::Component::CreateLabel(panelGrid,wxString(layerName));

			flexSz->Add(Antares::Component::CreateLabel(panelGrid, wxT("Map name : "), true), 0, wxALIGN_BOTTOM | wxALIGN_RIGHT);
			...
			label = Antares::Component::CreateLabel(panelGrid, wxString(wxT("   ")) << wxT("TIME FRAME") << wxT("       "), true);
			label->Enable(false);
			flexSz->Add(label, 0, wxRIGHT | wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL);
			flexSz->AddSpacer(5);
			flexSz->AddSpacer(5);

			flexSz->Add(Antares::Component::CreateLabel(panelGrid, wxString(wxT("   ")) << wxString(wxT("   ")) <<  wxT("From hour : "), true), 0, wxALIGN_BOTTOM | wxALIGN_RIGHT);
			...
			btn->enabled(false);
			hS->Add(btn);*/
			startingHourCtrl = new wxTextCtrl(panelGrid, wxID_ANY, wxT("1"), wxDefaultPosition, wxSize(40, 20));
			startingHourCtrl->Bind(wxEVT_TEXT, &ConstraintsBuilderWizard::updateBeginningHourLabel, this);
			...
			btn->enabled(false);
			hS->Add(btn);*/
			endHourCtrl = new wxTextCtrl(panelGrid, wxID_ANY, wxT("8760"), wxDefaultPosition, wxSize(40, 20));
			endHourCtrl->Bind(wxEVT_TEXT, &ConstraintsBuilderWizard::updateEndHourLabel, this);
			vS->Add(endHourCtrl, 0, wxALIGN_CENTER);
			hS->Add(vS);
			/*btn = new Component::Button(this, wxString::Format(wxT("%i"), 1), "images/16x16/hour_pref.png");
			btn->menu(true);*/
			...
			label = Antares::Component::CreateLabel(panelGrid, wxString(wxT("   ")) << wxT("LOOP FLOW") << wxT("       "), true);
			label->Enable(false);
			flexSz->Add(label, 0, wxRIGHT | wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL);
			flexSz->AddSpacer(5);
			flexSz->AddSpacer(5);
			...
			//Include passive loopflow
			{
				...
				pIncludeLoopFlow->Bind(wxEVT_CHECKBOX, &ConstraintsBuilderWizard::onIncludeLoopFlow, this);
				flexSz->Add(pIncludeLoopFlow, 0, wxALIGN_BOTTOM | wxALIGN_RIGHT);
				flexSz->Add(Antares::Component::CreateLabel(panelGrid, wxT(" Include passive Loop Flows "), true), 0, wxALIGN_BOTTOM | wxALIGN_LEFT);
				flexSz->AddSpacer(5);
			...
			//Include passive loopflow
			{
				...
				pIncludePhaseShifts->Bind(wxEVT_CHECKBOX, &ConstraintsBuilderWizard::onUsePhaseShift, this);
				flexSz->Add(pIncludePhaseShifts, 0, wxALIGN_BOTTOM | wxALIGN_RIGHT);
				flexSz->Add(Antares::Component::CreateLabel(panelGrid, wxT(" Include active phase-shifts "), true), 0, wxALIGN_BOTTOM | wxALIGN_LEFT);
				flexSz->AddSpacer(5);
		...
		pGrid = new Component::Datagrid::Component(panelGrid, pRenderer, wxEmptyString, false, true, true);
		...
		if (pDelete)
		{
			...
			pTextDelete = new wxStaticText(this, -1, "Warning: Previous generation was detected. Reset may be used to remove \"" + std::string(CB_PREFIX) + "\" constraints before generating new ones");
			wxSize size = GetSize();
		...
		wxWindow* pBtnDelete = Component::CreateButton(this, wxT(" Reset "), this, &ConstraintsBuilderWizard::onDelete);
		bottomSizer->Add(pBtnDelete, 0, wxALL|wxEXPAND);
		pBtnDelete->Enable(pDelete);

		bottomSizer->AddSpacer(5);
		wxWindow* pBtnStatus = Component::CreateButton(this, wxT(" Show Status "), this, &ConstraintsBuilderWizard::onUpdateLinesStatus);
		bottomSizer->Add(pBtnStatus, 0, wxALL | wxEXPAND);
		pBtnStatus->Enable(true);

		bottomSizer->AddSpacer(5);
		bottomSizer->Add(new wxStaticLine(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLI_VERTICAL), 0, wxALL | wxEXPAND);
		bottomSizer->AddSpacer(5);

		bottomSizer->AddSpacer(5);
		wxWindow* pBtnBuild = Component::CreateButton(this, wxT(" Run "), this, &ConstraintsBuilderWizard::onBuild);
		bottomSizer->Add(pBtnBuild, 0, wxALL|wxEXPAND);
		pBtnBuild->Enable(true);

		bottomSizer->AddSpacer(5);

		wxWindow* btnCancel = Component::CreateButton(this, wxT(" Cancel "), this, &ConstraintsBuilderWizard::onCancel);
	...
	void ConstraintsBuilderWizard::onUpdateInfiniteValue(wxCommandEvent& evt)
	{
		...
		{
			Window::Message message(this, wxT("Constraint Generator"),
	...
	void ConstraintsBuilderWizard::onBuild(void*)
	{
		...
		if (pDelete)
		{
			Window::Message message(this, wxT("Constraint Generator"),
				wxT("Constraint Generator"),
				wxT("Warning :  current study includes the result of previous generations. To remove all such constraints (named  \"@UTO-\" ) , use \"Reset\" before \"Run\"")
			);
			message.add(Window::Message::btnOk);
			message.add(Window::Message::btnCancel, true);
			if (message.showModal() == Window::Message::btnCancel)
			...
			
		{
			...
			{
				logs.error() << "Impossible to create a temporary file which would contain the required informations for the constraint generator";
			}
		}
	...
	void ConstraintsBuilderWizard::gridAppend(wxFlexGridSizer& sizer, const wxString& title, const wxString& key, wxWindow* value, bool bold)
	{
		if (title.empty())
		{
		...
		{
			auto* t = Antares::Component::CreateLabel(this, wxString(wxT("   ")) << title << wxT("       "), true);
			t->Enable(false);
	...
	void ConstraintsBuilderWizard::updateEndHourLabel(wxEvent& evt)
	{
		...
		{
			String::Vector split;
			Data::Study::Current::Get()->calendar.text.hours[num - 1].split(split, "-");			auto labelText = wxString("(") << wxStringFromUTF8(split[1]) << ")";
