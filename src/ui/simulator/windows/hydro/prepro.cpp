...
PreproHydrocluster::PreproHydrocluster(wxWindow* parent, Toolbox::InputSelector::HydroclusterCluster* notifier) :
 Component::Panel(parent),
 ...
 pIntermonthlyCorrelation(nullptr)
{
    OnStudyClosed.connect(this, &PreproHydrocluster::onStudyClosed);
    if (notifier)
        notifier->onClusterChanged.connect(this, &PreproHydrocluster::onClusterChanged);
}

void PreproHydrocluster::createComponents()
{
    if (pComponentsAreReady)
        return;
    pComponentsAreReady = true;
    ...
    ssGrids->Add(new Component::Datagrid::Component(
                   pSupport,
                   new Component::Datagrid::Renderer::HydroPreproHydroclusterCluster(this, pInputHydroclusterClusterSelector),
                   wxT("Overall monthly hydro Energy (ROR + Storage)")),
                 4,
                 wxALL | wxEXPAND,
                 5);
    ...
    ssGrids->Add(new Component::Datagrid::Component(
                   pSupport,
                   new Component::Datagrid::Renderer::InflowPatternHydroclusterCluster(this, pInputHydroclusterClusterSelector),
                   wxT("Inflow Pattern")),
                 2,
                 wxALL | wxEXPAND,
                 5);
    ...
    pIntermonthlyCorrelation->Connect(
      pIntermonthlyCorrelation->GetId(),
      wxEVT_COMMAND_TEXT_UPDATED,
      wxCommandEventHandler(PreproHydrocluster::onIntermonthlyCorrelationChanged),
      nullptr,
      this);
}

PreproHydrocluster::~PreproHydrocluster()
{
    destroyBoundEvents();
    // destroy all children as soon as possible to prevent against corrupt vtable
    DestroyChildren();
...
void PreproHydrocluster::onClusterChanged(Data::HydroclusterCluster* hydroclusterCluster)
{
    // pArea = area;
    ...
    if(pHydroclusterCluster and pHydroclusterCluster->prepro)    
    {
        ...
        pIntermonthlyCorrelation->ChangeValue(wxString()
                                              << pHydroclusterCluster->prepro->intermonthlyCorrelation);
    }
    else
    {
        if (pComponentsAreReady)
...
void PreproHydrocluster::onIntermonthlyCorrelationChanged(wxCommandEvent& evt)
{
    if (pHydroclusterCluster)
    {
        ...
        double d;
        evt.GetString().ToDouble(&d);
        if (not Math::Equals(d, pHydroclusterCluster->prepro->intermonthlyCorrelation))
        {
            if (d < 0.)
            {
                d = 0.;
            ...
            pHydroclusterCluster->prepro->intermonthlyCorrelation = d;
            MarkTheStudyAsModified();
        }
    }
}

void PreproHydrocluster::onStudyClosed()
{
    pHydroclusterCluster = nullptr;
