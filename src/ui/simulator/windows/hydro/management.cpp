...
namespace Hydro
{
	...
	void Management::createComponents()
	{
		...
		{
			pGrid->Add(Component::CreateLabel(pSupport, wxT("Initialize reservoir level on ")), 0, right);
		...
		pInterdailyBreakdown->Connect(pInterdailyBreakdown->GetId(), wxEVT_COMMAND_TEXT_UPDATED,
			wxCommandEventHandler(Management::onInterdailyBreakdownChanged), nullptr, this);
		pIntradailyModulation->Connect(pIntradailyModulation->GetId(), wxEVT_COMMAND_TEXT_UPDATED,
			wxCommandEventHandler(Management::onIntradailyModulationChanged), nullptr, this);

		pIntermonthlyBreakdown->Connect(pIntermonthlyBreakdown->GetId(), wxEVT_COMMAND_TEXT_UPDATED,
			wxCommandEventHandler(Management::onIntermonthlyBreakdownChanged), nullptr, this);

		pReservoirCapacity->Connect(pReservoirCapacity->GetId(), wxEVT_COMMAND_TEXT_UPDATED,
			wxCommandEventHandler(Management::onReservoirCapacityChanged), nullptr, this);

		pLeewayUpperBound->Connect(pLeewayUpperBound->GetId(), wxEVT_COMMAND_TEXT_UPDATED,
			wxCommandEventHandler(Management::onLeewayUpperBoundChanged), nullptr, this);
		pLeewayLowerBound->Connect(pLeewayLowerBound->GetId(), wxEVT_COMMAND_TEXT_UPDATED,
			wxCommandEventHandler(Management::onLeewayLowBoundChanged), nullptr, this);

		pPumpingEfficiency->Connect(pPumpingEfficiency->GetId(), wxEVT_COMMAND_TEXT_UPDATED,
			wxCommandEventHandler(Management::onPumpingEfficiencyChanged), nullptr, this);
	}
	...
	void Management::onAreaChanged(Data::Area* area)
	{
		...
		if (area and area->hydro.prepro)
		{
			...
			{
				pInitializeReservoirLevelDate->caption(Antares::Date::MonthToString(area->hydro.initializeReservoirLevelDate, 0));
			}
			else
			{
			...
			if ((area->hydro.useHeuristicTarget && area->hydro.useLeeway) or not area->hydro.useHeuristicTarget)
			{
	...
	void Management::onToggleReservoirManagement(Component::Button&, wxMenu& menu, void*)
	{
		...
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onEnableReserveManagement), nullptr, this);
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("No"), "images/16x16/light_orange.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onDisableReserveManagement), nullptr, this);
	}
	...
	void Management::onToggleFollowLoad(Component::Button&, wxMenu& menu, void*)
	{
		...
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Follow load modulations"),
			"images/16x16/light_green.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onFollowingLoadModulations), nullptr, this);
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("No"), "images/16x16/light_orange.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onUnfollowingLoadModulations), nullptr, this);
	}
	...
	void Management::onToggleUseWaterValue(Component::Button&, wxMenu& menu, void*)
	{
		...
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Use water value"),
			"images/16x16/light_green.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onEnableUseWaterValue), nullptr, this);
		if (pArea->hydro.useHeuristicTarget)
		{
			it = Menu::CreateItem(&menu, wxID_ANY, wxT("No"), "images/16x16/light_orange.png", wxEmptyString);
			menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
				wxCommandEventHandler(Management::onDisableUseWaterValue), nullptr, this);
		}
	}
	...
	void Management::onToggleHardBoundsOnRuleCurves(Component::Button&, wxMenu& menu, void*)
	{
		...
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Hard bounds on rule curves"),
			"images/16x16/light_green.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onEnableHardBoundsOnRuleCurves), nullptr, this);
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("No"), "images/16x16/light_orange.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onDisableHardBoundsOnRuleCurves), nullptr, this);
	}
	...
	void Management::onToggleUseHeuristicTarget(Component::Button&, wxMenu& menu, void*)
	{
		...
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Use heuristic target"),
		 	"images/16x16/light_green.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onEnableUseHeuristicTarget), nullptr, this);

		if (pArea->hydro.useWaterValue)
		{
			it = Menu::CreateItem(&menu, wxID_ANY, wxT("No"), "images/16x16/light_orange.png", wxEmptyString);
			menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
				wxCommandEventHandler(Management::onDisableUseHeuristicTarget), nullptr, this);
		}
	}
	...
	void Management::onToggleUseLeeway(Component::Button&, wxMenu& menu, void*)
	{
		...
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Use Leeway"),
			"images/16x16/light_green.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onEnableUseLeeway), nullptr, this);
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("No"), "images/16x16/light_orange.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onDisableUseLeeway), nullptr, this);
	}
	...
	void Management::onToggleInitializeReservoirLevelDate(Component::Button&, wxMenu& menu, void*)
	{
		...
		for (int i = 0; i < 12; i++)
		{
			it = Menu::CreateItem(&menu, i, Antares::Date::MonthToString(i, 0),
				NULL, wxEmptyString);
			menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
				wxCommandEventHandler(Management::onChangingInitializeReservoirLevelDate), nullptr, this);
		}
	}
	...
	void Management::onTogglePowerToLevel(Component::Button&, wxMenu& menu, void*)
	{
		...
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Power to Level Modulations"),
			"images/16x16/light_green.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onEnablePowerToLevel), nullptr, this);
		it = Menu::CreateItem(&menu, wxID_ANY, wxT("No"), "images/16x16/light_orange.png", wxEmptyString);
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(Management::onDisablePowerToLevel), nullptr, this);
	}
