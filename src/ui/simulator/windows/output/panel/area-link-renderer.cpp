...
namespace OutputViewerData
{
	...
	AreaLinkRenderer::AreaLinkRenderer(OutputViewerComponent* component, const Antares::Matrix<CellType>* matrix) :
	...
	wxColour AreaLinkRenderer::cellTextColor(int x, int y) const
	{
		...
		if ((uint) x < pMatrix->width && (uint) y < pMatrix->height)
		{
			...
			if (not text.empty())
			{
				switch (text[0])
				{
					case '0': return wxColour(164, 167, 183);
					case '-': return wxColour(82, 84, 100);
				}
			}
		}
