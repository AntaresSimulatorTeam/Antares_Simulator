...
namespace OutputViewer
{
	...
	void SpotlightProvider::search(Spotlight::IItem::Vector& out, const Spotlight::SearchToken::Vector& tokens, const Yuni::String& text)
	{
		assert(pLayer);
	...
	void SpotlightProviderGlobalSelection::appendSetName(Spotlight::IItem::Vector& out, const String& name,
		const char* grp)
	{
		auto* item = new ItemAreaLink(stArea, name);
		item->group(grp);
	...
	void SpotlightProviderGlobalSelection::appendAreaName(Spotlight::IItem::Vector& out, const String& name,
		const char* grp)
	{
		auto* item = new ItemAreaLink(stArea, name);
		item->group(grp);
	...
	void SpotlightProviderGlobalSelection::appendLinkName(Spotlight::IItem::Vector& out, const String& name,
		const char* grp)
	{
		auto* item = new ItemAreaLink(stLink, name);
		item->group(grp);
	...
	void SpotlightProviderGlobalSelection::search(Spotlight::IItem::Vector& out, const Spotlight::SearchToken::Vector& tokens, const Yuni::String& text)
	{
		auto& study = *Data::Study::Current::Get();
		...
		if (text == "Unknown")
		{
			...
			for (; i != iend; ++i)
			{
				...
				if (arealink && arealink->selectionType == stArea)
				{
					...
					{
						if (arealink->id == itArea->first)//this area is in the study (it is visible at least on layer 0)
						{
							foundInALayer = true;
							break;
						}
				...
				if (arealink && arealink->selectionType == stLink)
				{
					...
					{
						wxString outputStyledName = (wxString() << wxStringFromUTF8((*itLink)->from->name) << wxT(" - ") << wxStringFromUTF8((*itLink)->with->name)).MakeLower();
						if (arealink->id == std::string(outputStyledName.mb_str()))//this link is in the study (it is visible at least on layer 0)
						{
							foundInALayer = true;
							break;
						}
		...
		**/
		...
		for (; i != iend; ++i)
		{
			...
			if (arealink && arealink->selectionType == stArea)
			{
				...
				for (auto itArea = study.areas.begin(); itArea != aEnd; ++itArea)
				{
					if (arealink->id == itArea->first && itArea->second->isVisibleOnLayer(layerID))//this area is in the study (it is visible at least on layer 0)
					{
						layerFilteredItems.push_back(*i);
					}
				}
			...
			if (arealink && arealink->selectionType == stLink)
			{
				...
				{
					wxString outputStyledName = (wxString() << wxStringFromUTF8((*itLink)->from->name) << wxT(" - ") << wxStringFromUTF8((*itLink)->with->name)).MakeLower();
					if (arealink->id == std::string(outputStyledName.mb_str()) && (*itLink)->isVisibleOnLayer(layerID))//this link is in the study (it is visible at least on layer 0)
					{
						layerFilteredItems.push_back(*i);
					}
				}
	...
	void SpotlightProviderGlobalSelection::search(Spotlight::IItem::Vector& out, const Spotlight::SearchToken::Vector& tokens, Spotlight::IItem::Vector& in)
	{
		if (tokens.empty())
		{
	...
	void SpotlightProviderMCAll::search(Spotlight::IItem::Vector& out, const Spotlight::SearchToken::Vector& tokens, const Yuni::String& text)
	{
