...
void Component::updateFromExternalEvent(const Data::Output::List& list,
                                        const Data::Output::Ptr selection)
{
...
void Component::createAllControlsIfNeeded()
{
...

    {
        ...
        auto* content = new Component::Panel(pSplitter);
        {
            ...
            // Tabs
            {
                // sidebar collapse / expand
                {
                    ...
                    btn = new Button(content,
                                     wxEmptyString,
                                     "images/16x16/sidebar_collapse.png",
                                     this,
            ...
            // mc-all
            {
                ...
                btn = new Button(grp->subpanel,
                                 wxEmptyString,
                                 "images/16x16/calendar_previous.png",
                                 this,
                                 &Component::decrementYearByYear);
                btn->pushedColor(grp);
                btn->SetToolTip(wxT("Previous individual year"));
                ybySizer->Add(btn, 0, wxALL | wxEXPAND);
                btn = new Button(grp->subpanel,
                                 wxEmptyString,
                                 "images/16x16/calendar_next.png",
                                 this,
                                 &Component::incrementYearByYear);
                btn->pushedColor(grp);
        ...
        // Current node / link
        {
            ...
            if (!pParentWindow)
            {
                auto* btn = new Button(grp->subpanel,
                                       wxT("Back to input"),
                                       "images/16x16/close.png",
                                       this,
                                       &Component::backToInputData);
                hz->Add(btn, 0, wxALL | wxEXPAND);
...
void Component::onDropDownDetachMenu(Button&, wxMenu& menu, void* userdata)
{
    ...
    if (!uiinfo->detached)
    {
        ...
        {
        case stArea:
            caption = wxT("Pin the area");
            break;
        case stLink:
            caption = wxT("Pin the link");
            break;
        default:
            return; // Uh ?
        }
