...
namespace Window
{
	...
	void CalendarSelect::evtOnShow(wxShowEvent& evt)
	{
		if (evt.IsShown()) // show can hide !!!
		{
			...
			if (not pComponentCreated)
			{
				...
				{
					auto* btn = new Component::Button(standardview, wxT("(invalid)"), "images/16x16/calendar_month.png");
					btn->menu(true);
				...
				// Panel with buttons
				{
					...
					// Close button
					{
						auto* btn = Antares::Component::CreateButton(panel, wxT("Cancel"), this, &CalendarSelect::onClose);
						sizerBar->Add(btn, 0, wxFIXED_MINSIZE|wxALIGN_CENTRE_VERTICAL|wxALL);
						sizerBar->Add(5, 2);

						if (not allowQuickSelect)
						{
							btn = Antares::Component::CreateButton(panel, wxT("  Apply the new date  "), this, &CalendarSelect::onProceed);
							pApplyButton = btn;
							sizerBar->Add(btn, 0, wxFIXED_MINSIZE|wxALIGN_CENTRE_VERTICAL|wxALL);
						}
				...
				p.SetHeight(CalendarViewStandard::recommendedWindowHeight
					+ 22 + 7 + 7 /*panel ok/cancel*/ + 12/*selection button*/);
				SetSize(p);
	...
	void CalendarSelect::onPopupMenu(Component::Button&, wxMenu& menu, void*)
	{
		...
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(CalendarSelect::onSelectWholeYear), nullptr, this);

		it = Menu::CreateItem(&menu, wxID_ANY, wxT("Clear the selection"));
		menu.Connect(it->GetId(), wxEVT_COMMAND_MENU_SELECTED,
			wxCommandEventHandler(CalendarSelect::onSelectNone), nullptr, this);
	}
