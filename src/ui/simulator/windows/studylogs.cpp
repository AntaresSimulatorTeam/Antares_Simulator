...
namespace Window
{
			...
			for (auto i = info.file_begin(); i != info.file_end(); ++i)
			{
				...
				if (regex.Matches(name))
				{
					...
					// Date
					{
						...
						s << pS[0] << pS[1] << pS[2] << pS[3] << wxT('/')
							<< pS[4] << pS[5] << wxT('/') << pS[6] << pS[7];
					}
					// Separator
					s << wxT(", ");
					// Hour
	...
	protected:
		...
		bool readLogLine(StringT& line)
		{
			...
			if (entry->verbosityType == LogEntry::vtInfo)
			{
				...
				{
					if (message.startsWith("Years from") or message.startsWith("Year ") or message.startsWith("Summary"))
						entry->highlight = true;
				}
			}
	...
	StudyLogs::StudyLogs(wxFrame* parent) :
		Component::Frame::WxLocalFrame(parent, wxID_ANY, wxT("Logs"), wxDefaultPosition, wxSize(1000, 600),
		...
		pShowAllLogsFiles(false)
	{
...

		{
			...
			// Refresh all available files
			{
				...
				Component::Button* btnRefresh = new Component::Button(panelAllFiles,
						wxT("Refresh the file list"), "images/16x16/refresh.png",
						this, &StudyLogs::onButtonRefreshFileList);
			...
			// List of all available log files
			{
				...
				{
					wxBoxSizer* hz = new wxBoxSizer(wxHORIZONTAL);
					wxCheckBox* allLogFiles = new wxCheckBox(panelAllFiles, wxID_ANY, wxT(" Show all log info  (advanced) "),
					...
					allLogFiles->Connect(allLogFiles->GetId(), wxEVT_COMMAND_CHECKBOX_CLICKED,
						wxCommandEventHandler(StudyLogs::onChangeShowAllAdvanced), nullptr, this);
				}
			}
			...
			pBtnLogFilenameRefresh = new Component::Button(infopanel,
				wxEmptyString, "images/16x16/refresh_table.png",
				this, &StudyLogs::onReloadLogFile);
			...
			pLogDisplay = new Component::Datagrid::Component(notebook, pRendererEntries, wxEmptyString,
			...
			pLogDisplayErrors = new Component::Datagrid::Component(notebook, pRendererEntriesErrors, wxEmptyString,
				false, true, true, true);
	...
	void StudyLogs::reloadFromData(LogEntryContainer::Ptr entries, LogEntryContainer::Ptr errors)
	{
		...
		pBtnLogFilenameRefresh->caption(wxString(wxT("Reload  ")) << wxStringFromUTF8(entries->barefilename));
