...
namespace Window
{
	...
	SaveAs::SaveAs(wxFrame* parent) :
		wxDialog(parent, wxID_ANY, wxT("Save As"), wxDefaultPosition, wxDefaultSize,
	...
	void SaveAs::internalCreateComponents()
	{
		...
		propCaption(wxT("In the directory"));
		{
			...
			btnBrowse->Connect(btnBrowse->GetId(), wxEVT_COMMAND_BUTTON_CLICKED,
				wxCommandEventHandler(SaveAs::onBrowse), nullptr, this);


			pFolder = new wxTextCtrl(pnl, wxID_ANY, wxEmptyString,
		...
		if (not pStudy->folder.empty())
		{
			...
			pCopyOutput = (wxCheckBox*)propEdit(new wxCheckBox(pParentProperties, wxID_ANY,
				wxT("Copy the output folder")));
			pCopyOutput->SetValue(false);

			propCaption(wxEmptyString);
			pCopyUserData = (wxCheckBox*)propEdit(new wxCheckBox(pParentProperties, wxID_ANY,
				wxT("Copy the user data folder")));
			pCopyUserData->SetValue(true);

			propCaption(wxEmptyString);
			pCopyLogs = (wxCheckBox*)propEdit(new wxCheckBox(pParentProperties, wxID_ANY,
				wxT("Copy the log files")));
	...
	{
		wxDirDialog dialog(this, _T("Browse"),
			pFolder->GetValue(), wxDD_DEFAULT_STYLE & ~wxDD_DIR_MUST_EXIST);
		...
		if (CheckIfInsideAStudyFolder(path, true))
		{
			...
			if (Data::Study::TitleFromStudyFolder(path, title))
			{
				...
				Window::Message message(&mainFrm, wxT("Save As"),
					...
					wxString()
					<< wxT("The selected folder is an existing study. Do you really want to overwrite it ?")
					<< wxT("\n\ntitle : ") << wxStringFromUTF8(title)
					<< wxT("\nlocation : ") << wxStringFromUTF8(path)
				);
