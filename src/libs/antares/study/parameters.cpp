...
namespace Data
{
	...
	const char* StudyModeToCString(StudyMode mode)
	{
		switch (mode)
		{
			case stdmEconomy:       return "Economy";
			case stdmAdequacy:      return "Adequacy";
			case stdmAdequacyDraft: return "draft";
			case stdmMax:
			case stdmUnknown: return "Unknown";
		}
		return "Unknown";
	}
	...
	static bool ParametersInternalLoadProperty(Parameters& params, const String& key, const String& value, const String& rawvalue, uint version)
	{
		char c = key.first();
		if (c < 'a' || c > 'z')
			return false;
	...
	void Parameters::fixBadValues()
	{
		...
		{
			adequacyBlockSize = 100;
			logs.warning() << "The block size for the adequacy algorithm is invalid (100 <= blocksize <= 100000). Reset to " << adequacyBlockSize << '.';
		}
		...
		else
		{
			...
			if (nbYears > maximumMCYears)
			{
				...
				logs.error() << "The number of MC years is too high (>"
					<< (uint) maximumMCYears << ")";
		...
		if (timeSeriesLoad & timeSeriesToRefresh && 0 == refreshIntervalLoad)
		{
			...
			logs.error() << "The load time-series must be refreshed but the interval is equal to 0. Auto-Reset to a safe value (1).";
		}
		if (timeSeriesSolar & timeSeriesToRefresh && 0 == refreshIntervalSolar)
		{
			refreshIntervalSolar = 1;
			logs.error() << "The solar time-series must be refreshed but the interval is equal to 0. Auto-Reset to a safe value (1).";
		}
		if (timeSeriesHydro & timeSeriesToRefresh && 0 == refreshIntervalHydro)
		{
			refreshIntervalHydro = 1;
			logs.error() << "The hydro time-series must be refreshed but the interval is equal to 0. Auto-Reset to a safe value (1).";
		}
		if (timeSeriesWind & timeSeriesToRefresh && 0 == refreshIntervalWind)
		{
			refreshIntervalWind = 1;
			logs.error() << "The wind time-series must be refreshed but the interval is equal to 0. Auto-Reset to a safe value (1).";
		}
		if (timeSeriesThermal & timeSeriesToRefresh && 0 == refreshIntervalThermal)
		{
			refreshIntervalThermal = 1;
			logs.error() << "The thermal time-series must be refreshed but the interval is equal to 0. Auto-Reset to a safe value (1).";
		}
	}
	...
	void Parameters::prepareForSimulation(const StudyLoadOptions& options)
	{
		...
		else
		{
			...
			if (nbYearsDifferentFrom1 != 0)
            {
                ...
                std::stringstream ss;
                copy( maximumWeightYearsList.begin(), maximumWeightYearsList.end(), std::ostream_iterator<float>(ss, ","));
                std::string s = ss.str();
		...
		if (interModal == timeSeriesLoad || interModal == timeSeriesSolar || interModal == timeSeriesWind
			|| interModal == timeSeriesHydro || interModal == timeSeriesThermal)
		{
	...
	void Parameters::saveToINI(IniFile& ini) const
	{
		...
		// Optimization
		{
			...
			switch (simplexOptimizationRange)
			{
				case sorDay:  section->add("simplex-range", "day");break;
				case sorWeek: section->add("simplex-range", "week");break;
				case sorUnknown: break;
			}
			// Optimization preferences
			switch (transmissionCapacities)
			{
				case tncEnabled:  section->add("transmission-capacities", "true");break;
		...
		// Advanced parameters
		{
			...
			ParametersSaveTimeSeries(section, "accuracy-on-correlation", timeSeriesAccuracyOnCorrelation);
			// Adequacy Block size (adequacy draft)
	...
	bool Parameters::loadFromFile(const AnyString& filename, uint version, const StudyLoadOptions& options)
	{
