...
unsigned
BindingConstraintGroupRepository::size() const
{
    return groups_.size();
}

bool
BindingConstraintGroupRepository::buildFrom(const BindingConstraintsRepository& repository)
{
...
bool
BindingConstraintGroupRepository::timeSeriesWidthConsistentInGroups() const
{
            ...
            [](const auto& group)
            {
                ...
                bool isConsistent = std::all_of(
                        ...
                        [&width](const std::shared_ptr<BindingConstraint>& bc)
                        {
                            ...
                            {
                                logs.error()
                                        << "Inconsistent time series width for constraint of the "
                                           "same "
...
void
BindingConstraintGroupRepository::resizeAllTimeseriesNumbers(unsigned int nb_years)
{
...
BindingConstraintGroupRepository::operator[](const std::string& name) const
{
...
BindingConstraintGroupRepository::iterator
BindingConstraintGroupRepository::begin()
{
...
BindingConstraintGroupRepository::iterator
BindingConstraintGroupRepository::end()
{
    return groups_.end();
}

BindingConstraintGroupRepository::const_iterator
BindingConstraintGroupRepository::end() const
{
    return groups_.end();
}

BindingConstraintGroup*
BindingConstraintGroupRepository::add(const std::string& name)
{
    return groups_.emplace_back(std::make_unique<BindingConstraintGroup>(name)).get();
}

void
BindingConstraintGroupRepository::clear()
{
    groups_.clear();
}
} // namespace Antares::Data
