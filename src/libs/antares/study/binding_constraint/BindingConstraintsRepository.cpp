...
std::vector<std::shared_ptr<BindingConstraint>> BindingConstraintsRepository::activeContraints()
  const
{
    ...
    else
    {
        ...
        std::copy_if(
          constraints_.begin(), constraints_.end(), std::back_inserter(out), [](const auto& bc) {
              return bc->isActive();
          });
