...
namespace Data
{
	...
	namespace // anonymous
	{
		...
		static void
		PreflightVersion20_area(PathList& e, PathList& p, const Area* area, StringT& buffer)
		{
		...
		template<class StringT>
		static void
		PreflightVersion20_interco(PathList& p, const Area* area, StringT& buffer)
		{
			...
			for (auto i = area->links.begin(); i != end; ++i)
			{
				...
				buffer.clear() << "input" << SEP << "links"
					<< SEP << link.from->id
					<< SEP << link.with->id << ".txt";
				p.add(buffer);
			}
		}
