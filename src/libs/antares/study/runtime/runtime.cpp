...
namespace Data
{
			...
			if (area.hydro.prepro)
			{
				...
				for (uint i = 0; i != 12; ++i)
				{
					if (!Math::Zero(e[i]))
					{
						...
						// asserts
						assert(!Math::NaN(nE) && "Hydro: NaN value detected for hydro prepro expectation");
						assert(!Math::NaN(nS) && "Hydro: NaN value detected for hydro prepro expectation");
	...
	static void CopyBCData(BindingConstraintRTI& rti, const BindingConstraint& b)
	{
		switch (C)
		{
			case BindingConstraint::columnInferior : rti.operatorType = '<'; break;
			case BindingConstraint::columnSuperior : rti.operatorType = '>'; break;
			case BindingConstraint::columnEquality : rti.operatorType = '='; break;
			case BindingConstraint::columnMax      : rti.operatorType = '?'; break;
		}
	...
	void StudyRuntimeInfos::initializeRangeLimits(const Study& study, StudyRangeLimits& limits)
	{
		...
		else
		{
			if (stdmAdequacyDraft != study.parameters.mode)
			{
				...
				if ((b - a + 1) % 168)
				{
					...
					logs.info() << "    Time interval that has been requested: "
						<< (1 + a) << ".." << (1 + b);
					// Reducing
		...
		logs.info() << "Calendar: hours:" << (1 + a) << ".." << (1 + b)
			...
			<< ", weeks:"  << (1+limits.week[rangeBegin])  << ".." << (1+limits.week[rangeEnd])
			<< ", months:" << (1+limits.month[rangeBegin]) << ".." << (1+limits.month[rangeEnd])
			<< ", years:"  << (1+limits.year[rangeBegin])  << ".." << (1+limits.year[rangeEnd]);
		...
		{
			simulationDaysPerMonth[(uint)ca.month] = study.calendar.months[(uint)ca.month].days - ca.dayMonth;
	...
	bool StudyRuntimeInfos::loadFromStudy(Study& study)
	{
		...
		logs.info() << "     filtering:" << (gd.filtering ? "true" : "false");
	...
	Yuni::uint64 StudyRuntimeInfosMemoryUsage(StudyRuntimeInfos* r)
	{
		if (r)
		{
			return sizeof(StudyRuntimeInfos)
				+ sizeof(AreaLink*) * r->interconnectionsCount
