...
namespace Data
{
	...
	inline uint Equipments<ItemT>::remove(const ItemType* element)
	{
		return removeIf([&] (const ItemType& rhs) {
			return element == rhs;
		});
	}
	...
	{
		uint count = 0;
		pItems.erase(std::remove_if(pItems.begin(), p.end(), [&] (const ItemType* rhs) -> bool
		{
			if (predicate(*rhs))
			{
				++count;
	...
	bool Equipments<ItemT>::findNonExistingName(StringT& out, const AnyString& prefix, bool emptybefore) const
	{
		if (emptybefore)
			out.clear();
