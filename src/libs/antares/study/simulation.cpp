...
bool SimulationComments::loadFromFolder(const StudyLoadOptions& options)
{
    ...
    {
        pStudy.get().buffer.clear() << pStudy.get().folderSettings << SEP << "comments.txt";
        if (IO::errNone != IO::File::LoadFromFile(comments, pStudy.get().buffer))
            logs.warning() << pStudy.get().buffer << ": Impossible to read the file";
    }
    return true;
}
