		...
		if (ctx.area && ctx.extStudy && ctx.area->ui)
		{
			...
			if (source)
			{
				if (source != ctx.area)
				{
					...
					ctx.area->spreadUnsuppliedEnergyCost = source->spreadUnsuppliedEnergyCost; // MBO 23/05/2014 - #23
					ctx.area->spreadSpilledEnergyCost = source->spreadSpilledEnergyCost; // MBO 23/05/2014 - #23
				}
				return true;
			}
		}
