...
namespace Data
{
	...
	template<class T>
	inline Sets<T>::Sets()
		:pByIndex(NULL), pNameByIndex(NULL), pModified(false)
	{}


	template<class T>
	inline Sets<T>::Sets(const Sets& rhs)
		:pMap(rhs.pMap), pOptions(rhs.pOptions),
		pByIndex(NULL),
		pNameByIndex(NULL),
		pModified(false)
	{
	...
	void Sets<T>::rebuildFromRules(const IDType& id, HandlerT& handler)
	{
		...
		logs.debug() << "  > set :: " << opts.caption << ": applying " << opts.rules.size() << " rules, got "
			<< opts.resultSize << " items";
	}
