...
namespace Data
{
	...
	bool BindingConstraint::loadFromEnv(BindingConstraint::EnvForLoading& env)
	{
		...
		if (!ret or !pName or !pID or pOperator == opUnknown or pType == typeUnknown)
		{
			...
			if (!pName)
				logs.error() << env.iniFilename << ": in [" << env.section->name << "]: Invalid binding constraint name";
			if (!pID)
				logs.error() << env.iniFilename << ": in [" << env.section->name << "]: Invalid binding constraint id";
			if (pType == typeUnknown)
				logs.error() << env.iniFilename << ": in [" << env.section->name << "]: Invalid type [hourly,daily,weekly]";
			if (pOperator == opUnknown)
				logs.error() << env.iniFilename << ": in [" << env.section->name << "]: Invalid operator [less,greater,equal,both]";
		...
		if (env.version > 310)
		{
			...
			if (pValues.loadFromCSVFile(env.buffer, columnMax, (pType == typeHourly) ? 8784 : 366,
				Matrix<>::optImmediate | Matrix<>::optFixedSize, &env.matrixBuffer))
			{
				if (pComments.empty())
				{
		...
		else // studies <= 3.1
		{
			...
			::Matrix<> m;
			if (m.loadFromCSVFile(env.buffer, 1, 8760, Matrix<>::optImmediate | Matrix<>::optFixedSize, &env.matrixBuffer))
			{
				// Copying values
	...
	bool BindConstList::loadFromFolder(Study& study, const StudyLoadOptions& options, const AnyString& folder)
	{
		// Log entries
	...
	void BindConstList::mutateWeeklyConstraintsIntoDailyOnes()
	{
		each([&] (BindingConstraint& constraint)
		{
			...
			{
				logs.info() << "  The type of the constraint '" << constraint.name() << "' is now 'daily'";
	...
	void BindConstList::reverseWeightSign(const AreaLink* lnk)
	{
		each([&] (BindingConstraint& constraint)
		{
			constraint.reverseWeightSign(lnk);
		});
	}
	...
	void BindingConstraint::clearAndReset(const AnyString& name, BindingConstraint::Type newType,
