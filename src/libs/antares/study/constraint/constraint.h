...
namespace Data
{
	...
	public:
		...
		{
		public:
			EnvForLoading(AreaList& l, uint v) :
				areaList(l), version(v)
			{}
			//! INI file
	...
	public:
		...
		** \brief Copy all weights from another constraint
		*/
		void copyWeights(const Study& study, const BindingConstraint& rhs, bool emptyBefore,
		...
		** \brief Copy all offsets from another constraint
		*/
		void copyOffsets(const Study& study, const BindingConstraint& rhs, bool emptyBefore,
			Yuni::Bind<void (AreaName&, const AreaName&)>& translate);
	...
	public:
		...
		*/
		template<class PredicateT> void eachEnabled(const PredicateT& predicate);
		/*!
		** \brief Iterate through all enabled constraints (const)
		*/
		template<class PredicateT> void eachEnabled(const PredicateT& predicate) const;
