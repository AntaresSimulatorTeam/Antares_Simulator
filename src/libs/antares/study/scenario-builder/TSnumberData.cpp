...
bool hydroclusterTSNumberData::reset(const Study& study)
{
    const uint nbYears = study.parameters.nbYears;
    assert(pArea != nullptr);

    // If an area is available, it can only be an overlay for hydrocluster timeseries
    // WARNING: The total number of clusters may vary if used from the
    //   solver or not.
    // WARNING: At this point in time, the variable pArea->hydrocluster.clusterCount()
    //   might not be valid (because not really initialized yet)
    uint clusterCount = (study.usedByTheSolver)
                          ? pArea->hydrocluster.list.size()
                          : pArea->hydrocluster.list.size();
...
void hydroclusterTSNumberData::saveToINIFile(const Study& /* study */, Yuni::IO::File::Stream& file) const
{
    // Prefix
    CString<512, false> prefix;
    prefix += get_prefix();
    ...
    {
        assert(pTSNumberRules.width == pArea->hydrocluster.list.size());
    }
#endif

    // Foreach hydrocluster cluster...
    for (uint index = 0; index != pTSNumberRules.width; ++index)
    {
        // Foreach year ...
        for (uint y = 0; y != pTSNumberRules.height; ++y)
        {
            const uint val = get(pArea->hydrocluster.list.byIndex[index], y);
            // Equals to zero means 'auto', which is the default mode
            if (!val)
                continue;
            file << prefix << pArea->id << "," << y << ','
                 << pArea->hydrocluster.list.byIndex[index]->id() << " = " << val << '\n';
        }
    }
}

void hydroclusterTSNumberData::set(const Antares::Data::HydroclusterCluster* cluster,
                              const uint year,
                              uint value)
{
...
bool hydroclusterTSNumberData::apply(Study& study)
{
    bool ret = true;
    CString<512, false> logprefix;
    // Errors
    ...
    auto clusterCount = (uint)area.hydrocluster.clusterCount();

    const uint tsGenCountHydrocluster = get_tsGenCount(study);

    for (uint clusterIndex = 0; clusterIndex != clusterCount; ++clusterIndex)
    {
        auto& cluster = *(area.hydrocluster.clusters[clusterIndex]);
        // alias to the current column
...
uint hydroclusterTSNumberData::get_tsGenCount(const Study& study) const
{
    // General data
    auto& parameters = study.parameters;

    bool tsGenHydrocluster = (0 != (parameters.timeSeriesToGenerate & timeSeriesHydrocluster));
    return tsGenHydrocluster ? parameters.nbTimeSeriesHydrocluster : 0u;
}
