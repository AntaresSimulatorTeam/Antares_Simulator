...
Progression::Progression(Progression&& rhs) noexcept
{
    std::lock_guard guard(rhs.pProgressMeter.mutex);
    pStarted = std::exchange(pStarted, false);
}

Progression& Progression::operator=(Progression&& rhs) noexcept
{
    if (this != &rhs) {
        std::lock_guard guard(rhs.pProgressMeter.mutex);
        pStarted = std::exchange(pStarted, false);
