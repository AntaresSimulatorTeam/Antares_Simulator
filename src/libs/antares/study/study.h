...
namespace Data
{
	...
	*/
	...
	public:
		...
		// CompareAreaName : to control the order of areas in a set of areas. This order can have an effect,
		// even if tiny, on the results of aggregations.
	...
	public:
		...
		** \return True if the operation succeeded, false otherwise
		*/
		bool thermalClusterRename(ThermalCluster* cluster, ThermalClusterName newName, bool force = false);
		//@}
		...
		*/
		template<int TimeSeriesT> bool storeTimeSeriesNumbers();
		//@}
		...
		*/
		template<enum TimeSeries TS> void destroyTSGeneratorData();
		...
		** - All batches (or sets) of simultaneous years have the same size (obvious if a parallel run is not required : answer is yes).
		**
		** If these conditions are not met, some error message is raised, when attempting to run the study.
		*/
		...
		*/
		FILE* createFileIntoOutputWithExtension(const YString & prefix, const YString & extension, uint numSpace) const;
		...
		** \return a FILE structure (which may be null if any error occured)
		*/
		FILE* createMPSFileIntoOutput(uint numSpace) const {return createFileIntoOutputWithExtension("problem","mps", numSpace); } ;
		//@}
		...
		** \return a FILE structure (which may be null if any error occured)
		*/
		FILE* createCriterionFileIntoOutput(uint numSpace) const {return createFileIntoOutputWithExtension("criterion","txt", numSpace); } ;
		//@}
		...
		*/
		...
		//@{
		/*!
		** \brief Check validity between Min Stable Power and capacity modulation for each thermal cluster in the all area in the study. 
	...
	public:
		...
		// This raw number of cores is possibly reduced by the smallest TS refresh span or the total number of MC years.
		// In GUI, used for RAM estimation only.
		// In solver, it is the max number of years (actually run, not skipped) a set of parallel years can contain.
