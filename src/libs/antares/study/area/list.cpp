...
namespace Data
{
	...
	namespace // anonymous
	{
		...
		static bool AreaListLoadThermalDataFromFile(AreaList& list, const Clob& filename)
		{
				...
				for (IniFile::Property* p = section->firstProperty; p; p = p->next)
				{
					...
					if (area)
					{
						...
						//if (area->thermal.unsuppliedEnergyCost < 0.)
							//logs.error() << "Thermal: `" << area->name << "`: The unsupplied energy cost must be positive";
				...
				for (IniFile::Property* p = section->firstProperty; p; p = p->next)
				{
					...
					if (area)
					{
						...
						//if (area->thermal.spilledEnergyCost < 0.)
							//logs.error() << "Thermal: `" << area->name << "`: The spilled energy cost must be positive";
			...
			// logs.warning() << '`' << filename << "`: The section `unsuppliedenergycost` can not be found";
			return true;
		}
		...
		static bool AreaListSaveToFolderSingleArea(const Area& area, Clob& buffer, const AnyString& folder)
		{
			...
			// Nodal optimization
			{
				...
				if (file.openRW(buffer))
				{
					...
					buffer << "spread-unsupplied-energy-cost = "
						<< area.spreadUnsuppliedEnergyCost << '\n';
					buffer << "spread-spilled-energy-cost = "
						<< area.spreadSpilledEnergyCost << '\n';
			...
			buffer.clear() << folder << SEP << "input" << SEP << "reserves" << SEP
				<< area.id << ".txt";
			ret = area.reserves.saveToCSVFile(buffer) and ret;

			// Fatal hors hydro
			buffer.clear() << folder << SEP << "input" << SEP << "misc-gen" << SEP
				<< "miscgen-" << area.id << ".txt";
			ret = area.miscGen.saveToCSVFile(buffer) and ret;

			// NTC
			buffer.clear() << folder << SEP << "input" << SEP << "links" << SEP
				<< area.id;
			...
			// Load
			{
				...
				{
					buffer.clear() << folder << SEP << "input" << SEP << "load" << SEP << "prepro" << SEP << area.id;
			...
			// Solar
			{
				...
				{
					buffer.clear() << folder << SEP << "input" << SEP << "solar" << SEP << "prepro" << SEP << area.id;
			...
			// Hydro
			{
				buffer.clear() << folder << SEP << "input" << SEP << "hydro" << SEP
					<< "allocation" << SEP << area.id << ".ini";
			...
			// Wind
			{
				...
				{
					buffer.clear() << folder << SEP << "input" << SEP << "wind" << SEP << "prepro" << SEP << area.id;
	...
	Area* AreaListAddFromNames(AreaList& list, const AnyString& name, const AnyString& lname, uint nbParallelYears)
	{
		if (!name or !lname)
			return nullptr;
		// Look up
	...
	bool AreaList::loadListFromFile(const AnyString& filename)
	{
		...
		switch (areas.size())
		{
			case 0:  logs.info() << "  No area found"; break;
			case 1:  logs.info() << "  1 area found"; break;
			default: logs.info() << "  " << areas.size() << " areas found";
		}
		return true;
	}
	...
	bool AreaList::saveToFolder(const AnyString& folder) const
	{
		...
		ret = IO::Directory::Create(buffer) and ret;
		buffer.clear() << folder << SEP << "input" << SEP << "hydro" << SEP << "common" << SEP << "capacity";
		ret = IO::Directory::Create(buffer) and ret;
	...
	static bool AreaListLoadFromFolderSingleArea(Study& study, AreaList* list, Area& area,
		StringT& buffer, const StudyLoadOptions& options)
	{
		// Progression
		options.progressTicks = 0;
		options.progressTickCount = area.thermal.list.size() * (options.loadOnlyNeeded ? 1 : 2) //prepro+series
			+ 1  // links
		...
		if (study.header.version < 320)
		{
			...
			// DSM
			buffer.clear() << study.folderInput << SEP << "reserves-dsm" << SEP
				<< area.id << SEP << "dsm." << study.inputExtension;
			ret = Array1DLoadFromFile(buffer.c_str(), tmp, HOURS_PER_YEAR) and ret;
			area.reserves.pasteToColumn(fhrDSM, tmp);

			// Strategic Reserves
			buffer.clear() << study.folderInput << SEP << "reserves-dsm" << SEP
				<< area.id << SEP << "reserves." << study.inputExtension;
			ret = Array1DLoadFromFile(buffer.c_str(), tmp, HOURS_PER_YEAR) and ret;
			...
			// Primary reserves
			buffer.clear() << study.folderInput << SEP << "reserves-dsm" << SEP
				<< area.id << SEP << "primaryreserves." << study.inputExtension;
			ret = Array1DLoadFromFile(buffer.c_str(), tmp, HOURS_PER_YEAR) and ret;
		...
		{
			buffer.clear() << study.folderInput << SEP << "reserves"
				<< SEP << area.id << ".txt";
			ret = area.reserves.loadFromCSVFile(buffer, fhrMax, HOURS_PER_YEAR, Matrix<>::optFixedSize) and ret;
		}
		...
		if (study.header.version <= 320)
		{
			...
			buffer.clear() << study.folderInput << SEP << "misc-gen" << SEP << area.id
				<< SEP << "fatalhorshydro." << study.inputExtension;
			if (study.header.version < 210)
			{
				Matrix<double> m;
				bool r = m.loadFromCSVFile(buffer, 5, 8760 /* hardcoded in this version */,
				...
				if (r)
				{
					...
					(void) memcpy(area.solar.series->series[0], m[1], (size_t) (m.height * sizeof(double)));
					// Keep the user posted
					LogCompatibility("'%s': Misc Gen.: The data have been converted (changes from v2.1)",
						area.id.c_str());
				}
			}
			else
			{
				...
				if (r)
				{
					...
					(void) memcpy(area.miscGen[fhhGeoThermal], m[5], (size_t) (m.height * sizeof(double)));
					...
					(void) memcpy(area.miscGen[fhhRowBalance], m[8], (size_t) (m.height * sizeof(double)));

					(void) memcpy(area.solar.series->series[0], m[1], (size_t) (m.height * sizeof(double)));
					// Keep the user posted
					LogCompatibility("'%s': Misc Gen.: The data have been converted (changes from v3.3)",
						area.id.c_str());
				}
			}
		...
		{
			// >= 330
			buffer.clear() << study.folderInput << SEP << "misc-gen" << SEP << "miscgen-" << area.id << ".txt";
			ret = area.miscGen.loadFromCSVFile(buffer, fhhMax, HOURS_PER_YEAR, Matrix<>::optFixedSize) and ret;
		}
		// Check misc gen
		{
		...
		if (JIT::usedFromGUI)
		{
			...
			buffer.clear() << study.folderInput << SEP << "areas" << SEP
				<< area.id << SEP << "ui.ini";
		...
		// Load
		{
			if (area.load.prepro) // Prepro
			{
				...
				buffer.clear() << study.folderInput << SEP << "load" << SEP << "prepro" << SEP << area.id;
			...
			if (area.load.series and (!options.loadOnlyNeeded or !area.load.prepro)) // Series
			{
				...
				ret = DataSeriesLoadLoadFromFolder(study, area.load.series, area.id, buffer.c_str()) and ret;
			}
		...
		// Solar
		{
			...
			{
				// if changes are required, please update reloadXCastData()
				buffer.clear() << study.folderInput << SEP << "solar" << SEP << "prepro" << SEP << area.id;
			...
			if (area.solar.series and (!options.loadOnlyNeeded or !area.solar.prepro)) // Series
			{
				if (study.header.version >= 330)
				{
					...
					ret = DataSeriesSolarLoadFromFolder(study, area.solar.series, area.id, buffer.c_str()) and ret;
				}
			}
		...
		{
			// Allocation
			buffer.clear() << study.folderInput << SEP << "hydro" << SEP << "allocation" << SEP << area.id << ".ini";
		...
		// Wind
		{
			...
			{
				// if changes are required, please update reloadXCastData()
				buffer.clear() << study.folderInput << SEP << "wind" << SEP << "prepro" << SEP << area.id;
			...
			if (area.wind.series and (!options.loadOnlyNeeded or !area.wind.prepro)) // Series
			{
				...
				ret = DataSeriesWindLoadFromFolder(study, area.wind.series, area.id, buffer.c_str()) and ret;
			}
		...
		// Thermal cluster list
		{
			if (not options.loadOnlyNeeded)
			{
				...
				ret = ThermalClusterListLoadPreproFromFolder(study, options, &area.thermal.list, buffer) and ret;
				buffer.clear() << study.folderInput << SEP << "thermal" << SEP << "series";
				ret = ThermalClusterListLoadDataSeriesFromFolder(study, options, &area.thermal.list, buffer, options.loadOnlyNeeded) and ret;
			}
			else
			{
	...
	bool AreaList::loadFromFolder(const StudyLoadOptions& options)
	{
		...
		// Load the list of all available areas	
		{
			...
			buffer.clear() << pStudy.folderInput << SEP << "areas" << SEP << "list." << pStudy.inputExtension;
		...
		// Thermal data, specific to areas
		{
			...
			for (auto i = areas.begin(); i != end; ++i)
			{
				...
				{
					if (options.usedByTheSolver and area.thermal.list.size() > License::Limits::thermalClusterCount)
						return false;
		...
		{
			// Progression
			options.logMessage.clear() << "Loading the area " << (++indx) << '/' << areas.size()
				<< ": " << area.name;
	...
	Area* AreaList::findFromPosition(const int x, const int y) const
	{
		if (! this->areas.empty())
		{
			...
			for (auto i = this->areas.rbegin(); i != end; ++i)
			{
				...
				if(lastArea->ui)
				if (Math::Abs(lastArea->ui->x - x) < nearestDistance && Math::Abs(lastArea->ui->y - y) < nearestDistance)
				{
	...
	void AreaListEnsureDataThermalTimeSeries(AreaList* l)
	{
		...
		l->each([&] (Data::Area& area)
		{
			ThermalClusterListEnsureDataTimeSeries(&area.thermal.list);
		});
	}
	...
	void AreaListEnsureDataThermalPrepro(AreaList* l)
	{
		...
		l->each([&] (Data::Area& area)
		{
			ThermalClusterListEnsureDataPrepro(&area.thermal.list);
		});
	}
	...
	bool AreaList::invalidate(bool reload) const
	{
		...
		each([&] (Data::Area& area)
		{
			ret = area.invalidate(reload) and ret;
		});
		return ret;
	}
	...
	void AreaList::resizeAllTimeseriesNumbers(uint n)
	{
		...
		each([&] (Data::Area& area)
		{
			area.resizeAllTimeseriesNumbers(n);
		});
	}
	...
	void AreaList::removeLoadTimeseries()
	{
		each ([&] (Data::Area& area)
		{
			area.load.series->series.reset(1, HOURS_PER_YEAR);
		});
	}
	...
	void AreaList::removeSolarTimeseries()
	{
		each ([&] (Data::Area& area)
		{
			area.solar.series->series.reset(1, HOURS_PER_YEAR);
		});
	}
	...
	void AreaList::removeWindTimeseries()
	{
		each ([&] (Data::Area& area)
		{
			area.wind.series->series.reset(1, HOURS_PER_YEAR);
		});
	}
	...
	void AreaList::removeThermalTimeseries()
	{
		each ([&] (Data::Area& area)
		{
			area.thermal.list.each([&] (Data::ThermalCluster& cluster)
			{
				cluster.series->series.reset(1, HOURS_PER_YEAR);
			});
		});
	}
