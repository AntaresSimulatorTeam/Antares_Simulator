...
namespace Data
{
	...
	uint64 Study::memoryUsage() const
	{
		return folder.capacity()
			...
			+ buffer.capacity() + dataBuffer.capacity() + bufferLoadingTS.capacity()
			// Simulation
			...
			+ preproLoadCorrelation.memoryUsage()
			+ preproSolarCorrelation.memoryUsage()
			+ preproHydroCorrelation.memoryUsage()
			+ preproWindCorrelation.memoryUsage()
	...
	std::map<std::string, uint> Study::getRawNumberCoresPerLevel()
	{
		...
		{
			case 1:
				table["min"] = 1; table["low"] = 1; table["med"] = 1; table["high"] = 1; table["max"] = 1;
				break;
			case 2:
				table["min"] = 1; table["low"] = 1; table["med"] = 1; table["high"] = 2; table["max"] = 2;
				break;
			case 3:
				table["min"] = 1; table["low"] = 2; table["med"] = 2; table["high"] = 2; table["max"] = 3;
				break;
			case 4:
				table["min"] = 1; table["low"] = 2; table["med"] = 2; table["high"] =3 ; table["max"] = 4;
				break;
			case 5:
				table["min"] = 1; table["low"] = 2; table["med"] = 3; table["high"] = 4; table["max"] = 5;
				break;
			case 6:
				table["min"] = 1; table["low"] = 2; table["med"] = 3; table["high"] = 4; table["max"] = 6;
				break;
			case 7:
				table["min"] = 1; table["low"] = 2; table["med"] = 3; table["high"] = 5; table["max"] = 7;
				break;
			case 8:
				table["min"] = 1; table["low"] = 2; table["med"] = 4; table["high"] = 6; table["max"] = 8;
				break;
			case 9:
				table["min"] = 1; table["low"] = 3; table["med"] = 5; table["high"] = 7; table["max"] = 8;
				break;
			case 10:
				table["min"] = 1; table["low"] = 3; table["med"] = 5; table["high"] = 8; table["max"] = 9;
				break;
			case 11:
				table["min"] = 1; table["low"] = 3; table["med"] = 6; table["high"] = 8; table["max"] = 10;
				break;
			case 12:
				table["min"] = 1; table["low"] = 3; table["med"] = 6; table["high"] = 9; table["max"] = 11;
				break;
			default:
				table["min"] = 1;
	...
	void Study::getNumberOfCores(const bool forceParallel, const uint nbYearsParallelForced)
	{
		...
		*/
		...
		if (TSlimit < maxNbYearsInParallel) maxNbYearsInParallel = TSlimit;

		// Limiting the number of parallel years by the total number of years
		if (p.nbYears < maxNbYearsInParallel) maxNbYearsInParallel = p.nbYears;
		...
		for (uint y = 0; y < p.nbYears; ++y)
		{
			...
			refreshing = (p.timeSeriesToGenerate & timeSeriesLoad) && (p.timeSeriesToRefresh & timeSeriesLoad) && ( !y || ((y % p.refreshIntervalLoad) == 0));
			...
			//	- This is the first year after the previous set is full with years to be actually executed (not skipped).
		...
		// The max number of years to run in parallel is limited by the max number years in a set of parallel years.
		// This latter number can be limited by the smallest interval between 2 refresh points and determined by
		// the unrun MC years in case of play-list.
		...
		// GUI : storing max nb of parallel years (in a set of parallel years) in case parallel mode is enabled.
		...
		areas.each([&] (Data::Area& area)
		{
			...
			for (uint timeSeries = 0; timeSeries < matrix.width; ++timeSeries)
			{
				...
				for (uint h = 0; h < matrix.height; ++h)
				{
					...
					/*if (perHour[h] < 0.)
					{
						logs.warning() << area.id << ", hour " << h << ": `load - dsm` can not be negative. Reset to 0";
						perHour[h] = 0.;
					}*/
				}
			}
	...
	bool Study::prepareOutput()
	{
		...
		{
			case stdmEconomy:       folderOutput += "eco"; break;
			case stdmAdequacy:      folderOutput += "adq"; break;
			case stdmAdequacyDraft: folderOutput += "dft"; break;
			case stdmUnknown:
			case stdmMax:           break;
		}
		...
		// and the scenario builder data
		{
			...
			// Remove all areas
			{
				...
				for (auto i = arealist.begin(); i != end; ++i)
				{
					...
					logs.info() << "destroying the area " << count << '/' << arealist.size() << ": " << area.name;
	...
	bool Study::areasThermalClustersMinStablePowerValidity(std::map<int,YString>& areaClusterNames) const
	{
		YString areaname=""; 
		bool resultat = true;
	...
	namespace // anonymous
	{
		...
		struct TS final
		{
			static bool IsNeeded(const Study& s, const uint y)
			{
				...
				switch (T)
				{
					case timeSeriesLoad:    return (! (y % s.parameters.refreshIntervalLoad));
					case timeSeriesSolar:   return (! (y % s.parameters.refreshIntervalSolar));
					case timeSeriesWind:    return (! (y % s.parameters.refreshIntervalWind));
					case timeSeriesHydro:   return (! (y % s.parameters.refreshIntervalHydro));
					case timeSeriesThermal: return (! (y % s.parameters.refreshIntervalThermal));
				}
				return false;
			}
		};
		...
		if (not tsGeneratorOnly)
		{
			...
			if (parameters.mode != stdmAdequacyDraft)
			{
				ticksPerYear =
					// nb weeks
					((int)
					((double)(parameters.simulationDays.end - parameters.simulationDays.first) / 7)
					);
			}
			else
				ticksPerYear = 1;
	...
	bool Study::checkForFilenameLimits(bool output, const String& chfolder) const
	{
		...
		if (output)
		{
			...
			{
				logs.error()
					<< "OS Maximum path length limitation obtained with the link '"
					<< linkname << "' (got " << filename.size() << " characters)";
		...
		else
		{
			...
			if (not areaname.empty() and not clustername.empty())
			{
				...
				if (filename.size() >= limit)
				{
					logs.error()
						...
						<< areaname << "::" << clustername << "' (got " << filename.size() << " characters)";
					logs.error() << "You may experience problems while accessing to this file: "
						<< filename;
					return false;
				}
			...
			if (not areaname.empty())
			{
				...
				{
					logs.error()
						<< "OS Maximum path length limitation obtained with the area '"
