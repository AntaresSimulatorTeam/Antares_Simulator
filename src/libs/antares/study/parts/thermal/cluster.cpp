...
namespace Data
{
	...
	static bool
	ThermalClusterLoadFromSection(const AnyString& filename, ThermalCluster& cluster, const IniFile::Section& section)
	{
		...
		if (section.firstProperty)
		{
			...
			for (auto* property = section.firstProperty; property; property = property->next)
			{
				...
				{
					logs.warning() << '`' << filename << "`: `" << section.name << "`: Invalid key/value";
					continue;
				}
				if (not ThermalClusterLoadFromProperty(cluster, property))
				{
					logs.warning() << '`' << filename << "`: `" << section.name << "`/`" << property->value
	...
	static bool
	ThermalClusterLoadCouplingSection(const AnyString& filename, ThermalClusterList& list, const IniFile::Section* s)
	{
		if (s->firstProperty)
		{
			...
			for (const IniFile::Property* p = s->firstProperty; p; p = p->next)
			{
				...
				logs.info() << "  cluster coupling : " << clusterFrom->name() << " <-> " << clusterWith->name();
			}
		}
		return true;
	}
	...
	void ThermalClusterList::resizeAllTimeseriesNumbers(uint n)
	{
		...
		if (not cluster.empty())
		{
			if (0 == n)
			{
				...
				{
					cluster.series->timeseriesNumbers.clear();
				});
			}
			else
			{
				each([&] (ThermalCluster& cluster)
				{
					cluster.series->timeseriesNumbers.resize(1, n);
				});
			}
		}
	}
	...
	bool ThermalClusterListSaveToFolder(const ThermalClusterList* l, const AnyString& folder)
	{
		...
		if (IO::Directory::Create(folder))
		{
			...
			l->each([&] (const Data::ThermalCluster& cluster)
			{
				...
				buffer.clear() << folder << SEP << ".." << SEP << ".." << SEP << "prepro"
					<< SEP << cluster.parentArea->id
					<< SEP << cluster.id();
				if (IO::Directory::Create(buffer))
				{
					buffer.clear() << folder << SEP << ".." << SEP << ".." << SEP << "prepro"
						<< SEP << cluster.parentArea->id
						<< SEP << cluster.id() << SEP << "modulation.txt";
		...
		if (ini.open(study.buffer))
		{
			...
			if (ini.firstSection)
			{
				...
				for (auto* section = ini.firstSection; section; section = section->next)
				{
					...
					if (study.header.version <= 500)
					{
						...
						else
						{
							if (Math::Abs(cluster->marketBidCost) > 5.e4)
								(cluster->marketBidCost > 0) ? cluster->marketBidCost =  5.e4 :  cluster->marketBidCost = -5.e4;
							else
								cluster->marketBidCost = Math::Round(cluster->marketBidCost, 3);
						}
						...
						else
						{
							if (Math::Abs(cluster->marginalCost) > 5.e4)
								(cluster->marginalCost > 0) ? cluster->marginalCost =  5.e4 :  cluster->marginalCost = -5.e4;
							else
						...
						else
						{
							if (Math::Abs(cluster->fixedCost) > 5.e4)
								(cluster->fixedCost > 0) ? cluster->fixedCost =  5.e4 :  cluster->fixedCost = -5.e4;
							else
						...
						else
						{
							if (Math::Abs(cluster->startupCost) > 5.e6)
								(cluster->startupCost > 0) ? cluster->startupCost =  5.e6 :  cluster->startupCost = -5.e6;
							else
						...
						//			Math::Max(it->dailyMinimumCapacity, it->weeklyMinimumCapacity));
					...
					//		break;
					//	default:
					//		logs.error() << "Invalid 'Min. Up/Down time' for the cluster '" << it->name() << "'";
					//		it->hourlyMinimumCapacity = it->minStablePower;
					//		it->dailyMinimumCapacity  = 0;
					//		it->weeklyMinimumCapacity = 0;
					// }
					...
					if (study.header.version < 350)
					{
						...
						bool r = modulation.loadFromCSVFile(modulationFile, 2, HOURS_PER_YEAR, Matrix<>::optImmediate | Matrix<>::optMarkAsModified);
						if (r and modulation.width == 2)
						{
							...
							modulation.pasteToColumn(thermalModulationMarketBid, modulation[thermalModulationCost]);
					...
					else if (study.header.version <= 450)
					{
						...
						bool r = cluster->modulation.loadFromCSVFile(modulationFile, 3, HOURS_PER_YEAR, Matrix<>::optImmediate | Matrix<>::optMarkAsModified);
						...
						modulation.pasteToColumn(thermalMinGenModulation, modulation[thermalModulationMarketBid]);
						modulation.pasteToColumn(thermalModulationMarketBid, modulation[thermalModulationCapacity]);
						modulation.pasteToColumn(thermalModulationCapacity, modulation[thermalMinGenModulation]);
						modulation.fillColumn(thermalMinGenModulation, 0.);
						modulation.markAsModified();
						ret = ret and r;
					}
					else
					{
						...
						bool r = cluster->modulation.loadFromCSVFile(modulationFile, thermalModulationMax, HOURS_PER_YEAR, options);
	...
	bool ThermalClusterListLoadPreproFromFolder(Study& study, const StudyLoadOptions& options, ThermalClusterList* list, const AnyString& folder)
	{
		if (list->empty())
			return true;
		...
		list->each([&] (Data::ThermalCluster& cluster)
		{
			if (cluster.prepro)
			{
				...
				bool result = cluster.prepro->loadFromFolder(study, buffer, cluster.parentArea->id, cluster.id());
	...
	int ThermalClusterListSaveDataSeriesToFolder(const ThermalClusterList* l, const AnyString& folder, const String& msg)
	{
		if (l->empty())
			return 1;
	...
	int ThermalClusterListLoadDataSeriesFromFolder(Study& s, const StudyLoadOptions& options, ThermalClusterList* l, const AnyString& folder, int fast)
	{
		if (l->empty())
			return 1;
	...
	bool Data::ThermalCluster::integrityCheck()
	{
		...
		{
			logs.error() << "Thermal cluster " << parentArea->name << '/' << pName << ": NaN detected for marginal cost";
			return false;
		}
		...
		if (modulation.height > 0)
		{
			...
			buffer << "Thermal cluster: " << parentArea->name << '/' << pName
				<< ": Modulation";
	...
	const char* Data::ThermalCluster::GroupName(enum ThermalDispatchableGroup grp)
	{
		switch (grp)
		{
			case thermalDispatchGrpNuclear:   return "Nuclear";
			case thermalDispatchGrpLignite:   return "Lignite";
			case thermalDispatchGrpHardCoal:  return "Hard Coal";
			case thermalDispatchGrpGas:       return "Gas";
			case thermalDispatchGrpOil:       return "Oil";
			case thermalDispatchGrpMixedFuel: return "Mixed Fuel";
			case thermalDispatchGrpOther:     return "Other";
			case thermalDispatchGrpMax:       return "";
		}
		return "";
	}
	...
	bool ThermalClusterList::storeTimeseriesNumbers(Study& study)
	{
		...
		each([&] (const Data::ThermalCluster& cluster)
		{
			...
			study.buffer
				<< SEP << "ts-numbers"
				<< SEP << "thermal" << SEP << cluster.parentArea->id
				<< SEP << cluster.id() << ".txt";
			ret = cluster.series->timeseriesNumbers.saveToCSVFile(study.buffer, 0, true, predicate) and ret;
		});
		return ret;
	}
	...
	{
		minDivModulation.value = (modulation[thermalModulationCapacity][0]/Math::Ceil(modulation[thermalModulationCapacity][0]));
		minDivModulation.index = 0;

		for (uint t = 1; t < modulation.height; t++)
		{
			double div = modulation[thermalModulationCapacity][t]/ceil(modulation[thermalModulationCapacity][t]);
	...
	bool ThermalCluster::checkMinStablePower() 
	{
		...
		if (Math::Zero(1 - spinning/101))
			...
			minDivModulation.border = Math::Min(nomCapacityWithSpinning, minStablePower) / nomCapacityWithSpinning;
