...
namespace Data
{
	...
	bool PartHydro::LoadFromFolder(Study& study, const AnyString& folder)
	{
		...
		study.areas.each([&] (Data::Area& area)
		{
			...
			else
			{
				...
				if(!study.usedByTheSolver)
				{
					...
					{
						ret = area.hydro.maxPower.loadFromCSVFile(buffer, 4, DAYS_PER_YEAR,
			...
			if (study.header.version >= 620)
			{
				...
				ret = area.hydro.creditModulation.loadFromCSVFile(buffer, 101, 2,
					Matrix<>::optFixedSize, &study.dataBuffer) && ret;
			}
			else
			{
			...
			if (study.header.version >= 620)
			{
				...
				ret = area.hydro.reservoirLevel.loadFromCSVFile(buffer, 3, DAYS_PER_YEAR,
					Matrix<>::optFixedSize, &study.dataBuffer) && ret;
			}
			else if (study.header.version >= 390)
			{
				...
				static const uint daysPerMonthR[] = {
					study.calendar.months[0].days, study.calendar.months[1].days, study.calendar.months[2].days, study.calendar.months[3].days, study.calendar.months[4].days, study.calendar.months[5].days, study.calendar.months[6].days, study.calendar.months[7].days, study.calendar.months[8].days, study.calendar.months[9].days, study.calendar.months[10].days, study.calendar.months[11].days
				};
				...
				for (int x = area.hydro.minimum; x <= area.hydro.maximum; x++)
				{
					...
					for (int month = 0; month < 12; month++)
					{
						...
						{
							temp[x][day] = Math::Round(col[realMonth] + (day- firstDayMonth[month])*(valDiff/ daysPerMonthDecals[month]),3);
						}
					}
				}
			...
			if (study.header.version >= 620)
			{
				...
				ret = area.hydro.waterValues.loadFromCSVFile(buffer, 101, DAYS_PER_YEAR,
					Matrix<>::optFixedSize, &study.dataBuffer) && ret;
			}
			else
			{
			...
			if (study.header.version >= 620)
			{
				...
				ret = area.hydro.inflowPattern.loadFromCSVFile(buffer, 1, DAYS_PER_YEAR,
					Matrix<>::optFixedSize, &study.dataBuffer) && ret;
			}
			else
			{
			...
			if (study.usedByTheSolver)
			{
				...
				for (int day = 0; day < DAYS_PER_YEAR; day++)
				{
					if (!errorLevels && (colMin[day] < 0 || colAvg[day] < 0 || colMin[day] > colMax[day] || colAvg[day]>100 || colMax[day]>100))
					{
				...
				for (int i = 0; i < 101; i++) 
				{
					if ((area.hydro.creditModulation[i][0] < 0) || (area.hydro.creditModulation[i][1] < 0))
					{
		...
		// Check on reservoir capacity (has to be done after reservoir management and capacity reading, not before).
		// Some areas reservoir capacities may not be printed in hydro ini file when saving the study, because 
		// they are too small (< 1e-6).
		// We cannot have reservoir management = yes and capacity = 0 because of further division by capacity.
		// reservoir management = no and capacity = 0 is possible (no use of capacity further)
		study.areas.each([&](Data::Area& area)
		{
		...
		if (	
				( study.header.version >= 620 && study.header.version < 640 && (section = ini.find("leeway below")) )
		...
		if (
				( study.header.version >= 620 && study.header.version < 640 && (section = ini.find("leeway above")))
		...
		if (study.header.version >= 620)
		{
			// they are too small (< 1e-6). We cannot allow these areas to have reservoir management = true.
			study.areas.each([&](Data::Area& area)
			{
		...
		study.areas.each([&](Data::Area& area)
		{
			...
			{
				logs.error() << area.name << " : use water value = no conflicts with use heuristic target = no";
				ret = false && ret;
			}
		});
	...
	double getWeeklyModulation(const double & level /* format : in % of reservoir capacity */, Matrix<double, double> & creditMod, int modType)
	{
		...
		if ((int)(levelUp) == (int)(levelDown))
			...
			valueToReturn = 
			creditMod[(int)(levelUp)][modType] * (level - levelDown) +
