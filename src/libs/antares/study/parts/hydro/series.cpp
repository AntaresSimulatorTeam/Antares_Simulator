...
DataSeriesHydrocluster::DataSeriesHydrocluster() : count(0)
{
}

bool DataSeriesHydrocluster::saveToFolder(const AreaName& areaID, const AnyString& folder) const
{
    String buffer;
    buffer.clear() << folder << SEP << areaID;
    /* Make sure the folder is created */
...
bool DataSeriesHydrocluster::loadFromFolder(Study& study, const AreaName& areaID, const AnyString& folder)
{
    bool ret = true;
    auto& buffer = study.bufferLoadingTS;
...
bool DataSeriesHydrocluster::invalidate(bool reload) const
{
    bool ret = true;
    ret = ror.invalidate(reload) && ret;
    ret = storage.invalidate(reload) && ret;
...
void DataSeriesHydrocluster::markAsModified() const
{
    ror.markAsModified();
    storage.markAsModified();
    mingen.markAsModified();
...
void DataSeriesHydrocluster::reset()
{
    ror.reset(1, HOURS_PER_YEAR);
    storage.reset(1, DAYS_PER_YEAR);
    mingen.reset(1, HOURS_PER_YEAR);
...
#ifdef ANTARES_SWAP_SUPPORT
void DataSeriesHydrocluster::flush()
{
    ror.flush();
    storage.flush();
    mingen.flush();
...
uint64 DataSeriesHydrocluster::memoryUsage() const
{
    return sizeof(double) + ror.memoryUsage() + storage.memoryUsage() + mingen.memoryUsage();
}
