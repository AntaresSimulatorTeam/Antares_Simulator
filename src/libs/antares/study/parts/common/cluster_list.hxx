...
void ClusterList<ClusterT>::each(const PredicateT& predicate) const
{
    auto end = cluster.cend();
    for (auto i = cluster.cbegin(); i != end; ++i)
    {
        const ClusterT& it = *(i->second);
        predicate(it);
    }
}
...
template<class ClusterT>
template<class PredicateT>
void ClusterList<ClusterT>::each(const PredicateT& predicate)
{
    auto end = cluster.end();
    for (auto i = cluster.begin(); i != end; ++i)
    {
        ClusterT& it = *(i->second);
        predicate(it);
    }
}
...
template<class ClusterT>
void ClusterList<ClusterT>::estimateMemoryUsage(StudyMemoryUsage& u) const
{
    u.requiredMemoryForInput += (sizeof(void*) * 4 /*overhead map*/) * cluster.size();

    each([&](const Cluster& cluster) {
        u.requiredMemoryForInput += sizeof(ClusterT);
