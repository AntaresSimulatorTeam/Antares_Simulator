...
namespace Data
{
	...
	static inline void InterAreaCorrelationSetValue(Matrix<>& m, const char filename[],
		Area* from, Area* to, IniFile::Section* s, IniFile::Property* p)
	{
		double v;
		...
		if (to == from)
		{
			logs.warning() << filename << ": '" << s->name << "'/'" << p->key << "'='"
				<< p->value << "': This value of correlation is locked (equal to 1.0). The property is ignored";
			return;
		}
		...
		{
			logs.warning() << filename << ": '" << s->name << "'/'" << p->key << "'='"
				<< p->value << "': This correlation value is already set and will override the previous value";
		}
		// Try to convert the value
		if (!p->value.to<double>(v))
		{
			logs.warning() << filename << ": '" << s->name << "'/'" << p->key << "'='"
				<< p->value << "': A floatting point value is required. The property is ignored";
			return;
		}
		// Out of bounds
		if (v < -1. or v > 1.)
		{
			logs.error() << filename << ": '" << s->name << "'/'" << p->key << "'='"
				<< p->value << "': A correlation coefficient must be between -1.0 and +1.0";
			return;
		}
	...
	static inline void
	ReadCorrelationCoefficients(Correlation& correlation, Study& study, Matrix<>& m,
		const IniFile& ini, const IniFile::Section& section, bool warnings)
	{
		double v;
		AreaName nameFrom;
		AreaName nameTo;
		...
		for (auto* p = section.firstProperty; p; p = p->next)
		{
			...
			{
				if (warnings)
					logs.error() << ini.filename() << ": '" << nameFrom << "': impossible to find the area";
				continue;
			}
			if (!to)
			{
				if (warnings)
					logs.error() << ini.filename() << ": '" << nameTo << "': impossible to find the area";
				continue;
			}
			...
			else
			{
				if (warnings)
				{
					logs.warning() << ini.filename() << ": section '" << section.name
						<< "': invalid coefficient for the link '"
						<< from->id << "' -> '" << to->id << "'";
				}
			}
		}
	...
	static inline void
	ExportCorrelationCoefficients(Study& study, const Matrix<>& m, IO::File::Stream& file, const char* name)
	{
		...
		for (uint x = 0; x != m.width; ++x)
		{
			...
			for (uint y = 0; y < x; ++y)
			{
				if (!Math::Zero(col[y]))
				{
					file << from << '%' << study.areas.byIndex[y]->id
						<< " = " << col[y] << '\n';
				}
			}
		}
			...
			for (s = ini->firstSection; s != NULL; s = s->next)          /* Each section */
			{
				...
				if (from)
				{
					...
					for (p = s->firstProperty; p != NULL; p = p->next)   /* Each property*/
					{
						...
						{
							if (warnings)
								logs.warning() << "Correlation: " << s->name << ": " << p->key << ": Area not found";
						}
					}
				}
				else /* nullptr != from */
	...
	IniFile* InterAreaCorrelationSaveToIniFile(const Matrix<>* m, const AreaList* l)
	{
		...
		{
			logs.error() << "Correlation: The matrix does not have the good sie ("
				<< l->size() << 'x' << l->size() << " expected, got " << m->width << 'x' << m->height << ')';
		}
		else
		{
			uint x;
	...
	int InterAreaCorrelationSaveToFile(const Matrix<>* m, const AreaList* l, const char filename[])
	{
		...
		if (m->width != l->size() or m->height != l->size())
		{
			logs.error() << filename << ": The matrix does not have the good sie ("
				<< l->size() << 'x' << l->size() << " expected, got " << m->width << 'x' << m->height << ')';
		}
		else
		{
	...
	const char* Correlation::ModeToCString(Mode mode)
	{
		switch (mode)
		{
			case modeAnnual:  return "annual";
			case modeMonthly: return "monthly";
		...
		if (JIT::usedFromGUI or pMode == modeMonthly)
		{
			...
			for (auto* section = ini.firstSection; section; section = section->next)
			{
				if (section->name.to<int>(index))
				{
					...
					{
						ReadCorrelationCoefficients(*this, study, monthly[index], ini, *section, warnings);
						monthly[index].flush();
					}
					else
	...
	bool Correlation::internalLoadFromINIv32(Study& study, const IniFile& ini, bool warnings)
	{
		...
		for (auto* s = ini.firstSection; s; s = s->next)
		{
			...
			for (const IniFile::Property* p = s->firstProperty; p; p = p->next)
			{
				...
				{
					if (warnings)
						logs.warning() << correlationName << "*: " << s->name << ": " << p->key << ": Area not found";
				}
			}
		}
		annual->flush();
	...
	void Correlation::set(Matrix<>& m, const Area& from, const Area& to, double v)
	{
		...
		{
			logs.error() << correlationName << ": Impossible to overwrite the correlation coefficient for `"
				<< from.name << "` / `" << to.name << '`';
			return;
		}
	...
	static void CopyFromSingleMatrix(const Matrix<>& mxsrc, Matrix<>& mxout, const Study& studySource,
		 uint areaSource, uint areaTarget,
		 const Area::NameMapping& mapping, const Study& study)
	{
	...
	void Correlation::copyFrom(const Correlation& source, const Study& studySource, const AreaName& areaSource,
		const Area::NameMapping& mapping, const Study& study)
	{
		...
		std::cout << "ANNUAL\n";
		CopyFromSingleMatrix(*source.annual, *annual, studySource, areaSourceIndex, areaTargetIndex,
			mapping, study);
		...
		for (uint i = 0; i != 12; ++i)
		{
			CopyFromSingleMatrix(source.monthly[i], monthly[i], studySource, areaSourceIndex,
				areaTargetIndex, mapping, study);
		}
	}
