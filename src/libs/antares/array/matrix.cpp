							...
							if (size == file.read(u.buffer->data(), size))
							{
								...
								*(u.buffer) += '\n';
								...
								if (offset != String::npos)
								{
									String::Size n = 0;
									uint columns = 1;
									while ((n = u.buffer->find_first_of(ANTARES_MATRIX_CSV_SEPARATORS, n)) != String::npos and n < offset)
									{
										++columns;
										++n;
									}
		...
		if (m->width and m->height)
		{
			for (; x < m->width; ++x)
			{
				...
				for (y = 0; y < m->height; ++y)
				{
					if (col[y] < value)
					{
						logs.error() << msg << ": invalid value detected (at column " << x << ", row: " << y << ')';
						return 0;
					}
				}
			}
		...
		if (m->width and m->height)
		{
			for (uint x = 0; x < m->width; ++x)
			{
				...
				for (uint y = 0; y < m->height; ++y)
				{
					if (column[y] > 0.)
					{
						logs.error() << msg << ": positive value detected (at column " << x << ", row: " << y << ')';
						return 0;
					}
				}
			}
		...
		if (m->width and m->height and maxWidth)
		{
			for (uint x = 0; x < maxWidth; ++x)
			{
				...
				for (uint y = 0; y < m->height; ++y)
				{
					if (column[y] < 0.)
					{
						logs.error() << msg << ": negative value detected (at column " << x << ", row: " << y << ')';
						return 0;
					}
				}
			}
