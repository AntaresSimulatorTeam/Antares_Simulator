...
template<class U>
static int Array1DLoadFromBuffer(const char* filename, U& data, double values[],
	const int height)
{
		...
		if ((offset = data.find('\n', offset)) != Yuni::String::npos)
		{
			...
			else
			{
				...
				if (!cell.to(v))
				{
					if (errorCount)
					{
						logs.warning() << filename << ": invalid decimal value at line " << (y + 1) << ", assuming 0";
						if (!(--errorCount))
				...
				if (!(pend and '\0' == *pend))
				{
					...
					if (errorCount)
					{
						logs.warning() << '`' << filename
							<< "`: Invalid format: Decimal value expected (y:" << y << ", offset:" << (uint)pos << "byte)";
						if (!(--errorCount))
							logs.warning() << " ... (skipped)";
					}
				}
