...
namespace Antares
{
	...
	{
		assert((JIT::enabled or (jit == NULL)) and "Internal variable jit is set but JIT is not globally enabled (overflow?)");
		delete jit;
	...
	template<class T, class ReadWriteT>
	bool Matrix<T,ReadWriteT>::internalLoadJITData(const AnyString& filename, uint minWidth, uint maxHeight,
		uint options)
	{
	...
	{
		return loadFromCSVFile(filename, 1, 0, optImmediate|optNoWarnIfEmpty|optNeverFails|optQuiet);
	}


	template<class T, class ReadWriteT>
	inline bool Matrix<T,ReadWriteT>::loadFromCSVFile(const AnyString& filename, uint minWidth, uint maxHeight,
		BufferType* buffer)
	{
	...
	template<class T, class ReadWriteT>
	bool Matrix<T,ReadWriteT>::loadFromCSVFile(const AnyString& filename, uint minWidth, uint maxHeight,
		uint options, BufferType* buffer)
	{
			...
			if (data.size() > 1)
			{
				...
				if ((unsigned char)data[0] == 0xFE and ((unsigned char)data[1]) == 0xFF)
				{
					if (data.size() > 3 and !data[2] and !data[3])
						logs.error() << '`' << filename << "`: UTF-32 Little Endian encoding detected. ASCII/UTF-8 required.";
					else
						logs.error() << '`' << filename << "`: UTF-16 Big Endian encoding detected. ASCII/UTF-8 required.";
					return false;
				}
				// UTF-16 Little endian
				if ((unsigned char)data[0] == 0xFF and ((unsigned char)data[1]) == 0xFE)
				{
					logs.error() << '`' << filename << "`: UTF-16 Little Endian encoding detected. ASCII/UTF-8 required.";
					return false;
				}
				// UTF-8 Little endian
				if ((unsigned char)data[0] == 0xEF and ((unsigned char)data[1]) == 0xBB and data.size() > 2 and ((unsigned char)data[2]) == 0xBF)
				{
				...
				if (data.size() > 3)
				{
					if (!data[0] and !data[1] and ((unsigned char)data[2]) == 0xFE and ((unsigned char)data[3]) == 0xFF)
					{
						logs.error() << '`' << filename << "`: UTF-32 Big Endian encoding detected. ASCII/UTF-8 required.";
						return false;
					}
				}
			}
	...
	template<class T, class ReadWriteT>
	bool Matrix<T,ReadWriteT>::loadFromBuffer(const AnyString& filename, BufferType& data,
		uint minWidth, uint maxHeight, const int fixedSize, uint options)
	{
		...
		while (y < maxHeight and offset < (uint) data.size())
		{
			...
			while ((offset = data.find_first_of(ANTARES_MATRIX_CSV_SEPARATORS, offset)) != BufferType::npos)
			{
					...
					if (x >= width) // Out of bounds
					{
						if ((options & optNeverFails))
						{
							...
							else
							{
								...
								while ((newOffset = data.find_first_of(ANTARES_MATRIX_CSV_SEPARATORS, (String::Size)newOffset)) != BufferType::npos)
								{
									if (data[newOffset] == '\n')
									{
										++newWidth;
						...
						else
						{
							...
							if (not (options & optQuiet) and errorCount > 0)
							{
								logs.warning() << '`' << filename
									<< "`: Invalid format: Too many entry for the row " << y << " (offset: " << (uint)pos << "byte)";
								if (!(--errorCount))
					...
					else
					{
						...
						if (not MatrixStringConverter<ReadWriteType>::Do(converter, cellValue))
						{
							...
							if (not MatrixStringConverter<double>::Do(converter, fallback))
							{
								...
								{
									logs.warning() << '`' << filename
										<< "`: Invalid numeric value (x:" << x << ",y:" << y << ", offset: " << (uint)pos << "byte), text: `" << converter << " read:" << entry[x][y]<< '`';
									if (not (--errorCount))
										logs.warning() << " ... (skipped)";
								}
								...
								entry[x][y] = MatrixRound<T, ReadWriteType>::Value(static_cast<ReadWriteType>(fallback));
						}
						else
						{
			...
			if (x < width)
			{
				if (not (options & optNeverFails))
				{
					...
					if (not (options & optQuiet) and errorCount)
					{
						logs.warning() << filename << ": at line " << (y + 1) << ", not enough columns (expected "
							<< width << ", got " << x << ')';
						if (not (--errorCount))
							logs.warning() << " ... (skipped)";
					}
		...
		if (y < height)
		{
			...
			if (!(options & optQuiet))
			{
				logs.warning() << filename << ": not enough rows (expected "
					<< height << ", got " << y << ')';
			}
	...
	bool Matrix<T,ReadWriteT>::internalLoadCSVFile(const AnyString& filename,
		uint minWidth, uint maxHeight, uint options, BufferType* buffer)
	{
		using namespace Yuni;
			...
			case IO::errNone:
				{
					...
					*buffer += '\n';
					// Load the data
					result = loadFromBuffer(filename, *buffer, minWidth, maxHeight,
						(options & optFixedSize), options);
	...
	bool Matrix<T,ReadWriteT>::loadAllJITData() const
	{
		if (jit and not JIT::IsReady(jit))
		{
			return (const_cast<Matrix<T,ReadWriteT>*>(this))->
				loadFromCSVFile(jit->sourceFilename, jit->minWidth, jit->maxHeight, jit->options | optImmediate);
		}
		return true;
	}
	...
	template<class T, class ReadWriteT>
	inline void Matrix<T,ReadWriteT>::EstimateMemoryUsage(Antares::Data::StudyMemoryUsage& u, uint w, uint h)
	{
	...
	template<class T, class ReadWriteT>
	void Matrix<T,ReadWriteT>::estimateMemoryUsage(Antares::Data::StudyMemoryUsage& u, bool cond, uint w, uint h) const
	{
		if (cond)
