...
#ifdef YUNI_OS_WINDOWS
    typedef HRESULT WINAPI SHGetFolderPathWFn(
      HWND hwndOwner, int nFolder, HANDLE hToken, DWORD dwFlags, __out LPTSTR pszPath);

    HMODULE hShell32 = GetModuleHandleW(L"shell32.dll");
    if (hShell32)
    {
        // Windows XP
        SHGetFolderPathWFn* folderPathW
          = (SHGetFolderPathWFn*)(GetProcAddress(hShell32, "SHGetFolderPathW"));
        if (folderPathW)
        {
            ...
            if (!wpath)
                return false;
            folderPathW(
              0, (allusers ? CSIDL_COMMON_APPDATA : CSIDL_LOCAL_APPDATA), nullptr, 0, wpath);
