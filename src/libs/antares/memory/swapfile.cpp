...
#define LOG_SWAP_ERR_POSSIBLE_CAUSES \
		"The swap support is disabled. Possible causes: Not enough free disk space, not enough permissions or swap folder on a network drive"

#define LOG_SWAP_NOT_ENOUGH_DISK_SPACE \
			logs.warning() << "impossible to allocate "   \
				<< (::Antares::Memory::swapSize / 1024 / 1024) \
...
namespace Memory
{
	...
	SwapFileInfo::SwapFileInfo() :
		lastOffset(0),
		nbFreeBlocks(Antares::Memory::blockPerSwap)
	{
	...
	bool SwapFileInfo::openSwapFile(uint count)
	{
...

		{
			...
			logs.info() << "  memory pool: expanding to "
				<< size << "Mo. Reserving disk space in " << filename;
		}
		...
		// Open the file
		{
			...
			if (buffer.empty())
				return false;
			handle = CreateFileW(buffer.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE|FILE_SHARE_READ,
		...
		if (handle != INVALID_HANDLE_VALUE)
		{
			...
			::SetFilePointer(handle, ::Antares::Memory::swapSize + ::Antares::Memory::blockSize, 0, FILE_BEGIN);
			if (::SetEndOfFile(handle))
			{
				FILE_ZERO_DATA_INFORMATION fzdi;
				DWORD dwTemp;
				fzdi.FileOffset.QuadPart = 0;
				fzdi.BeyondFinalZero.QuadPart = ::Antares::Memory::swapSize + ::Antares::Memory::blockSize;
				// Write all zeros !
				if (!::DeviceIoControl(handle, FSCTL_SET_ZERO_DATA, &fzdi, sizeof(fzdi), nullptr, 0, &dwTemp, nullptr))
					LOG_SWAP_NOT_ENOUGH_DISK_SPACE_RETURN_FALSE;

				// Mapping
				LARGE_INTEGER offset;
				offset.QuadPart = Antares::Memory::swapSize;
				mappingHandle = CreateFileMapping(handle, nullptr, PAGE_READWRITE,
									offset.HighPart, offset.LowPart, nullptr);
		...
		{
			logs.warning()
				<< "[swap] Impossible to create a new swap file. Not enough free disk space or not enough permissions";
		}
