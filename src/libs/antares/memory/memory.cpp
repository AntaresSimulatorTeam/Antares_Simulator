...
namespace Antares
{
	...
	bool Memory::initialize()
	{
		...
		if (not pCacheFolder.empty() and not pAllowedToChangeCacheFolder and not IO::Directory::Create(pCacheFolder))
		{
			// The swap support is disabled.
			logs.warning() << "Swap support is disabled. Impossible to create the folder " << pCacheFolder;
			pCacheFolder.clear();
		}
		return true;
	}
		...
		if (not pSwapFile.empty())
		{
			...
			for (SwapFileList::const_iterator i = pSwapFile.begin(); i != end; ++i)
			{
				...
				logs.info() << "[memory][summary] -- swap " << index << ": "
					...
					<< (swapSize / 1024 / 1024) << "Mo, "
					<< s.nbFreeBlocks << " free blocks";
				++index;
			}
		}
		else
	...
	uint64 Memory::memoryUsage() const
	{
		...
		enum
		{
			mapOverheadPerElement = (sizeof(Handle) + sizeof(Mapping*) + sizeof(32/*std::_Rb_tree_node_base*/))
		};
	...
	Memory::Handle Memory::internalAllocate(size_t size)
	{
		...
		do
		{
			...
			while (i--)
			{
				...
				{
					const MappingMap::iterator end = pMapping.end();
					while (pMapping.find(++pNextHandle) != end or not pNextHandle) // or (uint) -1 == pNextHandle)
					{
						// does nothing
					}
				}
				...
				// std::cout << "  allocate handle " << pNextHandle << " of " << blockNeeded << " blocks" << '\n';
	...
	void* Memory::acquireMapping(Handle handle, volatile void* pointer)
	{
		...
		if (i != pMapping.end())
		{
			...
			try
			{
				...
				void* p = MapViewOfFile(m.swapFile->mappingHandle, FILE_MAP_ALL_ACCESS,
					offset.HighPart, offset.LowPart, m.nbBlocks * blockSize);
				if (p == nullptr)
				{
					...
					if (error == 8) // Not enough memory
					{
						logs.error() << "memory pool: impossible to access to the swap file : "
							<< m.swapFile->filename << " (offset: " << m.offset << ", Not enough memory)";
					}
					else
					{
						logs.error() << "memory pool: impossible to access to the swap file : "
							<< m.swapFile->filename << " (offset: " << (sint64)m.offset << ", err: " << error << ")";
					}
					return nullptr;
				}
				return p;
			...
			void* p = mmap(0, m.nbBlocks * blockSize, PROT_READ|PROT_WRITE, MAP_SHARED,
				m.swapFile->handle, m.offset * blockSize);
			if (p == MAP_FAILED)
			{
				logs.error() << "memory pool: impossible to access to the swap file : "
					<< m.swapFile->filename;
		...
		logs.error() << "memory pool: impossible to acquire the ressource " << handle << ": Resource ID not found";
		# ifndef NDEBUG
		{
			logs.info() << "trace: ";
			const MappingMap::iterator end = pMapping.end();
	...
	void Memory::EstimateMemoryUsage(size_t bytes, uint count, Data::StudyMemoryUsage& u, bool duplicateForParallelYears)
	{
	...
	void Memory::displayInfo() const
	{
		...
		SYSTEM_INFO info;
		GetSystemInfo(&info);
		logs.info() << "  memory pool: system info: page size: " << info.dwPageSize << ", granularity: " << info.dwAllocationGranularity;
		# else
