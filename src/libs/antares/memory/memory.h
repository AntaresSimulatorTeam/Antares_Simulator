...
namespace Antares
{
	...
	*/
	...
	public:
		...
		public:
			...
			*/
			...
			template<class U> Array(const Array<U>&);
	...
	public:
		# ifdef ANTARES_SWAP_SUPPORT
		template<class T> static void Allocate(Antares::Memory::Array<T>& out, size_t size);
		# else
		template<class T> static void Allocate(T*& out, size_t size);
		# endif

		# ifdef ANTARES_SWAP_SUPPORT
		template<class T> static void Flush(Antares::Memory::Array<T>& out);
		# else
		template<class T> static void Flush(T*&);
		# endif

		# ifdef ANTARES_SWAP_SUPPORT
		template<class T> static void Acquire(Antares::Memory::Array<T>& out);
		# else
		template<class T> static void Acquire(T*&);
		# endif

		# ifdef ANTARES_SWAP_SUPPORT
		template<class T> static bool Null(const Antares::Memory::Array<T>& out);
		# else
		template<class T> static bool Null(const T*& out);
		# endif

		# ifdef ANTARES_SWAP_SUPPORT
		template<class T> static bool StrictNull(const Antares::Memory::Array<T>& out);
		# else
		template<class T> static bool StrictNull(const T*& out);
		# endif

		/*!
		** \brief Release a raw pointer
		*/
		template<class T> static void Release(T*& pointer);

		/*!
		** \brief Release a managed pointer
		*/
		template<class T> static void Release(Array<T>& pointer);
		...
		*/
		static void EstimateMemoryUsage(size_t bytes, uint count, Data::StudyMemoryUsage& u, bool duplicateForParallelYears);
	...
	private:
		...
		*/
		template<class T> Handle allocate(size_t count);
