...
namespace Antares
{
	...
	void ATSP::Extrait_bloc(Matrix<>& out, uint indlig, uint indcol, const Matrix<>& MTRX,
	...
	double ATSP::Correlation(double* A, double* B, uint nblig, double EA, double EB, double SA, double SB, int code)
	{
		double sigma_A = 0.;
		double sigma_B = 0.;
		double expec_A = 0.;
	...
	void ATSP::Analyse_auto(double* A, int nblig, double auc, double aum, double hor, double& theta, double& mu)
	{
	...
	// ajustement de la densité de probabilité en fonction des bornes L, U, de l'espérance E et de l'écart-type S
	// selon une loi "type" : calcule alpha=A, beta = B, gamma = C, delta = D
	// retourne 0 si le calage est possible et 1 sinon */
	bool ATSP::Probab_density_funct(double L, double U, double E, double S,
		Data::XCast::Distribution law, double& A, double& B, double& C, double& D)
	{
		enum { dicho = 20 };
