        ...
        for feature in feature_files:
            with open(feature, "r") as f:
                for line in f.readlines():
                    if "Given the solver study path is \"resources/Antares_Simulator_Tests_NR/" in line:
                        study = line.strip()
                        study = study.replace("Given the solver study path is \"resources/Antares_Simulator_Tests_NR/", "")
                        study = study.replace("\"", "")
