...
BOOST_AUTO_TEST_CASE(thermal_cluster_fullfills_area_demand)
{
    ...
    std::iota(timeStamps.begin(), timeStamps.end(), 0);
    {
        ...
        gLinearProblemData
          = Antares::optim::api::LinearProblemData(timeStamps,
        ...
        auto balance = std::make_shared<Balance>("some_area",
                                                 std::vector<std::shared_ptr<Battery>>{},
                                                 std::vector<std::shared_ptr<Thermal>>({thermal}));
