...
public:
    Component(string id,
              Model model,
              map<string, double> doubleParameterValues,
              map<string, string> stringParameterValues) :
     id_(std::move(id)),
     model_(model),
    ...
    [[nodiscard]] double getDoubleParameterValue(const string& key) const
    {
        return doubleParameterValues_.at(key);
    }
    [[nodiscard]] string getStringParameterValue(const string& key) const
    {
