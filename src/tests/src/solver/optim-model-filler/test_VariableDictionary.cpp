...
{
    FullKey k("component", "variable", ScenarioAndTime::Scenario{3}, 4);
    BOOST_CHECK_EQUAL(k.getComponent(), "component");
    BOOST_CHECK_EQUAL(k.getVariable(), "variable");

    BOOST_CHECK_EQUAL(*k.getScenario(), ScenarioAndTime::Scenario{3});
...
std::pair<std::map<std::pair<ScenarioAndTime::Scenario, int>, std::string>, VariableDictionary>
namesFromDimensions(const Dimensions& dim)
{
    VariableDictionary vdict;
    std::map<std::pair<ScenarioAndTime::Scenario, int>, std::string> names;
    vdict.addVariable(
      dim,
      PartialKey("component", "variable"),
      [&names](const ScenarioAndTime& timeAndScenario, const std::string& name)
      {
...
BOOST_AUTO_TEST_CASE(addVariable_no_ts_no_sc)
{
    ...
    BOOST_CHECK_EQUAL(names.at(std::pair(ScenarioAndTime::Scenario{0}, 0)), "component.variable");
...
BOOST_AUTO_TEST_CASE(addVariable_no_ts_multiple_sc)
{
    ...
    BOOST_CHECK_EQUAL(names.at(std::pair(ScenarioAndTime::Scenario{0}, 0)),
                      "component.variable_s0");

    BOOST_CHECK_NO_THROW(dict("component", "variable", ScenarioAndTime::Scenario{1}, 0));
}
...
BOOST_AUTO_TEST_CASE(addVariable_multiple_ts_no_sc)
{
    ...
    BOOST_CHECK_NO_THROW(dict("component", "variable", ScenarioAndTime::Scenario{0}, 2));
}
...
BOOST_AUTO_TEST_CASE(addVariable_multiple_ts_sc)
{
    ...
    BOOST_CHECK_EQUAL(names.at(std::pair(ScenarioAndTime::Scenario{2}, 3)),
                      "component.variable_s2_t3");
    BOOST_CHECK(!names.contains(std::pair(ScenarioAndTime::Scenario{3}, 3)));

    BOOST_CHECK_THROW(dict("component", "variable", ScenarioAndTime::Scenario{3}, 2),
                      std::out_of_range);
    BOOST_CHECK_THROW(dict("component", "variable", ScenarioAndTime::Scenario{2}, 5),
                      std::out_of_range);
}
