...
{
    unsigned int nb_new_vars = 2;
    auto vars = pb->addIntVariable(std::vector<double>(nb_new_vars, 1),
                                   std::vector<double>(nb_new_vars, 5),
                                   "int_var",
                                   nb_new_vars);
...
{
    unsigned int nb_new_vars = 3;
    auto vars = pb->addNumVariable(std::vector<double>(nb_new_vars, 0.),
                                   std::vector<double>(nb_new_vars, 10),
                                   "var",
                                   nb_new_vars);
...
BOOST_FIXTURE_TEST_CASE(add_mixed_variables_to_problem___check_all_vars_exist, FixtureEmptyProblem)
{
    ...
    auto num_vars = pb->addVariable(std::vector<double>(nb_vars_per_type, 0),
                                    std::vector<double>(nb_vars_per_type, 20),
                                    false,
                                    "num_var",
                                    nb_vars_per_type);
    auto int_vars = pb->addVariable(std::vector<double>(nb_vars_per_type, 12),
                                    std::vector<double>(nb_vars_per_type, 34),
                                    true,
                                    "int_var",
                                    nb_vars_per_type);
                        ...
                        FixtureEmptyProblem)
{
    ...
    auto constraints = pb->addConstraint(std::vector<double>(nb_new_vars, 5),
                                         std::vector<double>(nb_new_vars, 15),
                                         "constraint",
                                         nb_new_vars);
...
BOOST_FIXTURE_TEST_CASE(mix_bulk_and_single_variable_additions, FixtureEmptyProblem)
{
    ...
    auto vars = pb->addNumVariable(std::vector<double>(2, 5),
