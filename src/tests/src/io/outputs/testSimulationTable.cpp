...
BOOST_AUTO_TEST_CASE(TableForStage_CreatesOnDemandAndKeepsPointersStable)
{
    ...
    SimulationTable* remix = tables.tableForStage(Antares::IO::Outputs::Stage::remixHydro);
    BOOST_REQUIRE(remix != nullptr);
    BOOST_CHECK_EQUAL(tables.stages().size(), 1u);

    // Asking again for the same stage returns the same table.
    BOOST_CHECK(tables.tableForStage(Antares::IO::Outputs::Stage::remixHydro) == remix);
    ...
    BOOST_CHECK(tables.tableForStage(Antares::IO::Outputs::Stage::remixHydro) == remix);
    BOOST_CHECK(tables.tableForStage(Antares::IO::Outputs::Stage::firstOptim) == first);
...
BOOST_AUTO_TEST_CASE(WriteTo_NamesOneFilePerStage)
{
    ...
    tables.tableForStage(Antares::IO::Outputs::Stage::remixHydro)->addEntry(entry);
...
BOOST_AUTO_TEST_CASE(WriteTo_SkipsStagesWithNoRows)
{
    ...
    tables.tableForStage(Antares::IO::Outputs::Stage::remixHydro);
...
{
    for (const auto stage: Antares::IO::Outputs::allStages)
    {
...
BOOST_AUTO_TEST_CASE(ParseStageSelection_TrimsSpacesAndRejectsUnknownNames)
{
    ...
    BOOST_CHECK(selection.contains(Antares::IO::Outputs::Stage::secondOptim));
    BOOST_CHECK(selection.contains(Antares::IO::Outputs::Stage::adequacyPatchCsr));
...
{
    OptimisationsSimulationTable tables;
    tables.selectStages({Antares::IO::Outputs::Stage::remixHydro});

    BOOST_CHECK(tables.tableForStage(Antares::IO::Outputs::Stage::remixHydro) != nullptr);
    BOOST_CHECK(tables.firstOptimSimulationTable() == nullptr);
    BOOST_CHECK(tables.secondOptimSimulationTable() == nullptr);
    BOOST_CHECK(tables.tableForStage(Antares::IO::Outputs::Stage::adequacyPatchCsr) == nullptr);
...
BOOST_AUTO_TEST_CASE(SelectStages_PostProcessStagesDriveTheModelerProblemRetention)
{
    ...
    for (const auto stage: {Antares::IO::Outputs::Stage::remixHydro, Antares::IO::Outputs::Stage::adequacyPatchCsr})
    {
        OptimisationsSimulationTable tables;
    ...
    optimOnly.selectStages({Antares::IO::Outputs::Stage::firstOptim, Antares::IO::Outputs::Stage::secondOptim});
    BOOST_CHECK(!optimOnly.anyPostProcessStageSelected());

    // Querying must not create anything.
    OptimisationsSimulationTable untouched;
    untouched.selectStages({Antares::IO::Outputs::Stage::remixHydro});
    BOOST_CHECK(untouched.isStageSelected(Antares::IO::Outputs::Stage::remixHydro));
    BOOST_CHECK(!untouched.isStageSelected(Antares::IO::Outputs::Stage::firstOptim));
...
{
    OptimisationsSimulationTable tables;
    tables.selectStages({Antares::IO::Outputs::Stage::firstOptim});
