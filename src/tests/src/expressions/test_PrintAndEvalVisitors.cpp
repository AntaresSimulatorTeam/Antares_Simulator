...
BOOST_AUTO_TEST_CASE(testDualOnDroppedConstraintKeepsDroppedTimestepAtZero)
{
    ...
    auto* next_var_node = registry.create<TimeShiftNode>(
      registry.create<VariableNode>("var1", 0, VariabilityType::VARYING_IN_TIME_ONLY),
      registry.create<LiteralNode>(1));
    auto* current_var_node = registry.create<VariableNode>("var1", 0, VariabilityType::VARYING_IN_TIME_ONLY);
    auto* ct_node = registry.create<EqualNode>(next_var_node, current_var_node);

    std::vector<Constraint> constraints;
    constraints.emplace_back("ct1",
                             Expression("ct1", NodeRegistry(ct_node, std::move(registry))),
                             Antares::Solver::Config::Location::SUBPROBLEMS,
                             OutOfBoundsProcessingMode::DROP);

    ModelBuilder modelBuilder;
    auto model = modelBuilder.withId("model").withConstraints(std::move(constraints)).build();

    ComponentBuilder componentBuilder;
    auto component = componentBuilder.withId("component")
                       .withIndex(0)
                       .withModel(&model)
                       .withParameterValues({})
    ...
    Antares::Optimisation::LinearProblemApi::FillContext fillContext{0, 2, 0, 2, 0};
    Antares::Optimisation::LinearProblemDataImpl::LinearProblemData data;
    auto scenarioGroupRepo = makeScenarioGroupRepo(component);

    EvalVisitor evalVisitor(optimContainer,
                            fillContext,
                            component,
                            &data,
                            &scenarioGroupRepo.scenario(component.getScenarioGroupId()));

    Registry<Nodes::Node> dualRegistry;
    auto* dualNode = dualRegistry.create<FunctionNode>(FunctionNodeType::dual,
                                                       dualRegistry.create<ParameterNode>("ct1"),
                                                       dualRegistry.create<LiteralNode>(0));

    const auto dualValues = evalVisitor.dispatch(dualNode).valuesAsVector();

    const std::vector<double> expectedDualValues = {2.5, -3.0, 0.0};
    BOOST_CHECK_EQUAL_COLLECTIONS(dualValues.begin(),
                                  dualValues.end(),
                                  expectedDualValues.begin(),
                                  expectedDualValues.end());
}
