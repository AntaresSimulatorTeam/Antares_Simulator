...
{
    ParameterNode root("my-param", TimeScenarioVariability::CONSTANT_IN_TIME_AND_SCENARIO);
                        ...
                        MyDummyFixture)
{
    ...
    ParameterNode root(id, TimeScenarioVariability::CONSTANT_IN_TIME_AND_SCENARIO);
    Model model = createModelWithParameters(
      {Parameter("my-param", TimeDependent::NO, ScenarioDependent::NO)});
...
{
    ParameterNode root = ParameterNode("my-param", TimeScenarioVariability::VARYING_IN_TIME_ONLY);
...
{
    ParameterNode paramNode("my-param", TimeScenarioVariability::VARYING_IN_TIME_ONLY);
...
BOOST_AUTO_TEST_CASE(functionNode_min_timeDepdentParameter)
{
    ...
    ParameterNode second("Param2", TimeScenarioVariability::VARYING_IN_TIME_ONLY);
...
BOOST_FIXTURE_TEST_CASE(testVariableNodeEvaluation, MyDummyFixture)
{
    ...
    Node* reducedCost = create<FunctionNode>(
      ...
      create<VariableNode>("my_const_variable", 0, TimeScenarioVariability::CONSTANT_IN_TIME_AND_SCENARIO));
    eval = visitor.dispatch(reducedCost).valueAsDouble();
    BOOST_CHECK_EQUAL(eval, 4.96);

    root = create<VariableNode>("my_const_variable", 0, TimeScenarioVariability::VARYING_IN_SCENARIO_ONLY);
    eval = visitor.dispatch(root).valueAsDouble();
    BOOST_CHECK_EQUAL(eval, 12.5);

    root = create<VariableNode>("my_non_const_variable", 1, TimeScenarioVariability::VARYING_IN_TIME_ONLY);
