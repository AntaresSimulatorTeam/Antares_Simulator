...
BOOST_FIXTURE_TEST_CASE(test_importCapacityReservation_allGood, OneProblemWithReservesTwoAreas)
{
    ...
    BOOST_CHECK_EQUAL(problemeHebdo->allReserves.at(indexA).areaCapacityReservations.size(),
                      2);
    BOOST_CHECK_EQUAL(problemeHebdo->allReserves.at(indexB).areaCapacityReservations.size(),
                      2);
