...
struct Fixture
{
	...
	{
      // We need to set a 0 offset in the timezone. Otherwise, function DateTime::TimestampToString (a wrapper around ::strftime) will add 1 or 2 hours. We don't want this.
      // See https://users.pja.edu.pl/~jms/qnx/help/watcom/clibref/global_data.html
      setenv("TZ", "GMT", 1);
	}
...
BOOST_AUTO_TEST_CASE(economy_legacyfiles_emptylabel)
{
    ...
    const YString actualOutput = StudyCreateOutputName(mode,
                                                       fmt,
                                                       tmp_path.string(),
                                                       label,
                                                       startTime);
...
BOOST_AUTO_TEST_CASE(adequacy_legacyfiles_label_now)
{
    ...
    const YString expectedOutput_suffix = (tmp_path / "output" / "20221230-0914adq-test-2").string();
    const YString actualOutput_suffix = StudyCreateOutputName(mode,
                                                              fmt,
                                                              tmp_path.string(),
                                                              label,
                                                              startTime);
