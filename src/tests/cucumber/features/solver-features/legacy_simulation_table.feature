Feature: Legacy variables in simulation table
  ...
  Scenario: Per-stage simulation tables show what the adequacy patch moved
    ...
    # remix hydro changes nothing and the shave-peaks stage is a faithful copy
    ...
    When I run antares simulator with --output=simulation-tables
    Then the simulation succeeds
    And the simulation tables cover exactly the stages "optim-nb-1, optim-nb-2, shave-peaks, adq-patch"
    ...
    And the modeler outputs from stage "shave-peaks" contain the following entries with relative tolerance 1e-4
      | block | component              | output            | timestep | scenario | value  |
      | 0     | areain-1_node          | unsupplied_energy | 0        | 0        | 0      |
      | 0     | areain-2_node          | unsupplied_energy | 0        | 0        | 400    |
      | 0     | areain-1_areain-2_link | flow              | 0        | 0        | -101   |
  ...
  Scenario: A single stage can be selected on its own
    Given the solver study path is "Antares_Simulator_Tests_NR/adequacy-patch-CSR/adq-patch-CSR-test-case-v02"
    When I run antares simulator with --output=simulation-tables --simulation-table-stages=shave-peaks
    Then the simulation succeeds
    And the simulation tables cover exactly the stages "shave-peaks"
  ...
  Scenario: A selected post-process stage keeps its modeler rows
    ...
    # pass writes a table of its own -- selecting only shave-peaks leaves both
  ...
  Scenario: The study can ask for its stages in generaldata.ini
    ...
    And the study asks for the simulation table stages "optim-nb-1, shave-peaks"
    When I run antares simulator with --output=simulation-tables
    Then the simulation succeeds
    And the simulation tables cover exactly the stages "optim-nb-1, shave-peaks"
