...
namespace Test::Modeler {
    ...
    void LinearProblemBuildingFixture::buildLinearProblem(
        ...
        for (auto &component: components) {
            auto cf = std::make_unique<Antares::Optimization::ComponentFiller>(component, variableDictionary);
        ...
        pb = std::make_unique<Antares::Optimisation::LinearProblemMpsolverImpl::OrtoolsLinearProblem>(
            false,
            "sirius");
        Antares::Optimisation::LinearProblemApi::LinearProblemBuilder linear_problem_builder(fillers_ptr);
    ...
    Antares::Expressions::Nodes::Node *LinearProblemBuildingFixture::parameter(const std::string &paramId,
                                                                               const
    ...
    Antares::Expressions::Nodes::Node *LinearProblemBuildingFixture::variable(const std::string &varId,
                                                                              const
                                                                              Antares::Expressions::Visitors::TimeIndex
                                                                              &timeIndex) {
    ...
    Antares::Expressions::Nodes::Node *LinearProblemBuildingFixture::multiply(Antares::Expressions::Nodes::Node *node1,
                                                                              Antares::Expressions::Nodes::Node *
                                                                              node2) {
        return nodes.create<Antares::Expressions::Nodes::MultiplicationNode>(node1, node2);
    }

    Antares::Expressions::Nodes::Node *LinearProblemBuildingFixture::negate(Antares::Expressions::Nodes::Node *node) {
        return nodes.create<Antares::Expressions::Nodes::NegationNode>(node);
    }

    void LinearProblemBuildingFixture::createModel(std::string modelId, std::vector<std::string> parameterIds,
        ...
        for (auto parameter_id: std::move(parameterIds)) {
            parameters.push_back(
                Antares::ModelerStudy::SystemModel::Parameter(
                    parameter_id, Antares::ModelerStudy::SystemModel::TimeDependent::NO,
    ...
    void LinearProblemBuildingFixture::createModelWithSystemModelParameter(
        ...
        auto createExpression = [this](Antares::Expressions::Nodes::Node *node) {
            ...
            Antares::ModelerStudy::SystemModel::Expression expression("expression", std::move(node_registry));
            return expression;
        };
        ...
        for (auto [id, type, lb, ub, timeDependent, scenarioDependent]: variablesData) {
            variables.emplace_back(id,
                                   ...
                                   Antares::ModelerStudy::SystemModel::fromBool<
                                       Antares::ModelerStudy::SystemModel::ScenarioDependent>(scenarioDependent));
        }
        std::vector<Antares::ModelerStudy::SystemModel::Constraint> constraints;
        for (auto [id, expression]: constraintsData) {
            constraints.push_back(
                std::move(Antares::ModelerStudy::SystemModel::Constraint(id, createExpression(expression))));
        }
    ...
    void LinearProblemBuildingFixture::createModelWithOneFloatVar(const std::string &modelId,
                                                                         const std::vector<std::string> &parameterIds,
                                                                         const std::string &varId,
                                                                         Antares::Expressions::Nodes::Node *lb,
                                                                         Antares::Expressions::Nodes::Node *ub,
                                                                         const std::vector<ConstraintData> &
                                                                         constraintsData,
                                                                         Antares::Expressions::Nodes::Node *objective,
                                                                         bool time_dependent) {
        createModel(modelId,
                    parameterIds,
                    {{varId, Antares::ModelerStudy::SystemModel::ValueType::FLOAT, lb, ub, time_dependent, false}},
                    constraintsData,
                    objective);
    }
}
