...
GIVEN("^the study path is \"(.+)\"$") {
    ...
    context->studyPath = std::filesystem::current_path().string() + "/src/tests/resources/Antares_Simulator_Tests/" + studyPath;
}
...
double read(string fileName, string tag) {
    ...
    // but FAIL() can only be used in functions that return void, as documented in https://google.github.io/googletest/reference/assertions.html
    return 0;
}
